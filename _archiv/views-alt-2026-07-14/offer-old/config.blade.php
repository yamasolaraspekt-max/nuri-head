    ...
    <div id="view-editor" class="view-section flex-1 overflow-hidden relative bg-slate-100">
        ...
        <div class="flex h-full overflow-hidden relative">
            ...
            <aside id="sidebar-left" class="w-[390px] bg-white border-r border-slate-200 flex flex-col z-20 shadow-xl flex-shrink-0 sidebar-panel no-print overflow-hidden">
                ...
                <!-- LIBRARY -->
                <div id="sidebar-content-lib" class="flex-1 flex flex-col min-h-0 bg-slate-50">
...
(function () {
    ...
    App.calcLaborAnalysis = function (item, options = {}) {
        ...
        if (rows.length === 0) {
            ...
            result.cost = toNumber(item?.purchase_price ?? item?.ek, 0) * multiplier;
    ...
    App.calcItemAnalysis = function (item, options = {}) {
        ...
        if (item.isPauschal) {
            ...
            result.cost = toNumber(item.purchase_price ?? item.ek, 0);
...
<script>
/**
 * OFFER_TOTAL_MIN_PROFIT_FIX
 *
 * Fixes two calculation problems:
 * 1) Imported master-set/product rows had `ek` but `purchase_price` stayed 0 from buildBaseItem().
 *    Calculations used `purchase_price ?? ek`, so 0 was treated as the real EK.
 * 2) Mindestgewinn warning was checked per article. It is now checked against the total offer:
 *    (total VK - total EK) / total EK * 100.
 */
(function () {
    if (typeof App === 'undefined' || typeof State === 'undefined') return;
    ...
    const safeDiv = (value, divisor) => {
        const d = toNum(divisor, 0);
        return d > 0 ? toNum(value, 0) / d : 0;
    };
    ...
    const addAnalysis = (target, source, multiplier = 1) => {
        Object.keys(blankAnalysis()).forEach((key) => {
            target[key] = toNum(target[key], 0) + (toNum(source[key], 0) * multiplier);
        });
        target.db = target.sales - target.cost;
        return target;
    };

    App.getEffectiveEK = function (item) {
        const purchase = toNum(item?.purchase_price, NaN);
        const ek = toNum(item?.ek, NaN);
        const cost = toNum(item?.cost, NaN);

        // Important: purchase_price=0 from the default object must NOT hide a real ek value.
        if (Number.isFinite(purchase) && purchase > 0) return purchase;
        if (Number.isFinite(ek) && ek > 0) return ek;
        if (Number.isFinite(cost) && cost > 0) return cost;

        return 0;
    };

    App.normalizeOfferCostFields = function () {
        const normalizeItem = (item) => {
            if (!item || typeof item !== 'object') return;

            const ek = App.getEffectiveEK(item);
            if (ek > 0) {
                item.ek = ek;
                item.purchase_price = ek;
            }

            if (Array.isArray(item.labor_rows)) {
                item.labor_rows.forEach((row) => {
                    row.ek = toNum(row.ek, 0);
            ...
            if (Array.isArray(item.subItems)) {
                item.subItems.forEach(normalizeItem);
            }
        };

        (State.sections || []).forEach((section) => {
            (section.items || []).forEach(normalizeItem);
        });
    };

    App.calcLaborAnalysis = function (item) {
        const result = blankAnalysis();
        const rows = Array.isArray(item?.labor_rows) ? item.labor_rows : [];

        if (rows.length === 0) {
            const multiplier = App.getItemMultiplier ? App.getItemMultiplier(item) : toNum(item?.qty, 1);
            result.sales = toNum(item?.price ?? item?.rate, 0) * multiplier;
            result.cost = App.getEffectiveEK(item) * multiplier;
            result.hours = toNum(item?.qty, 0);
        } else {
            rows.forEach((row) => {
                const qty = toNum(row.qty, 0);
                const ek = toNum(row.ek, 0);
                const rate = toNum(row.rate, 0);

                row.total = qty * rate;

                result.sales += qty * rate;
                result.cost += qty * ek;
                result.hours += qty;
                result.rows += 1;
            });
    ...
    App.calcItemAnalysis = function (item, options = {}) {
        ...
        const status = App.resolveLineStatus ? App.resolveLineStatus(item) : String(item.lineType || item.status || 'normal').toLowerCase();
        const kind = App.resolveItemKind ? App.resolveItemKind(item) : String(item.kind || item.item_type || 'material').toLowerCase();
        ...
        const hasChildren = Array.isArray(item.subItems) && item.subItems.length > 0 && !item.isPauschal;
        let directLine = false;

        if (item.isPauschal) {
            result.sales = toNum(item.price ?? item.rate, 0);
            result.cost = App.getEffectiveEK(item);
            directLine = true;
        } else if (kind === 'labor') {
            addAnalysis(result, App.calcLaborAnalysis(item));
        } else if (hasChildren) {
            ...
            const parentMultiplier = App.getItemMultiplier ? App.getItemMultiplier(item) : toNum(item.qty, 1);
            addAnalysis(result, childTotal, parentMultiplier);
        } else {
            const multiplier = App.getItemMultiplier ? App.getItemMultiplier(item) : toNum(item.qty, 1);
            result.sales = toNum(item.price ?? item.rate, 0) * multiplier;
            result.cost = App.getEffectiveEK(item) * multiplier;
    ...
    App.getQuoteMinProfitStatus = function () {
        App.normalizeOfferCostFields();

        const totals = App.computeQuoteTotals ? App.computeQuoteTotals(false) : {};
        const totalVK = toNum(totals.salesNet, 0);
        const totalEK = toNum(totals.sumEK, 0);
        const profit = totalVK - totalEK;

        // This matches your example: VK 1200, EK 1000 => 20%.
        const profitPctOnEK = totalEK > 0 ? (profit / totalEK) * 100 : 0;
        const profitPctOnVK = totalVK > 0 ? (profit / totalVK) * 100 : 0;
        const minProfit = toNum(State.config?.minProfit, 10);
        ...
        return {
            ...
            profitPctOnVK,
            minProfit,
            isBelow: totalEK > 0 && profitPctOnEK < minProfit
        };
    };

    App.checkQuoteMinProfit = function (showToast = false) {
        const status = App.getQuoteMinProfitStatus();

        if (!status.isBelow) {
            State._lastTotalMinProfitWarningKey = null;
            return true;
        }

        if (showToast && typeof App.toastConfirmShow === 'function') {
            const key = [
                Math.round(status.totalVK * 100),
                Math.round(status.totalEK * 100),
                Math.round(status.profitPctOnEK * 10),
                Math.round(status.minProfit * 10)
            ].join(':');

            if (State._lastTotalMinProfitWarningKey !== key) {
                State._lastTotalMinProfitWarningKey = key;

                App.toastConfirmShow({
                    title: 'Achtung: Gesamtgewinn zu niedrig!',
                    message:
                        `Der Mindestgewinn wird jetzt auf das ganze Angebot geprüft, nicht pro Artikel.<br><br>` +
                        `VK gesamt: <b>${fmtMoney(status.totalVK)}</b><br>` +
                        `EK gesamt: <b>${fmtMoney(status.totalEK)}</b><br>` +
                        `Gewinn: <b>${fmtMoney(status.profit)}</b><br>` +
                        `Gewinn auf EK: <b>${fmtPct(status.profitPctOnEK)}</b> ` +
                        `(Limit: ${fmtPct(status.minProfit)})`,
                    okText: 'Verstanden',
                    cancelText: ''
                });
    ...
    App.updatePosPriceCalc = function (sIdx, iIdx, subIdx, field, value) {
        ...
        let ek = App.getEffectiveEK(item);
        let marginType = item.marginType || 'percent';
        let margin = toNum(item.marginPercent ?? item.margin, 0);
        ...
        if (field === 'marginType') {
            const currentVK = toNum(item.price ?? item.rate, 0);
            item.marginType = value || 'percent';
            marginType = item.marginType;

            if (marginType === 'percent') {
                item.marginPercent = ek > 0 ? App.marginFromEkVk(ek, currentVK) : 0;
                item.margin = item.marginPercent;
            } else {
                item.margin = currentVK - ek;
                item.marginPercent = ek > 0 ? App.marginFromEkVk(ek, currentVK) : 0;
            }
        } else if (field === 'ek' || field === 'purchase_price') {
            ek = val;
            item.ek = val;
            item.purchase_price = val;

            const currentMargin = toNum(item.margin ?? item.marginPercent, 0);
            if (marginType === 'euro') {
                item.price = val + currentMargin;
            } else {
                item.price = App.vkFromEkMargin(val, currentMargin);
            }

            item.rate = item.price;
            item.marginPercent = val > 0 ? App.marginFromEkVk(val, item.price) : 0;
        } else if (field === 'margin' || field === 'marginPercent') {
            item.margin = val;

            if (marginType === 'euro') {
                item.price = ek + val;
                item.marginPercent = ek > 0 ? App.marginFromEkVk(ek, item.price) : 0;
            } else {
                item.marginPercent = val;
                item.price = App.vkFromEkMargin(ek, val);
            }
            ...
            item.rate = val;
            item.marginPercent = ek > 0 ? App.marginFromEkVk(ek, val) : 0;
            item.margin = marginType === 'euro' ? (val - ek) : item.marginPercent;
        } else if (field === 'qty') {
            item.qty = val;
        } else if (field === 'price_unit_value') {
            item.price_unit_value = Math.max(0.000001, val);
        ...
        if (item.kind === 'labor' && Array.isArray(item.labor_rows) && typeof App.recalcLaborCarrier === 'function') {
            App.recalcLaborCarrier(sIdx, iIdx, isSub ? subIdx : null);
        }
        ...
        if (typeof App.flashListViewRow === 'function') {
            App.flashListViewRow(sIdx, iIdx, isSub ? subIdx : null);
        }

        // No per-line Mindestgewinn warning anymore.
        // Only the total offer decides.
        if (['ek', 'purchase_price', 'margin', 'marginPercent', 'price', 'rate', 'qty', 'price_unit_value'].includes(field)) {
            App.checkQuoteMinProfit(true);
        }
    };

    // Normalize immediately after importing master sets/products.
    if (typeof App.handleItemAdd === 'function' && !App.handleItemAdd.__totalMinProfitWrapped) {
        const originalHandleItemAdd = App.handleItemAdd;
        App.handleItemAdd = async function (...args) {
            const result = await originalHandleItemAdd.apply(this, args);
            App.normalizeOfferCostFields();
            return result;
        };
        App.handleItemAdd.__totalMinProfitWrapped = true;
    }

    if (typeof App.addLibraryItemAsSubPosition === 'function' && !App.addLibraryItemAsSubPosition.__totalMinProfitWrapped) {
        const originalAddLibraryItemAsSubPosition = App.addLibraryItemAsSubPosition;
        App.addLibraryItemAsSubPosition = async function (...args) {
            const result = await originalAddLibraryItemAsSubPosition.apply(this, args);
            App.normalizeOfferCostFields();
            return result;
        };
        App.addLibraryItemAsSubPosition.__totalMinProfitWrapped = true;
    }

    if (typeof App.performSave === 'function' && !App.performSave.__totalMinProfitWrapped) {
        const originalPerformSave = App.performSave;
        App.performSave = async function (...args) {
            App.normalizeOfferCostFields();
            return originalPerformSave.apply(this, args);
        };
        App.performSave.__totalMinProfitWrapped = true;
    }

    // Keep old saved offers safe too.
    App.normalizeOfferCostFields();
...
/* ============================================================
   OFFER_ROW_BOOT_RECALC_FIX_V4
   Fix:
   - Old saved rows are normalized/recalculated immediately after load.
   - EK total / VK total no longer stay at 0 until the user changes a field.
   - Fixed-€ margin mode supports both "fixed" and older "euro".
   - Mindestgewinn stays checked on the complete offer total, not per row.
============================================================ */
(function () {
    if (typeof App === 'undefined' || typeof State === 'undefined') return;
    ...
    const isFixedMargin = (type) => {
        const value = String(type || '').toLowerCase().trim();
        return value === 'fixed' || value === 'euro' || value === 'amount';
    };

    const readPositive = (...values) => {
        for (const value of values) {
    ...
    const lineStatus = (item) => {
        if (typeof App.resolveLineStatus === 'function') return App.resolveLineStatus(item);
        const raw = String(item?.lineType || item?.status || 'normal').toLowerCase().trim();
        return raw === 'standard' ? 'normal' : (raw || 'normal');
    };

    const itemKind = (item) => {
        if (typeof App.resolveItemKind === 'function') return App.resolveItemKind(item);
        const raw = String(item?.kind || item?.item_type || 'material').toLowerCase().trim();
        if (raw === 'article') return 'material';
        return raw || 'material';
    };

    const multiplier = (item) => {
        if (item?.isPauschal) return 1;
        if (typeof App.getItemMultiplier === 'function') return App.getItemMultiplier(item);
        const qty = (item?.qty === undefined || item?.qty === null || item?.qty === '') ? 1 : n(item.qty, 0);
        const baseQty = Math.max(0.000001, n(item?.price_unit_value, 1));
        return qty / baseQty;
    };
    ...
    App.getEffectiveVK = function (item) {
        return readPositive(
            item?.price,
            item?.rate,
            item?.vk,
    ...
    App.normalizeOfferCostFields = function () {
        const normalizeItem = (item) => {
            ...
            if (kind !== 'note') {
                ...
                if (!n(item.price_unit_value, 0)) item.price_unit_value = 1;
            }
            ...
            let vk = App.getEffectiveVK(item);
            const marginType = item.marginType || 'percent';
            const margin = n(item.margin ?? item.marginPercent, 0);

            // If old saved rows have EK + margin but no VK, rebuild VK immediately.
            if (vk <= 0 && ek > 0 && kind !== 'note') {
                vk = isFixedMargin(marginType)
                    ? ek + margin
                    : (typeof App.vkFromEkMargin === 'function' ? App.vkFromEkMargin(ek, margin) : ek * (1 + margin / 100));
                item.price = vk;
                item.rate = vk;
            }
            ...
            if (ek > 0 && vk > 0) {
                const pct = typeof App.marginFromEkVk === 'function'
                    ? App.marginFromEkVk(ek, vk)
                    : ((vk - ek) / ek) * 100;

                item.marginPercent = pct;
                if (!isFixedMargin(marginType)) item.margin = pct;
            }
            ...
            if (Array.isArray(item.labor_rows)) {
                item.labor_rows.forEach((row) => {
                    row.qty = n(row.qty, 0);
                    row.ek = n(row.ek, 0);
                    row.rate = n(row.rate, 0);

                    if (row.rate <= 0 && row.ek > 0) {
                        const rowMargin = n(row.margin_percent, typeof App.getDefaultMargin === 'function' ? App.getDefaultMargin('labor') : 0);
                        row.rate = typeof App.vkFromEkMargin === 'function'
                            ? App.vkFromEkMargin(row.ek, rowMargin)
                            : row.ek * (1 + rowMargin / 100);
                    }
    ...
    App.calcItemAnalysis = function (item, options = {}) {
        ...
        const add = (src, factor = 1) => {
            Object.keys(result).forEach((key) => {
                result[key] += n(src[key], 0) * factor;
            });
            result.db = result.sales - result.cost;
        };
        ...
        if (item.isPauschal) {
            result.sales = App.getEffectiveVK(item);
            result.cost = App.getEffectiveEK(item);
        } else if (kind === 'labor' && Array.isArray(item.labor_rows) && item.labor_rows.length > 0) {
            item.labor_rows.forEach((row) => {
                const qty = n(row.qty, 0);
                const ek = n(row.ek, 0);
                const rate = n(row.rate, 0);
                row.total = qty * rate;
            ...
            const children = {
                sales: 0, cost: 0, db: 0, hours: 0,
                materialSales: 0, materialCost: 0,
                laborSales: 0, laborCost: 0,
                externalSales: 0, externalCost: 0,
                logisticsSales: 0, logisticsCost: 0,
                optionalSales: 0, alternativeSales: 0,
                rows: 0
            };

            item.subItems.forEach((sub) => {
                const child = App.calcItemAnalysis(sub, opts);
                Object.keys(children).forEach((key) => {
                    children[key] += n(child[key], 0);
                });
            });

            add(children, multiplier(item));
        } else {
            const factor = multiplier(item);
            result.sales = App.getEffectiveVK(item) * factor;
            result.cost = App.getEffectiveEK(item) * factor;

            if (kind === 'labor') {
                result.laborSales = result.sales;
                result.laborCost = result.cost;
                result.hours = n(item.qty, 0);
            } else if (kind === 'external') {
                result.externalSales = result.sales;
                result.externalCost = result.cost;
            } else if (kind === 'logistics') {
    ...
    App.recalculateOfferRowsForDisplay = function () {
        App.normalizeOfferCostFields();

        (State.sections || []).forEach((section, sIdx) => {
            (section.items || []).forEach((item, iIdx) => {
                if (!item || !Array.isArray(item.subItems) || item.subItems.length === 0 || item.isPauschal) return;

                let totalVK = 0;
                let totalEK = 0;

                item.subItems.forEach((sub) => {
                    if (!sub || sub.active === false || lineStatus(sub) !== 'normal' || itemKind(sub) === 'note') return;
                    totalVK += App.calcItemGross(sub);
                    totalEK += App.calcItemCost(sub);
                });

                item.price = totalVK;
                item.rate = totalVK;
                item.ek = totalEK;
                item.purchase_price = totalEK;
                item.marginPercent = totalEK > 0 ? ((totalVK - totalEK) / totalEK) * 100 : (totalVK > 0 ? 100 : 0);

                if (!isFixedMargin(item.marginType)) {
                    item.margin = item.marginPercent;
                }

                if (typeof App.syncParentTotals === 'function') {
                    App.syncParentTotals(sIdx, iIdx);
                }
            });
        });
    ...
    const originalUpdatePosPriceCalc = App.updatePosPriceCalc;
    App.updatePosPriceCalc = function (sIdx, iIdx, subIdx, field, value) {
        const isSub = subIdx !== null && subIdx !== undefined && subIdx !== 'null';
        const item = isSub
            ? State.sections?.[sIdx]?.items?.[iIdx]?.subItems?.[subIdx]
        ...
        if (field === 'marginType') {
            ...
            const currentVK = App.getEffectiveVK(item);

            if (isFixedMargin(marginType)) {
                item.margin = currentVK - ek;
                item.marginPercent = ek > 0 ? ((currentVK - ek) / ek) * 100 : 0;
            } else {
                item.marginPercent = ek > 0 ? ((currentVK - ek) / ek) * 100 : 0;
                item.margin = item.marginPercent;
            }
            ...
            const currentMargin = n(item.margin ?? item.marginPercent, 0);
            item.price = isFixedMargin(marginType)
                ? val + currentMargin
                : (typeof App.vkFromEkMargin === 'function' ? App.vkFromEkMargin(val, currentMargin) : val * (1 + currentMargin / 100));
            item.rate = item.price;
            item.marginPercent = val > 0 ? ((item.price - val) / val) * 100 : 0;
        } else if (field === 'margin' || field === 'marginPercent') {
            item.margin = val;

            if (isFixedMargin(marginType)) {
                item.price = ek + val;
                item.marginPercent = ek > 0 ? ((item.price - ek) / ek) * 100 : 0;
            } else {
                item.marginPercent = val;
                item.price = typeof App.vkFromEkMargin === 'function' ? App.vkFromEkMargin(ek, val) : ek * (1 + val / 100);
            }
            ...
            item.price = val;
            item.rate = val;
            item.marginPercent = ek > 0 ? ((val - ek) / ek) * 100 : 0;
            item.margin = isFixedMargin(marginType) ? (val - ek) : item.marginPercent;
        } else if (field === 'qty') {
            item.qty = val;
        } else if (field === 'price_unit_value') {
            item.price_unit_value = Math.max(0.000001, val);
        } else if (typeof originalUpdatePosPriceCalc === 'function') {
            return originalUpdatePosPriceCalc.apply(this, arguments);
        }
        ...
        App.recalculateOfferRowsForDisplay();
        ...
        if (['ek', 'purchase_price', 'margin', 'marginPercent', 'price', 'rate', 'qty', 'price_unit_value'].includes(field)
            && typeof App.checkQuoteMinProfit === 'function') {
            App.checkQuoteMinProfit(true);
        }
    };

    if (typeof App.renderQuotePage === 'function' && !App.renderQuotePage.__rowBootRecalcWrapped) {
        const originalRenderQuotePage = App.renderQuotePage.bind(App);

        App.renderQuotePage = function (forPrint = false) {
            if (!State.__rowBootRecalcRunning) {
                State.__rowBootRecalcRunning = true;
                try {
                    App.recalculateOfferRowsForDisplay();
                } finally {
                    State.__rowBootRecalcRunning = false;
                }
            }

            return originalRenderQuotePage(forPrint);
        };

        App.renderQuotePage.__rowBootRecalcWrapped = true;
    }

    if (typeof App.startQuote === 'function' && !App.startQuote.__rowBootRecalcWrapped) {
        const originalStartQuote = App.startQuote.bind(App);

        App.startQuote = function (...args) {
            const wasClean = !State.hasUnsavedChanges;
            App.recalculateOfferRowsForDisplay();

            const result = originalStartQuote(...args);

            App.recalculateOfferRowsForDisplay();

            if (App.Tabs?.current === 'list' && App.ListView && typeof App.ListView.render === 'function') {
                App.ListView.render();
            }

            if (State.loadedSavedDetail && wasClean) {
                State.hasUnsavedChanges = false;
            }
        ...
        App.startQuote.__rowBootRecalcWrapped = true;
    }

    if (typeof App.loadSavedDocumentIfAvailable === 'function' && !App.loadSavedDocumentIfAvailable.__rowBootRecalcWrapped) {
        const originalLoadSavedDocumentIfAvailable = App.loadSavedDocumentIfAvailable.bind(App);

        App.loadSavedDocumentIfAvailable = async function (...args) {
            const result = await originalLoadSavedDocumentIfAvailable(...args);

            App.recalculateOfferRowsForDisplay();

            if (State.loadedSavedDetail && typeof App.renderQuotePage === 'function') {
                App.renderQuotePage(false);
        ...
        App.loadSavedDocumentIfAvailable.__rowBootRecalcWrapped = true;
    }

    // One more safe pass after the DOM and the async saved document load have had time to finish.
    window.addEventListener('DOMContentLoaded', function () {
        [0, 100, 500].forEach((delay) => {
            setTimeout(() => {
                if (!Array.isArray(State.sections) || State.sections.length === 0) return;
