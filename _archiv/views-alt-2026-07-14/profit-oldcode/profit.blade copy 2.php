          ...
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="flex items-center gap-2">
              <img
                ...
                alt="Solar Aspekt"
                style="height:32px; object-fit:contain; width:190px;"
              />
            </div>
          </div>
  ...
  try {
    ...
    App.calculateErgebnis = function () {
      ...
      const effWallboxCost =
        (activeAuto && formData.includeSolar)
      ...
      saisonVerteilung.forEach(s => {
        ...
        if (formData.includeSolar) {
          ...
          const dailyFeedIn = dailyRestYield - charge;

          sCovered = dailyTotalCovered * s.tage;
          sFeedIn = dailyFeedIn * s.tage;
    ...
    App.getDashboardCounters = function (ergebnis) {
      const activeDash = this.state.stage === 'DASHBOARD';
      const selectedYears = this.state.selectedYears;

      const kfwCounter = this.countUp('kfwCounter', ergebnis.kfwZuschussMax, 2000, activeDash);
      const fossilCounter = this.countUp('fossilCounter', Math.round(ergebnis.opexFossilReihe?.[selectedYears] || 0), 1000, activeDash);
      const opexSolarCounter = this.countUp('opexSolarCounter', Math.round(ergebnis.opexSolarReihe?.[selectedYears] || 0), 1000, activeDash);
      const ersparnisOpexCounter = this.countUp('ersparnisOpexCounter', Math.round(ergebnis.ersparnisOpexReihe?.[selectedYears] || 0), 1000, activeDash);
...
App.renderPdfPreview = function (ergebnis) {
  ...
  const brandLogo = () => `
    ${
      window.APP_LOGO_URL
        ? `<img src="${window.APP_LOGO_URL}" alt="Logo" style="height:32px; object-fit:contain; width:190px;" />`
        : `
          <div style="font-size:16pt; font-weight:900; letter-spacing:.02em; line-height:1; display:flex; align-items:center; gap:6px;">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="${COLORS.green}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"></circle><path d="M12 2v2"></path><path d="M12 20v2"></path><path d="m4.93 4.93 1.41 1.41"></path><path d="m17.66 17.66 1.41 1.41"></path><path d="M2 12h2"></path><path d="M20 12h2"></path><path d="m6.34 17.66-1.41 1.41"></path><path d="m19.07 4.93-1.41 1.41"></path></svg>
            <span style="color:#1e293b;">SOLAR<span style="color:${COLORS.blue};">ASPEKT</span></span>
          </div>
        `
    }
  `;
  ...
  const renderPageShell = (body, { title = '', pageNo = '', noBreak = false } = {}) => `
    <section class="pdf-page ${noBreak ? 'no-page-break' : ''}" style="width:794px; min-height:1123px; box-sizing:border-box; padding:34px 38px 30px 38px; background:#ffffff; page-break-after:${noBreak ? 'auto' : 'always'}; break-after:${noBreak ? 'auto' : 'page'}; display:flex; flex-direction:column;">
      <div style="margin-bottom:22px;">
        ...
        <div style="height:2px; background:${COLORS.green}; margin-bottom:16px;"></div>
        <div style="display:flex; justify-content:space-between; align-items:flex-end; gap:12px;">
          <div style="font-size:17px; font-weight:900; line-height:1.15; text-transform:uppercase; color:${COLORS.green};">${escapeHtml(title)}</div>
      ...
      <div style="margin-top:24px; padding-top:8px; border-top:2px solid ${COLORS.green};"></div>
    </section>
  `;

  // NEUES DECKBLATT NACH DEM REACT-DESIGN
  const renderCoverPage = () => `
    <div style="width: 794px; height: 1123px; box-sizing: border-box; page-break-after: always; break-after: page;" class="relative bg-white flex flex-col overflow-hidden pdf-page">
      <div class="absolute top-0 left-0 w-full h-[55%] bg-[#f8fafc]">
        <div class="absolute top-0 left-0 w-full h-full opacity-30 mix-blend-multiply" style="background-image: url('https://www.transparenttextures.com/patterns/cubes.png');"></div>
      ...
      <div class="relative z-10 p-16 flex flex-col h-full">
        ...
        <div>
          ...
          <div class="mb-8">
            <div style="font-size:30px; line-height:0.95; font-weight:900; text-transform:uppercase; color:#93c21c;">Ihr Weg zur eigenen Energieautarkie </div>
            <div style="font-size:30px; line-height:0.95; font-weight:900; text-transform:uppercase; color:#c0d8ea;">Weniger Netz. Mehr Kontrolle.</div> 
            <div style="font-size:30px; line-height:1.02; font-weight:500; text-transform:uppercase; color:#74b2d4;">Maximale Effizienz – jeden Tag</div>
          </div>

          <div class="bg-white/90   p-8 rounded-2xl max-w-[95%]">
            ...
            <p class="text-[10pt] leading-relaxed text-slate-600 mb-4">
              Auf den folgenden Seiten sehen Sie transparent, wie sich Ihr Energieprofil durch die intelligente Vernetzung von Wärmepumpe, Photovoltaik und Speicher optimieren lässt. Zudem haben wir alle relevanten staatlichen Förderungen integriert, um Ihre Netto-Investition so effizient wie möglich zu gestalten.
            </p>
            ...
            <ul class="text-[10pt] text-slate-600 space-y-2 list-none m-0 p-0">
              <li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Ausgangslage:</strong> Ihre heutigen Energiekosten</span></li>
              <li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Umstellung auf Strom:</strong> Elektrifizierung von Wärme ${ergebnis.activeAuto ? '(und Mobilität)' : ''}</span></li>
              ...
              ${hasPv ? `<li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Systemauslegung:</strong> Dimensionierung von Photovoltaik und Batteriespeicher</span></li>` : ''}
              ...
              <li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Kennzahlen:</strong> Amortisation, Rendite und Solarstrompreis (Modellwerte)</span></li>
              ...
              <li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Klimaschutz:</strong> Ihre persönliche CO₂-Reduktion</span></li>
              <li class="flex items-start gap-2"><svg class="w-4 h-4 text-[#93c21c] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span><strong>Wie es weitergeht:</strong> Nächste Schritte und Ihr persönlicher Vorteil</span></li>
            </ul>
            ...
            <p class="text-[10pt] text-slate-700 font-bold m-0">
              Mit freundlichen Grüßen<br/>Ihr SOLAR ASPEKT-Team
            </p>
          </div>
        </div>
  ...
  const renderRing = (percent, primary = COLORS.green, secondary = '#d7e4ee', size = 86, stroke = 8, label = true) => {
    ...
    return `
      <div style="position:relative; width:${size}px; height:${size}px; display:flex; align-items:center; justify-content:center;">
        <svg viewBox="0 0 80 80" style="width:${size}px; height:${size}px; transform:rotate(-90deg);">
          <circle cx="40" cy="40" r="${r}" fill="none" stroke="${secondary}" stroke-width="${stroke}"></circle>
          <circle cx="40" cy="40" r="${r}" fill="none" stroke="${primary}" stroke-width="${stroke}" stroke-linecap="round" stroke-dasharray="${c}" stroke-dashoffset="${offset}"></circle>
        </svg>
  ...
  const investmentCards = [
    hasWp ? { title: `Wärmepumpe (${formatInt(ergebnis.wpGroesseKW)} kW)`, line1Label: 'Investition', line1Value: `${formatInt(ergebnis.investWPBrutto)} €`, line2Label: '– KfW-Förderung', line2Value: `-${formatInt((ergebnis.kfwZuschussMax || 0) + (ergebnis.f_wp_zusatz || 0))} €`, totalLabel: 'Wärmepumpe', totalValue: `${formatInt(ergebnis.investWpNetto)} €`, accent: COLORS.blue } : null,
    hasPv ? { title: `Photovoltaik (${escapeHtml(ergebnis.pvGroesse)} kWp)`, line1Label: 'Investition', line1Value: `${formatInt(ergebnis.investPVOnly)} €`, line2Label: 'Förderung', line2Value: `-${formatInt(ergebnis.f_pv || 0)} €`, totalLabel: 'Photovoltaik', totalValue: `${formatInt(ergebnis.investPvNetto)} €`, accent: COLORS.green } : null,
    hasPv ? { title: `Speicher (${escapeHtml(ergebnis.speicherGroesse)} kWh)`, line1Label: 'Investition', line1Value: `${formatInt(ergebnis.investSpeicher)} €`, line2Label: 'Förderung', line2Value: `-${formatInt(ergebnis.f_speicher || 0)} €`, totalLabel: 'Speicher', totalValue: `${formatInt(ergebnis.investSpeicherNetto)} €`, accent: COLORS.green } : null
  ].filter(Boolean);

  // --- HTML für WP-Formel ---
  const wpFormelHtml = hasWp ? `
    <div style="border: 1px solid #d7dee7; border-radius: 12px; padding: 16px; margin-bottom: 16px; page-break-inside: avoid;">
      <div style="font-size: 11pt; font-weight: 900; color: ${COLORS.text}; margin-bottom: 12px; display: flex; align-items: center; gap: 6px;">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${COLORS.blue}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.9 4 6.5 2 1.6 3 3.5 3 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 2.5 2.5z"/></svg>
        Die Wärmepumpen-Formel: Effizienz im Detail
      </div>
      <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px;">
        <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px;">
          <div style="font-size: 8pt; font-weight: 900; color: #334155; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px; margin-bottom: 8px;">
            1. Reale Nutzwärme
          </div>
          <div style="font-size: 7.5pt; line-height: 1.6; color: ${COLORS.textSoft};">
            Ihre Altanlage (Alter: ${alterHeizung} J.) hat ca. <strong>${wirkungsgradVerlustPct}% Systemverlust</strong>. Von ${formatInt(ergebnis.waermeBedarfKWhFossil)} kWh gekauftem Brennstoff kommen nur <strong style="color:${COLORS.blue}">${formatInt(ergebnis.nutzWaermeKWh)} kWh</strong> als echte Heizenergie in den Räumen an.
          </div>
        </div>
        <div style="background: #f0f7ff; border: 1px solid #c0d8ea; border-radius: 8px; padding: 12px;">
          <div style="font-size: 8pt; font-weight: 900; color: ${COLORS.blue}; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #c0d8ea; padding-bottom: 6px; margin-bottom: 8px;">
            2. Umweltenergie
          </div>
          <div style="font-size: 7.5pt; line-height: 1.6; color: ${COLORS.textSoft};">
            Dank der Jahresarbeitszahl (JAZ) von <strong>${jaz}</strong> zieht die Anlage <strong style="color:${COLORS.green}">${Math.round(umweltEnergiePct)}% (${formatInt(umweltEnergieKWh)} kWh)</strong> kostenlos aus der Außenluft. Nur für den Rest wird Antriebsstrom (<strong>${formatInt(ergebnis.wpStrombedarf)} kWh</strong>) benötigt.
          </div>
        </div>
        <div style="background: #f4fae6; border: 1px solid #cfe09b; border-radius: 8px; padding: 12px;">
          <div style="font-size: 8pt; font-weight: 900; color: ${COLORS.green}; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #cfe09b; padding-bottom: 6px; margin-bottom: 8px;">
            3. Leistung & Ersparnis
          </div>
          <div style="font-size: 7.5pt; line-height: 1.6; color: ${COLORS.textSoft};">
            Ausgelegt auf <strong>${ergebnis.wpGroesseKW} kW Nennleistung</strong> sparen Sie sofort: statt ${formatInt(ergebnis.heizKostenHeute)} € fossil zahlen Sie nur noch ${formatInt(ergebnis.kostenWPNeu)} € für Netzstrom. <strong>Gewinn: +${formatInt(ergebnis.heizKostenHeute - ergebnis.kostenWPNeu)} € / Jahr</strong>.
          </div>
        </div>
      </div>
    </div>
  ...
  // --- HTML für Systemdimensionierung ---
  const systemDimHtml = hasPv ? `
    <div style="background: #f4fae6; border: 1px solid #cfe09b; border-radius: 12px; padding: 16px; margin-bottom: 16px; page-break-inside: avoid;">
      <div style="font-size: 11pt; font-weight: 900; color: ${COLORS.green}; margin-bottom: 8px; text-transform: uppercase; display: flex; align-items: center; gap: 6px;">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="${COLORS.green}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
      ...
      <div style="font-size: 8.5pt; line-height: 1.5; color: ${COLORS.textSoft};">
        Die Systemauslegung orientiert sich präzise an Ihrem zukünftigen Gesamtbedarf aus Haushaltsstrom, Wärmepumpe und E-Mobilität. So bleibt Ihr Netzbezug minimal, während der Batteriespeicher die Nachtstunden und Übergangszeiten zuverlässig für Sie abfedert.
      </div>
    </div>
  ` : '';
  ...
  // Page 1: Transformation & Overview
  const page1Html = `
    <div style="margin-bottom:18px;">
      <div style="font-size:15px; font-weight:900; color:${COLORS.blue}; margin-bottom:8px;">Wie aus teurem Fossil effizienter Strombedarf wird</div>
      <p style="font-size:11px; line-height:1.65; color:${COLORS.textSoft}; margin:0;">Um deine Wirtschaftlichkeit präzise zu berechnen, wandeln wir deine bisherigen fossilen Verbräuche in den zukünftigen vernetzten Strombedarf um.</p>
    </div>
    <div class="grid grid-cols-2 gap-4" style="margin-bottom:16px;">
      <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:12px; padding:14px;">
        <div style="font-size:8.4pt; font-weight:900; color:${COLORS.blue}; margin-bottom:10px;">Vorher (Fossil)</div>
        <div style="font-size:7.6pt; line-height:1.8; color:${COLORS.textSoft};">
          <div class="flex justify-between"><span>Hausstrom:</span><strong>${formatInt(ergebnis.stromKostenHeute)} €</strong></div>
          <div class="flex justify-between"><span>Heizung:</span><strong>${formatInt(ergebnis.heizKostenHeute)} €</strong></div>
          ${ergebnis.activeAuto ? `<div class="flex justify-between"><span>Mobilität (Sprit)</span><strong>${formatInt(ergebnis.kostenAutoHeute)} €</strong></div>` : ''}
          <div class="flex justify-between"><span>Wartung, Schornsteinfeger & Rep.</span><strong>${formatInt(ergebnis.wartungAlt_pa)} €</strong></div>
          <div class="flex justify-between" style="margin-top:8px; padding-top:8px; border-top:1px solid:${COLORS.blue}; font-weight:900; color:${COLORS.blue};">
            <span>Gesamtkosten vorher</span><span>${formatInt(totalVorherJahr)} € / Jahr</span>
          </div>
        </div>
      </div>
      <div style="background:#f8fcff; border:1px solid ${COLORS.blueSoft}; border-radius:12px; padding:14px;">
        <div style="font-size:8.4pt; font-weight:900; color:${COLORS.blue}; margin-bottom:10px;">Nachher (Vernetzt)</div>
        <div style="font-size:7.6pt; line-height:1.8; color:${COLORS.textSoft};">
          <div class="flex justify-between"><span>Allgemeiner Hausstrom:</span><strong>${formatInt(ergebnis.kostenHausNeu)} €</strong></div>
          ${hasWp ? `<div class="flex justify-between"><span>Wärmepumpe:</span><strong>${formatInt(ergebnis.kostenWPNeu)} €</strong></div>` : ''}
          ${ergebnis.activeAuto ? `<div class="flex justify-between"><span>E-Auto:</span><strong>${formatInt(ergebnis.kostenAutoNeu)} € </strong></div>` : ''}
          <div class="flex justify-between" style="margin-top:8px; padding-top:8px; border-top:1px solid ${COLORS.blueSoft}; font-weight:900; color:${COLORS.blue};">
    ...
    ${hasPv ? `
      <div style="margin-bottom:10px; page-break-inside: avoid;">
        <div style="font-size:15px; font-weight:900; color:${COLORS.blue}; margin-bottom:14px; margin-top:10px;">Deine Gesamtbilanz auf einen Blick</div>
        <div class="grid grid-cols-3 gap-4" style="margin-bottom:24px;">
          <div style="border:1px solid #93c21c; border-radius:12px; padding:14px; text-align:center;">
            <div style="display:flex; justify-content:center; margin-bottom:10px;">${renderRing(ergebnis.autarkieQuote, COLORS.green)}</div>
            ...
            <div style="font-size:7.1pt; line-height:1.7; color:${COLORS.textSoft}; border-top:1px solid #d7e4ee; padding-top:7px;">
              <div class="flex justify-between"><span>Deckung</span><strong style="color:${COLORS.green};">${formatInt(ergebnis.genutzterPvStrom)} kWh</strong></div>
              <div class="flex justify-between"><span>Netzbezug</span><strong>${formatInt(ergebnis.restStromBezug)} kWh</strong></div>
            </div>
          </div>
          <div style="border:1px solid #93c21c; border-radius:12px; padding:14px; text-align:center;">
            <div style="display:flex; justify-content:center; margin-bottom:10px;">${renderRing(ergebnis.eigenverbrauchQuote, COLORS.green)}</div>
            <div style="font-size:7.8pt; font-weight:900; color:${COLORS.text}; text-transform:uppercase;">Eigenverbrauchsquote</div>
            <div style="font-size:6.6pt; color:${COLORS.text}; font-weight:700; margin-bottom:10px;">Nutzung des PV-Stroms</div>
            <div style="font-size:7.1pt; line-height:1.7; color:${COLORS.textSoft}; border-top:1px solid #d7e4ee; padding-top:7px;">
              <div class="flex justify-between"><span>Genutzt</span><strong style="color:${COLORS.blue};">${formatInt(ergebnis.genutzterPvStrom)} kWh</strong></div>
              <div class="flex justify-between"><span>Einspeisung</span><strong>${formatInt(ergebnis.netzEinspeisung)} kWh</strong></div>
            </div>
          </div>
          <div style="border:1px solid #93c21c; border-radius:12px; padding:14px; text-align:center;">
            <div style="display:flex; justify-content:center; margin-bottom:10px;">${renderRing(ergebnis.finanzAutarkieQuote, COLORS.green)}</div>
            ...
            <div style="font-size:7.1pt; line-height:1.7; color:${COLORS.textSoft}; border-top:1px solid #d7e4ee; padding-top:7px;">
              <div class="flex justify-between"><span>Ersparnis</span><strong style="color:${COLORS.green};">+${formatInt(annualSavingsNow)} €</strong></div>
              <div class="flex justify-between"><span>Vorher</span><strong>${formatInt(totalVorherJahr)} €</strong></div>
            </div>
          </div>
        </div>
      </div>
  ...
  // Page 2: Chart Dashboard (Only if PV is selected)
  let page2Html = '';
  if (hasPv) {
    // 1. Single Rings
    const singleRings = (ergebnis.saisonDaten || []).map((s) => `
      <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:10px; padding:10px;">
        <div style="display:flex; align-items:center; gap:5px; margin-bottom:8px;">
          <span style="width:7px; height:7px; border-radius:999px; background:${COLORS.green}; display:inline-block;"></span>
          <span style="font-size:7pt; font-weight:900; color:${COLORS.green}; text-transform:uppercase;">${escapeHtml(s.name)}</span>
        </div>
        <div style="display:flex; justify-content:center; margin-bottom:4px;">${renderRing(s.autarkie, COLORS.green, '#d7e4ee', 60, 5)}</div>
        ...
        <div style="font-size:6.5pt; line-height:1.6; color:${COLORS.textSoft}; border-top:1px solid #e2e8f0; padding-top:6px;">
          <div class="flex justify-between"><span>Bedarf:</span><strong>${formatInt(s.verbrauch)} kWh</strong></div>
          <div class="flex justify-between"><span style="color:${COLORS.green};">PV/Akku:</span><strong style="color:${COLORS.green};">${formatInt(s.covered)} kWh (${s.pctCovered}%)</strong></div>
          <div class="flex justify-between"><span>Zukauf:</span><strong>${formatInt(s.restbezug)} kWh (${s.pctRestbezug}%)</strong></div>
          <div class="flex justify-between" style="padding-top:4px; margin-top:4px; border-top:1px solid #e2e8f0;"><span style="color:${COLORS.blue};">Einsp.:</span><strong style="color:${COLORS.blue};">${formatInt(s.einspeisung)} kWh (${s.pctEinspeisung}%*)</strong></div>
        </div>
      </div>
    `).join('');

    // 2. Dual Rings WP vs Solar
    let dualRingsWp = '';
    if (hasBoth) {
      const dualRingsHtml = (ergebnis.saisonDaten || []).map((s) => {
        ...
        return `
          <div style="text-align:center;">
            ...
            <div style="display:flex; justify-content:center; margin-bottom:10px;">${renderDualRing(pctErtrag, pctHeiz, COLORS.green, COLORS.blue)}</div>
            <div style="font-size:7.2pt; line-height:1.65; color:${COLORS.textSoft}; text-align:left; display:inline-block; width:100%; max-width: 120px;">
              <div style="display:flex; justify-content:space-between; margin-bottom:2px;"><span>Ertrag:</span> <strong style="color:${COLORS.green};">${formatInt(s.ertrag)} kWh (${pctErtrag}%)</strong></div>
              <div style="display:flex; justify-content:space-between;"><span>Heizung:</span> <strong style="color:${COLORS.blue};">${formatInt(s.heizbedarf)} kWh (${pctHeiz}%)</strong></div>
            </div>
          </div>
        `;
      }).join('');
      dualRingsWp = `
        <div style="margin-bottom:10px; page-break-inside: avoid; margin-top:24px;">
          <div style="font-size:11pt; font-weight:900; color:${COLORS.green}; margin-bottom:7px; text-transform:uppercase;">Heizbedarf vs. Solarertrag</div>
          <p style="font-size:9pt; line-height:1.5; color:${COLORS.textSoft}; margin:0 0 12px 0;">Diese Gegenüberstellung zeigt deinen Wärmepumpen-Strombedarf und den PV-Ertrag im Jahresverlauf als Doppel-Ringdiagramm.</p>
          <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:14px; padding:16px;">
            <div class="grid grid-cols-4 gap-4" style="margin-bottom:12px;">${dualRingsHtml}</div>
            <div style="border-top:1px solid #d7e4ee; padding-top:12px; text-align:center; font-size:7.1pt; font-weight:900; color:${COLORS.text};"><span style="color:${COLORS.green}; font-size:10pt;">○</span> Äußerer Ring: Solarertrag &nbsp;&nbsp;&nbsp;&nbsp; <span style="color:${COLORS.blue}; font-size:10pt;">○</span> Innerer Ring: Heizbedarf WP</div>
          </div>
        </div>
      `;
    }

    // 3. Dual Rings Total vs Solar
    const totalRingsHtml = (ergebnis.saisonDaten || []).map((s) => {
      ...
      return `
        <div style="text-align:center;">
          <div style="font-size:8.4pt; font-weight:900; letter-spacing:.12em; color:${COLORS.text}; text-transform:uppercase; margin-bottom:8px;">${escapeHtml(s.name)}</div>
          <div style="display:flex; justify-content:center; margin-bottom:10px;">${renderDualRing(pctErtrag, pctGesamt, COLORS.green, COLORS.blue)}</div>
          <div style="font-size:7.2pt; line-height:1.65; color:${COLORS.textSoft}; text-align:left; display:inline-block; width:100%; max-width: 120px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:2px;"><span>Ertrag:</span> <strong style="color:${COLORS.green};">${formatInt(s.ertrag)} kWh (${pctErtrag}%)</strong></div>
            <div style="display:flex; justify-content:space-between;"><span>Gesamt:</span> <strong style="color:${COLORS.blue};">${formatInt(s.verbrauch)} kWh (${pctGesamt}%)</strong></div>
          </div>
        </div>
      `;
    }).join('');
    ...
    const dualRingsTotal = `
      <div style="margin-bottom:10px; page-break-inside: avoid; margin-top:24px;">
        <div style="font-size:11pt; font-weight:900; color:${COLORS.blue}; margin-bottom:7px; text-transform:uppercase;">Gesamtbedarf vs. Solarertrag</div>
        <p style="font-size:9pt; line-height:1.5; color:${COLORS.textSoft}; margin:0 0 12px 0;">Diese Gegenüberstellung zeigt deinen gesamten Strombedarf und deinen PV-Ertrag im Jahresverlauf.</p>
        <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:14px; padding:16px;">
          <div class="grid grid-cols-4 gap-4" style="margin-bottom:12px;">${totalRingsHtml}</div>
          <div style="border-top:1px solid #d7e4ee; padding-top:12px; text-align:center; font-size:7.1pt; font-weight:900; color:${COLORS.text};"><span style="color:${COLORS.green}; font-size:10pt;">○</span> Äußerer Ring: Solarertrag &nbsp;&nbsp;&nbsp;&nbsp; <span style="color:${COLORS.blue}; font-size:10pt;">○</span> Innerer Ring: Gesamtbedarf</div>
        </div>
      </div>
    `;

    page2Html = renderPageShell(`
      <div style="font-size:10pt; font-weight:900; color:${COLORS.blue}; text-transform:uppercase; margin-bottom:10px;">Der Verlauf über das Jahr</div>
      <div class="grid grid-cols-4 gap-3" style="margin-bottom:24px;">${singleRings}</div>
      <p style="font-size:7pt; color:${COLORS.textMute}; margin-top:-14px; margin-bottom:24px; text-align:center;">* Der Prozentwert der Einspeisung bezieht sich auf den gesamten PV-Ertrag der jeweiligen Jahreszeit.</p>
      ${dualRingsWp}
      ${dualRingsTotal}
    `, { title: '2. Saisonale Verteilung & Autarkie', pageNo: 2 });
  }

  // Page 3: Finances
  const page3Html = `
    <div class="grid ${hasPv ? 'grid-cols-3' : 'grid-cols-2'} gap-4" style="margin-bottom:18px;">
      <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:10px; padding:12px; text-align:center;">
        <div style="font-size:14pt; font-weight:900; color:${COLORS.blue}; line-height:1.1;">${escapeHtml(this.formatAmortisation(ergebnis))}</div>
      ...
      <div style="background:#f7fceb; border:1px solid ${COLORS.greenSoft}; border-radius:10px; padding:12px; text-align:center;">
        <div style="font-size:14pt; font-weight:900; color:${COLORS.green}; line-height:1.1;">${escapeHtml(ergebnis.roi)} %</div>
        <div style="font-size:6.8pt; font-weight:900; color:${COLORS.textMute}; text-transform:uppercase; letter-spacing:.08em; margin-top:4px;">Rendite p.a. (Start)</div>
      </div>
      ${hasPv ? `
        <div style="background:#f8fafc; border:1px solid #d7dee7; border-radius:10px; padding:12px; text-align:center;">
    ...
    <div class="grid grid-cols-2 gap-6">
      <div>
        <div style="font-size:8pt; font-weight:900; color:${COLORS.blue}; text-transform:uppercase; margin-bottom:8px; padding-bottom:4px; border-bottom:1px solid ${COLORS.blueSoft};">Investitionsübersicht</div>
        <div class="space-y-4">
          ${investmentCards.map(card => `
            <div style="background:#f8fafc; border:1px solid #d7dee7; border-left:3px solid ${card.accent}; border-radius:8px; padding:12px;">
              <div style="font-size:8pt; font-weight:900; color:${COLORS.text}; margin-bottom:8px;">${card.title}</div>
              <div style="font-size:7.2pt; line-height:1.7; color:${COLORS.textSoft};">
                <div class="flex justify-between"><span>${card.line1Label}</span><strong>${card.line1Value}</strong></div>
                <div class="flex justify-between"><span>${card.line2Label}</span><strong style="color:${COLORS.green};">${card.line2Value}</strong></div>
              </div>
              <div style="margin-top:7px; padding-top:7px; border-top:1px solid #d7dee7; font-size:7.5pt; font-weight:900; color:${card.accent}; display:flex; justify-content:space-between;">
                <span>${card.totalLabel}</span><span>${card.totalValue}</span>
              </div>
            </div>
          `).join('')}
          <div style="background:#e8f2fb; border:1px solid ${COLORS.blueSoft}; border-radius:8px; padding:10px 12px; display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:8pt; font-weight:900; color:${COLORS.blue};">Gesamtinvestition</span>
            <span style="font-size:10pt; font-weight:900; color:${COLORS.blue};">${formatInt(ergebnis.nettoInvestition)} €</span>
          </div>
        </div>
      </div>

      <div>
        <div style="font-size:8pt; font-weight:900; color:${COLORS.blue}; text-transform:uppercase; margin-bottom:8px; padding-bottom:4px; border-bottom:1px solid ${COLORS.blueSoft};">Betriebskosten-Verlauf</div>
        <div class="space-y-4">
          ${yearsList.map(y => {
            ...
            return `
              <div style="background:#f8fafc; border:1px solid ${COLORS.greenSoft}; border-right:3px solid ${COLORS.green}; border-radius:8px; padding:10px 12px;">
                <div style="display:flex; gap:10px; align-items:center;">
                  <div style="width:34px; height:34px; border-radius:999px; border:2px solid #d7dee7; background:#fff; display:flex; flex-direction:column; align-items:center; justify-content:center; flex-shrink:0;">
                    <span style="font-size:7pt; font-weight:900; color:${COLORS.blue}; line-height:1;">${y}</span>
                    <span style="font-size:4.8pt; font-weight:900; color:${COLORS.textMute}; text-transform:uppercase; line-height:1.1;">Jahr</span>
                  </div>
                  <div style="flex:1;">
                    <div class="grid grid-cols-2 gap-4" style="font-size:6.8pt; line-height:1.45;">
                      ...
                      <div style="text-align:right;"><div style="font-weight:900; color:${COLORS.textMute}; text-transform:uppercase;">Nachher</div><div style="font-size:8.5pt; font-weight:900; color:${COLORS.blue};">${formatInt(after)} €</div></div>
                    </div>
                    <div style="margin-top:5px; padding-top:5px; border-top:1px solid #d7dee7; display:flex; justify-content:space-between; align-items:center;">
                      <span style="font-size:6.8pt; font-weight:900; color:${COLORS.blue}; text-transform:uppercase;">Ersparnis</span><span style="font-size:8.5pt; font-weight:900; color:${COLORS.green};">+${formatInt(save)} €</span>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <div style="padding-top:18px; margin-bottom:18px;">
      <div style="font-size:10px; font-weight:900; color:${COLORS.green}; text-transform:uppercase; margin-bottom:10px;">Dein aktiver Klimaschutz</div>
      <p style="font-size:8pt; line-height:1.7; color:${COLORS.textSoft}; margin:0 0 14px 0; max-width:100%;">Durch deinen Systemwechsel sparst du massiv CO₂ ein und entziehst dich den kommenden CO₂-Strafsteuern auf fossile Brennstoffe. Hier ist deine Umweltbilanz:</p>
      <div class="grid grid-cols-4 gap-3" style="margin-bottom:14px;">
        <div style="background:#fff; border:1px solid ${COLORS.greenSoft}; border-radius:8px; padding:12px 8px; text-align:center;"><div style="font-size:6.7pt; font-weight:900; color:${COLORS.text}; text-transform:uppercase; letter-spacing:.08em; margin-bottom:6px;">Pro Jahr</div><div style="font-size:12.5pt; font-weight:900; color:${COLORS.green}; line-height:1;">${(ergebnis.co2ErsparnisPerYear || 0).toFixed(1).replace('.', ',')} t</div></div>
        <div style="background:#fff; border:1px solid ${COLORS.greenSoft}; border-radius:8px; padding:12px 8px; text-align:center;"><div style="font-size:6.7pt; font-weight:900; color:${COLORS.text}; text-transform:uppercase; letter-spacing:.08em; margin-bottom:6px;">10 Jahre</div><div style="font-size:12.5pt; font-weight:900; color:${COLORS.green}; line-height:1;">${((ergebnis.co2ErsparnisPerYear || 0) * 10).toFixed(1).replace('.', ',')} t</div></div>
        <div style="background:#fff; border:1px solid ${COLORS.greenSoft}; border-radius:8px; padding:12px 8px; text-align:center;"><div style="font-size:6.7pt; font-weight:900; color:${COLORS.text}; text-transform:uppercase; letter-spacing:.08em; margin-bottom:6px;">20 Jahre</div><div style="font-size:12.5pt; font-weight:900; color:${COLORS.green}; line-height:1;">${((ergebnis.co2ErsparnisPerYear || 0) * 20).toFixed(1).replace('.', ',')} t</div></div>
        <div style="background:#fff; border:1px solid ${COLORS.greenSoft}; border-radius:8px; padding:12px 8px; text-align:center;"><div style="font-size:6.7pt; font-weight:900; color:${COLORS.text}; text-transform:uppercase; letter-spacing:.08em; margin-bottom:6px;">30 Jahre</div><div style="font-size:12.5pt; font-weight:900; color:${COLORS.green}; line-height:1;">${((ergebnis.co2ErsparnisPerYear || 0) * 30).toFixed(1).replace('.', ',')} t</div></div>
      </div>
      <div style="background:#fff; border:1px solid ${COLORS.greenSoft}; border-radius:10px; padding:12px 14px; font-size:7.4pt; line-height:1.6; color:${COLORS.textSoft}; margin-bottom:26px;">
        Deine jährliche Einsparung entspricht der CO₂-Speicherkraft von <strong style="color:${COLORS.green};">${formatInt(ergebnis.co2Baeume || 0)} Bäumen</strong> oder einer Mischwaldfläche von <strong style="color:${COLORS.green};">${formatInt(ergebnis.co2FlaecheQm || 0)} m²</strong>.
      </div>
    </div>
  `;

  // Page 4: Offer / Gutschein
  const page4Html = `
    <div style="text-align:center; margin-bottom:22px;">
      <div style="justify-items:anchor-center;margin-bottom:22px;">
        ${brandLogo()}
      </div>
      <div style="font-size:17pt; font-weight:500; color:#63a6d9; margin-bottom:10px;">Gehe keine Kompromisse ein.</div>
      <p style="font-size:8.7pt; line-height:1.6; color:${COLORS.text}; max-width:420px; margin:0 auto;">Ein Systemwechsel ist eine Entscheidung für Jahrzehnte. Kontaktiere uns jetzt für einen unverbindlichen Vor-Ort-Termin und löse deinen persönlichen Gutschein ein.</p>
    </div>
    @include('admin.checklist.profitablity_calculation.bonus')
    <div style="font-size:6.9pt; color:#6b7b8c; text-align:center; max-width: 727px; text-align: justify; margin-top: 273px;">
    ...
    <div class="fixed inset-0 z-[100] bg-slate-900/90 overflow-y-auto flex flex-col items-center animate-fade-in">
      <div class="sticky top-0 w-full bg-slate-900/80 backdrop-blur-md p-4 flex justify-center gap-4 z-20 shrink-0">
        <button data-action="print-pdf-direct" class="bg-white text-slate-800 font-black py-3 px-6 rounded-xl flex items-center gap-2 hover:bg-slate-100 transition-colors">
            ${this.renderIcon('external-link', 'w-4 h-4')}
            Drucken
          </button>
      ...
      <div class="w-full pb-10 overflow-x-auto flex flex-col items-center pt-8">
        <div id="pdf-export-wrapper" class="bg-white text-left shadow-2xl mx-auto" style="width:794px; color:${COLORS.text};">
          ...
          ${renderPageShell(page4Html, { title: '4. Klimaschutz & Gutschein', pageNo: hasPv ? 4 : 3, noBreak: true })}
        </div>
      </div>
    </div>
  `;
  ...
  App.renderDashboard = function () {
    ...
    return ` 
        ...
        <main class="animate-fade-in relative z-10 max-w-7xl mx-auto px-4 pb-24">
          ...
          ${formData.includeWp ? `
            <div class="bg-white p-6 rounded-3xl border border-slate-200  relative overflow-hidden flex flex-col hover:border-[#74b2d4] hover:shadow-2xl transition-all">
              ...
              <div class="space-y-3 mt-auto">
                ...
                <div class="flex justify-between items-center text-sm text-[#93c21c] p-2">
                  <input type="text" data-model="wpZusatzFoerderName" placeholder="Zusatzförderung" value="${escapeHtml(formData.wpZusatzFoerderName)}" class="w-24 bg-transparent border-b border-dashed border-slate-300 font-bold outline-none focus:border-[#93c21c] placeholder:text-[#93c21c]/50 text-xs pb-0.5"/>
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="font-bold">-</span>
                    <input type="number" data-model="wpZusatzFoerderSumme" placeholder="0" value="${escapeHtml(formData.wpZusatzFoerderSumme)}" class="w-20 text-right bg-transparent border-b border-dashed border-slate-300 font-black outline-none focus:border-[#93c21c] pb-0.5"/>
          ...
          ${formData.includeSolar ? `
            <div class="bg-white p-6 rounded-3xl border border-slate-200  relative overflow-hidden flex flex-col hover:border-[#93c21c] hover:shadow-2xl transition-all">
              ...
              <div class="space-y-3 mt-auto">
                ...
                <div class="flex justify-between items-center text-sm text-[#93c21c] p-2 mt-2">
                  <input type="text" data-model="pvZusatzFoerderName" placeholder="Zusatzförderung" value="${escapeHtml(formData.pvZusatzFoerderName)}" class="w-24 bg-transparent border-b border-dashed border-slate-300 font-bold outline-none focus:border-[#93c21c] placeholder:text-[#93c21c]/50 text-xs pb-0.5"/>
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="font-bold">-</span>
                    <input type="number" data-model="pvZusatzFoerderSumme" placeholder="0" value="${escapeHtml(formData.pvZusatzFoerderSumme)}" class="w-20 text-right bg-transparent border-b border-dashed border-slate-300 font-black outline-none focus:border-[#93c21c] pb-0.5"/>
                    <span class="font-black">€</span>
                  </div>
                </div>
            ...
            <div class="bg-white p-6 rounded-3xl border border-slate-200  relative overflow-hidden flex flex-col hover:border-[#93c21c] hover:shadow-2xl transition-all">
              ...
              <div class="space-y-3 mt-auto">
                ...
                <div class="flex justify-between items-center text-sm text-[#93c21c] p-2 mt-2">
                  <input type="text" data-model="speicherZusatzFoerderName" placeholder="Zusatzförderung" value="${escapeHtml(formData.speicherZusatzFoerderName)}" class="w-24 bg-transparent border-b border-dashed border-slate-300 font-bold outline-none focus:border-[#93c21c] placeholder:text-[#93c21c]/50 text-xs pb-0.5"/>
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="font-bold">-</span>
                    <input type="number" data-model="speicherZusatzFoerderSumme" placeholder="0" value="${escapeHtml(formData.speicherZusatzFoerderSumme)}" class="w-20 text-right bg-transparent border-b border-dashed border-slate-300 font-black outline-none focus:border-[#93c21c] pb-0.5"/>
                    <span class="font-black">€</span>
                  </div>
                </div>
          ...
          ${(formData.includeSolar && ergebnis.activeAuto) ? `
            <div class="bg-white p-6 rounded-3xl border border-slate-200  relative overflow-hidden flex flex-col hover:border-[#74b2d4] hover:shadow-2xl transition-all">
              ...
              <div class="space-y-3 mt-auto">
                ...
                <div class="flex justify-between items-center text-sm text-[#93c21c] p-2 mt-2">
                  <input type="text" data-model="wallboxZusatzFoerderName" placeholder="Zusatzförderung" value="${escapeHtml(formData.wallboxZusatzFoerderName)}" class="w-24 bg-transparent border-b border-dashed border-slate-300 font-bold outline-none focus:border-[#93c21c] placeholder:text-[#93c21c]/50 text-xs pb-0.5"/>
                  <div class="flex items-center gap-1.5 shrink-0">
                    <span class="font-bold">-</span>
                    <input type="number" data-model="wallboxZusatzFoerderSumme" placeholder="0" value="${escapeHtml(formData.wallboxZusatzFoerderSumme)}" class="w-20 text-right bg-transparent border-b border-dashed border-slate-300 font-black outline-none focus:border-[#93c21c] pb-0.5"/>
                    <span class="font-black">€</span>
                  </div>
                </div>
      ...
      ${(formData.includeWp && formData.includeSolar) ? `
        <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl mt-8">
          ...
          <p class="text-sm text-slate-600 mb-8">
            ...
            Diese Gegenüberstellung visualisiert das dynamische Zusammenspiel deines Wärmepumpen-Lastprofils und deines PV-Ertrags im Jahresverlauf. Der äußere Ring (Grün) zeigt den Anteil des Solarertrags, der innere (Blau) den Anteil des Heizbedarfs am jeweiligen Jahresgesamtwert.
          </p>
      ...
      ${formData.includeSolar ? `
        <div class="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl mt-8">
          ...
          <p class="text-sm text-slate-600 mb-8">
            ...
            Diese Gegenüberstellung zeigt deinen gesamten Strombedarf und deinen PV-Ertrag im Jahresverlauf. Der äußere Ring (Grün) zeigt deinen Ertragsanteil in der jeweiligen Jahreszeit. Der innere Ring (Blau) zeigt, wie viel Prozent deines Jahres-Gesamtbedarfs in diese Zeit fallen.
          </p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          ${formData.includeSolar ? `
            ...
            <div class="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm">
              ...
              <h4 class="text-lg font-black text-slate-800 mb-3">LiFePO4 Batteriespeicher</h4>
              <ul class="text-sm text-slate-600 space-y-3 list-none">
...
App.handleModelUpdate = function (key, value) {
  ...
  const current = { ...this.state.formData, ...patch };

  let needsRender = false;

  if (key === 'personenAnzahl') {
    const val = Math.max(1, parseInt(next || '1', 10) || 1);
    patch.stromverbrauch = String(val * 1200 + 500);
    needsRender = true; // because another field changes visibly
  }

  if (key === 'heizungsArt' && ENERGIE[next]) {
    patch.heizPreis = String(ENERGIE[next].preis);
    patch.heizVerbrauch =
      next === 'Heizöl' ? '2500' :
      next === 'Pellets' ? '5000' :
      '20000';
    needsRender = true; // dependent labels/values change
  }

  if (key === 'wohneinheitenBewohnt') {
    const bewohnt = toNumber(next, 0);
    const bonus = toNumber(current.eigentuemerUnter40k, 0);
    if (bonus > bewohnt) {
      patch.eigentuemerUnter40k = String(bewohnt);
    }
    needsRender = true; // conditional block can appear/disappear
  }

  if (key === 'gebaeudeArt') {
    needsRender = true; // quiz layout changes
    if (next === 'Einfamilienhaus') {
      patch.wohneinheitenGesamt = '2';
      patch.wohneinheitenBewohnt = '1';
      if (toNumber(current.eigentuemerUnter40k, 0) > 1) {
        patch.eigentuemerUnter40k = '1';
      }
    }
  }
  ...
  // no full rerender while typing normal fields
  if (needsRender) {
    this.render();
  }

  // postcode lookup only when complete
...
App.bindEvents = function () {
  ...
  this.root.addEventListener('input', (e) => {
    ...
    if (el.matches('[data-project-name]')) {
      this.setState({ newProjectName: el.value });
    }
  });
