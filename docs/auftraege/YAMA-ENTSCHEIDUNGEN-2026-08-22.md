...
| 9 | Y-13 GRANT (Test-DB) | **ERLEDIGT ohne root — Stufe 1:** `ticket_g1b1_testing` über Socket (ticket_user@localhost hat ALL, 446 Tabellen), eine Test-DB, DB-Lease serialisiert, TEST_ROLLE Pflicht; ENV_BLOCKED-Ursache gemessen (.env.testing → ticket_testing über 127.0.0.1, dort nur ticket.*). Stufe 2 (vier DBs) optional mit root: GRANT auf BEIDE Hosts (localhost + 127.0.0.1) | gemessen 15:0x | Planner (Z0-I1 Stufe 1) | ja |
