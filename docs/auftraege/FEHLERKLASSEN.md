...
| **F-16** | **Das Werkzeug liest ein Papier als Auftrag, das keiner ist** — `auftrag-pruefen.mjs` wertet JEDEN `yaml`-Block in `docs/auftraege/` als Blatt aus, und PB-019 sucht `status:` + `aktiv` als reinen TEXT ueber die ganze Datei. `AUFTRAGSSCHEMA.md` (Doku, kein Auftrag) zaehlte dadurch als **zweites aktives Blatt** — S-01 sperrte jeden Verzeichnislauf — und der Validator fuhr seinen Beispielbefehl `npm run test:hausplaner` wirklich aus: ein npm-Gate, exit 1 als Dauerbefund, und der Grund, warum der Lauf ueber die volle Schlange in 45 s nicht durchkam | **1** | 01.08. 19:4x | **noch keine Barriere — heute nur eine Regel:** Doku-Papiere in `docs/auftraege/` benutzen ` ```text ` statt ` ```yaml ` und schreiben **keinen** Beispielwert `aktiv` hinter `status:`. Bei der zweiten Wiederholung kommt eine Ausnahme-Markierung ins Werkzeug, kein dritter Vorsatz (R9) | ❌ offen · *Planner-Fehler, vom eigenen Werkzeug beim ersten Verzeichnislauf gefunden* |
| **F-17** | **Ein unbekannter `typ:` verschwindet lautlos aus dem Bericht** — der Validator listet nur die Typen, die er kennt. Ein Kriterium mit `typ: verschoben` (oder einem Tippfehler wie `pressence`) taucht **gar nicht** auf: nicht als OK, nicht als NICHT MASCHINELL, nicht als Fehler. Gemessen 01.08.: Z-05 meldete nach dem Verschieben 7 Eintraege statt 9, ohne ein Wort dazu. **Ein Tippfehler im `typ` macht ein P1-Kriterium unsichtbar** | **1** | 01.08. 19:4x | **noch keine Barriere — heute nur eine Regel:** wer einen `typ` schreibt, prueft im Bericht nach, dass die Zahl der Eintraege stimmt. Die Barriere waere: unbekannter `typ` = Fehlschlag, nicht Schweigen | ❌ offen |
