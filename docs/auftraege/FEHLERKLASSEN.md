...
| **F-04** | **Zahl oder Artefakt behauptet statt gemessen** — ein Auftrag nennt eine Datei, Zusage oder Zahl, die niemand geprüft hat. **02.08.: DREI neue Vorkommen, und alle drei in PROSA statt in einem `ausgangswert`** — dort greift S-08 nicht. (a) Werkzeug-Bauplan: *„ein Werkzeug erreichbar machen kostet vier Stellen"* → gemessen sind **drei von vier schon gefüllt**, es fehlt nur die Registry. (b) W-01: *„60 Commits ungepusht"* → waren **13**. (c) **W-06 Ausschluss: *„ein Parser zöge eine Abhängigkeit in ein bisher abhängigkeitsfreies Werkzeug"* → `typescript 5.9.3` liegt in `node_modules`, steht in `package.json`, und `scripts/hook-abhaengigkeiten.mjs:70` ruft `ts.createSourceFile` bereits.** Alle drei gefunden von jemand anderem: (a) beim eigenen Nachmessen, (b) vom Evaluator, (c) **vom Generator, der VOR dem Bau gemessen hat — mein Fix hätte 2 von 61 Fällen gelöst** | **8** | 30.07. 06:25 · zuletzt 02.08. 16:0x | **R11** + **S-08** decken den `ausgangswert` ab und haben dort mehrfach getroffen. **Sie decken NICHT: Prosa im Fließtext und `ausschluesse[].grund`** — und genau dort sind alle drei neuen Fälle passiert. **Vorschlag S-13 (noch nicht geschnitten): ein `ausschluss`, dessen `grund` eine TATSACHE behauptet, braucht ein Feld `beleg:` mit einem Befehl; ein `grund`, der eine ENTSCHEIDUNG ist, wird als solche markiert.** *Der Unterschied ist der ganze Punkt: „wir wollen das hier nicht" braucht keinen Beleg, „das geht nicht" schon* | ⚠ **halb** — Barriere für `ausgangswert` steht seit 01.08. 19:1x; für Prosa und Ausschlussgründe steht sie nicht, und R9 ist mit drei Wiederholungen an einem Tag deutlich überschritten |
