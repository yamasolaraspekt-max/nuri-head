...
| **F-03** | **Messung älter als der Baum** — gemessen, dann minutenlang geschrieben, während andere committen | **3** | 29.07. 00:55 | **R14** — `git log -1` + `git status` **unmittelbar** vor dem Schreiben; bei Tafelzeilen zuerst prüfen, ob der Bauende sie schon gesetzt hat | ⚠ Regel, keine Barriere |
| **F-04** | **Zahl oder Artefakt behauptet statt gemessen** — ein Auftrag nennt eine Datei, Zusage oder Zahl, die niemand geprüft hat | **3** | 29.07. 08:29 | **R11** — jedes Kriterium trägt den Befehl, der es belegt, **und der Planner hat ihn gefahren**, bevor das Blatt liegt | ⚠ Regel, keine Barriere |
...
| **F-07** | **Bestand nicht gemessen, sondern nachgebaut** | **3** | 29.07. 08:15 | **R15** — vor jedem Entwurf die **Shell der Heimat-App** messen (Layout, Seitenleisten, Navigationseinträge, Klappzustände) **und** `docs/planner/` lesen | ⚠ Regel, keine Barriere |
...
| **F-08b** | **`status:` im Auftragsblatt bleibt stehen, wenn die Marke entsperrt** — die Tafel sagt `AKTIV`, das Blatt sagt `gesperrt` | **3** | 29.07. 12:22 | — | ❌ offen · *der Generator meldet es jedes Mal; das ist Bringschuld statt Barriere* |
...
| **F-12** | **Der vorlaufende Baum kostet eine Messung** — der Pruefende misst einen Stand, den der Bauende schon verlassen hat | **3** | 29.07. 21:00 | **R10-Zusatz** — eine Sperre endet mit dem Bau, **es sei denn, der Folgeauftrag zerstoert einen Pruefstand**, den die offene Abnahme braucht | ⚠ Regel · **beim dritten Mal wurde eine Messung vernichtet, nicht nur verzoegert** |
