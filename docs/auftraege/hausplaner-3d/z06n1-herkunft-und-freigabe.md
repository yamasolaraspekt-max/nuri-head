...
auftrag:
  ...
  status: gebaut   # NACHBESSERUNG 03.08. 21:5x auf den ROT-Befund des Evaluators (6eb3d3fe): die Insel hob auf v3, der Server stand auf v2, jeder Speicherversuch am Objekt endete in 422. Naht geschlossen - SpeichereHausplanerDokumentRequest 'in:2' -> 'in:3'. Roter Ausgangsstand selbst gemessen (2 failed/11 passed), danach 15 passed / 37 Zusicherungen. MUTATION 'in:3'->'in:2,3' kam beim ersten Anlauf DURCH: keine Zusage bewachte die Regel. Zwei neue Zusagen ergaenzt, danach faellt sie. OFFEN und NICHT meins: public/hausplaner/ traegt noch den v2-Bau (R18 - Sichtprobe beauftragt), und L-01 zeigt auf studio.blade, eine Flaeche OHNE Persistenz - die Sichtprobe gehoert auf objekt.blade. | NACHBESSERUNG 2 (03.08. 22:3x): K-N3 geschlossen - die Verdrahtungs-Zusage bewacht jetzt BEIDE Felder (Mutation 'freigabe hinter dem Spread' vom Evaluator selbst nachgestellt: kam bei 1667/0 durch; danach an Decke UND Dach gefangen, 3/3). K-N4 geschlossen - npm run build:hausplaner gefahren, Bundle traegt schemaVersion:3 3x, :2 0x, geometrieHerkunft 7, freigabe 9. ANMERKUNG zu K-N4: Bundle und Insel-Zeile liegen in ZWEI Commits (80c23274, dieser) - die Insel-Zeile war eine ZUSAGE, keine Quelle; die quellseitige v3-Hebung von 6d93fc97 ist es, die den Bau brauchte, und sie deckt sich jetzt mit ihm.
