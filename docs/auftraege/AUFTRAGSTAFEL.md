...
| Nr | Auftrag | Rolle | Status | Datei / Beleg |
|---|---|---|---|---|
| **AUF-70** | **Eine Werkzeugzeile — und der gesperrte Zustand wird ablesbar** (Wunsch Yama, 26.07.: *„alle Werkzeuge auf eine Linie … auch 2D, Split, 3D, Rückgängig"*, dazu *„Rückgängig und das Gegenteil funktionstüchtig machen"*). **Der zweite Teil entfällt: Rückgängig und Wiederholen funktionieren** — im laufenden Programm durchgespielt (Befehl ⇒ frei; Rückgängig ⇒ Wiederholen frei; Wiederholen ⇒ Rückgängig frei). **Der Befund ist die Darstellung:** ein gesperrter Knopf dieser Zeile ist von einem freien **nicht zu unterscheiden** — Deckkraft 1, Mauszeiger `pointer`, Schrift, Rahmen und Hintergrund gemessen identisch. `knopf()` kennt einen Zustand; **AUF-59 hat die drei Zustände nur für `OpBtn` hergestellt.** Dazu die Zusammenlegung: heute stehen Werkzeuge in zwei Zeilen (y 179 und y 269) | Evaluator | `BERICHTET` — **`4c1ce13` + Bundle `0045ea2`, 26.07., 09:27.** *(Bericht im Ledger steht noch aus — der Posten ist gebaut, die Übergabe fehlt.)* **Vom Planner unabhängig im Browser nachgemessen, 1440 px:** die Werkzeugzeile trägt **16** Knöpfe in **einer** Zeile (vorher 11 in y 269 plus 5 in y 179); oben stehen nur noch Geschosse und Speichern. **Der gesperrte Zustand ist ablesbar:** Rückgängig gesperrt = Deckkraft **0,6** · `not-allowed` · Schrift rgb(167,174,183) · Grund rgb(242,244,246) gegen Split frei = **1** · `pointer` · rgb(35,42,49) · rgb(255,255,255) — **vier** unterschiedliche Werte, vorher **keiner**. Dritter Zustand vorhanden (2D aktiv trägt Markenfarbe). **Kein Überlauf:** `docOverflow` = 0. **Abstands-Kriterium erfüllt:** 38 px innerhalb der Gruppen gegen 52–57 px zwischen ihnen — der Abstand trägt die Gliederung weiter. Generator meldet Tests 1020 → **1033**, Mutation **6 rot** | `generator-auftrag-auf70-eine-werkzeugzeile.md` · Commit-Nachricht `4c1ce13` |
