...
| **AUF-21** | **Werkzeug-Paket einsortieren (I1–I4)** — 110 SVGs nach `public/hausplaner/icons/tools/`; Katalog-Austausch mit Adapter Paket→`ToolDefinition` (Konflikt-Regel: der neue Code passt sich dem Bestand an); die 47 DTP-Reste belegt stilllegen; `canPin`/`priority` in die Zonen-Kuratierung führen. **Gesperrt bis AUF-20** | Planner → Generator | ⚡ **AKTIV — I4** · I1 `UMGESETZT` (`7bbf9ff`, Abnahme offen) · I2 `BERICHTET` (`289ccc8`) · I3 `BERICHTET` (`ccdc93b`) · **I4 `IN ARBEIT — Generator (nativ)`: die 110 sichtbar machen** — Auftrag `generator-auftrag-i4-werkzeuge-sichtbar.md`, Vorbedingung AUF-31 erfüllt | `docs/planner/inventur-werkzeug-icons-2026-07-25.md` §7 | **Neue Priorität 25.07.: I2 + I3 sind Schritt 3 und 4 zur fertigen Werkzeugleiste — Vorrang vor L2/L3.**
