...
**Eine Frage, und sie blockiert die Kette nicht.** Sie steht hier, weil keine Instanz sie in Yamas
Vertretung entscheidet (§5).

| Nr | Frage | Was daran haengt | Was ich empfehle |
|---|---|---|---|
| **W-Projektliste** | **Darf der Controller die Liste der eigenen Objekte an die Insel durchreichen?** Gemessen: `HausplanerController::seite()` reicht `objekt`, `dokument`, `uebernahme` und seit AUF-69 `hpRechte` durch — **keine Objektliste**. `StartView` nimmt sie seit Teil A als Eigenschaft entgegen, **Grundwert leer** | **Der Startbildschirm zeigt heute ehrlich „noch kein Projekt geoeffnet" — auch dann, wenn du zwanzig Projekte hast.** Ehrlich, aber nutzlos. Mit der Zulieferung wird aus dem Leerzustand eine echte Liste, und „Weiterarbeiten" bekommt sein Ziel | **Ja, freigeben.** Es ist eine Variable mehr in einer Methode, die schon vier durchreicht — **derselbe Mechanismus, kein neuer.** Kein Schema, keine Migration, keine Route. **Der Generator hat es ausdruecklich nicht nebenbei gebaut**, obwohl es eine Zeile ist: *Tor 1 ueberspringt man nicht, weil etwas klein ist.* Das war richtig — und deshalb liegt die Frage jetzt hier statt im Diff |
