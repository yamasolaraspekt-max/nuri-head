...
| **AUF-52** | **L3 — die übrigen zwölf Rechen-Engines nach dem Treppen-Muster** (`engine-fbh · -heizkoerper · -heizkreis · -abwasser · -kueche · -pv · -uwert · -fensterprodukt · -sparren · -holzmengen · -holzbauteile · -schifter`). Das Muster ist abgenommen (AUF-33 L2, Votum `7293a2d`): `EnginePanel → EngineFlaeche → Fachflächen-Hülle → vorhandene Engine`. **Gruppenweise, nicht alle zwölf auf einmal** — Vorschlag: `dach-zimmerei` zuerst (Sparren · Holzmengen · Holzbauteile · Schifter, vier mit Render-Bezug), dann Haustechnik, dann der Rest. **Die drei Grenzen aus AUF-33 §3 gelten unverändert:** keine Rechenlogik im Panel, kein dynamischer Import, kein Schreiben ins Modell. Jede Engine schaltet erst auf `verfuegbar`, wenn sie wirklich angeschlossen ist | Planner → Generator | **`SCHEIBE C BERICHTET — wartet auf Evaluator`** — Code `97e941a1` + Bundle `7cf7f6ae`; **vier von fuenf angeschlossen**, `engine-uwert` zurueckgegeben (Schichtliste **mit Lambda**, die es im Modell nicht gibt). **L3 damit durch: acht von dreizehn Engines haben eine Flaeche, fuenf sind mit Messung zurueckgegeben.** Gates 0/0/0/0 + `test:dom` 0; Insel 1278 auf 1288, PHP 789; Mutation 1 rot. · `SCHEIBE B BERICHTET — wartet auf Evaluator` — Code `b5af4196` + Bundle `890a3f70`; **zwei von drei angeschlossen** (`engine-fbh`, `engine-heizkoerper`), **`engine-heizkreis` zurueckgegeben** (Listeneingang). Gates 0/0/0/0 + `test:dom` 0; Insel 1268 auf 1278, PHP 789; Mutation 1 rot. Nach B: **vier verfuegbare Engines, vier Flaechen — exakt gleich** (K6). Scheibe A (freigegeben `497215c6`): — Code `644d7be` + Bundle `e47ef915`. **Eins von vier angeschlossen** (`engine-sparren`), **drei begruendet zurueckgegeben**: zweimal fehlt der Eingang (eine Holzliste gibt es im Modell nicht — `grep` ueber `domain`/`store`/`app` = 0), einmal die Ergebnisform. Gates 0/0/0/0 + `test:dom` 0; Insel 1256 auf 1268, PHP 789; **Mutation erst nach Reparatur des eigenen Tests 2 rot**. Sichtprobe 1440 und 375, kein Ueberlauf. `sichtbar`. Auftrag liegt: **drei Scheiben** (dach-zimmerei 4 · tga-heizung 3 · Rest 5), jede einzeln abgenommen. Gemessen: **alle zwölf Module existieren**. Ausdrücklich erwartet, dass nicht alle zwölf anschließbar sind — begründete Rückgabe ist ein gültiges Ergebnis **Scheibe A (dach-zimmerei) ABGENOMMEN** (Votum `497215c6`, FREIGABE): `engine-sparren` angeschlossen, drei mit Messung zurueckgegeben — **anderer Fehlertyp als "Empfaenger fehlt": diesen dreien fehlt der Eingang.** Scheibe B (tga-heizung) laeuft. **Scheibe B (tga-heizung) BERICHTET:** 2 von 3 (`engine-fbh`, `engine-heizkoerper`), `engine-heizkreis` zurueck — **dritter Fall Listeneingang.** Scheibe C offen. **ALLE DREI SCHEIBEN BERICHTET — L3 durch:** 8 von 13 Engines haben eine Flaeche, 5 begruendet zurueck (4x Listeneingang, 1x Ergebnisform). | `generator-auftrag-auf52-l3-zwoelf-engines.md` |
| **AUF-38** | **Inline-Styles ablösen** (Entscheidung Yama, 25.07., aus AUF-14) — 331 `style={{` in 35 Dateien wandern in eine echte Stilschicht; `build:hausplaner` erzeugt künftig `public/hausplaner/hausplaner.css`, die vorhandene `@if (file_exists(…))`-Bewachung in beiden Blades greift dann von selbst. **In Scheiben, nach Datei geschnitten** (HausplanerApp 132 · GuidedView 41 · ConfigWizard 39 · HausplanerStudio 34 · FachFlaeche 27 · StartView 20 · Rest 38) — nie zwei Scheiben gleichzeitig, weil dieselben Dateien der Werkzeugleiste gehören. **Wert- und verhaltenstreu:** kein gerenderter Farbwert ändert sich, `studioDaten.ts` bleibt die Quelle | Generator | `⚡ AKTIV` · **`SCHEIBE 2 IN ARBEIT — Generator (nativ)`** (`StartView.tsx`) · `SCHEIBE 1 ABGENOMMEN` (Votum `ba47815`, FREIGABE); laeuft erst nach AUF-52, ein Bauender je Posten (§13) — Code `cca1837` + Artefakte `022021f` (`hausplaner.js` **und** die neu entstandene `hausplaner.css`, nach K8 im selben Commit). Gates 0/0/0/0 + `test:dom` 0; Insel 1246 auf 1256, PHP 789; Mutation 2 rot. **Kriterium 9 buchstaeblich belegt: dieselbe Seite mit und ohne die neue CSS, drei Viewports, Bildschirmfotos sha256-verglichen — PIXELGLEICH.** Keine Bau-, keine Blade-Aenderung noetig; **keine einzige `style`-Stelle umgestellt** (Scheiben 2-8 offen) **Er laeuft neben nichts** — acht Oberflaechendateien, darunter `HausplanerApp.tsx`. Auftrag liegt: **acht Scheiben**, Scheibe 1 (Grundgerüst) wird eigens abgenommen. **Scheibe 7 ist entsperrt** — AUF-35a ist abgenommen; sie bleibt trotzdem die letzte, weil `HausplanerApp.tsx` auch AUF-43/45/48 trägt **Marke 26.07., 23:20:** AUF-52 hat fertig gebaut, **Scheibe 2 ist frei und wird gezogen.** **Mit Yamas Entscheid fuer Weg B ist AUF-38 kein Aufraeumposten mehr, sondern der tragende:** die Token-Quelle, zu der der Rest ziehen soll. | `generator-auftrag-auf38-inline-styles.md` |
...
|---|---|---|---|---|
| **AUF-52 Sch. C** | 26.07., 23:14 | Code `97e941a1` · Artefakt `7cf7f6ae` | **Zuerst die Plaketten-Regel — die einzige Erweiterung der Huelle.** Sie wird nur gezeigt, wenn die Engine ein Bestehens-Merkmal liefert; `berechneUw` und `pvSchnellBelegung` rechnen Werte aus und **bestehen nichts**. Pruefen: erscheint dort wirklich **keine** Plakette, bei Abwasser und Kueche eine? *Eine Plakette „nicht bestanden" waere eine erfundene Bewertung* — der gefaehrlichere Fehler als eine fehlende. **Danach die Rueckgabe `engine-uwert`:** er sagt, `WallNode.schichten` fuehre kein Lambda. **Gegenprobe des Planners: `app/Models/Material.php` fuehrt `lambda_w_mk` (Migration 05.07.).** Es fehlt also kein Feld, sondern ein Nachschlag ueber `materialId`. **Zuletzt** K8/K6. §13.6 gilt. | Evaluator |
| **AUF-52 Sch. B** | 26.07., 22:55 | Code `b5af4196` · Artefakt `890a3f70` | **Zuerst die Umbenennung, sie ist die einzige Stelle mit Bedeutung.** `bewerteDeckung` nennt sein Merkmal **`ausreichend`**, die Huelle liest **`bestanden`**. Er sagt: unveraendert durchgereicht, nur unter anderem Namen. **Die Frage ist nicht, ob der Wert gleich bleibt, sondern ob die beiden Woerter dasselbe bedeuten** — „ausreichend" ist eine Aussage ueber eine Deckung, „bestanden" eine ueber eine Pruefung. Wurde dabei ein Grenzwert erfunden? Gegen-Beweis: Wert am Ursprung verfaelschen und sehen, ob er ungefiltert ankommt. **Danach:** ob die beiden Seiten des K8-Vergleichs wirklich getrennt laufen (er sagt, er habe es nach dem Fehler aus Scheibe A bewusst getrennt gefuehrt) — und die Rueckgabe `engine-heizkreis`. §13.6 gilt. | Evaluator |
