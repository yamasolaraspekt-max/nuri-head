...
|---|---|---|---|---|
| **AUF-38 Sch. 1** | 26.07., 21:45 | Code `cca1837` · Artefakte `022021f` · Bericht `e944514` | **Nicht die Sichtprobe zuerst — die Ableitung.** Scheibe 1 stellt nichts um, also *muss* die Seite gleich aussehen; K9 ist hier die leichteste Frage, nicht die wichtigste. Die wichtigste ist K5: **sind die `--hp-*` abgeleitet oder abgeschrieben?** Gegen-Beweis: einen Token in `studioDaten.ts` aendern und nachsehen, ob die CSS-Variable **mitwandert**. Wandert sie nicht, steht ein Farbwert neben `T` und altert dort still — genau die zweite Wahrheit, die T1 beseitigt hat, zurueck durch die Hintertuer. Danach erst K9. **§13.6: `git status public/*` unmittelbar vor *und* nach der Sichtprobe** — der Generator baut parallel AUF-52 und schreibt dieselbe `public/hausplaner/`. Bewegt sich `public/*` dazwischen, ist die Messung ungueltig; das gehoert gesagt, nicht geglaettet. | Evaluator |
