...
| Nr | Auftrag | Rolle | Status | Datei / Beleg |
|---|---|---|---|---|
| **AUF-69** | **Die Rechteberechnung zieht aus dem Blade in den Controller** — der Weg, den mein AUF-64-Auftrag entschieden hatte und den der Generator aus gemessenem Grund nicht gegangen ist. **Kein Fehler mehr, sondern Aufräumen:** `auth()->user()`, eine `collect()`-Kette und vier `hasPermission`-Aufrufe sind Anwendungslogik und gehören nicht in eine Vorlage; `HausplanerController::seite()` reicht ohnehin schon drei Variablen durch. Im Blade bliebe **eine** Zeile `data-rechte="{{ $hpRechte }}"` | Evaluator | `BERICHTET` — **`ea60d9e`, 26.07., 01:50** (Commit-Titel nennt AUF-64; es ist dieser Posten). Vier Dateien: Controller, Blade, `rechte.test.ts`, neuer `HausplanerRechteTest.php`. **PHP-Suite `tests/Feature/Hausplaner` 38 → 44**, alle grün — die Blade-Regel §9 ist damit im ersten Posten nach ihrer Aufnahme eingehalten. **`public/*`: null Zeilen** (selbst nachgemessen), kein Bundle nötig. Im Blade steht nur noch `data-rechte="{{ $hpRechte }}"` (Z. 107). **Zu prüfen mit Vorrang: ohne angemeldeten Nutzer bleibt der Wert leer, nicht voll** — das ist der Kern von AUF-60, und Umzüge sind die Gelegenheit, bei der so etwas still verschwindet | Ledger „PLANNER 26.07. — Evaluator-Votum AUF-60" |
