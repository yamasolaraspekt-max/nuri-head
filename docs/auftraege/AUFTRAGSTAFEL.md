...
| **AUF-82** | **Der Wächter heilt sich ohne `rm`, und sein Scheitern wird sichtbar** — aus der Messung des Evaluators vom 26.07., 13:53 (`1a8f43e`), **nicht aus einer Vermutung**. Gemessen: 65 Aufrufe, davon **45 echte Prüfläufe** und **3 stumme** (`OHNE lebenden Halter`, exit 2). **Schaden bis heute null** — alle drei stummen tragen keinen Commit. **Aber:** der Hook schluckt den exit 2 vollständig (`>/dev/null 2>&1 &` + `exit 0`), der eigens gebaute Fehlerausgang erreicht den einzigen realen Auslöse-Pfad nie. Ursache ist eine Umgebung, kein Denkfehler: `erobern()` und der Trap benutzen `rm -rf`, und über die Cowork-Brücke ist `unlink` verboten | Generator | `⚡ AKTIV` — **gezogen 26.07., 16:45: AUF-78 ist berichtet, die Marke rueckt** (§1c: sie wandert beim Bericht, nicht bei der Abnahme). **Spur A, klein.** Zwei unabhängige Stücke: **(a)** `mv` statt `rm -rf` (auf dem Mount belegt erlaubt), Drei-Fälle-Logik Zeichen für Zeichen unverändert — **es ändert sich das Werkzeug, nicht das Verhalten**; **(b)** ein Wächter-Ende ≠ 0 hinterlässt eine auffindbare Spur, **ohne** dass der Hook blockiert oder etwas anderes als 0 zurückgibt. **Die stehende Sperre (pid 76) wird nicht von Hand geräumt** — sie ist der Prüfstein. **Kein Einwand gegen die AUF-80-Abnahme:** die Logik stimmt, geprüft wurde sie im `/tmp`-Mini-Repo, wo `rm` erlaubt ist | `generator-auftrag-auf82-waechter-unlinkfrei.md` |
