...
| **AUF-54** | **Farbe als Parameter statt in `geometry/`** (Entscheidung Yama, 25.07., aus AUF-5 und AUF-10) — `geometry/treppeSvg.ts` führt heute sechs rohe Farbwerte, darunter `#93c21c` für die Lauflinie, und wird an **neun Stellen** ohne Farbparameter aufgerufen. Die Farbe wird künftig hereingereicht; `studioDaten.ts` bleibt die einzige Quelle. **Wert- und verhaltenstreu:** der gerenderte Farbwert bleibt zunächst derselbe, nur seine Herkunft ändert sich | Generator | `⚡ AKTIV` — **IN ARBEIT — Generator (nativ)**, gezogen 26.07., Auftragsdatei vollstaendig gelesen (108 Zeilen, 3 Posten; nur AUF-54 traegt die Marke). **Warum nicht AUF-77:** dessen Sperre lautet woertlich *bis AUF-76 **abgenommen** ist* — AUF-76 ist **berichtet**, nicht abgenommen. **§7.6: Bedingungen werden nicht im Moment ausgelegt.** Auftrag liegt. `BERICHTET — wartet auf Evaluator` — Code `29e83f4`; **kein Bundle-Commit noetig, das Artefakt ist bytegleich** (belegt per sha256) — der Beweis, dass sich nichts Ausgeliefertes aendert. Gates 0/0/0/0, Insel 1143→1154, PHP 789 unveraendert; rohe Farbwerte in `treppeSvg.ts` **0**; Byte-Gleichheit ueber **vier** Treppenarten gegen `d8038bf`; Mutation ⇒ 6 rot. `Vorarbeit`. **Auftragspraemisse korrigiert: nicht neun Aufrufstellen, sondern zwei** — beide im Test, `treppeAlsSvg` wird aus dem ausgelieferten Code gar nicht aufgerufen | `generator-auftrag-auf54-55-56-klein.md` |
