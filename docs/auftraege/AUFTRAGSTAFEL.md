...
**20** Arbeitsvorrat · **2** Abnahme · **1** bei Yama · **36** im Archiv — Summe geprüft, 59.
...
|---|---|---|---|---|
| **AUF-51** ⚡ **AKTIV** | **Die Zeichenfläche ist verschiebbar erklärt, aber der Verschub wird verworfen** (Playwright meldete eine react-konva-Warnung; **Ursache selbst gemessen**) — `HausplanerApp.tsx:1280` setzt am `<Stage>` `draggable`, **ohne jeden Drag-Handler**. Die vier Node-Ebenen (Z. 1332/1386/1481/1521) haben `onDragStart` und `onDragEnd`, **die Bühne selbst hat keinen**. Zugleich sind `x={80}` und `y={hoehe - 80}` **gesteuerte Werte ohne Zustand dahinter** (`grep` nach `setPan`, `panX`, `panY`: **leer**), und `onMouseMove` ruft `setCursor(...)` — **jede Mausbewegung erzeugt ein Rendern**, das die Bühne auf `x=80` zurücksetzt. Der Nutzer schiebt, und es springt zurück. `weltPunkt` (Z. 626-634) liest `stage.x()`, also die **echte** Position — für die Dauer des Zurückspringens widersprechen sich Anzeige und Koordinate | Generator | `OFFEN` ⚡ — **AKTIV.** Kein Layout-Posten, ein Richtigkeitsfehler: entweder Pan-Zustand einführen oder `draggable` an der Bühne entfernen, bis es einen gibt | Ledger „Playwright-Audit nachgemessen" (25.07.) |
