...
| **AUF-68** ⚡ **AKTIV** | **Die drei Gruppenwörter aus der Icon-Zeile entfernen** (Wunsch Yama, 26.07., wörtlich: *„kannst du die Wörter Ansicht, Messen & Export sowie Bearbeiten weg machen"*) — gemessen sind es **genau drei Aufrufe** einer Hilfsfunktion (`HausplanerApp.tsx:1144, 1160, 1170`) und die Funktion selbst (`:651`), die **keinen weiteren Aufrufer** hat. **Die Gruppierung geht dabei nicht verloren:** die Trennstriche (`opSep`) liegen bereits zwischen den Gruppen. Rund **150 px** der Zeile werden frei | Generator | `OFFEN` ⚡ — **gezogen, klein, Auftrag liegt.** Bedingung, unter der das Wort gehen darf: **der Name bleibt als `aria-label` an einer `role="group"`-Umhüllung**, damit die Gruppen für Vorleseprogramme nicht verschwinden. Kriterien: `grep -c "opLbl"` = 0; Knopfzahl unverändert (11, davon 3 gesperrt); **keine Sperre geändert**; Sichtprobe in der Abnahme | `generator-auftrag-auf68-gruppenwoerter.md` |
| **AUF-69** | **Die Rechteberechnung zieht aus dem Blade in den Controller** — der Weg, den mein AUF-64-Auftrag entschieden hatte und den der Generator aus gemessenem Grund nicht gegangen ist. **Kein Fehler mehr, sondern Aufräumen:** `auth()->user()`, eine `collect()`-Kette und vier `hasPermission`-Aufrufe sind Anwendungslogik und gehören nicht in eine Vorlage; `HausplanerController::seite()` reicht ohnehin schon drei Variablen durch. Im Blade bliebe **eine** Zeile `data-rechte="{{ $hpRechte }}"` | Generator | `GESPERRT` — **bis AUF-64 committet und AUF-60 abgenommen ist.** Vorher wäre es eine zweite Änderung an derselben zerbrechlichen Datei. Ohne Dringlichkeit: die Route ist mit AUF-64 heil, dieser Posten macht sie nur wartbar. Kriterium, das nicht verlorengehen darf: **ohne angemeldeten Nutzer bleibt der Wert leer**, nicht voll | Ledger „PLANNER 26.07. — Evaluator-Votum AUF-60" |
...
**Zwei Posten, und sie hängen zusammen.** AUF-60 wartet nur noch, weil AUF-64 seinen Fehler
behoben hat; wer AUF-64 prüft, hat AUF-60 halb mitgeprüft. Niemand nimmt eigene Arbeit ab (§1.4).
