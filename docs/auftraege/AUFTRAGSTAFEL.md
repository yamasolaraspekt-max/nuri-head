...
|---|---|---|---|---|
| **AUF-66** | 26.07., 17:34 | Code `c3de05a` · Bundle `3d54ad0` | **Ein Klick genuegt — und zwar der richtige.** Zuerst Kriterium 2: traegt jeder Eintrag die Adresse des **eigenen** Objekts (`id 203` ⇒ Ziel enthaelt 203, `id 7` ⇒ **nicht** 203)? Der haeufigste Fehler solcher Listen ist die **geteilte** Adresse. Dann **K6-Mutation**: Adresse aus dem Blade entfernt ⇒ der Eintrag darf **keine Schaltflaeche** mehr sein — **kein Ziel, kein Versprechen**. Dazu: **die Insel baut keine URL** (`grep '/objekt/'` in `resources/planner/` = 0), Tastatur (Enter **und** Leertaste, Fokusring, erster in der Tastfolge), Leerzustand aus AUF-40 Teil A zeichengleich, und die Sichtprobe bei **1024×768** ohne Scrollen | Evaluator |
