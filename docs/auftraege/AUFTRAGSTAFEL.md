...
| **AUF-67** | **Die Befehlspalette wird globale Navigation** (UX-Bewertung 26.07., P1) — sie funktioniert (⌘K, Gründe bei gesperrten Befehlen, Kürzel, Sofortsuche) und **erfasst heute nur Werkzeuge**. Sie soll auch Projekte, Geschosse, Wizard-Schritte, Fachplaner, offene Prüfungen und Ansichten finden — „Dach" führt dann zum Dachwerkzeug **und** zum Dachschritt **und** zu vorhandenen Dachobjekten. **Dazu die Lücke aus AUF-49:** Fokusfalle und Fokus-Rückgabe fehlen für **diese** Palette noch (die drei Dialoge sind versorgt) | Generator | `⚡ AKTIV` · `BERICHTET — wartet auf Evaluator` — Code `5bcca43` + Bundle `5e090cf`; Gates 0/0/0/0 + `test:dom` 0; Insel 1216 auf 1231, PHP 789. **Fuenf Arten, je ein Register, Mutation je Art belegt.** Sichtprobe 1024x768 mit offener Palette: „dach" trifft Werkzeug UND Bauteil, Ueberstand 0, kein Ueberlauf. `sichtbar`. **Zurueckgegeben: die Fokusfalle fehlt DIESER Palette weiter** (AUF-49-Luecke aus der Gegenstandsspalte, in den Abnahmekriterien des Auftrags nicht enthalten) **Der Controller nennt genau diesen Posten als Prioritaet 6** — Projekte, Geschosse, Wizard-Schritte, offene Pruefungen, Objekte: **das ist wortgleich der Auftrag, der seit 19:20 liegt.** Sperre gefallen, gemessen 26.07., 19:20: AUF-65 ist abgenommen und im Archiv** — die Bedingung lautete *erst wenn AUF-65 die Aufgaben liefert*. **Auftrag liegt jetzt vor** (er fehlte, obwohl die Zeile seit dem 25.07. steht). Unveraendert gilt: **eine Quelle je Art**: die Palette darf nichts eigenes wissen, sie fragt die vorhandenen Register ab. Erst wenn AUF-65 die Aufgaben liefert, gibt es etwas zu finden | `generator-auftrag-auf67-palette-navigation.md` |
