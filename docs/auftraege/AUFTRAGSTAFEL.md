...
**17** Arbeitsvorrat · **7** Abnahme · **10** bei Yama · **19** im Archiv — Summe geprüft, 53.
...
| **AUF-51** | **Die Zeichenfläche ist verschiebbar erklärt, aber der Verschub wird verworfen** (Playwright meldete eine react-konva-Warnung; **Ursache selbst gemessen**) — `HausplanerApp.tsx:1280` setzt am `<Stage>` `draggable={werkzeug === 'auswahl'}`, **ohne jeden Drag-Handler**. Die vier Node-Ebenen (Z. 1332/1386/1481/1521) haben `onDragStart`+`onDragEnd`, **die Bühne selbst hat keinen**. Zugleich sind `x={80}` und `y={hoehe - 80}` **gesteuerte Werte ohne Zustand dahinter** (`grep` auf `setPan|panX|panY` = **leer**), und `onMouseMove` ruft `setCursor(...)` — **jede Mausbewegung erzeugt ein Rendern**, das die Bühne auf `x=80` zurücksetzt. Der Nutzer schiebt, und es springt zurück. `weltPunkt` liest `stage.x()`, also die **echte** Position — für die Dauer des Zurückspringens widersprechen sich Anzeige und Koordinate | Generator | `OFFEN` — **kein Layout-Posten, ein Richtigkeitsfehler.** Entweder Pan-Zustand einführen (`onDragEnd` → State → `x`/`y`) oder `draggable` an der Bühne entfernen, bis es einen gibt. Der Zwischenzustand — verschiebbar aussehen und nicht sein — ist der schlechteste | Ledger „Playwright-Audit nachgemessen" (25.07.) |
