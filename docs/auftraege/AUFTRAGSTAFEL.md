...
| **AUF-35b** | **Flächen- und Zonenauswahl** — Wandseite und einzelne Dachfläche greifen (fehlt vollständig); Zone markieren für die **6** Schema-Typen, von denen das Paket nur `raum` kennt und die Commands **keinen**. **Die Naht zu den 13 Engines.** Braucht AUF-35a als Fundament | Generator | `⚡ AKTIV` · `BERICHTET — wartet auf Evaluator` — Code `1164321` + Bundle `7fd8749`; **halber Posten mit Begruendung** (Auftrag §6 sieht diesen Ausgang vor). Gebaut: Teil-Identitaet rein und ohne DOM, kein Schema/Command/Persistenz (null Treffer in `domain`/`store`/`geometry`/`renderers`), Mutation 1 rot. Gates 0/0/0/0 + `test:dom` 0; Insel 1231 auf 1246, PHP 789. **Zurueckgegeben: das 3D-Picking kann ohne Aenderung an `renderers/` keine Teil-Information liefern** (`szene.ts:659-661` reduziert sofort auf `nodeId`) **und es gibt kein Werkzeug, das Flaechen waehlt** — ohne Ausloeser nichts sichtbar. Deshalb `Vorarbeit` statt `sichtbar`, **Kriterium 10 ausdruecklich NICHT erfuellt** Zonen fallen weg: kein Zone-Command, keine Darstellung — **man kann nicht auswählen, was nicht dargestellt wird.** Kern ist die fehlende **Teil-Identität**: Auswahl ist heute knotenweise (`userData.nodeId`), die zwei Wandseiten sind implizit, `surfaceId` lebt nur **innerhalb** von `dachAusschnitt.ts`. Gebaut wird eine **abgeleitete** Teil-Kennung in der App-Schicht — **kein Schema, kein Command, keine Persistenz** | `generator-auftrag-auf35b-flaechenauswahl.md` |
