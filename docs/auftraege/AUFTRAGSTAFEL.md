...
**17** Arbeitsvorrat · **4** Abnahme · **0** bei Yama · **43** im Archiv — Summe geprüft, 64.
...
| Nr | Auftrag | Rolle | Status | Datei / Beleg |
|---|---|---|---|---|
| **AUF-53** | **Import-Recht `Hausplaner,import` einführen** (Entscheidung Yama, 25.07., aus AUF-41) — gemessen kennt `routes/web.php` heute nur `Hausplaner,read` und `Hausplaner,update`; die **acht** Import-Werkzeuge sind deshalb dauerhaft gesperrt mit dem Grund „Keine Berechtigung zum Importieren." Das neue Recht in die Rechteverwaltung, an die Import-Routen hängen, und die Fähigkeit an `AktivierungsKontext.permissions` reichen — **die Vorbedingung `permission.import` steht bereits im Funktionsvertrag** und geht von selbst auf grün. **Kein Werkzeug wird freigeschaltet, das das Recht nicht hat** | Planner → Generator | `BERICHTET — wartet auf Evaluator` — umgesetzt `b4e5f03`, Bundle `581f457`, 956/956. **Tor 1 nicht berührt:** `routes/` und `database/migrations/` sind **null Zeilen**. Die Falle ist bestätigt; zugeordnet ist `Hausplaner,add`. Gemessen 79 → 71 gesperrte Werkzeuge. **§4 zurückgegeben: die Insel kennt keine Nutzerrechte, das Recht wird nicht durchgereicht** — eigener Posten | `generator-auftrag-auf53-import-recht.md` + `evaluator-auftrag-stapel-2026-07-25.md` |
| **AUF-49** | **Dialog-Fokus und Tastatur unvollständig** (externe Bewertung Nr. 6, nachgemessen) — `FachFlaeche` hat `role="dialog"`, `aria-modal` und Escape, aber **keinen Fokuswechsel beim Öffnen, keine Fokusfalle, keine Fokus-Rückgabe beim Schließen**. `ConfigWizard.tsx:56` trägt die Dialogsemantik gar nicht. Dazu gemessen: **8×** `role="button"`, **10×** `key === "Enter"`, **1×** Leertaste — selbstgebaute Schaltflächen reagieren fast durchweg nicht auf die Leertaste (WCAG 2.1.1) **Playwright-Lauf 25.07., nachgemessen:** `ConfigWizard.tsx` hat **weder `role="dialog"` noch `aria-modal` noch einen Escape-Handler** (`grep` = 0 Treffer). Fokus bleibt nach dem Öffnen auf dem Hintergrundknopf, der erste Tab-Sprung landet **hinter** dem Dialog, Escape schließt nicht, Fokus wird weder eingeschlossen noch zurückgegeben. **Zum Vergleich:** `FachFlaeche.tsx:125,139` (AUF-33, heute gebaut) hat Escape, `role="dialog"` und `aria-modal` — die neue Schicht ist richtig, die Schuld liegt in der alten. Dazu **Zielgrößen**: Chips 27–40 px, CAD-Schaltflächen ~30×32 px (WCAG 2.5.5 verlangt 44). | Generator | `BERICHTET` — umgesetzt `f83cf11`, Bundle `c4e8cc4`. `dialogFokus.ts` einmal für alle drei Dialoge: Fokus hinein, Tab im Kreis, Escape, Fokus zurück. ConfigWizard hatte **gar keine** Dialogsemantik — jetzt `role`/`aria-modal`/Escape. Leertaste an **sieben** Stellen ergänzt (WCAG 2.1.1). Tore 0/0/0/0, 971→982, Mutation 3 rot. Sichtprobe: alle vier Zusagen einzeln belegt. **Zielgrößen (WCAG 2.5.5) zurückgegeben** — Layout-Posten, würde AUF-59 wieder aufmachen. **Ballbesitz → Evaluator** | `evaluator-auftrag-stapel-2026-07-25.md` |
| **AUF-59** | **Die Icon-Zeile mischt drei Zustände, die man nicht auseinanderhalten kann** (Yama, 25.07.: „einige Icon, die sind eingerahmt, manche sind aktiv manche nicht"). **Gemessen bei 1440 px:** 15 quadratische Knöpfe in einer Zeile, 9 gesperrt. Drei Zustände — **(1)** Schalter EIN: grüner Rand, grüner Grund (Raster, Fang) · **(2)** bedienbar: weiß, grauer Rand, dunkles Icon · **(3)** gesperrt: weiß, grauer Rand, blasses Icon. **Zustand 2 und 3 unterscheiden sich ausschließlich in der Icon-Farbe** (`rgb(35,42,49)` gegen `rgb(167,174,183)`) und im Cursor; Rahmen, Hintergrund und Deckkraft sind **identisch**. Die Zeile liest sich als Reihe gleichwertiger Kästchen. **Dazu die Dublette:** „Grundriss spiegeln" existiert **zweimal** — als Icon mit Tooltip in dieser Zeile (32x30) und als ausgeschriebener Textknopf im Eigenschaften-Panel (117x43, **ohne** `title`). Yamas Punkt: die Icon-Form gibt es bereits, der Text ist die Dopplung | Generator | `BERICHTET — wartet auf Evaluator` — umgesetzt `8f34fc5`, Bundle `ece8e43`. Drei Zustände jetzt an **drei** Merkmalen unterscheidbar statt an einem; der Rahmen trägt nur noch den **Schalter**, die Text-Dublette im Panel ist den vorhandenen Icons gewichen. **Keine Sperre geändert** — Sichtprobe bei 1440 gemessen | Planner-Sichtprobe 25.07. 23:10 + `evaluator-auftrag-stapel-2026-07-25.md` |
| **AUF-44** | **Zwei „(geplant)"-Knöpfe und das Spiegeln im leeren Plan** (Befund B2) — `title` sagt es selbst: „Auswahl um 90° drehen **(geplant)**", „Als PDF-Planblatt exportieren **(geplant)**". Dieselbe Sorte falsches Versprechen, die I2 aus dem Katalog entfernt hat, in der Icon-Zeile stehengeblieben. Dazu nehmen „Grundriss spiegeln links/rechts + oben/unten" den prominentesten Platz im Eigenschaften-Panel ein — bei **0 Räumen · 0.00 m²** | Generator | `BERICHTET — wartet auf Evaluator` — umgesetzt `47addd1`, Bundle `0bde0d9`. **Die Tafelzeile nannte zwei, gemessen waren es fünf.** Vier davon sind **Dubletten** (`drehen`, `distanz-messen`, `bemassen`, `pdf` stehen in ihrer Themen-Gruppe mit ehrlichem Zustand) — entfernt wurde die tote Kopie, **nicht das Werkzeug**; Bilanz unverändert 110. Icon-Zeile **15 → 11**. Der fünfte („Ansicht einpassen") hat kein Gegenstück und wird als Willensfrage zurückgegeben | `docs/planner/ux-befund-layout-alle-ebenen-2026-07-25.md` + `evaluator-auftrag-stapel-2026-07-25.md` |
...
**43 Posten**: AUF-1 · AUF-2 · AUF-3 · AUF-4 · AUF-9 · AUF-11 · AUF-12 · AUF-14 · AUF-15a · AUF-16 ·
