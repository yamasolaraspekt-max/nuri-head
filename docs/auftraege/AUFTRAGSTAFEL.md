...
| **AUF-70** ⚡ **AKTIV** | **Eine Werkzeugzeile — und der gesperrte Zustand wird ablesbar** (Wunsch Yama, 26.07.: *„alle Werkzeuge auf eine Linie … auch 2D, Split, 3D, Rückgängig"*, dazu *„Rückgängig und das Gegenteil funktionstüchtig machen"*). **Der zweite Teil entfällt: Rückgängig und Wiederholen funktionieren** — im laufenden Programm durchgespielt (Befehl ⇒ frei; Rückgängig ⇒ Wiederholen frei; Wiederholen ⇒ Rückgängig frei). **Der Befund ist die Darstellung:** ein gesperrter Knopf dieser Zeile ist von einem freien **nicht zu unterscheiden** — Deckkraft 1, Mauszeiger `pointer`, Schrift, Rahmen und Hintergrund gemessen identisch. `knopf()` kennt einen Zustand; **AUF-59 hat die drei Zustände nur für `OpBtn` hergestellt.** Dazu die Zusammenlegung: heute stehen Werkzeuge in zwei Zeilen (y 179 und y 269) | Generator | `IN ARBEIT — Generator (nativ)` — gezogen 26.07., AUF-68 ist freigegeben (373dfe9) | `generator-auftrag-auf70-eine-werkzeugzeile.md` |
