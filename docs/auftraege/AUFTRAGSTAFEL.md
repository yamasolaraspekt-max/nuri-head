...
| **AUF-62** ⚡ **AKTIV** | **„Ansicht einpassen" bauen** (Entscheidung Yama, 25.07., aus AUF-61) — der einzige der fünf toten „(geplant)"-Knöpfe **ohne Gegenstück im Katalog**; die anderen vier waren Dubletten und sind mit AUF-44 entfernt. Er soll die Ansicht so setzen, dass der **gesamte Grundriss des aktiven Geschosses** sichtbar ist. **Technisch klein:** es braucht kein Modell und keinen Command — nur die Bounding-Box der Knoten des Geschosses, den vorhandenen Zoom und den **Pan-Zustand aus AUF-51** (`74fdcb4`), der genau dafür jetzt existiert. **Kein Schreiben ins Dokument** — die Ansicht ist Anzeige, kein Modellzustand | Generator | `OFFEN` ⚡ — **gezogen 26.07., 07:40. Spur A** (der Posten rechnet Bounding-Box, Maßstab und Verschub — das ist Logik, kein Markup). **Auftrag liegt.** Gemessen: alles Nötige ist vorhanden — `zoom` (`:348`), `pan` aus AUF-51 (`:354`), `bbox()` in `geometry/editierGeometrie.ts:63`. Es fehlt **eine Rechnung und ihre Verdrahtung**. Die Kanten stehen im Auftrag, darunter zwei, die man beim Bauen nicht vor Augen hat: **leeres Geschoss ⇒ kein Sprung** und **Split ⇒ einpassen in `stageBreite`, nicht in `breite`** | `generator-auftrag-auf62-ansicht-einpassen.md` |
