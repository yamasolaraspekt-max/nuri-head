...
**17** Arbeitsvorrat · **3** Abnahme · **2** bei Yama · **41** im Archiv — Summe geprüft, 63.
...
|---|---|---|---|---|
| **AUF-49** ⚡ **AKTIV** | **Dialog-Fokus und Tastatur unvollständig** (externe Bewertung Nr. 6, nachgemessen) — `FachFlaeche` hat `role="dialog"`, `aria-modal` und Escape, aber **keinen Fokuswechsel beim Öffnen, keine Fokusfalle, keine Fokus-Rückgabe beim Schließen**. `ConfigWizard.tsx:56` trägt die Dialogsemantik gar nicht. Dazu gemessen: **8×** `role="button"`, **10×** `key === "Enter"`, **1×** Leertaste — selbstgebaute Schaltflächen reagieren fast durchweg nicht auf die Leertaste (WCAG 2.1.1) **Playwright-Lauf 25.07., nachgemessen:** `ConfigWizard.tsx` hat **weder `role="dialog"` noch `aria-modal` noch einen Escape-Handler** (`grep` = 0 Treffer). Fokus bleibt nach dem Öffnen auf dem Hintergrundknopf, der erste Tab-Sprung landet **hinter** dem Dialog, Escape schließt nicht, Fokus wird weder eingeschlossen noch zurückgegeben. **Zum Vergleich:** `FachFlaeche.tsx:125,139` (AUF-33, heute gebaut) hat Escape, `role="dialog"` und `aria-modal` — die neue Schicht ist richtig, die Schuld liegt in der alten. Dazu **Zielgrößen**: Chips 27–40 px, CAD-Schaltflächen ~30×32 px (WCAG 2.5.5 verlangt 44). | Generator | `OFFEN` ⚡ — **AKTIV.** Gute Reihenfolge: die Icon-Zeile ist gerade angefasst worden (AUF-59), die Zielgrößen gehören dazu. Umfang: `ConfigWizard` bekommt Dialogsemantik (heute **0** Treffer für `role="dialog"`, `aria-modal`, Escape), Fokusfalle und Fokus-Rückgabe; die eigenen Schaltflächen reagieren auf die **Leertaste** (gemessen 8× `role="button"`, 10× Enter, **1×** Leertaste); Zielgröße 44 px statt 32×30 | Ledger „Externe Frontend-Bewertung" (25.07.), nachgemessen |
...
**41 Posten**: AUF-1 · AUF-2 · AUF-3 · AUF-4 · AUF-9 · AUF-11 · AUF-12 · AUF-14 · AUF-15a · AUF-16 ·
