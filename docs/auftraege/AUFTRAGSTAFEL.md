...
| **AUF-70** | **Eine Werkzeugzeile — und der gesperrte Zustand wird ablesbar** (Wunsch Yama, 26.07.: *„alle Werkzeuge auf eine Linie … auch 2D, Split, 3D, Rückgängig"*, dazu *„Rückgängig und das Gegenteil funktionstüchtig machen"*). **Der zweite Teil entfällt: Rückgängig und Wiederholen funktionieren** — im laufenden Programm durchgespielt (Befehl ⇒ frei; Rückgängig ⇒ Wiederholen frei; Wiederholen ⇒ Rückgängig frei). **Der Befund ist die Darstellung:** ein gesperrter Knopf dieser Zeile ist von einem freien **nicht zu unterscheiden** — Deckkraft 1, Mauszeiger `pointer`, Schrift, Rahmen und Hintergrund gemessen identisch. `knopf()` kennt einen Zustand; **AUF-59 hat die drei Zustände nur für `OpBtn` hergestellt.** Dazu die Zusammenlegung: heute stehen Werkzeuge in zwei Zeilen (y 179 und y 269) | Generator | `GESPERRT` — **bis AUF-68 committet ist.** Beide fassen dieselben Zeilen an; zwei gleichzeitige Änderungen an derselben Stelle sind der Fehler von heute Nacht. Kriterien: gesperrt unterscheidet sich in **mindestens einem** gemessenen Wert (heute: keinem); Mauszeiger nicht `pointer`; **eine** Wahrheit über Zustände, keine zweite neben AUF-59; Sperr-Menge identisch; `store/history.ts` **null Zeilen**; kein waagerechter Überlauf bei 1440 **und** 1024 mit dann 16 statt 11 Knöpfen | `generator-auftrag-auf70-eine-werkzeugzeile.md` |
| **AUF-69** | **Die Rechteberechnung zieht aus dem Blade in den Controller** — der Weg, den mein AUF-64-Auftrag entschieden hatte und den der Generator aus gemessenem Grund nicht gegangen ist. **Kein Fehler mehr, sondern Aufräumen:** `auth()->user()`, eine `collect()`-Kette und vier `hasPermission`-Aufrufe sind Anwendungslogik und gehören nicht in eine Vorlage; `HausplanerController::seite()` reicht ohnehin schon drei Variablen durch. Im Blade bliebe **eine** Zeile `data-rechte="{{ $hpRechte }}"` | Generator | `OFFEN` — **entsperrt am 26.07., 01:40:** AUF-64 ist committet **und** freigegeben (`f43bc14`), AUF-60 mit ihm. Beide Vorbedingungen erfüllt. **Kein Tor 1:** die Rechteberechnung aus einer Vorlage in ihren Controller zu ziehen ist Software-Struktur, keine Fachentscheidung — die Rechte-Wahrheit (`User::hasPermission`) wird **nicht** angefasst. *(Der Generator hat Tor 1 vermutet; das war vorsichtig und ist hiermit beantwortet.)* **Die Arbeit liegt bereits im Arbeitsbaum** (`HausplanerController.php` M, `HausplanerRechteTest.php` neu). Kriterien: **ohne angemeldeten Nutzer bleibt der Wert leer**, nicht voll; `data-rechte` trägt denselben Wert wie vorher; `BladeKompiliertTest` bleibt grün; die PHP-Suite läuft mit (§9 Blade-Regel) | Ledger „PLANNER 26.07. — Evaluator-Votum AUF-60" |
