...
| Nr | Auftrag | Rolle | Status | Datei / Beleg |
|---|---|---|---|---|
| **AUF-62** | **„Ansicht einpassen" bauen** (Entscheidung Yama, 25.07., aus AUF-61) — der einzige der fünf toten „(geplant)"-Knöpfe **ohne Gegenstück im Katalog**; die anderen vier waren Dubletten und sind mit AUF-44 entfernt. Er soll die Ansicht so setzen, dass der **gesamte Grundriss des aktiven Geschosses** sichtbar ist. **Technisch klein:** es braucht kein Modell und keinen Command — nur die Bounding-Box der Knoten des Geschosses, den vorhandenen Zoom und den **Pan-Zustand aus AUF-51** (`74fdcb4`), der genau dafür jetzt existiert. **Kein Schreiben ins Dokument** — die Ansicht ist Anzeige, kein Modellzustand | Evaluator | `BERICHTET` — Code `bae4596` + Bundle `38a855e`, Bericht `e634e73`. Neu `app/dashboard/einpassen.ts` — rein, ohne DOM, ohne Store, ohne Befehl; `bbox()` **gelesen**, `geometry/` null Zeilen. **Alle sechs Kanten beantwortet**, darunter die zwei, die ich benannt hatte: Split passt in `stageBreite` (Sichtprobe belegt den **Unterschied**: 2D 4 % → 7 %, Split 4 % → **3 %** — mit `breite` stünden beide auf 7 %), und das y-Vorzeichen ist mit zwei Seitenverhältnissen geprüft. Tests **1034 → 1051**; Mutationen: Rand→0 **1 rot**, `stageBreite`→`breite` **1 rot**, Vorzeichen gedreht **7 rot**. `public/*` im Code-Commit **0**. **Drei geerbte Zusagen offengelegt und neu formuliert statt gestrichen** — die AUF-44-Zusage verriegelt jetzt, dass die Zahl der `geplant`-Knöpfe **nicht wieder steigt**. **Rückgabe: die Bühne ragt unter das Fenster — als AUF-72 beauftragt** | `generator-auftrag-auf62-ansicht-einpassen.md` |
| **AUF-72** | **🔴 Die Bühne ist höher als das Fenster** — **Rückgabe des Generators aus AUF-62**, von mir unabhängig nachgemessen. `HausplanerApp.tsx:1058` rechnet `window.innerHeight - 96`; die **96** stammt aus einer Zeit mit **einer** Leiste über der Bühne. Seither sind Arbeitsbereich-Wähler (AUF-34), Werkzeugzeile und Optionszeile dazugekommen — **gemessen stehen heute 323–369 px darüber**. **Generator bei 1440×900: 227 px unter dem Fenster, 72 % sichtbar. Planner bei 1440×813: 273 px, nur 62 % sichtbar** — und die Seite scrollt nur 46 px, der Rest ist **unerreichbar**. Je kürzer das Fenster, desto schlimmer; Laptops sind kürzer als 900 px | Evaluator | `BERICHTET` — Code `2e56fcb` + Bundle `c5a7ee5`, Bericht `c23ec6a`. **Überstand in allen drei Formaten 0** (1440×900: 227 → **0** · 1440×813: 273 → **0** · 1024×768: **0**), keine Seite scrollt. Die Höhe kommt jetzt von der **Inhaltsreihe**, die die Bühne trägt — **es gibt keine Zahl mehr, die jemand nachpflegen müsste.** `ResizeObserver` **am Element**, nicht nur `resize` am Fenster: erscheint oben eine Zeile, ändert sich das Fenster nicht — der reine Fenster-Zuhörer hätte ausgerechnet den Fall verpasst, der den Fehler erzeugt hat. Kanten beantwortet: Ersatzhöhe **700** bei gemessener 0, Mindesthöhe **200**, `null` bleibt von `0` unterschieden, **Verschub des Nutzers überlebt unverändert**, kein Flackern (Reihe trägt `overflow: hidden`, Zustand wird nur bei echter Änderung gesetzt). Tests **1051 → 1060**, keine Zusage verschwunden. **K8 belegt: nach „Ansicht einpassen" steht der ganze Grundriss im *sichtbaren* Bereich** — damit leistet AUF-62, was es versprochen hat | `generator-auftrag-auf72-buehnenhoehe.md` |

### 3c. Bei Yama — Willensfragen

**Eine Frage, und sie blockiert die Kette nicht** — sie hält nur einen einzelnen Beleg offen.
Sie steht hier, weil keine Instanz sie in Yamas Vertretung entscheidet (§5).

| Nr | Frage | Was daran hängt | Was ich empfehle |
|---|---|---|---|
| **W-Login** | **Ein funktionierender lokaler Zugang zu `/admin/hausplaner/objekt/{id}`.** Der im Wissensstand hinterlegte Zugang `admin@ticket.test` wird abgewiesen — *„These credentials do not match our records."* | **Die Konsolenprüfung im echten Browser** für jeden Posten, der die Objekt-Route berührt — zuletzt AUF-64. Serverseitig ist die Route belegt (HTTP 200, keine Fehlerspuren); was fehlt, ist der Blick in die Browser-Konsole | **Du gibst einen Zugang, oder du legst den Nutzer selbst an.** Der Generator hat es ausdrücklich **gelassen** — einen Nutzer anzulegen wäre ein Schreibvorgang auf der Arbeits-Datenbank `ticket`, und das ist kein Test-Beifang. Das war richtig. **Bis dahin gilt: Posten an der Objekt-Route werden serverseitig belegt, und die Konsolenprüfung wird als offen benannt — nicht stillschweigend übersprungen** |
