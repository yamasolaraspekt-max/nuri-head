...
| **AUF-67** | **Die Befehlspalette wird globale Navigation** (UX-Bewertung 26.07., P1) — sie funktioniert (⌘K, Gründe bei gesperrten Befehlen, Kürzel, Sofortsuche) und **erfasst heute nur Werkzeuge**. Sie soll auch Projekte, Geschosse, Wizard-Schritte, Fachplaner, offene Prüfungen und Ansichten finden — „Dach" führt dann zum Dachwerkzeug **und** zum Dachschritt **und** zu vorhandenen Dachobjekten. **Dazu die Lücke aus AUF-49:** Fokusfalle und Fokus-Rückgabe fehlen für **diese** Palette noch (die drei Dialoge sind versorgt) | Generator | `OFFEN` — **Sperre gefallen, gemessen 26.07., 19:20: AUF-65 ist abgenommen und im Archiv** — die Bedingung lautete *erst wenn AUF-65 die Aufgaben liefert*. **Auftrag liegt jetzt vor** (er fehlte, obwohl die Zeile seit dem 25.07. steht). Unveraendert gilt: **eine Quelle je Art**: die Palette darf nichts eigenes wissen, sie fragt die vorhandenen Register ab. Erst wenn AUF-65 die Aufgaben liefert, gibt es etwas zu finden | `generator-auftrag-auf67-palette-navigation.md` |
| **AUF-63** | **jsdom für den Testlauf** (Bestandsmeldung des Evaluators, 26.07., Frage 3) — AUF-30 hat `.tsx` im Testlauf übersetzbar gemacht, **jsdom fehlt aber weiter**. Folge, von ihm benannt: **Fokusfalle und `getComputedStyle` sind nur im Browser messbar, nicht im Gate.** Nicht blockierend — die iframe-Sichtprobe trägt — aber jedes Fokus- und Zustands-Kriterium hängt damit an einer Person mit offenem Browser statt an einem Testlauf. **Zu prüfen ist auch, ob es sich lohnt:** jsdom ist eine Abhängigkeit mehr, und die Testumgebung läuft heute bewusst schlank | Generator | `⚡ AKTIV` — **IN ARBEIT — Generator (nativ)**, gezogen 26.07., Auftragsdatei vollstaendig gelesen (100 Zeilen, 6 Abschnitte). **Laeuft allein** — er aendert den Testlaeufer selbst, und wer daneben misst, misst mit einem Geraet, das gerade umgebaut wird. **Auftrag liegt, Preis vorher gemessen.** jsdom kann **Fokus, `getComputedStyle` und Tastatur** — aber `getBoundingClientRect().width` ist dort **0**: **keine Layout-Engine.** Alle Geometrie bleibt Browser-Sache. Preis 39 Pakete / 27 MB (happy-dom 9 / 23 MB als zulässige Alternative). **Der DOM-Testlauf setzt seine Grenze selbst durch** | `generator-auftrag-auf63-jsdom.md` |
...
| **AUF-42** | **`viewport.ready` ist heute eine Vereinfachung** — die Fähigkeit wird gesetzt, sobald `HausplanerApp` rendert; einen echten Renderer-Bereitschaftszustand führt der Store nicht. Der Generator hat das bei AUF-36 offengelegt statt es für gemessen auszugeben. **Folgenlos, solange es keine Ladeanimation gibt** — der Posten existiert, damit die Vereinfachung nicht später für eine Messung gehalten wird | Generator | `OFFEN` — **Auftrag liegt jetzt vor.** **Gemessen 26.07.:** `FAEHIGKEIT_ANSICHT_BEREIT` steht **ohne Bedingung** in der Liste (`HausplanerApp.tsx:435`) — die Faehigkeit sagt **immer ja**, der Grundtext *Die Zeichenflaeche ist noch nicht bereit* wird **nie gezeigt**, und **fuenf** Werkzeuge tragen eine Vorbedingung, die nichts prueft. **Der erste Schritt ist eine Messung, kein Bau** — und **den Posten zu schliessen ist ein zulaessiger Ausgang**: eine Vorbedingung, die nie eintritt, ist eine Luege in die andere Richtung. Frueher gesperrt: klein, ohne Dringlichkeit; erst wenn ein Ladezustand sichtbar wird | `generator-auftrag-auf42-ansicht-bereit.md` |
...
|---|---|---|---|---|
| **AUF-77** | 26.07., 19:40 | Code `a331fce`, **kein Bundle** (bytegleich — eine reine Rechen-Datei, die noch niemand aufruft, erscheint nicht im Buendel) | **Zuerst K4 — der Typfehler und wie er belegt ist.** Zur Laufzeit laesst sich Unmoeglichkeit nicht pruefen, und die Testdateien sind aus `tsconfig` **ausgenommen**, ein `@ts-expect-error` dort waere wirkungslos. Er hat eine **Typprobe mit fremder Endung** gebaut, die im Test kopiert und durch `tsc --noEmit` geschickt wird — **der Lauf ist gruen, WEIL der Fehler eintritt**; Mutation `bezug` optional ⇒ 1 rot. Dann: **K5 Handrechnung als Zahl** (12,5 / 1,68 / **10,82 m²**) · **K8 genau eine Rundungsstelle** · **sechs Meldefaelle, keiner liefert eine Zahl**, darunter der selbst ergaenzte (**fremde Oeffnung**: `hostWallId` zeigt woanders hin ⇒ gemeldet statt stillschweigend abgezogen) · **Ueberlappung in zwei Achsen** — Tuer + Oberlicht rechnen 2,4 m² und melden **nicht** | Evaluator |
...
| **AUF-56.1** | Auflage aus dem Votum `e715b03` | — | **Dieselbe ausstehende Live-Sichtprobe wie 55.1, hier fuer die zwei Fussleisten** (Ueberlauf 1440 · 1024 · 375). Der Evaluator schlaegt vor, sie **mit 55.1 zu buendeln** — angenommen: **ein Browserlauf statt zwei**, dieselbe Fenstergroessen, dieselbe Grundlinie. Die Substanz beider Posten ist bereits belegt; offen ist allein der Ueberlauf-Blick. **Faellig vor dem naechsten Merge nach `main`** | Evaluator |
