...
Ist der aktive Posten fertig **und abgenommen**, rückt die Markierung auf den nächsten der Kette.
...
| **AUF-31** ⚡ **AKTIV** | **IDs der 110 eindeutschen** — `werkzeugPaket.ts` trägt die englischen Paket-IDs (`u-value`, `thermal-envelope`, `floor-heating` …), weil die Namenstabelle bei I2 noch uncommittet war. Vorlage: `docs/planner/eindeutschung-110-paket-ids.md` (führend). Umbenennen von `werkzeugPaket.ts` + den 110 Icon-Dateien; die 9 Bestands-IDs bleiben byte-genau; die 16 schema-gebundenen behalten ihren englischen Schutzwert im Adapter | Generator | `OFFEN` — **entsperrt 25.07., Planner-Entscheidung Weg 1: zusammenführen.** Die 9 kollidierenden Paket-Einträge entfallen, ihre Metadaten wandern additiv in die Registry; Umfang 101 statt 110 IDs. Nachtrag im Auftrag | Planner-Befund 1 zu I2 |
