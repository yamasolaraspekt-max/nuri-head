...
**10** Arbeitsvorrat · **6** Abnahme · **10** bei Yama · **18** im Archiv — Summe geprüft, 44.
...
> Sperren nannten eine Reihenfolge, die nicht gemessen war (AUF-38/39/40) — sie nennen jetzt den echten
> Grund: **die Auftragsdatei fehlt, und die schulde ich.**
...
| **AUF-38** | **Inline-Styles ablösen** (Entscheidung Yama, 25.07., aus AUF-14) — 331 `style={{` in 35 Dateien wandern in eine echte Stilschicht; `build:hausplaner` erzeugt künftig `public/hausplaner/hausplaner.css`, die vorhandene `@if (file_exists(…))`-Bewachung in beiden Blades greift dann von selbst. **In Scheiben, nach Datei geschnitten** (HausplanerApp 132 · GuidedView 41 · ConfigWizard 39 · HausplanerStudio 34 · FachFlaeche 27 · StartView 20 · Rest 38) — nie zwei Scheiben gleichzeitig, weil dieselben Dateien der Werkzeugleiste gehören. **Wert- und verhaltenstreu:** kein gerenderter Farbwert ändert sich, `studioDaten.ts` bleibt die Quelle | Planner → Generator | `GESPERRT` — **Auftragsdatei fehlt (Planner)**, und die Umsetzung muss hinter AUF-35a bleiben: `HausplanerApp.tsx` trägt 132 der 331 Stellen und ist dieselbe Datei, die „Markieren" anfasst. **Das ist die einzige gemessene Kollision** unter den Layout-Posten | Ledger-Block „Zwei Willensentscheidungen" (25.07.) |
| **AUF-39** | **L5 — Wizard-Schritte aus dem Modell ableiten** statt hartkodierter Status/Prüfungen. Steht seit `5af3e18` im Layout-Fahrplan und hatte bis 25.07. **keinen Posten** — gemessen: null Treffer auf `L5` in dieser Tafel. Guardrail aus dem Fahrplan: an vorhandene Services andocken, **kein zweiter Snapshot-/Hash-/Projektions-Mechanismus** | Planner → Generator | `GESPERRT` — **Auftragsdatei fehlt (Planner).** *Korrektur 25.07.: die frühere Sperre „hinter AUF-33" war nicht gemessen — AUF-39 fasst `GuidedView.tsx` und `studioDaten.ts` an und kollidiert mit keinem laufenden Posten. Sobald der Auftrag liegt, ist er ziehbar.* | `docs/fahrplan-frontend-layout-hausplaner.md` Z. 90 |
| **AUF-40** | **L6 — Start/Zuletzt an echte Projekte + Konfigurator-Paket serverseitig speichern.** Gemessen offen: `app/StartView.tsx:75` rendert die Demo-Liste `ZULETZT`, `app/ConfigWizard.tsx` lädt das `ConfiguratorPackage` als JSON **herunter**, statt es zu speichern. Hatte bis 25.07. ebenfalls **keinen Posten** (null Treffer auf `L6`). Braucht Backend-Anschluss, blockiert kein Layout | Planner → Generator | `GESPERRT` — **Auftragsdatei fehlt (Planner)** *und* Yamas Fach-Freigabe: als einziger Layout-Posten berührt er `app/Http/` und `routes/` (Tor 1). *Korrektur 25.07.: die Sperre „hinter AUF-39" war nicht gemessen — die Dateien überschneiden sich nicht.* | `docs/fahrplan-frontend-layout-hausplaner.md` Z. 91 |
...
**18 Posten**: AUF-1 · AUF-2 · AUF-3 · AUF-4 · AUF-9 · AUF-11 · AUF-12 · AUF-14 · AUF-15a · AUF-16 ·
AUF-19 · AUF-20 · AUF-24 · AUF-25 · AUF-26 · AUF-28 · AUF-31 · AUF-32. Nicht gelöscht, nur nicht mehr im Weg.
