...
| **AUF-53** | **Import-Recht `Hausplaner,import` einführen** (Entscheidung Yama, 25.07., aus AUF-41) — gemessen kennt `routes/web.php` heute nur `Hausplaner,read` und `Hausplaner,update`; die **acht** Import-Werkzeuge sind deshalb dauerhaft gesperrt mit dem Grund „Keine Berechtigung zum Importieren." Das neue Recht in die Rechteverwaltung, an die Import-Routen hängen, und die Fähigkeit an `AktivierungsKontext.permissions` reichen — **die Vorbedingung `permission.import` steht bereits im Funktionsvertrag** und geht von selbst auf grün. **Kein Werkzeug wird freigeschaltet, das das Recht nicht hat** | Planner → Generator | `GESPERRT` — **Auftrag liegt**, wartet nur noch auf Yamas Freigabe zum Bauen. **Der Zuschnitt hat sich beim Messen halbiert:** `permission:Hausplaner,import` würde **nichts schützen** — `User::hasPermission` kennt nur vier feste Aktionen, eine unbekannte fällt auf `is_read`. Stattdessen **`Hausplaner,add`**: Spalte existiert seit 2023, Item existiert, **0 Routen nutzen es**. **Keine Migration, keine Änderung an `hasPermission`** | `generator-auftrag-auf53-import-recht.md` |
