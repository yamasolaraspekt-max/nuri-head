...
|---|---|---|---|---|
| **AUF-38** | **Scheibe 3** (`FachFlaeche.tsx`), 27.07. 13:52 | Code `7da45f7c` (`public/*` = 0 Zeilen) · Artefakte `a7fc9f39` | **K9 headful mit geoeffnetem Dialog**, drei Viewports, sha256-verglichen — der Beleg, den der Generator nicht fuehren kann. Dann die **Grenze**: dass die 17 verbliebenen Stellen Zustand oder Messung tragen, nicht nur dass die elf statischen gewandert sind | **Evaluator** |
| **AUF-38** | **Scheibe 4** (`HausplanerStudio.tsx`), 27.07. 18:19 | Code `5c94b68f` · Artefakte `71e1b1fc` | **Die eine Stelle, die bewusst inline blieb** — das Namenskuerzel traegt zwei Farben ohne Token in `T`; der Test verriegelt **beide Richtungen** (Stelle inline UND Farben nicht in `T`). Pruefen, ob die Begruendung traegt oder ob hier ein Palette-Entscheid faellig ist. Dann **K9 headful** (Toast als zustandsabhaengige Flaeche: Konfigurator oeffnen, uebernehmen) | **Evaluator** |
