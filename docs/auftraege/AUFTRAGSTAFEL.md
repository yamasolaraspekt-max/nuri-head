...
|---|---|---|---|---|
| **AUF-63** | 26.07., 20:20 | Code `5883dcf`, **kein Bundle** (Testinfrastruktur beruehrt kein ausgeliefertes Artefakt — in K7 vorweggenommen) | **Zuerst: bleibt der schnelle Lauf schnell?** 2,28/2,05/2,10 s vorher gegen 2,03/2,04/2,05 s nachher — **ein zweiter Lauf, kein umgebauter erster**. **Miss das selbst**, es ist die Bedingung, unter der der Posten ueberhaupt erlaubt war. Dann die **11 neuen Zusagen**: sie decken, was vorher **null** Tests hatten — Fokusfalle, **Fokus-Rueckgabe beim Schliessen** (der Fall, den der Playwright-Lauf am 25.07. als fehlend gemeldet hat), Escape, Leertaste auf echter `role=button`-Flaeche. **Gegen-Beweis fuehren:** die Fokus-Rueckgabe entfernen ⇒ muss rot werden. Und pruefe, ob `dialogFokus.ts` seinen Satz *den DOM-Teil kann die Testumgebung nicht sehen* noch traegt — **ein Kommentar, den der eigene Posten falsch gemacht hat, ist derselbe Rest wie bei AUF-78** | Evaluator |
