...
|---|---|---|---|---|
| **AUF-40** ⚡ **AKTIV** | **L6 — Start/Zuletzt an echte Projekte + Konfigurator-Paket serverseitig speichern.** Gemessen offen: `app/StartView.tsx:75` rendert die Demo-Liste `ZULETZT`, `app/ConfigWizard.tsx` lädt das `ConfiguratorPackage` als JSON **herunter**, statt es zu speichern. Hatte bis 25.07. ebenfalls **keinen Posten** (null Treffer auf `L6`). Braucht Backend-Anschluss, blockiert kein Layout **Zielbild aus der UX-Bewertung 26.07.:** je Eintrag Projektname · letzter Zeitpunkt · aktueller Schritt · offene Aufgaben · Speicherstatus · **direkte Fortsetzen-Aktion**. | Planner → Generator | **Teil A `IN ARBEIT — Generator (nativ)`** — gezogen 26.07., Auftragsdatei vollstaendig (111 Zeilen) · **Teil B `BEI YAMA`** — Migration + Route + Controller berühren Tor 1 und werden nicht ohne seine Freigabe gezogen **VORGEZOGEN 26.07., 09:00: das ist der letzte Befund der Layout-Inventur, der ohne Yama baubar ist.** Von neun Befunden (B1-B9) sind **sieben geschlossen**, einer war beim Nachmessen kein Defekt (B9). Uebrig: **B4** (drei erfundene Projekte, drei Karten ein Ziel) = **Teil A**, und **B7** (Konfigurator sagt speicherbar, speichert aber nicht) = **Teil B**. **Teil A ist reine Insel-Arbeit und sofort baubar. Teil B braucht Migration und Route und damit Yamas Freigabe** — er beginnt nicht ohne sie **Teil B: Entscheidung Yama 26.07. — noch nicht bauen.** Statt der Persistenz wird der Satz ehrlich gemacht: **AUF-74**. Teil B bleibt hier stehen, **unveraendert und ungezogen** — nicht gestrichen, nicht dran | `generator-auftrag-auf40-start-und-persistenz.md` |
| **AUF-74** | **Der Konfigurator sagt, was wirklich passiert** (Befund **B7** der Layout-Inventur, **Entscheidung Yama 26.07.: erst den Satz ehrlich machen, nicht bauen**). Selbst nachgemessen: `grep -rl ConfiguratorPackage app/ database/migrations/ routes/` = **leer** — serverseitig existiert nichts. Was „speichern" heisst, ist `a.download` einer JSON-Datei (`ConfigWizard.tsx:236`). Der Nutzer liest derweil an drei Stellen „speicherbar", „gespeichert" und „spaeter verlustfrei ins Projekt". **Die staerkste Flaeche im Programm macht das leerste Versprechen** | Generator | `OFFEN` — **Spur A, klein.** Es wird **nichts** gespeichert und **nichts** migriert: `routes/`, `app/Http/`, `database/` tragen null Zeilen — das ist hier kein Formalkriterium, sondern der Inhalt der Entscheidung. Zwei bindende Punkte: **kein „noch nicht" ohne Aussage darueber, was stattdessen geht** (der Download **ist** ein Ergebnis), und **kein Versprechen auf spaeter** — genau die Sorte Satz hat AUF-44 aus der Icon-Zeile entfernt. **Die wahre Aussage bleibt stehen:** der Nicht-autarke Fall („als ein Command ins Gebaeudemodell") stimmt und ist testverriegelt | `generator-auftrag-auf74-konfigurator-ehrlich.md` |
| **AUF-58** | **`.gitignore` für die Sichtprobe-Kladden** (Meldung des Evaluators, 25.07.) — der native Generator legt bei jeder sichtbaren Scheibe zwei Hilfsdateien an (`public/_aufNN-sichtprobe.html`, `sichtprobe-aufNN.tmp.mjs`) und räumt sie hinterher weg. **Gemessen: in der gesamten Historie wurde noch **keine** committet** — die Disziplin hat siebenmal gehalten. Der Posten ist **Vorbeugung, keine Reparatur**: zwei Zeilen in `.gitignore` (`sichtprobe-*.tmp.mjs`, `public/_auf*-sichtprobe.html`) nehmen das Risiko dauerhaft heraus, dass eine Kladde in `public/` neben dem Bundle landet | Generator | `OFFEN` — **vorgezogen 26.07., 09:15, und der Grund ist neu: Spur B.** Der Befund ist unveraendert klein (gemessen **neun** Kladden an einem Tag, **keine committet** — die Disziplin haelt). **Geaendert hat sich die Umgebung: seit gestern gilt §10.2** — fremde untracked Dateien sind ein Haltesignal. **Ich bin an einem Vormittag viermal darauf getroffen** (AUF-62, 64, 71, 73), viermal war es eine Kladde. **Eine Regel, deren Ausloeser meistens harmlos ist, wird ueberlesen** — §10.2 ist einen Tag alt und schon dabei, stumpf zu werden. Kriterium: enges Muster, vorgefuehrt an `public/_auf99-sichtprobe.html` (verschwindet) **und** `public/auf99-sichtprobe.html` ohne Unterstrich (**erscheint**) — die zweite Probe ist die, die zaehlt | `generator-auftrag-auf58-kladden.md` |
| **AUF-75** | **Der Waechter — es soll etwas von selbst laufen** (Yamas Frage nach einem lokalen Qualitaetsdienst; **erste und einzige Scheibe davon**). **Gemessen: in diesem Repository laeuft nichts automatisch** — keine CI (`.github/workflows` fehlt), kein Hook (`.git/hooks` leer), kein Playwright-Setup, kein MCP. **Jede Pruefung laeuft, weil sich jemand erinnert.** Was das gekostet hat: **AUF-64** — vier Gates gruen, 1007 Tests gruen, und `objekt/203` lag tot im Hauptzweig, weil die PHP-Suite nicht gefahren wurde. §9 verlangt sie seither; **durchgesetzt wird sie von nichts** | Generator | `OFFEN` — **Spur A. Deterministisch: kein Sprachmodell, keine Reparatur, keine Bewertung.** Ein Skript, das nach Betroffenheit aus dem Diff die vorhandenen Gates faehrt und das Ergebnis anhaengt, plus ein **nicht blockierender** `post-commit`-Hook. **Wichtigstes Kriterium: ein nicht gelaufener Test darf nie wie ein bestandener aussehen.** Beweiskriterium: der Waechter muss gegen `e0d1144` **rot** melden — den Commit, der die Route zerbrochen hat. Und er darf den geteilten Baum nicht stoeren: `--no-optional-legacy` nein — **`--no-optional-locks` an jedem git-Aufruf**, nichts ausserhalb von `docs/befunde/` | `generator-auftrag-auf75-waechter.md` |
