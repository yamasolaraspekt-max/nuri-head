...
| **AUF-55** | **Snapshot-Fläche ehrlich ausweisen** (Entscheidung 25.07., aus AUF-13) — das Blade setzt `data-snapshots-url`, drei Routen existieren, `main.tsx` liest **null** Verweise. Bis die Anbindung kommt, wird die Fläche als `in Entwicklung` gekennzeichnet — mit demselben `ZustandBadge` und demselben Blindtext-Verbot wie die Fachplaner-Flächen aus AUF-25. **Die tote URL im Blade bleibt stehen** und wird nicht entfernt: sie ist die Naht, an der die spätere Anbindung ansetzt | Generator | `⚡ AKTIV` · `BERICHTET — wartet auf Evaluator` — Code `53ede12` + Bundle `7033b86`; Gates 0/0/0/0, Insel 1154 auf 1164, PHP 789 unveraendert; Mutation ergibt 4 rot; Sichtprobe 1440/1024/375, kein waagerechter Ueberlauf. `sichtbar`. **Auftragspraemisse gedreht: es gab keine wirkungslose Snapshot-Flaeche, sondern gar keine** — der ehrliche Zustand steht jetzt im vorhandenen Reiter Historie, kein neuer Reiter. `views/` und `routes/` null Zeilen. **Befund gemeldet: zwei Flaechen tragen „Module folgen“** (`HausplanerApp.tsx:1445`, `HausplanerStudio.tsx:174`) **Die tote URL im Blade bleibt stehen** (Naht für die spätere Anbindung); `routes/` und `views/` null Zeilen Diff. Sichtbar | `generator-auftrag-auf54-55-56-klein.md` |
...
|---|---|---|---|---|
| **AUF-54** | 26.07., 18:04 | Code `29e83f4`, **kein Bundle-Commit** | **Zuerst die fehlende zweite Zeile — und sie fehlt zu Recht.** Er belegt: `build:hausplaner` Exit 0, Artefakt **bytegleich** (`sha256 a49f3ab9…`), `treppeAlsSvg` wird aus dem ausgelieferten Code **gar nicht aufgerufen**, Farbwerte im Buendel **0 Treffer**. **Pruefe genau das nach** — stimmt es, ist §8 2b **erfuellt**, nicht umgangen. Dann: **K1 in strengster Form** (`farben` ist **Pflicht**, kein Standardwert, hinter dem ein Wert ueberleben koennte) · **Schichtrichtung**: `geometry/` importiert **nicht** aus `app/` — *der bequeme Fehler waere gewesen, die Palette in die Geometrie zu holen; dann laege der Wert woanders, aber die Geometrie hinge weiter am Aussehen, nur unsichtbarer* · **K2 wertgleich Byte fuer Byte ueber vier Treppenarten**, dazu die sechs Farbwerte **einzeln** gesucht, weil eine Pruefsumme allein gruen bliebe, wenn zwei Farben getauscht waeren | Evaluator |
