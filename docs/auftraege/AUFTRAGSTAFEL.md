...
|---|---|---|---|---|
| **AUF-64** ⚡ **AKTIV** | **🔴 `objekt/203` ist kaputt — PHP-ParseError, Route nicht benutzbar** (Planner-Messung 26.07., 00:40). `e0d1144` (AUF-60) hat einen **`@php … @endphp`-Block** in `objekt.blade.php:97-103` eingefügt. Dieselbe Datei trägt in **Zeile 62** ein **inline** `@php($szeneLeer = …)`. **Beleg aus dem Kompilat** (`storage/framework/views/a3018339….php`): dort steht `<?php($szeneLeer = empty(…))` — **ohne Semikolon, ohne schließendes `?>`**; PHP frisst das folgende HTML und stolpert über `class` im `<form>`. Ignition meldet deshalb Zeile 63. **Die Blade-Änderung ist committet** — der Fehler liegt im Hauptzweig, nicht in einem Arbeitsbaum. `/studio` ist **nicht** betroffen (eigenes Blade, kein Fehler in der Konsole) | Generator | `OFFEN` ⚡ — **AKTIV, Vorrang vor allem. Auftrag liegt.** **Weg entschieden, nicht zur Wahl gestellt:** die Rechte werden im **Controller** berechnet (`HausplanerController::seite`, Z. 29-38 reicht schon drei Variablen durch), im Blade bleibt **eine** Zeile `data-rechte="{{ $hpRechte }}"`. Damit kann der Fehler nicht wiederkommen — es bleibt kein `@php`-Block übrig. **Dazu die Regressionssperre:** ein Test, der die Hausplaner-Blades **kompiliert**. 1007 Tests haben das nicht gefunden, weil keiner ein Blade anfasst **STAND 26.07., 01:20 (Planner-Messung + Evaluator-Votum `e4f2a25`):** der Fix ist **gebaut, aber NICHT committet** — `git status` zeigt `objekt.blade.php` als `M` und `tests/Feature/Hausplaner/BladeKompiliertTest.php` als `??`. **Deshalb ist `objekt/203` nur im Arbeitsbaum heil; im ausgelieferten Stand ist die Route weiter kaputt.** Solange nichts committet ist, ist nichts erledigt. **Weg-Abweichung angenommen:** gebaut wurde die **einzeilige** Form statt der Controller-Variante aus meinem Auftrag §2. Angenommen, weil die gemessene Ursache genauer ist als meine — Blade zieht Rohblöcke **non-greedy und vor** dem Entfernen der Kommentare, das schließende Gegenstück paart also mit der früheren einzeiligen Öffnung, sogar über Kommentartext hinweg. Die Regressionssperre leistet strukturell, was ich vom Controller wollte. **Der Umzug in den Controller bleibt richtig und wird AUF-69** — ohne Dringlichkeit. | `generator-auftrag-auf64-objekt-route.md` |
| **AUF-60** | **Die Insel muss die Rechte des Nutzers kennen** (Rückgabe aus AUF-53 §4) — gemessen: die React-Insel kennt **genau ein** Recht (`RECHT_BEARBEITEN`), und es stammt **nicht aus dem angemeldeten Nutzer**, sondern wird gesetzt. Damit ist die Zuordnung aus AUF-53 richtig und **wirkungslos zugleich**. **Keine Sicherheitslücke** — der Server schützt jede Route weiter über `CheckUserPermission` — sondern eine **Anzeige-Lüge in beide Richtungen**: ein Werkzeug erscheint bedienbar, das der Server verweigern wird, oder gesperrt, obwohl der Nutzer es dürfte. Der Weg ist die vorhandene Blade-Naht, die schon `data-speichern-url` trägt | Planner → Generator | `BERICHTET — wartet auf Evaluator` — Code e0d1144 + Bundle 416aeb2 **VOTUM 26.07. (`e4f2a25`): NACHBESSERN.** Die Rechte-Logik ist freigabereif (fehlendes Attribut ⇒ **leere** Liste = Minimum, `rechte.ts:31`; 1008/1008 Tests; 15 Untertests). **Blockierend ist die Blade-Regression** aus demselben Commit — reproduziert über Laravels echten Compiler, `php -l` meldet Parse error. **AUF-60 wird grün, sobald AUF-64 committet ist**, nicht vorher. | `generator-auftrag-auf60-rechte-durchreichen.md` |
