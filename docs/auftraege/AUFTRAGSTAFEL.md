...
|---|---|---|---|---|
| **AUF-76** | 26.07., 17:43 | Code `26a544f` · Bundle `8b43e13` | **Dass der Bestand gueltig bleibt — an einer echten Fixture, nicht an einem erfundenen Dokument.** Der Generator hat seinen ersten Anlauf selbst verworfen, weil das handgeschriebene Testdokument nur bewies, dass er das Schema erraten hatte. Dazu: **K3** kein persistierter Wert umbenannt (DAUERDIREKTIVE) — `thickness` bleibt die Wahrheit fuer den Rohbau, die Liste sagt **woraus** die Wand besteht, nicht **wie dick** · **K7 feldgleich mit der Decke am erzeugten Schema**, nicht am Quelltext (*gleich aussehender Zod-Code kann verschiedenes Schema erzeugen*) · **K6** `0`, `-5`, `12.5` und ein Tippfehler-Feld werden abgelehnt · PHP-Suite **unveraendert**, wie es bei einem rein additiven Feld sein muss | Evaluator |
