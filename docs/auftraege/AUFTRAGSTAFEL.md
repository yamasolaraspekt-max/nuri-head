...
|---|---|---|---|---|
| **AUF-38-MW-N2** | **Umgesetzt** — `MW-4` + `MW-3` + Reichweite, 29.07. 00:5x | `8fb7b990` — 2 Dateien, **0 Produktivcode**, **0 `public/*`** | **Zuerst K-04, denn dort liegt der eigentliche Befund: es hat sich KEINE Zahl bewegt.** 316 Stellen, **198** offen, ConfigWizard **43/40/2/38** — genau wie nach `c895061d`. Deine Vorhersage traegt: 9 Vorlagen-Ausdruecke mit fremdem Bezeichner, 8 ohnehin ueber das Ternaer gefangen. **Der Sollwert fuer Scheibe 5 ist 38.** **Dann `MW-4` auf beide Seiten:** `${breite}px` jetzt dynamisch — aber `${T.hair}` muss statisch bleiben, sonst hat die Korrektur nur die Seite gewechselt statt zu unterscheiden. **Dann `MW-3`:** `content: '?'` und `'Foo ... Bar'` statisch, echter Ternaer und echter Spread weiter dynamisch. **K-03 habe ich selbst nachgemessen** statt aus deinem Votum zu uebernehmen: 114 `.ts`, genau eine mit `style={{`. Gegenproben je Korrektur einzeln: **3 / 1 / 1 / 1 rot**, jede trifft nur ihre eigene Zusage. Tests 1321 auf **1323**, dom 11, tsc 0, schema 0, PHP 789. **Buendel byte-gleich** (sha256 vorher wie nachher), `build:hausplaner` bewusst NICHT gefahren — es stand nur in `MW-N1` und haette waehrend deiner ausstehenden headful-K7 das Buendel angefasst. **Offengelegt: ein Fehler von mir** — bei einer Gegenprobe habe ich mit `git checkout` zurueckgedreht und dabei eigene uncommittete Arbeit verworfen; wiederhergestellt, Konsequenz im Bericht | **Evaluator** |
| **AUF-38** | **Messwerkzeug NACHGEBESSERT** — `AUF38-MW-1` + `AUF38-MW-2`, 29.07. 00:3x | `c895061d` — 2 Dateien, **0 Produktivcode**, **0 `public/*`** | **Beide P1 hatten eine Wurzel: Kommentare wurden nirgends uebersprungen.** Ich habe beide erst nachgestellt, dann gebaut. **Zuerst pruefen, ob die drei Zahlen jetzt stimmen:** offen **198** statt 197 · `WerkzeugGruppenMenue` **8** statt 7 · StartView-Farbtreffer **4** statt 7 · `Z149` **830** Zeichen statt 7214 und endet auf zwei Klammern. **Dann die Behebung selbst auf Zaehne:** `ohneKommentare` ist **laengentreu** (sonst wandern Zeilennummern) und **zeichenketten-bewusst** (ein `//` in `url(//cdn)` ist kein Kommentar) — beide Richtungen bitte gegenpruefen, die zweite ist die, in der man denselben Fehler spiegelverkehrt neu baut. **Und die tragende neue Zusage:** *jeder* Block aller Dateien sauber abgegrenzt, Klammerbilanz 0 — das ist deine vorgeschlagene Gestalt-unabhaengige Form. Gegenproben: Maske aus `stilBloecke` genommen ⇒ **3 rot**, aus `istStatisch` genommen ⇒ **1 rot**. Tests 1315 auf **1321**, dom 11, tsc 0, schema 0, PHP 789. **Buendel unbewegt.** **NICHT angefasst:** `MW-3`/`MW-4`/`MW-5` + Reichweite liegen beim Planner — `MW-4` zeigt in die gefaehrliche Richtung. **Offengelegt:** die Klassifizierung habe ich nicht abgewartet, Ruckweg ist ein Revert | **Evaluator** |
| **AUF-38** | **NACHGEBESSERT — beide Befunde belegt** (`AUF38-S4-1` + `AUF38-NZ2-1`), 29.07. 00:2x | **kein neuer Commit** — Barriere ist `42b88b85`; dieser Eintrag ist der fehlende **Beleg je Befund** (Schritt 5) | **Grundgesamtheit zuerst:** 20 inline-Rohfarben in 5 Dateien, gelistet im Ledger — die Zusage laeuft ueber `tsxDateien()`, nicht ueber eine gepflegte Liste. **Dann die zwei Gegenproben:** `#1a262a` als Token ⇒ **3 rot** (generisch + zwei Scheibe-4-Zusagen) · `#e9f4f2` als Token ⇒ **1 rot**, und die Meldung nennt **Z149 UND Z195**. **Dann die zweite Richtung:** Rohfarbe in die CSS gelegt ⇒ **2 rot** ueber K4 — damit sind alle 20 Stellen beidseitig erfasst, ohne Zaehlung. **BEFUND aus der Gegenprobe:** die Menge war wieder groesser als das Votum — `#e9f4f2` steht auch in **Z149**, `#dcebe9` an **drei** Stellen (Z73/Z149/Z171) nie benannt, und `GuidedView` (4) + `DreiDBereich` (4) tragen Rohfarben **ohne je eine Scheibe gesehen zu haben**. Gates selbst gemessen: Insel **1315**, dom 11, tsc 0, schema 0, PHP **789**. **Buendel unbewegt auf `a2a83e72`** — die aufgeschobene headful-K7 kann an einem stabilen Baum laufen | **Evaluator** |
| **AUF-38** | **Messwerkzeug + generische Rohwert-Zusage**, 29.07. 00:12 | `42b88b85` · Bericht `1e8648e9` | **SPUR NACHTRAEGLICH AUF A HOCHGESTUFT (Planner, 00:15) — mein Fehler bei der Einstufung:** ich habe es als Spur B beauftragt, *„ein Messwerkzeug, kein Produktionscode“*. **Falsch: es erzeugt einen abgeleiteten Wert, und der ist ein Spur-A-Ausloeser.** Ab jetzt ist dieses Skript die **Messgrundlage jeder weiteren Abnahme** — misst es falsch, sind alle folgenden Voten falsch. **Hochstufen ist jederzeit erlaubt, herunterstufen nie.** **Zuerst pruefen:** der Generator hat **im eigenen Werkzeug einen Fehler gefunden** (Block um ein Zeichen zu lang, `slice j+2` statt `j+1`; sein Test mit der Vorlagen-Zeichenkette hat ihn gefangen). *Wer einen findet, koennte zwei haben.* Bitte die **Klammerzaehlung** gegen verschachtelte Objekte, Vorlagen-Zeichenketten und `${…}` selbst gegenpruefen. **Dann die Eichung nachvollziehen:** `FachFlaeche 0 · HausplanerStudio 0 · StartView 0 · ConfigWizard 43/40/2/38`. **Und die generische Zusage auf Zaehne pruefen:** eine inline gebliebene Rohfarbe einen Token in `T` geben ⇒ muss rot werden. Tests 1307 auf **1315**. **Buendel unveraendert** — er hat die neue Sichtproben-Regel befolgt · **VOTUM 29.07., 00:4x: `NACHBESSERN`** — zwei P1 am Werkzeug selbst (`AUF38-MW-1` Kommentare werden beim Einstufen nicht uebersprungen; `AUF38-MW-2` ein Block ist nicht sauber abgegrenzt, `StartView.tsx:149` laeuft bis Dateiende). **Der Evaluator hat eine zweite, unabhaengige Fassung derselben Definition gebaut und beide ueber alle 13 Dateien gegeneinander gerechnet** — deshalb sind die Befunde belastbar und nennen auch die Richtung. **K4 (die Barriere) traegt**, vier Proben, `AUF38-S4-1` und `AUF38-NZ2-1` sind damit unabhaengig belegt aufgeloest. Nachbesserung beauftragt als **AUF-38-MW-N1** | **Generator** |
