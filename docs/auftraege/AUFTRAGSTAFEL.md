...
**17** Arbeitsvorrat · **1** Abnahme · **0** bei Yama · **47** im Archiv — Summe geprüft, 65.
...
|---|---|---|---|---|
| **AUF-57** | **Der Wegweiser braucht einen Anlass und einen Ort** (Rückgabe des Generators aus AUF-45) — der Mechanismus ist gebaut, gemessen und verriegelt, **erscheint aber nie**: `activeLevel.exists` ist nie verletzt, weil eine Szene immer mindestens ein Geschoss trägt. **Die Zahlen meines Auftrags waren überholt** (78/44/16 stammten aus der Zeit vor AUF-34; gemessen sind es 73/53/28). Der real größte Block sind **28 Werkzeuge, die einem anderen Arbeitsbereich gehören** — nicht das Geschoss. Der Planner entscheidet Anlass und Ort; Vorschlag: ein Hinweis am Arbeitsbereich-Wähler, der die messbare Zahl nennt, statt eines globalen ersten Schritts | Planner → Generator | `BERICHTET` — umgesetzt `7cac7cb`, Bundle `e391e73`. Anzeige hängt am **Ort** statt am hartkodierten Grund (`grep` = 0). **Auswahl ist der Anlass, der feuert: 25 Werkzeuge**, Ort = Werkzeug-Schiene, verschwindet nach dem Auswählen. **Arbeitsbereich ist KEIN Anlass — gemessen −26/−26/−22/−19**, der Mechanismus lehnt ihn selbst ab. Aktivierung unverändert (73/53/28). Tore 0/0/0/0, 987→993, Mutation 1 rot. Sichtprobe: Satz steht genau einmal. **Ballbesitz → Evaluator** | `generator-auftrag-auf57-wegweiser-ort.md` |
...
| **AUF-63** | **jsdom für den Testlauf** (Bestandsmeldung des Evaluators, 26.07., Frage 3) — AUF-30 hat `.tsx` im Testlauf übersetzbar gemacht, **jsdom fehlt aber weiter**. Folge, von ihm benannt: **Fokusfalle und `getComputedStyle` sind nur im Browser messbar, nicht im Gate.** Nicht blockierend — die iframe-Sichtprobe trägt — aber jedes Fokus- und Zustands-Kriterium hängt damit an einer Person mit offenem Browser statt an einem Testlauf. **Zu prüfen ist auch, ob es sich lohnt:** jsdom ist eine Abhängigkeit mehr, und die Testumgebung läuft heute bewusst schlank | Planner → Generator | `OFFEN` — Planner misst zuerst den Preis (Abhängigkeit, Laufzeit, was dadurch wirklich prüfbar wird), dann erst der Auftrag | Ledger „EVALUATOR-BESTANDSMELDUNG" (26.07.), Frage 3 |
...
**47 Posten**: AUF-1 · AUF-2 · AUF-3 · AUF-4 · AUF-9 · AUF-11 · AUF-12 · AUF-14 · AUF-15a · AUF-16 ·
