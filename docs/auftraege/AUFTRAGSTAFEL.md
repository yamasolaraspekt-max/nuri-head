...
| **AUF-38** | **Scheibe 4 NACHGEBESSERT** (`HausplanerStudio.tsx`), vollstaendig | `37094c5b` · Artefakte `0d5cd975` | **Zuerst die Vollstaendigkeit:** von 27 Inline-Stilen bleiben **10**, jeder mit Grund (8 dynamisch, 2 Rohwert ohne Token). **Dann die Wirkung:** dieselbe Zusage wie Scheibe 3, Gegenprobe 1 rot. **Abweichung geprueft:** Generator misst **17** statische, nicht 19 — `navBreit` ist dynamisch, eine Zeile traegt `#3f464e` roh. Gates 0/0/0/0 + `test:dom` 0; Insel 1300 auf 1302, PHP 789 | `generator-auftrag-auf38-inline-styles.md` + **`generator-auftrag-auf38-scheibe3.md`** (Blatt fuer Scheibe 3, 27.07.) |
...
|---|---|---|---|---|
| **AUF-38** | **Scheibe 3 NACHGEBESSERT** (`FachFlaeche.tsx`), 27.07. 20:18 | `01b9933e` · Artefakte `2798120f` | **Zuerst die Vollstaendigkeit, dann die Wirkung:** von 17 Inline-Stilen bleiben **3**, und die neue Zusage verlangt fuer *jeden* verbliebenen einen von zwei Gruenden. Gegenprobe des Generators: eine zurueckgedrehte Stelle ⇒ **2 rot**. Pruefen, ob die **drei Ausnahmen** tragen (`gesperrtStil.ts` als eine Wahrheit · zwei `rgba` ohne Token, AUF-56-Linie). Dann **K9 headful mit geoeffnetem Dialog** — jetzt aussagekraeftig, weil der Stand vollstaendig ist | **Evaluator** |
