...
|---|---|---|---|---|
| **AUF-59** ⚡ **AKTIV** | **Die Icon-Zeile mischt drei Zustände, die man nicht auseinanderhalten kann** (Yama, 25.07.: „einige Icon, die sind eingerahmt, manche sind aktiv manche nicht"). **Gemessen bei 1440 px:** 15 quadratische Knöpfe in einer Zeile, 9 gesperrt. Drei Zustände — **(1)** Schalter EIN: grüner Rand, grüner Grund (Raster, Fang) · **(2)** bedienbar: weiß, grauer Rand, dunkles Icon · **(3)** gesperrt: weiß, grauer Rand, blasses Icon. **Zustand 2 und 3 unterscheiden sich ausschließlich in der Icon-Farbe** (`rgb(35,42,49)` gegen `rgb(167,174,183)`) und im Cursor; Rahmen, Hintergrund und Deckkraft sind **identisch**. Die Zeile liest sich als Reihe gleichwertiger Kästchen. **Dazu die Dublette:** „Grundriss spiegeln" existiert **zweimal** — als Icon mit Tooltip in dieser Zeile (32x30) und als ausgeschriebener Textknopf im Eigenschaften-Panel (117x43, **ohne** `title`). Yamas Punkt: die Icon-Form gibt es bereits, der Text ist die Dopplung | Generator | `OFFEN` ⚡ — **AKTIV.** Grundlage aktualisiert: die Zeile trägt nach AUF-44 nur noch **11** Knöpfe statt 15. **Forderung, kein Entwurf:** gesperrt unterscheidet sich in **mindestens zwei** Merkmalen von bedienbar, nicht nur in der Icon-Farbe · der Rahmen trägt den **Schalter**-Zustand, nicht jeden Knopf · die Textknöpfe im Panel weichen den vorhandenen Icons | Planner-Sichtprobe 25.07. 23:10 |
