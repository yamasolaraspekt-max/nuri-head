...
| **AUF-75** ⚡ **AKTIV** | **Der Waechter — es soll etwas von selbst laufen** (Yamas Frage nach einem lokalen Qualitaetsdienst; **erste und einzige Scheibe davon**). **Gemessen: in diesem Repository laeuft nichts automatisch** — keine CI (`.github/workflows` fehlt), kein Hook (`.git/hooks` leer), kein Playwright-Setup, kein MCP. **Jede Pruefung laeuft, weil sich jemand erinnert.** Was das gekostet hat: **AUF-64** — vier Gates gruen, 1007 Tests gruen, und `objekt/203` lag tot im Hauptzweig, weil die PHP-Suite nicht gefahren wurde. §9 verlangt sie seither; **durchgesetzt wird sie von nichts** | Generator | `OFFEN` ⚡ — **gezogen 26.07., 12:20 — bewusst vor AUF-78 und AUF-76: der Waechter fasst weder PHP noch die Insel an, der laufende Suite-Messstand bleibt gueltig.** **Spur A. Deterministisch: kein Sprachmodell, keine Reparatur, keine Bewertung.** Ein Skript, das nach Betroffenheit aus dem Diff die vorhandenen Gates faehrt und das Ergebnis anhaengt, plus ein **nicht blockierender** `post-commit`-Hook. **Wichtigstes Kriterium: ein nicht gelaufener Test darf nie wie ein bestandener aussehen.** Beweiskriterium: der Waechter muss gegen `e0d1144` **rot** melden — den Commit, der die Route zerbrochen hat. Und er darf den geteilten Baum nicht stoeren: `--no-optional-legacy` nein — **`--no-optional-locks` an jedem git-Aufruf**, nichts ausserhalb von `docs/befunde/` | `generator-auftrag-auf75-waechter.md` |
