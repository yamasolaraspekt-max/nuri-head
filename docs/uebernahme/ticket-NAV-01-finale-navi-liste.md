...
| CRM›Leads/Kunden(6) | **✓ Tab-Leiste seit `5680a71`** (NAV III/F3 · Design A · 3a): 5 Tabs (Aktiv/Meine/Warteschleife/Junk/Gelöscht) über 3 Views (`customer_view`/`waiting_loops`/`reason_list`); die zuvor **unverlinkte `my_leads` (Meine)** jetzt erreichbar. **Flag-Include** (Option a): `customer_view` zeigt Tabs nur bei `showLeadTabs` (index/my_lead), `new.leads` bleibt tab-frei. **Daten-Grain-Notiz:** „Warteschleife" filtert `lead_product_lists.employee_id IS NULL` (Gewerk-Ebene, **kein `lead_stage`**), die anderen 4 auf `new_leads`. **NEUE Weiche W-new-leads-Sicht:** `new.leads` (`@new_lead:4818`) = **6. Lead-Listen-Sicht** (Quellen-Spalten `source`/`quelle`), nicht in den Tabs, nicht im Konzept — **Yama-Entscheid:** als 6. Tab aufnehmen / zusammenlegen / Streichkandidat; bis dahin unverändert, tab-frei. **Geparkt (3b):** Sidebar-Reduktion + „Neuer Lead"→+Aktion, wartet auf Kollisionsbereinigung. „CRM"-Etikett entfällt (A1). |
