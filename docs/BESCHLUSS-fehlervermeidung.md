...
| **B10** | **Automatisch erzeugte oder geschätzte Geometrie darf ihren Unsicherheits- und Herkunftsstatus durch Speichern, Laden, Export oder Sitzungswechsel nicht verlieren.** Ein Hinweis, der nur in der laufenden Sitzung lebt, ist kein Status, sondern eine Höflichkeit. Wer die Decke morgen öffnet, muss ihr ansehen, dass sie geraten ist — und solange sie nicht bestätigt ist, darf sie für Dach, Mengen, Statik oder jeden anderen Folgeprozess **nicht** als verlässliche Grundlage dienen | **4** | *Yama, 02.08. — Anlass war der Näherungs-Hinweis aus Z-06, den ich als Notiz behandeln wollte. Er ist kein kosmetischer Hinweis, sondern ein Persistenz- und Vertrauensproblem* | alle |
| **B11** | **Ein Handgriff, den eine Rolle mehr als zweimal von Hand macht, ist ein Befund und kein Umstand.** Wer ihn zum dritten Mal macht, hört auf und misst, warum er nötig ist. *Der Planner hat am 02.08. **vierzig** Mal einen Lock von Hand beiseitegeschoben und es für eine Eigenart des Mounts gehalten. Gemessen war es ein Baufehler im Commit-Tor: es räumt NACH dem Commit auf, während der blockierende Lock schon VORHER daliegt (W-09)* | **4** | *Yama, 03.08.: „sorg dafür dass dieses problem nie wieder passiert" — 855 beiseitegelegte Locks, 40 davon an einem Tag, alle vom Planner.* **BELEG KORRIGIERT 03.08. auf den Befund des Generators:** die erste Fassung nannte als Folge „vier fertige Scheiben lagen elf Stunden unverbucht". Das war eine Kausalität ohne Messung — der Generator hat `commit-pruefen.sh` **nullmal** aufgerufen; seine Arbeit lag am Regelkonflikt, nicht am Lock. *Eine Regel mit falschem Beleg ist schwächer als eine ohne: wer den Beleg kippen sieht, hält die Regel für widerlegt* | alle |
| **B12** | **SICHERN ist nicht VERÖFFENTLICHEN, und nur das Zweite gehört Yama.** Ein Commit auf den eigenen Arbeitszweig ist eine Sicherung: er kann nichts kaputtmachen, was nicht schon lokal ist, und er ist per `git revert` zurückdrehbar — **jede Rolle darf ihn jederzeit setzen, ohne Rückfrage.** `git push` · `main` · Tags · `upstream` · jedes `--force` · jedes Löschen sind Veröffentlichung: **unumkehrbar, Tor 2, Yama allein.** *Der RÜCKWEG entscheidet, wem eine Handlung gehört — nicht ihr Gewicht und nicht ihr Name* | **3** | *Yama, 03.08. — Auflösung des Konflikts zwischen BETRIEBSORDNUNG §2.1 („Commit … ohne Yama in Serie", 02.08.) und CLAUDE.md („Commits nur auf Yamas Wort", 01.08.). **Gefunden vom Generator, der 16 Stunden gewartet und damit richtig gehandelt hat** — während der Planner 31 Commits setzte, ohne den Konflikt zu bemerken. Dieselbe Trennung hatte PW-01 für den Sicherungs-Push schon aufgestellt* | alle |
| **B13** | **WERKZEUGSTOPP. Der Strang `werkzeuge` ruht, bis das Geschoss steht.** Kein neues W-/S-Blatt wird geschnitten. Die sechs gebauten werden noch abgenommen, danach ruht der Strang. **Werkzeugbefunde werden im Ledger NOTIERT, nicht geschnitten — außer sie blockieren einen laufenden Produkt-Bau; dann Spur B, kleinstmöglich, und der Grund steht im Blatt.** *Gemessen am 03.08.: seit dem 02.08. gab es **2** Commits am Produkt (`resources/planner/hausplaner`), **10** an der Werkstatt (`scripts/`) und **85** nur an Papier (`docs/`). 15 Werkzeug-Blätter gegen 12 Produkt-Blätter. **Wir haben an der Werkstatt gebaut und über das Bauen geschrieben.*** Der Planner misst sich ab jetzt an Produkt-Fortschritt, nicht an Blättern | **3** | *Yama, 03.08.: „wir kommen nicht voran, so geht es nicht weiter" — und die Messung dahinter, die ihm recht gibt* | alle |
| **B14** | **Der Zug endet mit dem Eintrag — den Statuswechsel schreibt, wer das Ereignis erzeugt.** Der Gegenleser setzt `bereit` (bei TRÄGT ohne sperrende Auflage), der Generator setzt `gebaut`, der Evaluator setzt `abgenommen` beim Votum — jeweils im selben Commit wie der Befund. **Der Planner setzt nur noch `entwurf`, `gesperrt` und Umverteilungen.** *Gemessen: 11 von den Planner-Commits zweier Tage waren reines Abtippen fremder Entscheidungen, und dreimal wurde es vergessen — Voten lagen bis zu zwei Tage uneingetragen, die Schlange zählte falsch, alle warteten. Von 7 Zügen je Blatt waren 3 Kopier-Züge; sie entfallen ersatzlos.* **Die Urteilstrennung bleibt unberührt: wer baut, votet nicht — der Statuswechsel ist Buchhaltung, kein Urteil** | **4** | *Yama, 03.08.: „Produktivität und Effizienz steigern ohne Fehler und Qualitätsverlust" — und die drei Eintrags-Versäumnisse des Planners als Anlass* | alle |
| **B15** | **Prosa-Diät — die Fehlerfläche schrumpft mit dem Text.** Ledger-Einträge: Messwerte + Entscheidung, Richtwert ≤ 20 Zeilen. Blatt-Abschnitt „Warum": ≤ 10 Zeilen mit Messwerten. **Die STAND-Schlange wird nicht mehr von Hand gepflegt — sie IST der Lauf** `node scripts/auftrag-pruefen.mjs docs/auftraege/<strang>/*.md` *(S-01/S-06/S-14 zeigen aktiv/baubar/unverbucht live statt abgeschrieben).* *Gemessen: 85 von 97 Commits zweier Tage waren Papier; alle 10 F-04-Fehler des Planners standen in PROSA — nicht in Kriterien, nicht im Code. Weniger Abschrift = weniger Stellen, an denen eine Zahl ohne Befehl stehen kann* | **3** | *Yama, 03.08., dieselbe Anweisung — und S-13s Befund als Beleg, WO die Fehler wohnen* | alle |
