...
| **F — Formulare (Synthese)** | ticket · (später Worktree `strang/formulare`) | **Exklusiv `docs/formular-*`** (+ später FS-Umsetzung nach Yama-Entscheid: `ProductFormula`, `app/Services/Formular/*`, Formula-Views). **`sidebar.blade.php` + `routes/web.php` tabu** (nur gelesen). Read-only außerhalb Doku-Scope. | **Befund** (Arbeitspaket) | (dieser Commit) |
