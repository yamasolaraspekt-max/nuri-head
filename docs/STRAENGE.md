...
- **VENDOR-REGEL (ratifiziert 2026-07-06):** Jeder Worktree hat sein **eigenes reales `vendor/`** (`cp -a` aus main + `composer dump-autoload`, oder `composer install`). **NIE symlink-`vendor` + `composer dump-autoload`** — das biegt die geteilte Autoload-Map auf den fremden Worktree um (Vorfall 2026-07-06: main + accounting luden App-Klassen aus dem formulare-Worktree). `vendor/` ist gitignored → Worktree-lokal.
- **TEST-DB-TRENNUNG (beschlossen 2026-07-06):** **eine Test-DB je Worktree** — `ticket_testing_{main,accounting,formulare,c,…}` (Yama/Ramin legt an bzw. erteilt `ticket_user` CREATE auf `ticket_testing_%`). Je Worktree eine **lokale, UNTRACKED `.env.testing`/phpunit-Env** mit eigenem `DB_DATABASE` — nur dieses **Muster** steht hier, die Datei selbst nie committen. **Übergangsregel bis dahin:** volle Suiten (inkl. Prüfer-G1) laufen **serialisiert** (Koordinator-Lock) — **kein paralleles `migrate:fresh` auf `ticket_testing`**. DB-freie (PHPUnit) oder isoliert-SQLite-Tests sind rennfrei und davon ausgenommen.
...
| **F — Formulare (Synthese)** | ticket · Worktree **`strang/formulare`** | **Exklusiv `docs/formular-*`** + **`app/Services/Form/*`** + **`tests/{Unit,Feature}/Form/*`** (+ später FS-07+ nach Yama: `ProductFormula`, Formula-Views). **`sidebar.blade.php` + `routes/web.php` tabu.** Read-only außerhalb Scope. | **Bau** (FS-03) | `6b1eccc` |
