...
### Stufe (ii) — Norm-Außentemperatur = eine Wahrheit (2026-07-07)

- **KLIMA-EINE-WAHRHEIT · `NormTempService` auf `klima_plz` umgeleitet.** Befund (Explore-Agent): keine vor-bestehende PLZ→Norm-Tabelle in ticket (Import war keine Tabellendopplung), **aber** `app/Services/NormTempService.php` war ein hartkodierter Heuristik-Estimator für denselben Wert (Stadt-Stichworte + Höhe; eigener Kommentar: „replace with a real table when available"). Parallel bezog `AnforderungsprofilService` den Wert bereits aus `klima_plz`/`KlimaPlzService`. **Gewählt:** `klima_plz` führend; `NormTempService` delegiert bei vorhandener PLZ (aus Adresse extrahiert) an `KlimaPlzService`, Heuristik nur noch Fallback (keine PLZ / kein Treffer). Öffentliche Signatur unverändert. Commit `c365f15`. Beleg: 80331 München → −14 (Tabelle) statt −16 (Heuristik). **NICHT zusammengeführt:** `temperatures` (Ist-Wetter-Zeitreihe) und `climate_*`-Modul (DWD-Monatsmittel/Gradtage/Solar) — anderer Sachverhalt, bleiben eigenständig.
