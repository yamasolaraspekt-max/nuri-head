...
| B14 Stückliste/Material-Liste | VORHANDEN (ticket) | `master_sets`/`master_set_components`(`type='zubehoer'`)/`deal_measurement_items`; Anzeige-Builder `DealMaterialListController` | **Korrektur (v-c-Befund):** ein CSV-/Excel-/PDF-Export existiert **nicht**. „Vorhanden" = der Material-Listen-**Anzeige**-Builder. HK-Andock = **additive `deal_measurement_items`-Zeilen** (`kind='heizkoerper'`, Provenance in `raw_snapshot`) → erscheinen dank Filter `extractMaterialsFromDealMeasurementItems` automatisch in der bestehenden Liste; **kein neuer Export, nicht duplizieren** (Stufe v-c) |
...
| **(v) UI** | Alpine einführen (**CLAUDE.md-Eintrag zuerst**); Routen `heizkoerper.*` (Bereich 8 „Energie & Auslegung", ticket-Web-Guard); Views: Aufnahme = Reuse `radiator.config.*`, SVG-Schema (Front+Seitenprofil, v-d), Stücklisten-Ansicht je Raum (v-c-1); **Andock statt Export**: HK erzeugt additive `deal_measurement_items` → bestehende Material-Liste (v-c-2) | Feature: Rechen-Endpunkt liefert EN-442-Tabelle+Ampel (✅ v-a); Stückliste-Anzeige (✅ v-c-1); **Übernahme erzeugt Positionen** (v-c-2). Deal-Flow-Integration (Konfigurator aus Deal öffnen) = M4-Folgepunkt |
...
| (v) UI | ✅ **grün** (v-a/v-b/v-c-1/v-c-2); v-d (SVG) = **Backlog** | Endpunkte + Alpine-Konfigurator + Stückliste-Anzeige + Übernahme-Endpunkt, hinter Feature-Flag | v-a: Rechen-/Kompatibilitäts-Endpunkte (EN-442-Durchstich 663 über HTTP, Datenlage-Stufen, Flag-off→404 ohne HK-Query, 422). v-b: erste Alpine-Fläche `heizkoerper.konfigurator` (Live-EN-442+Ampel+Kompatibilität, ehrliches Datenlage-Label). v-c-1: Stückliste-Anzeige je Raum (regel-kandidaten → „—", kein erfundener Preis). v-c-2: `heizkoerper.stueckliste.uebernehmen` — additive `deal_measurement_items` (`kind='heizkoerper'`, Provenance in `raw_snapshot`), **Replace-per-Raum**, Bestands-Guards (Flag/auth/423/422); erscheinen in bestehender Material-Liste (`DealMaterialListController`, ungetouched). **Ownership-Gap eskaliert → `docs/sicherheits-backlog.md` SEC-DM** (nicht hier gefixt). Aufnahme = Reuse `radiator.config.*`. **v-d SVG = Backlog** (kein Abnahmekriterium). **M4-b (Sidebar/Nav) geparkt** bis Parallelstrang-Commit. **M5 fährt die HK-Migrationen produktiv UND schaltet das M4-Flag (`HEIZKOERPER_MODULE_ENABLED`) frei** — bis dahin Modul prod-OFF. **M5 enthält zusätzlich: (a) additive `nullable`-Migration für `deal_measurement_items.unit_price/purchase_price/total_price`** (sonst 0,00 statt „kein Preis" in der Material-Liste — siehe `sicherheits-backlog.md` SEC-DM-3); **(b) Owner-Backfill-Lauf** `deal-measurements:backfill-owner` (SEC-DM-a). |
