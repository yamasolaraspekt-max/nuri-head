...
| (v) UI | 🟡 **v-a/v-b/v-c-1 grün**; v-c-2 (Andock) + v-d (SVG) offen | Endpunkte + Alpine-Konfigurator + Stückliste-Anzeige hinter Feature-Flag | v-a: Rechen-/Kompatibilitäts-Endpunkte (EN-442-Durchstich 663 über HTTP, Datenlage-Stufen, Flag-off→404 ohne HK-Query, 422). v-b: erste Alpine-Fläche `heizkoerper.konfigurator` (Live-EN-442+Ampel+Kompatibilität, ehrliches Datenlage-Label). v-c-1: Stückliste-Anzeige je Raum (regel-kandidaten → „—", kein erfundener Preis), kein B14-Write. Aufnahme = Reuse `radiator.config.*`. **M4-b (Sidebar/Nav) geparkt** bis Parallelstrang-Commit. **M5 fährt die HK-Migrationen produktiv UND schaltet das M4-Flag (`HEIZKOERPER_MODULE_ENABLED`) frei** — bis dahin Modul prod-OFF. |
