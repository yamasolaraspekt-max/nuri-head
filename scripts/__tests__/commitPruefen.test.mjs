...
test('A-02 Kante 2: eine HAENGENDE Halterauskunft laeuft in die Zeitgrenze und blockiert', async () => {
  // **Nachbesserung auf den Evaluator-Befund (05.08.).** Bis dahin behaupteten zwei Kommentare
  // eine Zeitgrenze, und der Aufruf hatte keine — *ein Kommentar, der etwas zusagt, was der Code
  // nicht tut, beendet die Suche an genau der Stelle, an der sie anfangen muesste.*
  //
  // Gemessen wurde in drei Runden, jede hat eine eigene Luecke gezeigt:
  //   1. ohne Zeitgrenze                     -> Tor lief 31 s weiter
  //   2. `perl alarm` in einer Pipe          -> immer noch 31 s: SIGALRM toetet die Shell, aber
  //                                             ein Enkelkind haelt das Pipe-Ende offen, und die
  //                                             Substitution wartet auf EOF statt auf den Prozess
  //   3. ueber eine Datei, Exitcode geprueft -> 2 s, exit 3, Lock liegt
  const verz = wegwerfRepo();
  writeFileSync(join(verz, 'anfang.txt'), 'zweiter Stand\n');
  lockSetzen(verz, '', 400);                     // 0 Byte und alt: der Rueckfall WUERDE ihn raeumen

  // Ein `lsof`, das nicht antwortet — untergeschoben ueber PATH, ohne echte Verzoegerung im Test.
  const bin = join(verz, 'fake-bin');
  execFileSync('mkdir', ['-p', bin]);
  writeFileSync(join(bin, 'lsof'), '#!/bin/sh\nsleep 30\n');
  execFileSync('chmod', ['+x', join(bin, 'lsof')]);

  const start = Date.now();
  let r;
  try {
    const aus = execFileSync('bash', ['scripts/commit-pruefen.sh', 'Probe: haengende Auskunft', 'anfang.txt'],
      { cwd: verz, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], timeout: 25_000,
  ...
  assert.ok(dauer < 10, `das Tor lief ${dauer.toFixed(1)} s — die Zeitgrenze greift nicht`);
  assert.equal(r.code, 3, `erwartet 3 (Umgebung), kam ${r.code}`);
  assert.equal(existsSync(join(verz, '.git', 'index.lock')), true,
    'der Lock wurde geraeumt, obwohl die Auskunft haengt — eine gestoerte Umgebung sagt nichts ueber ihn');
  assert.match(r.text, /ENV_BLOCKED: halterauskunft haengt/, 'die Blockade nennt ihren Grund nicht');
});

test('A-02 Kante 2 ROT: „lsof fehlt" ist NICHT „lsof haengt" — der Rueckfall bleibt getrennt', () => {
  // *Beide Faelle liefern „kein Halter feststellbar", und trotzdem sind sie verschieden:* fehlt
  // `lsof` ganz, ist die Lage bekannt und dauerhaft — dann traegt der konservative Rueckfall
  // (A-02-3). Haengt es, ist die Umgebung gestoert. **Ohne diese Zusage waere eine Fassung gruen,
  // die beides gleich behandelt — und genau die habe ich zuerst gebaut.**
  const verz = wegwerfRepo();
  writeFileSync(join(verz, 'anfang.txt'), 'zweiter Stand\n');
  lockSetzen(verz, '', 400);
  ...
  let r;
  try {
    const aus = execFileSync('bash', ['scripts/commit-pruefen.sh', 'Probe: ohne lsof', 'anfang.txt'],
      { cwd: verz, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'],
        env: { ...process.env, PATH: '/usr/bin:/bin' } });      // kein lsof in /usr/bin
    r = { code: 0, text: aus };
  } catch (e) {
    r = { code: e.status ?? -1, text: `${e.stdout ?? ''}${e.stderr ?? ''}` };
  }

  // 0 Byte UND alt: der konservative Rueckfall darf ihn raeumen — er raeumt WENIGER, nicht mehr.
  assert.equal(r.code, 0, `ohne lsof wurde blockiert statt konservativ zurueckgefallen: ${r.text.slice(0, 140)}`);
  assert.doesNotMatch(r.text, /haengt/, 'der Fall „fehlt" wird als „haengt" gemeldet — zwei Lagen, eine Meldung');
});
