...
use App\Http\Controllers\Customer\Offer\WpAngebotsreifeController;
...
Route::group(['middleware' => ['web', 'auth']], function(){
    ...
    // AP-3a — READ-ONLY Konfigurationsprojekt-Sicht je Objekt (Aggregation aktivierte Gewerke; kein Write, keine neue Tabelle).
    Route::get('/objekt/{alternative}/konfiguration', [\App\Http\Controllers\Customer\Offer\KonfigurationsprojektController::class, 'show'])
        ->whereNumber('alternative')
        ->name('objekt.konfiguration');
...
Route::middleware('auth')->prefix('offers')->name('offers.')->group(function () {
    ...
    // Paket 1 — READ-ONLY Angebotsreife-Anzeige je Gewerkzeile (kein Angebotslogik-Eingriff).
    Route::get('/angebotsreife/{leadProductList}', [WpAngebotsreifeController::class, 'show'])
        ->whereNumber('leadProductList')
        ->name('angebotsreife');
    Route::get('/angebotsreife/{leadProductList}/json', [WpAngebotsreifeController::class, 'json'])
        ->whereNumber('leadProductList')
        ->name('angebotsreife.json');
    // Paket 1b-a — nur das Panel-Partial (Lazy-AJAX-Einbettung, read-only).
    Route::get('/angebotsreife/{leadProductList}/panel', [WpAngebotsreifeController::class, 'panelPartial'])
        ->whereNumber('leadProductList')
        ->name('angebotsreife.panel');
    // Paket 1b-b — READ-ONLY Batch-Status für Kanban-Badges (?ids=1,2,3; nur numerisch, cap 100).
    Route::get('/angebotsreife-batch', [WpAngebotsreifeController::class, 'index'])
        ->name('angebotsreife.index');
    // P3-c-preview — READ-ONLY Auslegungs-Positionsvorschau (kein Write, kein Preis, keine Übernahme).
    Route::get('/auslegung-vorschau/{leadProductList}', [\App\Http\Controllers\Customer\Offer\AuslegungVorschlagController::class, 'panelPartial'])
        ->whereNumber('leadProductList')
        ->name('auslegung.vorschau');
    // P3-d0a — READ-ONLY WP-Katalog-Matching-Diagnose (kein Preisanker, kein component_id, kein Write).
    Route::get('/wp-katalog-matching/{leadProductList}', [\App\Http\Controllers\Customer\Offer\WpKatalogMatchingController::class, 'panelPartial'])
        ->whereNumber('leadProductList')
        ->name('wp-katalog-matching');
    // Paket 4a — READ-ONLY WP-Angebotsworkflow-Cockpit (Führung/Status, kein Write, kein Preisanker).
    Route::get('/workflow/{leadProductList}', [\App\Http\Controllers\Customer\Offer\WpAngebotsWorkflowController::class, 'show'])
        ->whereNumber('leadProductList')
        ->name('workflow');
...
Route::middleware(['auth'])->prefix('admin/hausplaner')->name('hausplaner.objekt.')->group(function () {
    ...
    Route::get('/objekt/{objekt}/snapshots', [\App\Http\Controllers\Hausplaner\HausplanerController::class, 'snapshotListe'])
        ->whereNumber('objekt')->middleware('permission:Hausplaner,read')->name('snapshots.liste');
