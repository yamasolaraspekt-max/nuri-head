...
class OfferReadinessGateTest extends TestCase
{
     ...
     */
    private function wpKombi(int $seed, bool $reif, array $pii = []): array
    {
        $customer = $seed + 1;
        $alt = $seed + 2;
        ...
        DB::table('lead_product_lists')->insert([
            'id' => $seed, 'customer_id' => $customer, 'alternative_id' => $alt, 'product_id' => 2,
            'status' => 'lead', 'interest' => 'WP gewünscht', 'service_id' => 1,
    ...
    public function test_pfad1_wizard_reif_erstellt_offer(): void
    {
        ...
        $this->actingAs($this->admin())->postJson(route('offers.wizard.create'), [
            'customer_id' => $k['customer'], 'alternative_id' => $k['alternative'], 'product_ids' => [2],
        ]);

        $this->assertGreaterThan(0, DB::table('offers')->count(), 'Reifer Vorgang muss ein Angebot anlegen (Gate lässt durch).');
    }
    ...
    public function test_pfad2_offercontroller_reif_erstellt_offer(): void
    {
        ...
        $this->actingAs($this->admin())->postJson(route('admin.offers.store'), [
            'customer_id' => $k['customer'], 'alternative_id' => $k['alternative'], 'product_id' => 2,
            'folder_name' => 'Ordner', 'folder_color' => '#fff',
        ]);
