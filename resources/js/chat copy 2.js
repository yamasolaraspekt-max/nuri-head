...
(function echoReverbViteDiagnostics() {
  const diag = {
    ...
    driver: null,
    ws: { host: null, port: null, secure: null, transports: null, key: null, cluster: null },
    state: null,
    lastError: null,
  ...
  const pusherLike = connector && (connector.pusher || connector.connection || connector.channels);
  ...
  diag.ws.secure = !!(opts.forceTLS ?? opts.encrypted ?? (location.protocol === "https:"));
  ...
  console.table({
    ...
    "enabledTransports": Array.isArray(diag.ws.transports) ? diag.ws.transports.join(",") : String(diag.ws.transports),
  ...
  if (connection?.bind) {
    ...
    console.warn("Echo connector has no connection.bind API. Check Echo init.");
  }
  ...
  try {
    ...
    const url = `${proto}://${host}:${port}/app/${encodeURIComponent(diag.ws.key || "app")}?protocol=7&client=js&version=8.x&flash=false`;
  ...
  function tryPingVite() {
    ...
    fetch(url, { mode: "no-cors" })
      ...
      .catch(() => {
        diag.vite.dev = false;
        diag.vite.ok = false;
        console.log("Vite dev server not detected. Running built assets or different port.");
      });
  }
  ...
  function classifySocketError(err) {
    ...
    if (msg.includes("failed") && msg.includes("tls")) {
      console.warn("Hint: TLS failure. Check REVERB_SCHEME=wss and proxy SSL termination.");
    } else if (msg.includes("401") || msg.includes("403") || msg.includes("unauthorized")) {
      console.warn("Hint: Auth error. Check /broadcasting/auth CSRF, Sanctum cookie, and auth middleware.");
    } else if (msg.includes("closed before connection established")) {
      console.warn("Hint: Port blocked or server not running. Verify reverb:start and firewall rules.");
    } else if (msg.includes("net::err")) {
      console.warn("Hint: Network error. Check wsHost/wsPort, container networking, and CORS for websockets.");
    }
  }
})();
    ...
    fetch("/chat/employee")
        ...
        .then(users => {
            users.forEach(u => {
                ...
                opt.dataset.avatar = u.avatar || u.image || "/images/gender/users.png";
            ...
            if (window.$ && $.fn.select2) {
                $(selectEl).select2({
                    ...
                    templateResult: function (user) {
                        ...
                        const avatar = $(user.element).data("avatar") || "/images/gender/users.png";
                        return `
...
function initDragAndDrop() {
    ...
    window.addEventListener(
        ...
        (e) => {
            ...
            // Only send if a chat is selected
            if (!selectedUserId && !selectedGroupId) {
                Swal?.fire?.(
                    "Kein Chat ausgewählt",
                    "Bitte zuerst einen Chat oder eine Gruppe auswählen.",
                    "info"
                );
                return;
            }
        ...
        .map((r) => {
            ...
            return `
                <div class="py-0.5 text-xs flex items-center justify-between gap-2">
                    ${escapeHtml(name)} - <span class="text-slate-400">${escapeHtml(  label )}</span> 
                </div>
            `;
        })
        .join("");
...
submitBtn.addEventListener("click", async () => {
    ...
    const customerId = document.getElementById("groupCustomerId")?.value || null;
    const alternativeId =
    ...
    const avatarPath = document.getElementById("groupAvatarPath")?.value || null;
