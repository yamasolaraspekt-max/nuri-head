                                        ...
                                        <article style="display: flex; align-items: center;">
                                            ...
                                            <div class="settings col-2"
                                                style="display: flex !important; align-items: flex-start;flex-direction: column; row-gap: 3px;"> 
                                                  <a type="button" class="btn btn-icon btn-icon rounded-circle btn-warning mr-1 mb-1 waves-effect waves-light heart-button" 
                                                    href="{{ url('filter_task_product/'.request()->id.'/'.$item->id) }}" 
                                                    onclick="toggleSelectedClass(event, this)">
                                    ...
                                    @foreach ($task_phase as $task)
                                        @if ( $task->order == 0)
                                            <tr>  
                                                        ...
                                                        @if($done->phase_id == $task->id && $done->customer_id == request()->id && $task->p_active_id == $done->activities_id)
                                                            <fieldset>
                                                                <div class="custom-control custom-checkbox">
                                                                    ...
                                                                    <input type="checkbox" class="custom-control-input" checked name="customCheck" id="done_checkbox" disabled>
                                                                    <label class="custom-control-label" for="done_checkbox"></label>
                                                                </div>
                                                            </fieldset> 
                                                ...
                                                <th scope="row">{{ $task->p_active_id }} .{{ $task->phase_name }}</th>
                                                <td>{{ $task->title }}</td>
                                                                            ...
                                                                            <div class="card-content">
                                                                                <div class="card-body"> 
                                                                                    <div class="form-body">
                                                                                        ...
                                                                                        <div class="col-12">
                                                                                                <div class="form-group">
                                                                                                    <input type="text" class="delete_id" style="color:white; width:0; border:0;">
                                        ...
                                        <div class="float-right">
                                            ...
                                            <div class="modal fade text-left" id="next" tabindex="-1" role="dialog" aria-labelledby="myModalLabel19" aria-hidden="true" style="display: none;">
                                                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-sm" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h4 class="modal-title" id="myModalLabel19">Aufgabenvalidierung</h4>
                                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                <span aria-hidden="true">×</span>
                                                            </button>
                                                        </div>
                                                        ...
                                                        <div class="modal-footer">
                                                            <a type="button" class="btn btn-primary waves-effect waves-light" href="{{ url('/customer_details?search='.request()->id) }}" >Ja</a>
...
@section('script')
  ...
  <script>
   function toggleSelectedClass(event, element) {
    // Find the closest article element
    const article = element.closest('article');
    if (article) {
        // Find the currently selected element and remove the 'selected' class
        const currentlySelected = document.querySelector('.products.selected');
        if (currentlySelected && currentlySelected !== article.querySelector('.products')) {
            currentlySelected.classList.remove('selected');
        }

        // Find the element with the 'products' class within the article
        const productElement = article.querySelector('.products');
        if (productElement) {
            // Toggle the 'selected' class on the product element
            productElement.classList.add('selected');
        }
    }
    
    // Allow the default action (GET request) to proceed
}
