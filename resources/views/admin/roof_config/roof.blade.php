...
<title>Dachstuhl Wizard – Walmdach</title>
<style>
  :root{
  ...
  *{box-sizing:border-box}
  ...
  /* Toolbar */
  #toolbar { position: absolute; top: 0; right: 0; width: 180px; height: 100%;
    background: #1c2a38; color: white; display: flex; flex-direction: column;
    align-items: stretch; padding: 10px; gap: 10px; border-left: 2px solid #2f3d4f; z-index: 200; }
  #toolbar h3 { text-align: center; margin: 10px 0; font-size: 16px; border-bottom: 1px solid #444; padding-bottom: 5px; }
  .tool-btn { background: #2f3d4f; border: none; color: #fff; padding: 10px; cursor: pointer; text-align: left; border-radius: 5px; transition: background 0.2s; }
  .tool-btn:hover { background: #3f5066; }
  #window-list { display: flex; flex-direction: column; gap: 8px; margin-top: 10px; }
  .window-card { background: #2a3b4f; padding: 8px; border-radius: 5px; font-size: 13px; }
  .window-card h4 { margin: 0 0 5px; font-size: 14px; color: #fff; }
  .window-card label { display: block; margin: 4px 0; color: #ddd; }
  .window-card input { width: 60px; margin-left: 5px; }
  .window-card button { margin-top: 4px; background: #444; color: #fff; border: none; padding: 4px 6px; cursor: pointer; border-radius: 3px; }
  .window-card button:hover { background: #666; }
</style>
</head>
<body>
...
<div class="app">
  <div class="top">
    <div class="brand">Dachstuhl Wizard — Walmdach</div>
    ...
    <aside class="sidebar">
      <div class="panel">
        ...
        <div class="grid">
          ...
          <div class="field"><div class="label">Breite/Spannweite S (m)</div><input id="S" value="8"></div>
...
for (const cdn of cdns) {
  try { THREE = await import(cdn.three);
        ({ OrbitControls } = await import(cdn.orbit));
        ({ TransformControls } = await import(cdn.transform));
        break; } catch (e) { console.warn("CDN fail, next…", e); }
}
if (!THREE) throw new Error("THREE failed");
...
const grid=new THREE.GridHelper(200,200,0x3b5e8a,0x223b5a); grid.material.transparent=true; grid.material.opacity=0.35; scene.add(grid);

const ro=new ResizeObserver(()=>{ const w=stage.clientWidth, h=stage.clientHeight;
  if (w>0&&h>0){ renderer.setSize(w,h,false); camera.aspect=w/h; camera.updateProjectionMatrix(); }});
ro.observe(stage);

/* ============================
   Graph groups & materials
============================ */
const ROOT=new THREE.Group(); scene.add(ROOT);
const G={};
[
"Mauerwerk","Pfetten","Sparren","Konter","Eindeck","Ziegel","Overlay",
...
function P(){
  ...
  const hipTaper = Math.min(S/2, L/2);
  const ridgeLen = Math.max(0, L - S);

  return {L,S,HT,HF,aDeg:a,alpha,tW,UL,UR,UT,Bf,Hf,Bfu,Hfu,Br,Hr,Bk,Hk,Be,He,eSpacing,Ltr,Leff,s_final,zMin,zMax,hipTaper,ridgeLen};
}

function S_eff_atZ(z, p){
  const zCore = clamp(z, 0, p.L);
  const d = Math.min(zCore, p.L - zCore);
  if (p.ridgeLen <= 0){
    return (p.S/2) * (d / p.hipTaper);
  }
  if (zCore >= p.hipTaper && zCore <= (p.L - p.hipTaper)) return p.S/2;
  return (p.S/2) * (Math.min(d, p.hipTaper) / p.hipTaper);
}

/* ============================
   Bases & geometry helpers
============================ */
function makeTileQuaternion(normal, upslope, referenceAxis){
  const n = normal.clone().normalize();
  let across = referenceAxis.clone().sub(n.clone().multiplyScalar(referenceAxis.dot(n)));
  if (across.lengthSq() < 1e-12) {
    across = new THREE.Vector3().crossVectors(upslope, n);
  } else {
    across.normalize();
  }
  if (across.dot(referenceAxis) < 0) across.multiplyScalar(-1);
  const u = upslope.clone().sub(n.clone().multiplyScalar(upslope.dot(n))).normalize();
  across.sub(n.clone().multiplyScalar(across.dot(n))).normalize();
  const M = new THREE.Matrix4().makeBasis(across, u, n);
  return new THREE.Quaternion().setFromRotationMatrix(M);
}

function makePfanneTileGeometry(w=0.300, h=0.335, t=0.018){
  const segX = 12, segY = 2;
  const geo = new THREE.BoxGeometry(w, h, t, segX, segY, 1);
  ...
  for (let i=0;i<pos.count;i++){
    ...
    if (z > halfT - 1e-6){
      const u = (x + w/2) / w;
      const ripple = 0.007*Math.sin(2*Math.PI*u) + 0.004*Math.sin(4*Math.PI*u + Math.PI*0.3);
      pos.setZ(i, z + ripple);
    }
  }
...
function Z_run_atX(x, p) {
  const halfS = p.S / 2;
  const ax = Math.min(Math.abs(x), halfS);
  const factor = 1 - (ax / Math.max(1e-6, halfS));
  return Math.max(0, p.hipTaper * factor);
}
...
/* ============================
   Windows (unchanged core)
============================ */
let windowCounter=0, selectedWindow=null, placingWindow=false, ghostWindow=null;
const windowList=document.getElementById("window-list");
function createWindowMesh(width=1.2,height=1.0){
  const group=new THREE.Group();
  const depth=0.15;
  const frameMat=new THREE.MeshStandardMaterial({color:"#555",roughness:0.85,metalness:0.2});
  const glassMat=new THREE.MeshPhysicalMaterial({color:"#99ccee",transmission:0.8,opacity:0.6,transparent:true,roughness:0.05,metalness:0.1,clearcoat:0.8,clearcoatRoughness:0.1});
  const frame=new THREE.Mesh(new THREE.BoxGeometry(width,height,depth),frameMat);
  const glass=new THREE.Mesh(new THREE.PlaneGeometry(width*0.9,height*0.9),glassMat); glass.position.z=depth/2-0.01;
  group.add(frame,glass);
  group.userData={type:"window",id:++windowCounter,width,height};
  return group;
}
$("#addWindowBtn").addEventListener("click", ()=>{
  if (placingWindow) return;
  ghostWindow=createWindowMesh(); scene.add(ghostWindow); placingWindow=true;
});
...
renderer.domElement.addEventListener("pointermove",(event)=>{
  ...
  if (hits.length>0){
    ...
    ghostWindow.lookAt(hit.point.clone().add(hit.face.normal));
  }
});
renderer.domElement.addEventListener("pointerdown",(event)=>{
  if (placingWindow && ghostWindow){
    const finalWindow=ghostWindow; placingWindow=false; ghostWindow=null;
    addWindowCard(finalWindow); scene.add(finalWindow); transformControls.attach(finalWindow); selectedWindow=finalWindow; return;
  }
  mouse.x=(event.clientX/renderer.domElement.clientWidth)*2-1;
  mouse.y=-(event.clientY/renderer.domElement.clientHeight)*2+1;
  raycaster.setFromCamera(mouse,camera);
  ...
  if (intersects.length>0){
    ...
    if (obj.userData.type==="window") selectWindow(obj);
  }
});
function resizeWindow(mesh,width,height){
  const depth=0.15; mesh.clear();
  const frameMat=new THREE.MeshStandardMaterial({color:"#555",roughness:0.85,metalness:0.2});
  const glassMat=new THREE.MeshPhysicalMaterial({color:"#99ccee",transmission:0.8,opacity:0.6,transparent:true,roughness:0.05,metalness:0.1,clearcoat:0.8,clearcoatRoughness:0.1});
  const frame=new THREE.Mesh(new THREE.BoxGeometry(width,height,depth),frameMat);
  const glass=new THREE.Mesh(new THREE.PlaneGeometry(width*0.9,height*0.9),glassMat); glass.position.z=depth/2-0.01;
  mesh.add(frame,glass); mesh.userData.width=width; mesh.userData.height=height;
}
function addWindowCard(mesh){
  const li=document.createElement("li");
  li.dataset.id=mesh.userData.id;
  li.innerHTML=`<strong>Window #${mesh.userData.id}</strong><br>
  ...
  const wi=li.querySelector(".win-width"), hi=li.querySelector(".win-height"), del=li.querySelector(".delete-btn");
  wi.addEventListener("change", ()=> resizeWindow(mesh, parseFloat(wi.value), mesh.userData.height));
  hi.addEventListener("change", ()=> resizeWindow(mesh, mesh.userData.width, parseFloat(hi.value)));
  del.addEventListener("click", ()=>{ scene.remove(mesh); transformControls.detach(); li.remove(); });
  li.addEventListener("click", ()=> selectWindow(mesh));
}
function selectWindow(mesh){
  selectedWindow=mesh; transformControls.attach(mesh);
}
...
$("#rafters")?.addEventListener("change", ()=>{
  const vals=$("#rafters").value.split(",").map(v=>parseFloat(v.trim())).filter(Number.isFinite);
  if (vals.length>0){ rafterPositions=vals; rebuild(); }
});

/* ============================
   Build structure (Walmdach)
============================ */
let WALLS={};
const WALL_OFFSETS={ left:new THREE.Vector3(), right:new THREE.Vector3(), front:new THREE.Vector3(), back:new THREE.Vector3() };

function quatForHipFaceLongX(nVec){
  const n = nVec.clone(); if (n.lengthSq()<1e-12) n.set(0,1,0); n.normalize();
  let Y = new THREE.Vector3(1,0,0);
  Y.sub(n.clone().multiplyScalar(Y.dot(n)));
  if (Y.lengthSq()<1e-12){
    Y.set(0,0,1); Y.sub(n.clone().multiplyScalar(Y.dot(n)));
    if (Y.lengthSq()<1e-12){ Y.set(0,1,0); Y.sub(n.clone().multiplyScalar(Y.dot(n))); }
  }
  Y.normalize();
  const Z = new THREE.Vector3().crossVectors(n, Y).normalize();
  return new THREE.Quaternion().setFromRotationMatrix(new THREE.Matrix4().makeBasis(n, Y, Z));
}

function quatForHipFaceLongZ(nVec){
  const n = nVec.clone();
  ...
  Y.normalize();
  const Z = new THREE.Vector3().crossVectors(n, Y).normalize();
  const M = new THREE.Matrix4().makeBasis(n, Y, Z);
  return new THREE.Quaternion().setFromRotationMatrix(M);
}

function buildStructure(stepMax=1){
  const p=P();
  dbg(`Walmdach • α=${p.aDeg.toFixed(1)}°, L=${p.L.toFixed(2)}m, S=${p.S.toFixed(2)}m, First=${p.ridgeLen.toFixed(2)}m`);
  ...
  if (stepMax>=1){
    ...
    const backRect = frontRect.slice();

    const leftWall  = createWall(leftPts,  { thickness: p.L, z: 0 });
    const rightWall = createWall(rightPts, { thickness: p.L, z: 0 });
    const frontWall = createWall(frontRect, { thickness: p.tW, z: 0 });
    const backWall  = createWall(backRect,  { thickness: p.tW, z: p.L - p.tW });

    const setWallMeta=(mesh,name)=>{ mesh.name=name; mesh.userData.kind="wall"; mesh.userData.wallName=name; mesh.userData.basePos=mesh.position.clone(); };
    setWallMeta(leftWall,"left"); setWallMeta(rightWall,"right"); setWallMeta(frontWall,"front"); setWallMeta(backWall,"back");
  ...
  if (stepMax>=2){
    ...
    const ridgeCenterZ = p.L/2;
    if (p.ridgeLen>0.001){
      const first = new THREE.Mesh(new THREE.BoxGeometry(p.Bf, p.Hf, p.ridgeLen + p.UL + p.UR), MAT.wood);
      first.position.set(0, p.HF - p.Hf/2, ridgeCenterZ);
      G.Pfetten.add(first);
    }
    const footL = new THREE.Mesh(new THREE.BoxGeometry(p.Bfu, p.Hfu, p.L + p.UL + p.UR), MAT.wood);
    footL.position.set(-p.S/2, p.HT + p.Hfu/2, zC);
    const footR = footL.clone(); footR.position.x=+p.S/2;
    G.Pfetten.add(footL, footR);
  }

  // 3) Sparren with hip taper
  ...
  let zList;
  if (Array.isArray(rafterPositions) && rafterPositions.length>0){ zList=rafterPositions.slice(); }
  else{
    ...
    rafterPositions=[...zList]; syncRafterInput();
  }

  let xList;
  {
    const countX = Math.max(1, Math.round((p.S/2) / Math.max(0.1, p.s_final)));
    const insideX = Array.from({length: countX*2+1}, (_,k)=> -p.S/2 + k*(p.S/(countX*2)));
    xList = insideX;
  }
  ...
  if (stepMax>=3){
    ...
    for (let i=0;i<zList.length;i++){
      ...
      const halfSpan = S_eff_atZ(clamp(z,0,p.L), p);
      const runAlongSlope = halfSpan / Math.max(1e-6, cosA);
      const cutL = (p.Bf/2) / Math.max(1e-6, Math.abs(bL.u.x));
      const cutR = (p.Bf/2) / Math.max(1e-6, Math.abs(bR.u.x));
      const lenY_L = Math.max(0.01, l_over + Math.max(0.01, runAlongSlope - cutL));
      const lenY_R = Math.max(0.01, l_over + Math.max(0.01, runAlongSlope - cutR));

      const geoL = makeRafterGeometry(lenY_L, p.Hr, p.Br, p.alpha, +1);
      const meshL=new THREE.Mesh(geoL, MAT.wood);
      ...
      const geoR = makeRafterGeometry(lenY_R, p.Hr, p.Br, p.alpha, -1);
      const meshR=new THREE.Mesh(geoR, MAT.wood);
  ...
  if (stepMax >= 3) {
  const l_overZ = p.UT / Math.max(1e-6, cosA);
  const uF = new THREE.Vector3(0, (p.HF - (p.HT + p.Hfu)) / Math.max(1e-6, p.hipTaper), +1);
  const uB = new THREE.Vector3(0, (p.HF - (p.HT + p.Hfu)) / Math.max(1e-6, p.hipTaper), -1);
  const bF = basisRafter(uF);
  const bB = basisRafter(uB);

  for (let i = 0; i < xList.length; i++) {
    const x = xList[i];
    const zRun = Z_run_atX(x, p);
    if (zRun <= 1e-4) continue;
    const runAlongSlope = zRun / Math.max(1e-6, cosA);

    {
      const geo = makeRafterGeometry(runAlongSlope, p.Hr, p.Br, p.alpha, +1);
      const mesh = new THREE.Mesh(geo, MAT.wood);
      const foot = new THREE.Vector3(x, p.HT + p.Hfu, 0);
      const start = foot.clone().add(bF.u.clone().multiplyScalar(-l_overZ));
      const end   = foot.clone().add(bF.u.clone().multiplyScalar(runAlongSlope - l_overZ));
      const center= start.clone().add(end).multiplyScalar(0.5).add(bF.n.clone().multiplyScalar(p.Hr/2));
      mesh.position.copy(center); mesh.quaternion.copy(bF.q);
      mesh.userData = { kind:"rafter", side:"F", index:i, Hr:p.Hr, Br:p.Br };
      G.Sparren.add(mesh);
    }

    {
      const geo = makeRafterGeometry(runAlongSlope, p.Hr, p.Br, p.alpha, -1);
      const mesh = new THREE.Mesh(geo, MAT.wood);
      const foot = new THREE.Vector3(x, p.HT + p.Hfu, p.L);
      const start = foot.clone().add(bB.u.clone().multiplyScalar(-l_overZ));
      const end   = foot.clone().add(bB.u.clone().multiplyScalar(runAlongSlope - l_overZ));
      const center= start.clone().add(end).multiplyScalar(0.5).add(bB.n.clone().multiplyScalar(p.Hr/2));
      mesh.position.copy(center); mesh.quaternion.copy(bB.q);
      mesh.userData = { kind:"rafter", side:"B", index:i, Hr:p.Hr, Br:p.Br };
      G.Sparren.add(mesh);
    }
  }
}
  ...
  if (stepMax>=4){
    ...
    function placeK(i,z,side){
      ...
      const halfSpan = S_eff_atZ(clamp(z,0,p.L), p);
      const runAlongSlope = halfSpan / Math.max(1e-6, Math.abs(Math.cos(p.alpha)));
      const foot=new THREE.Vector3(side==="L"? -p.S/2 : p.S/2, p.HT + p.Hfu, z);
      const start=foot.clone().add(b.u.clone().multiplyScalar(-l_over));
      const end=foot.clone().add(b.u.clone().multiplyScalar(Math.max(0.01, runAlongSlope - 0.001)));
      const mid=start.clone().add(end).multiplyScalar(0.5);
      const c = mid.add(b.z.clone().multiplyScalar(p.Hr + p.Hk/2 + eps));
      const mat=new THREE.Matrix4().compose(c, b.q, new THREE.Vector3(1, end.distanceTo(start), 1));
      (side==="L"?kL:kR).setMatrixAt(i, mat);
    }
    for (let i=0;i<zList.length;i++){ const z=zList[i]; if(!Number.isFinite(z)) continue; placeK(i,z,"L"); placeK(i,z,"R"); }
  ...
  if (stepMax >= 4) {
    ...
    const Bk_eff = (br, bk) => Math.min(bk, Math.max(0.01, 0.9 * br));
    const geoK_F = new THREE.BoxGeometry(p.Hk, 1.0, Bk_eff(p.Br, p.Bk));
    const geoK_B = new THREE.BoxGeometry(p.Hk, 1.0, Bk_eff(p.Br, p.Bk));
    const kF = new THREE.InstancedMesh(geoK_F, MAT.konter, xList.length);
    const kB = new THREE.InstancedMesh(geoK_B, MAT.konter, xList.length);

    function placeK_FB(i, x, side) {
      const b = (side === "F") ? bF : bB;
      const zRun = Z_run_atX(x, p);
      if (zRun <= 1e-4) return;
      const runAlongSlope = zRun / Math.max(1e-6, cosA);
      const foot = new THREE.Vector3(x, p.HT + p.Hfu, (side === "F") ? 0 : p.L);
      const start = foot.clone().add(b.u.clone().multiplyScalar(-l_overZ));
      const end   = foot.clone().add(b.u.clone().multiplyScalar(Math.max(0.01, runAlongSlope - 0.001)));
      const mid = start.clone().add(end).multiplyScalar(0.5);
      const c = mid.add(b.z.clone().multiplyScalar(p.Hr + p.Hk/2 + eps));
      const mat = new THREE.Matrix4().compose(c, b.q, new THREE.Vector3(1, end.distanceTo(start), 1));
      (side === "F" ? kF : kB).setMatrixAt(i, mat);
    }
    ...
    kF.instanceMatrix.needsUpdate = kB.instanceMatrix.needsUpdate = true;
    G.Konter.add(kF, kB);
  }
 ...
 // =====================
  if (stepMax >= 5) {
    ...
    // LEFT / RIGHT faces: trim in Z (core only)
    const qLatL = quatForHipFaceLongZ(bL.z);
    const qLatR = quatForHipFaceLongZ(bR.z);

    const spanZ = p.L;

    const halfSpanCenter = S_eff_atZ(p.L / 2, p);
    const lenRoofSide = (halfSpanCenter + p.UT) / Math.max(1e-6, cosA);
    const rowsSide = Math.max(1, Math.ceil(lenRoofSide / Math.max(0.05, p.eSpacing)));

    const eGeoSide = new THREE.BoxGeometry(p.He, spanZ, p.Be);
    const eL = new THREE.InstancedMesh(eGeoSide, MAT.batten, rowsSide);
    const eR = new THREE.InstancedMesh(eGeoSide, MAT.batten, rowsSide);

    const l_over_side = p.UT / Math.max(1e-6, cosA);

    function allowedZIntervalForRow(dist) {
      const reqHalf = Math.min(p.S/2, Math.max(0, cosA * (dist - l_over_side)));
      const margin = p.hipTaper * (reqHalf / Math.max(1e-6, p.S/2));
      return { zMin: margin, zMax: p.L - margin };
    }
    ...
    for (let r = 0; r < rowsSide; r++) {
      ...
      // LEFT
      {
        const { zMin, zMax } = allowedZIntervalForRow(dist);
        const allowed = zMax - zMin;
        if (allowed <= 1e-3) continue;
        const zMid = 0.5 * (zMin + zMax);

        const base = new THREE.Vector3(-p.S/2, p.HT + p.Hfu, zMid)
          .add(bL.u.clone().multiplyScalar(dist - l_over_side))
          .add(bL.z.clone().multiplyScalar(p.Hr + p.Hk + p.He/2 + eps));

        const scaleY = allowed / spanZ;
        eL.setMatrixAt(r, new THREE.Matrix4().compose(base, qLatL, new THREE.Vector3(1, scaleY, 1)));
      }
      ...
      // RIGHT
      {
        ...
        const base = new THREE.Vector3(+p.S/2, p.HT + p.Hfu, zMid)
          .add(bR.u.clone().multiplyScalar(dist - l_over_side))
          .add(bR.z.clone().multiplyScalar(p.Hr + p.Hk + p.He/2 + eps));

        const scaleY = allowed / spanZ;
        eR.setMatrixAt(r, new THREE.Matrix4().compose(base, qLatR, new THREE.Vector3(1, scaleY, 1)));
      }
    }
    eL.instanceMatrix.needsUpdate = eR.instanceMatrix.needsUpdate = true;
    G.Eindeck.add(eL, eR);

    // FRONT / BACK faces: trim in X
    const uF = new THREE.Vector3(0, (p.HF - (p.HT + p.Hfu)) / Math.max(1e-6, p.hipTaper), +1);
    const uB = new THREE.Vector3(0, (p.HF - (p.HT + p.Hfu)) / Math.max(1e-6, p.hipTaper), -1);
    const bF = basisRafter(uF), bB = basisRafter(uB);

    const qF = quatForHipFaceLongX(bF.z);
    const qB = quatForHipFaceLongX(bB.z);

    const zRunCenter = Math.min(p.hipTaper, p.L/2);
    const lenRoofFB  = (zRunCenter + p.UT) / Math.max(1e-6, cosA);
    const rowsFB     = Math.max(1, Math.ceil(lenRoofFB / Math.max(0.05, p.eSpacing)));

    const spanX = p.S;
    const eGeoFB = new THREE.BoxGeometry(p.He, spanX, p.Be);
    const eF = new THREE.InstancedMesh(eGeoFB, MAT.batten, rowsFB);
    const eB = new THREE.InstancedMesh(eGeoFB, MAT.batten, rowsFB);

    const l_over_FB = p.UT / Math.max(1e-6, cosA);
    const footF = new THREE.Vector3(0, p.HT + p.Hfu, 0);
    const footB = new THREE.Vector3(0, p.HT + p.Hfu, p.L);

    function allowedXSpanAtZ(z){
      const zCore = clamp(z, 0, p.L);
      const half  = S_eff_atZ(zCore, p);
      return 2 * Math.max(0, half);
    }

    for (let r = 0; r < rowsFB; r++){
      const along = Math.max(0, (r + 0.5) * Math.max(0.05, p.eSpacing) - l_over_FB);

      // FRONT
      {
        const pt = footF.clone().add(bF.u.clone().multiplyScalar(along));
        const allowed = allowedXSpanAtZ(pt.z);
        if (allowed <= 1e-3) continue;
        const xScale  = allowed / spanX;
        const center = pt.add(bF.z.clone().multiplyScalar(p.Hr + p.Hk + p.He/2 + eps));
        eF.setMatrixAt(r, new THREE.Matrix4().compose(center, qF, new THREE.Vector3(1, xScale, 1)));
      }

      // BACK
      {
        const pt = footB.clone().add(bB.u.clone().multiplyScalar(along));
        const allowed = allowedXSpanAtZ(pt.z);
        if (allowed <= 1e-3) continue;
        const xScale  = allowed / spanX;
        const center = pt.add(bB.z.clone().multiplyScalar(p.Hr + p.Hk + p.He/2 + eps));
        eB.setMatrixAt(r, new THREE.Matrix4().compose(center, qB, new THREE.Vector3(1, xScale, 1)));
      }
    }
    eF.instanceMatrix.needsUpdate = eB.instanceMatrix.needsUpdate = true;
    G.Eindeck.add(eF, eB);
  }

  return {p, bL, bR, l_over};
}
...
function buildDims(ctx, stepMax){
  ...
  if($("#chk-Exterior")?.checked){
    ...
    const a2=new THREE.Vector3(0,0.05,0), b2=new THREE.Vector3(0,0.05,p.L);
  ...
  if ($("#chk-First")?.checked) {
    const ridgeCenterZ = p.L / 2;
    const a = new THREE.Vector3(0, p.HF + 0.20, ridgeCenterZ - p.ridgeLen / 2);
    const b = new THREE.Vector3(0, p.HF + 0.20, ridgeCenterZ + p.ridgeLen / 2);
    const label = p.ridgeLen > 0.001 ? `Firstlänge = ${fmt(p.ridgeLen)}` : `Firstlänge = ${fmt(0)}`;
    G["Dims:First"].add(dimChain(a, b, new THREE.Vector3(1, 0, 0), 0.28, DCLR.First, label));
  }
  if ($("#chk-SparrenLen")?.checked) {
    const halfSpanCenter = S_eff_atZ(p.L / 2, p);
    const l_UT = (halfSpanCenter + p.UT) / Math.max(1e-6, Math.abs(Math.cos(p.alpha)));
    const footL = new THREE.Vector3(-p.S / 2, p.HT + p.Hfu, p.L / 2);
    const footR = new THREE.Vector3(+p.S / 2, p.HT + p.Hfu, p.L / 2);
    const bLdir = basisRafter(new THREE.Vector3(+(p.S / 2), (p.HF - (p.HT + p.Hfu)), 0)).u;
    const bRdir = basisRafter(new THREE.Vector3(-(p.S / 2), (p.HF - (p.HT + p.Hfu)), 0)).u;
    const aL = footL.clone().add(bLdir.clone().multiplyScalar(-l_over));
    const bLend = footL.clone().add(bLdir.clone().multiplyScalar(l_UT - l_over));
    const aR = footR.clone().add(bRdir.clone().multiplyScalar(-l_over));
    const bRend = footR.clone().add(bRdir.clone().multiplyScalar(l_UT - l_over));
    G["Dims:SparrenLen"].add(dimChain(aL, bLend, new THREE.Vector3(0, 1, 0), 0.25, DCLR.SparrenLen, `Sparrenlänge ≈ ${fmt(l_UT)}`));
    G["Dims:SparrenLen"].add(dimChain(aR, bRend, new THREE.Vector3(0, 1, 0), 0.25, DCLR.SparrenLen, `Sparrenlänge ≈ ${fmt(l_UT)}`));
  }
  ...
  if ($("#chk-Lattung")?.checked) {
    ...
    const bLdir = basisRafter(new THREE.Vector3(+(p.S / 2), (p.HF - (p.HT + p.Hfu)), 0)).u;
    const bRdir = basisRafter(new THREE.Vector3(-(p.S / 2), (p.HF - (p.HT + p.Hfu)), 0)).u;
    G["Dims:Lattung"].add(dimChain(baseL.clone(), baseL.clone().add(bLdir.clone().multiplyScalar(l_UT)), new THREE.Vector3(0, 1, 0), 0.22, DCLR.Lattung, `eSpacing = ${fmt(p.eSpacing)}  (Reihen ≈ ${rows})`));
    G["Dims:Lattung"].add(dimChain(baseR.clone(), baseR.clone().add(bRdir.clone().multiplyScalar(l_UT)), new THREE.Vector3(0, 1, 0), 0.22, DCLR.Lattung, `eSpacing = ${fmt(p.eSpacing)}  (Reihen ≈ ${rows})`));
  }
...
function getPickables() { return editMode === "wall" ? Object.values(WALLS) : G.Sparren.children; }
...
function initInteraction() {
  ...
  renderer.domElement.addEventListener("pointermove", (e) => {
    ...
    if (hits.length) {
      ...
      tip.textContent = u.kind === "wall"
        ? `Wand: ${u.wallName}`
        : `Sparren ${u.side ?? "?"}-${String((u.index ?? 0) + 1).padStart(2, "0")}  z=${(obj.position.z).toFixed(2)} m`;
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (key === "r" && !e.ctrlKey && !e.metaKey) { editMode = "rafter"; if (selectedObj && selectedObj.userData.kind !== "rafter") { resetMaterial(selectedObj); transformControls.detach(); selectedObj = null; } }
    if (!selectedObj) return;
...
============================ */
function pfanneGaugeForAngle(aDeg){
  if (aDeg > 30) return 0.345;
  if (aDeg >= 22) return 0.335 + (aDeg-22)/8*(0.345-0.335);
  const d = Math.min(22 - aDeg, 6);
  return 0.320 - (d/6)*(0.320 - 0.312);
}
