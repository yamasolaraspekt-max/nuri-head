    ...
    <div class="content-wrapper">
        <div class="content-header row mb-2">
            <div class="col-12 d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h2 class="content-header-title float-left mb-0 border-0">ANFRAGE</h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ url('/employee_dashboard') }}">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="{{ url('/inquiry_view') }}">Anfrageliste</a></li>
                            <li class="breadcrumb-item active">{{ $data->firma ?? '' }} - {{ $data->name ?? '' }} {{ $data->lastname ?? '' }}</li>
                        </ol>
                    </div>
                </div>
                ...
                <div class="mt-1 mt-md-0">
                    @if($data->status != "Published")
                        <a class="btn btn-primary" href="{{ url('inquiry_edit/'.$data->id) }}">
                                ...
                                <div class="tab-content">
                                                                ...
                                                                <div class="kanban-card draggable {{ $alreadyVerified ? 'kanban-card-verified' : '' }}"
                                                                   ...
                                                                   @php
                                                                        ...
                                                                        $initials = function ($text) {
                                                                            ...
                                                                            if (!$words || count($words) === 0) return 'PR';
                                    ...
                                    <div class="tab-pane fade" id="verify-view" role="tabpanel">
                                        @php
                                            ...
                                            $targetType = null; $targetLabel = null; $targetUrl = null;
                                            if ($lead) {
                                                $targetType = 'Lead'; $targetLabel = 'Lead #'.$lead->id; $targetUrl = url('new_lead_profile/'.$lead->id);
                                            } else {
                                                ...
                                                if ($dist) { $targetType = 'Lieferant'; $targetLabel = $dist->name; $targetUrl = route('distributor'); }
                                                elseif ($brand) { $targetType = ($brand->type === 'brand' ? 'Hersteller' : $brand->type); $targetLabel = $brand->name; $targetUrl = route('brand.info'); }
                                            }
