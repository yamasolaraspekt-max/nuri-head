                        ...
                        <div class="card mb-1 shadow-sm">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-1">
                                    ...
                                    <div class="col-md-5">
                                        <label>Vorname</label>
                                        <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $data->name) }}" list="name-options">
                                        <datalist id="name-options"></datalist>
                                    </div>
                                    <div class="col-md-5">
                                        <label>Nachname</label>
                                        <input type="text" class="form-control" id="lastname" name="lastname" value="{{ old('lastname', $data->lastname) }}" list="lastname-options">
                               ...
                               <div class="form-group mb-1">
                                    ...
                                    <input type="text" id="full_address" class="form-control" name="full_address"
                                           value="{{ old('full_address', $data->full_address ?? trim(($data->street ?? '').' '.($data->house_number ?? '').', '.($data->postcode ?? '').' '.($data->city ?? ''))) }}"
                                           placeholder="Straße Hausnummer, PLZ Ort" autocomplete="off">
                                    ...
                                    <input type="hidden" id="route-input" name="route" value="{{ old('route', $data->route ?? '') }}">
                                    <input type="hidden" id="street_number-input" name="house_number" value="{{ old('house_number', $data->house_number ?? '') }}">
...
@section('script')
    ...
    <!-- Name and Lastname suggestions  -->
     ...
     <script>
       $(document).ready(function() {
        $('#lastname, #name, #full_address, #street-input, #postal_code-input, #locality-input').on('change', function() {
            // Collect form data
            var lastname = $('#lastname').val();
            var name = $('#name').val();
            var street = $('#street-input').val() || $('#full_address').val();
    ...
    <script>
        ...
        window.initMap = function () {
            ...
            map = new google.maps.Map(mapDiv, {
                center: position,
                zoom: latInput?.value && lngInput?.value ? 18 : 13,
                mapTypeId: 'roadmap',
            ...
            mapMarker.addListener('dragend', function () {
                ...
                setValue('latitude-input', pos.lat());
                setValue('longitude-input', pos.lng());
                updateElevation(pos);
                reverseGeocode(pos);
            });
        ...
        function getAddressComponent(components, type, useShortName = false) {
            ...
            if (!item) return '';
            return useShortName ? (item.short_name || item.long_name || '') : (item.long_name || item.short_name || '');
        }

        function parseAddressComponents(components) {
            const route = getAddressComponent(components, 'route');
            const streetNumber = getAddressComponent(components, 'street_number');
            const postcode = getAddressComponent(components, 'postal_code') || getAddressComponent(components, 'postal_code_prefix');
        ...
        function fillAddressFields(place) {
            ...
            const formattedAddress = place.formatted_address || [parsed.street, parsed.postcode, parsed.city].filter(Boolean).join(', ');

            setValue('route-input', parsed.route);
            setValue('street_number-input', parsed.streetNumber);
            setValue('street-input', parsed.street);
            setValue('postal_code-input', parsed.postcode);
            setValue('locality-input', parsed.city);
            setValue('full_address', formattedAddress);
        }

        function updateElevation(location) {
            if (!elevationService || !location) return;

            elevationService.getElevationForLocations({ locations: [location] }, function (results, status) {
                const elevation = (status === google.maps.ElevationStatus.OK && results && results[0])
                    ? Number(results[0].elevation).toFixed(2)
                    : '';
                setValue('elevation-input', elevation);
            });
        }
        ...
        function initAutocomplete() {
            ...
            if (!fullAddressInput || !google.maps.places) return;
            ...
            autocomplete.addListener('place_changed', function () {
                ...
                if (!place || !place.geometry || !place.geometry.location) {
                    Swal.fire({
                        ...
                        text: 'Bitte wählen Sie eine Adresse aus der Google-Liste aus.'
                    });
                    return;
                }
                ...
                setValue('latitude-input', location.lat());
                setValue('longitude-input', location.lng());
