...
@php
    ...
    $canVerify = $permissions['canVerify'] ?? true;

    $productListGrouped = $productListGrouped ?? collect();
...
.oc-btn-ic.purple:hover{border-color:var(--purple)}
...
.verify-drawer-badge--ok{background:#ecfdf3;color:#166534}
.verify-drawer-badge--warn{background:#fef3c7;color:#92400e}
.verify-drawer-badge--lead{background:#eff6ff;color:var(--blue)}
.verify-drawer-badge--other{background:#e5e7eb;color:#374151}
.verify-drawer-name{
    font-size:.9rem;
    font-weight:600;
    color:#111827;
        ...
        <div class="oc-toolbar-left">
            <div class="oc-filter-block search">
                ...
                <input
                    ...
                    placeholder="Suche nach Name, Firma, Adresse, Produkt, Typ, Status, Notiz, Junk-Grund"
                    name="search"
            ...
            <div class="oc-filter-block">
                ...
                <select class="oc-select" name="status_filter">
                    ...
                    <option value="Unpublished" {{ request('status_filter') === 'Unpublished' ? 'selected' : '' }}>Nicht verifiziert</option>
                    <option value="Published" {{ request('status_filter') === 'Published' ? 'selected' : '' }}>Veröffentlicht</option>
        ...
        <div class="oc-list">
            @forelse($data as $item)
                @php
                    ...
                    $customerProducts = $productListGrouped[$item->id] ?? collect();
                    if ($customerProducts instanceof \Illuminate\Support\Collection === false) {
                        $customerProducts = collect($customerProducts);
                    }
                            ...
                            <div class="oc-main">
                                <div class="d-flex align-items-start justify-content-between flex-wrap" style="gap:10px;">
                                    ...
                                    <div class="d-flex" style="gap:8px;">
                                        <button type="button"
                                                class="oc-btn-ic primary js-open-contact"
                                                ...
                                                data-firma="{{ $fs($item->firma ?? null, 'Keine Firma') }}"
                                                title="Kontakt">
                                        ...
                                        @if(!empty($item->note))
                                            <button type="button"
                                                    class="oc-btn-ic purple js-open-note"
                                                    data-name="{{ $safeFullName }}"
                        ...
                        <div class="oc-cell">
                            ...
                            <div class="oc-main">
                                ...
                                <div class="d-flex align-items-center flex-wrap mt-2" style="gap:8px;">
                                    <i class="feather icon-alert-circle {{ (!empty($item->periority) && strtolower($item->periority) === 'sehr dringend') ? 'text-danger' : 'text-muted' }}"></i>
                                    <i class="feather icon-bell {{ $hoursDifference > 48 ? 'text-primary' : 'text-muted' }}"></i>
                                    <i class="feather icon-star {{ $hoursDifference <= 48 ? 'text-warning' : 'text-muted' }}"></i>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="oc-main">
                                @if($customerProducts->isNotEmpty())
                                    <div class="oc-flow-list">
                                        @foreach($customerProducts->unique('id') as $product)
                                            @php
                                            ...
                                            <div class="oc-flow-card">
                                                <div class="oc-flow-top">
                                                    ...
                                                    <img src="{{ $insideImg }}"
                                                         alt="Innendienst"
                                                         class="oc-flow-avatar select-employee"
                                                         data-type="employee"
                                                         data-id="{{ $product->id }}"
                                                         title="{{ $insideTitle }}">
                                                    ...
                                                    <img src="{{ $fieldImg }}"
                                                         alt="Außendienst"
                                                         class="oc-flow-avatar outside select-employee"
                                ...
                                @if($isJunkRow)
                                    <div class="oc-junk-box">
                                        ...
                                        <div class="oc-junk-text">
                                            <strong>Grund:</strong> {{ $fs($item->junk_reason ?? null, 'Kein Grund hinterlegt') }}
                                        </div>
                                ...
                                <div class="oc-row-menu" data-menu>
                                    ...
                                    <div class="oc-row-dropdown" data-menu-panel>
                                        ...
                                        @if(!$item->deleted_at && !$isJunkPage && !$isJunkRow && $canVerify)
                                            <button type="button" class="oc-row-dropdown-item verify-btn" data-info='@json($info)'>
                                        ...
                                        @if($canDelete && !$item->deleted_at && !$isJunkPage)
                                            <button type="button" class="oc-row-dropdown-item addNewProduct" data-id="{{ $item->id }}">
                                        ...
                                        @if(!$item->deleted_at)
                                            @if($isJunkPage || $isJunkRow)
                                                <button type="button"
                                                        class="oc-row-dropdown-item js-unjunk-btn"
                                                        data-id="{{ $item->id }}"
                                                        data-name="{{ $safeFullName }}">
                                            ...
                                            @elseif($canDelete)
                                                <button type="button"
                                                        class="oc-row-dropdown-item js-junk-btn"
                                                        data-id="{{ $item->id }}"
                                                        data-name="{{ $safeFullName }}">
                                        ...
                                        @if($canDelete && !$item->deleted_at)
                                            <button type="button"
                                                    class="oc-row-dropdown-item text-danger js-open-delete"
                                                    data-name="{{ $safeFullName }}"
...
<div class="oc-modal-backdrop" id="globalDeleteModal">
    <div class="oc-modal oc-modal-md">
        ...
        <div class="oc-modal-b" id="globalDeleteModalBody">
            Möchten Sie diese Anfrage wirklich löschen? Diese Aktion kann später über die gelöschte Liste rückgängig gemacht werden.
        </div>
        <div class="oc-modal-f">
...
<div id="verifyDrawer" class="verify-drawer">
    ...
    <div class="verify-drawer-body">
        <div class="mb-2">
            ...
            <small class="text-muted d-block mt-1">Hinweis: Für Lead/Kunde sollten bereits Produkte hinterlegt sein.</small>
        </div>
...
function fillEmployeesSelect($sel, list, placeholder){
    ...
    (list || []).forEach(emp=>{
        $sel.append(`<option value="${emp.id}" data-img="${emp.image||''}" data-positions="${(emp.positions||[]).join(', ')}">${emp.name} ${emp.lastname}</option>`);
    });
...
function initRow(idx){
    ...
    $p.on('change', async () => {
        ...
        try {
            ...
            const external = Array.isArray(res?.external_employees) && res.external_employees.length ? res.external_employees : internal;
...
function refreshEmployees(idx){
    ...
    fetchEmployees({ pid, did, sid, stage:'inquiry' }).done(res=>{
        if (res?.department_id && String(res.department_id)!==String(did || '')) {
            const $d = $(`.department-select[data-index="${idx}"]`);
            ensureOption($d, res.department_id, DEPTS.find(x=>String(x.id)===String(res.department_id))?.department_name || `Abt. ${res.department_id}`);
...
$(document).ready(function () {
    ...
    $('#addProductForm').on('submit', function (e) {
        ...
        $.post(URL_SAVE, payload)
            .done(res => {
                Swal.fire({ icon:'success', title:'Gespeichert', text: res.message || 'Erfolgreich gespeichert.' })
                    .then(()=> location.reload());
            })
            .fail(xhr => {
