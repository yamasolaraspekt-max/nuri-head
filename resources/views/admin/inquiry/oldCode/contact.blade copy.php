{{-- resources/views/admin/inquiries/create.blade.php --}}
...
<style>
  ...
  .card-header { border: 0; background: transparent; }
  ...
  .autosave-dot{ width:10px; height:10px; border-radius:50%; background:#9ca3af; }
  .autosave-pill.is-dirty .autosave-dot{ background:#f59e0b; }
  .autosave-pill.is-saving .autosave-dot{ background:#3b82f6; animation:pulse 1s infinite; }
  .autosave-pill.is-saved .autosave-dot{ background:#22c55e; }
  .autosave-pill.is-failed .autosave-dot{ background:#ef4444; }
  ...
  .card .card-header h5 { margin:0; font-weight:800; font-size:14px; color:#111827; }
  .card .card-body label { font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; }

  /* ===== Mini calendar styling (kept, compact & clean) ===== */
  ...
  #inquiry-mini-calendar .fc-toolbar-title { font-size: 14px; font-weight: 800; }
  #inquiry-mini-calendar .fc-button { background: #f3f3f3; border: 0; color: #333; padding: 4px 8px; border-radius: 10px; box-shadow: none; }
  #inquiry-mini-calendar .fc-button:focus, #inquiry-mini-calendar .fc-button:active { outline: none; box-shadow: none; }
  #inquiry-mini-calendar .fc-col-header-cell { background: #f8f8f8; }
  #inquiry-mini-calendar .fc-col-header-cell-cushion { padding: 6px 0; font-weight: 800; }
  #inquiry-mini-calendar .fc-timegrid-slot { height: 32px; }
  #inquiry-mini-calendar .fc-timegrid-axis-cushion { font-size: 10px; color: #8a8a8a; padding: 0 6px; }
  #inquiry-mini-calendar .fc-day-today { background: #f9fff2; }
  #inquiry-mini-calendar .fc-timegrid-now-indicator-line { border-color: #e50056; }
  #inquiry-mini-calendar .fc-event { border: 0; border-radius: 10px; padding: 2px 6px; box-shadow: 0 1px 0 rgba(0,0,0,.05); }

  /* ===== VERIFY MODAL CSS (kept) ===== */
  ...
  .verify-item .txt{font-weight:800;color:#222}
  ...
  .verify-pulse{outline:3px solid rgba(229,0,86,.45);box-shadow:0 0 0 6px rgba(229,0,86,.12);border-radius:8px;transition:all .2s ease}
  ...
  .apm-chip{display:inline-flex;align-items:center;gap:8px;border:1px solid #eee;background:#fafafa;border-radius:999px;padding:6px 10px;font-size:12px;font-weight:800;color:#222}
...
<div class="app-content content">
  ...
  <div class="content-wrapper">
      ...
      <div class="content-header-left col-12 mb-2">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-1">
          ...
          <div class="d-flex align-items-center gap-1">
            <span id="autosavePill" class="autosave-pill is-saved" title="Autosave">
              <span class="autosave-dot"></span>
              <span id="autosaveText">Bereit</span>
            </span>

            <button type="button" class="btn btn-outline-danger" id="discardDraftBtn">
              <i class="feather icon-trash-2"></i> Draft verwerfen
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="content-body">
      ...
      <form id="inquiryForm"
            class="leadForm form-horizontal custom-file-upload"
            method="post"
        ...
        <div class="card mb-1">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h5><i class="feather icon-search"></i> Kunde suchen</h5>
            <div class="d-flex gap-1">
              <button type="button" class="btn btn-primary" id="btnCustomerSearch">
                <i class="feather icon-search"></i> Suchen
              </button>
              <button type="button" class="btn btn-outline-secondary" id="btnCustomerClear">
        ...
        <div class="row">
          ...
          <div class="col-lg-8">
            <div class="card mb-1">
              ...
              <div class="card-body">
                <div id="inquiry-mini-calendar" style="height: 420px; border: 1px solid #e5e5e5; border-radius: 10px;"></div>
                <div class="mt-1">
                  <small class="text-muted">
                    Termine pro Produktzeile wählen (unten) – dies ist nur eine Übersicht.
                  </small>
                </div>
              </div>
            </div>
            ...
            <div class="card mb-1">
              <div class="card-header d-flex justify-content-between align-items-center">
                <h5><i class="feather icon-box"></i> Produkt / Dienstleistung / Abteilung / Personal</h5>
                <button type="button" class="btn btn-primary" id="addRow">
                  <i class="feather icon-plus"></i> Zeile
                </button>
              </div>
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-hover mb-0" id="inquiryProductTable">
                    <thead class="thead-light text-center">
                      <tr>
                        <th style="min-width:180px">Produkt</th>
                        <th style="min-width:180px">Dienstleistung</th>
                        <th style="min-width:170px">Abteilung</th>
                        <th style="min-width:200px">Innendienst</th>
                        <th style="min-width:200px">Außendienst</th>
                        <th style="min-width:220px">Termin</th>
              ...
              {{-- Sticky actions --}}
              <div class="inquiry-sticky-actions">
        ...
        <div class="apm-card">
          ...
          <div class="apm-card-b">
            ...
            <div class="mt-1">
              ...
              <div id="apmEmployeesEmpty" class="apm-empty mt-50 hidden">Keine Mitarbeiter in Produktzeilen ausgewählt.</div>
...
<script>
  // If editing: pass existing id; if create: 0
  window.INQUIRY_ID = {{ (int) ($data->id ?? ($inquiry->id ?? 0)) }};
</script>
...
$(document).ready(function() {
  ...
  $('select[name="branch_id"]').select2({ placeholder: 'Betrieb' });
  $('#source').select2({ tags: true, placeholder: "Quelle auswählen", allowClear: true });

  // customerSearch select2 can be configured here (AJAX) if you have endpoint.
...
{{-- Google Places (recommend: move key to env/config) --}}
...
<script>
function initMap() { initAutocomplete(); }
...
function initAutocomplete() {
  ...
  function parseAddressComponents(components) {
    const address = { street_number:"", route:"", locality:"", postal_code:"", administrative_area_level_1:"", administrative_area_level_2:"" };
...
{{-- Calendar availability (fixed: expose calendar globally) --}}
<script>
...
{{-- Appointment Modal (your logic kept; only small safety fixes) --}}
<script>
(function(){
  'use strict';

  const CSRF = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
  window.__CURRENT_INQUIRY_ID = window.INQUIRY_ID || null;

  const API = {
    customers: "{{ route('api.appointments.customers') }}",
    alternatives: (leadId) => "{{ url('/api/appointments/customers') }}/" + leadId + "/alternatives",
    leadProducts: (leadId) => "{{ url('/api/appointments/customers') }}/" + leadId + "/products",
    store: "{{ route('main_appointments.store_modal') }}"
  };
  ...
  function readEmployeeTextById(id){
    ...
    return opt.length ? opt.text().trim() : `Mitarbeiter #${id}`;
  }
    ...
    employeeItems.forEach(e => {
      $wrap.append(`
        <span class="apm-chip" data-id="${e.id}">
          <span class="dot"></span>
          ${e.text}
          <button type="button" class="rm" title="Entfernen" data-apm-rm-emp="${e.id}">
            <i class="feather icon-x"></i>
          </button>
        </span>
      `);
  ...
  function openModal(){ $('#apmModal').removeClass('hidden').attr('aria-hidden','false'); }
  function closeModal(){ $('#apmModal').addClass('hidden').attr('aria-hidden','true'); }
  ...
  $(document).on('click','[data-apm-rm-emp]', function(){
    ...
    $('#apmEmployeesChips .apm-chip').each(function(){
      ...
      if (cid !== id) items.push({id: cid, text: readEmployeeTextById(cid)});
    });
    renderEmployeeChips(items);
  });
  ...
  async function saveAppointment(){
    ...
    if (!payload.name) return Swal.fire({ icon:'warning', title:'Titel fehlt', text:'Bitte Titel eingeben.' });
    if (!payload.start) return Swal.fire({ icon:'warning', title:'Start fehlt', text:'Bitte Start-Datum/Zeit setzen.' });
    if (!payload.employee_ids.length) return Swal.fire({ icon:'warning', title:'Keine Mitarbeiter', text:'Bitte Mitarbeiter in Produktzeilen auswählen.' });
    ...
    if (!r.ok){
      return Swal.fire({ icon:'error', title:'Fehler', text: data?.message || 'Speichern fehlgeschlagen.' });
    }
...
{{-- Product rows (fixed: professional validations + row class) --}}
<script>
(() => {
  'use strict';
  ...
  $(function () {
    $('#addRow').on('click', function () {
      ...
      if ($lastRow.length) {
        ...
        if (missing.length) {
          Swal.fire({
            icon: 'error',
            title: `Letzte Zeile unvollständig`,
            html: `Bitte füllen Sie aus: <strong>${missing.join(', ')}</strong>`,
      ...
      const row = `
        <tr data-index="${idx}" class="align-middle product-row">
          <td>
      ...
      initSelects(idx);
      window.InquiryAutosave?.markDirty(); // trigger autosave
    });
    ...
    function initSelects(i) {
      ...
      [$product, $service, $dept].forEach($s => $s.select2({ width:'100%' }));
    ...
    function formatEmployee(opt) {
      ...
      return `
        <div style="display:flex;align-items:center;gap:10px;">
          ${img ? `<img src="${img}" class="rounded-circle" style="width:36px;height:36px;object-fit:cover;">`
               : `<div class="rounded-circle" style="width:36px;height:36px;background:#e5e7eb;"></div>`}
          <div>
            <div style="font-weight:800;line-height:1.1">${opt.text}</div>
            <div style="font-size:11px;color:#6b7280">${pos}</div>
          </div>
        </div>`;
    }
    ...
    $(document).on('click', '.removeRow', function () {
      $(this).closest('tr').fadeOut(200, function () {
        $(this).remove();
        window.InquiryAutosave?.markDirty();
      });
    });
  });
})();
...
{{-- VERIFY + SUBMIT (cleaned + fixed contact3 editor) --}}
<script>
(function () {
  'use strict';
  ...
  function openVerifyModal() { $('#verifyModal').removeClass('hidden').attr('aria-hidden', 'false'); }
  function closeVerifyModal() { $('#verifyModal').addClass('hidden').attr('aria-hidden', 'true'); $('.verify-inline-editor').remove(); }
  ...
  function openInlineEditor($item, edit) {
    if (edit.mode === 'gmap' || edit.type === 'gmap') { openGoogleAutocompleteForAddress(); return; }

    // ✅ contact3 editor (email + mobile + telephone)
      ...
      const html = `
        <div class="verify-inline-editor" style="margin-top:10px;padding:10px;border:1px solid #eee;border-radius:12px;background:#fafafa">
          ...
          <div class="form-group mb-2">
            <label style="font-size:12px;font-weight:800;color:#444;margin-bottom:4px">E-Mail</label>
            <input type="email" class="form-control verify-inline-email" value="${escapeHtml($email.val())}" placeholder="name@mail.de">
          </div>

          <div class="form-group mb-2">
            <label style="font-size:12px;font-weight:800;color:#444;margin-bottom:4px">Mobil</label>
            <input type="text" class="form-control verify-inline-mobile" value="${escapeHtml($mobile.val())}" placeholder="+49 ...">
          </div>

          <div class="form-group mb-2">
            <label style="font-size:12px;font-weight:800;color:#444;margin-bottom:4px">Festnetz</label>
    ...
    if (!$target.length) return Swal.fire({ icon:'warning', title:'Nicht gefunden', text:`Feld nicht gefunden: ${edit.selector}` });
    ...
    let html = `<div class="verify-inline-editor" style="margin-top:10px;padding:10px;border:1px solid #eee;border-radius:12px;background:#fafafa">
  ...
  $(document).on('click', '.verify-inline-save', function () {
    ...
    const val = $editor.find('.verify-inline-input').val();
  ...
  $(document).on('click', '.verify-inline-save-contact', function () {
    const $editor = $(this).closest('.verify-inline-editor');
    $('input[name="email"]').val($editor.find('.verify-inline-email').val()).trigger('change');
    $('input[name="phone"]').val($editor.find('.verify-inline-mobile').val()).trigger('change');
    $('input[name="telephone"]').val($editor.find('.verify-inline-telephone').val()).trigger('change');
    $editor.remove();
    renderChecklist(buildClientVerification());
  });
  ...
  function buildClientVerification() {
    ...
    checks.push(checkRow('pre_type', 'Art des Kontakts (pre_type) gewählt', preTypeOk, { selector:'select[name="pre_type"]', type:'select2', mode:'inline' }));
    if (!preTypeOk) missing.push('Art des Kontakts (pre_type) fehlt.');

    const nameOk = filled($('input[name="name"]').val());
    checks.push(checkRow('name', 'Vorname vorhanden', nameOk, { selector:'input[name="name"]', type:'focus', mode:'inline' }));
    if (!nameOk) missing.push('Vorname fehlt.');

    const lastnameOk = filled($('input[name="lastname"]').val());
    checks.push(checkRow('lastname', 'Nachname vorhanden', lastnameOk, { selector:'input[name="lastname"]', type:'focus', mode:'inline' }));
    if (!lastnameOk) missing.push('Nachname fehlt.');

    const addressOk = firstFilled($('#full_address').val(), $('#street-input').val());
    checks.push(checkRow('address', 'Adresse vorhanden', addressOk, { selector:'#full_address', type:'gmap', mode:'gmap' }));
    if (!addressOk) missing.push('Adresse fehlt.');

    const contactOk = firstFilled($('input[name="phone"]').val(), $('input[name="telephone"]').val(), $('input[name="email"]').val());
    checks.push(checkRow('contact', 'Kontakt vorhanden (Mobil/Festnetz/E-Mail)', contactOk, { selector:'input[name="phone"]', type:'focus', mode:'contact3' }));
    if (!contactOk) missing.push('Kontakt fehlt (Mobil oder Festnetz oder E-Mail).');

    const $rows = $('#inquiryProductTable tbody tr');
    const hasRows = $rows.length > 0;
    checks.push(checkRow('products', 'Mindestens 1 Produktzeile hinzugefügt', hasRows, { selector:'#addRow', type:'click' }));
    ...
    $rows.each(function (idx) {
      ...
      checks.push(checkRow(`row.${idx+1}.product`, `Produkt (${rowLabel})`, prodOk, { selector:`.product-select[data-index="${dataIndex}"]`, type:'select2', mode:'inline' }));
      if (!prodOk) { missing.push(`${rowLabel}: Produkt fehlt.`); rowsOk = false; }

      const srvOk = filled(sid);
      checks.push(checkRow(`row.${idx+1}.service`, `Dienstleistung (${rowLabel})`, srvOk, { selector:`.service-select[data-index="${dataIndex}"]`, type:'select2', mode:'inline' }));
      if (!srvOk) { missing.push(`${rowLabel}: Dienstleistung fehlt.`); rowsOk = false; }

      const depOk = filled(did);
      checks.push(checkRow(`row.${idx+1}.department`, `Abteilung (${rowLabel})`, depOk, { selector:`.department-select[data-index="${dataIndex}"]`, type:'select2', mode:'inline' }));
      if (!depOk) { missing.push(`${rowLabel}: Abteilung fehlt.`); rowsOk = false; }

      const empOk = filled(eid);
      checks.push(checkRow(`row.${idx+1}.employee`, `Innendienst (${rowLabel})`, empOk, { selector:`.employee-select[data-index="${dataIndex}"]`, type:'select2', mode:'inline' }));
      if (!empOk) { missing.push(`${rowLabel}: Innendienst fehlt.`); rowsOk = false; }
    });

    checks.push(checkRow('products_details', 'Produkte vollständig (Produkt/Dienstleistung/Abteilung/Innendienst)', hasRows && rowsOk, { selector:'#inquiryProductTable', type:'focus' }));
  ...
  function renderChecklist(payload) {
    ...
    checks.forEach(c => {
      ...
      const actionHtml = hasEdit
        ? `<button type="button" class="btn btn-sm btn-outline-primary verify-edit-btn"
  ...
  $(document).on('click', '.verify-edit-btn', function () {
    ...
    if (edit.mode === 'gmap' || edit.type === 'gmap') return openGoogleAutocompleteForAddress();
  ...
  $(document).on('input change', '.leadForm input, .leadForm textarea, .leadForm select, #inquiryProductTable input, #inquiryProductTable select', recheckDebounced);
  ...
  function ajaxSubmitForm($form) {
    ...
    return $.ajax({
      ...
      processData: false,
      beforeSend: function () {
        Swal.fire({ title: 'Speichern...', text: 'Ihre Anfrage wird verarbeitet.', showConfirmButton: false, allowOutsideClick: false, didOpen: () => Swal.showLoading() });
      }
    });
  }
  ...
  $('#btnVerifyConfirm').on('click', function () {
    ...
    .done(function (resp) {
      ...
      if (!inquiryId) {
        Swal.close(); setConfirmLoading(false);
        return Swal.fire({ icon:'error', title:'Fehler', text:'inquiry_id fehlt im Store-Response.' });
      }
      ...
      .done(function (resp2) {
        Swal.close();
        if (resp2?.redirect_url) return window.location.href = resp2.redirect_url;
        setConfirmLoading(false);
        Swal.fire({ icon:'warning', title:'Kein Redirect', text:'confirm hat keine redirect_url geliefert.' });
      })
      .fail(function (xhr) {
        Swal.close(); setConfirmLoading(false);
        Swal.fire({ icon:'error', title:'Fehler', text: xhr?.responseJSON?.message || 'Verifizierung fehlgeschlagen.' });
      });
    })
    .fail(function (xhr) {
      Swal.close(); setConfirmLoading(false);
      ...
      let errorMessages = '';
      if (serverErrors) {
        $.each(serverErrors, function (_k, v) { errorMessages += `<li>✅ ${escapeHtml(v)}</li>`; });
      } else {
  ...
  $('#btnVerifyRefresh').on('click', function(){
    renderChecklist(buildClientVerification());
  });
...
{{-- ✅ AUTOSAVE (SweetAlert for each autosave) --}}
<script>
(function(){
  'use strict';
  ...
  const Autosave = {
    ...
    setState(state, text){
      if (!$pill || !$txt) return;
      $pill.classList.remove('is-dirty','is-saving','is-saved','is-failed');
    ...
    markDirty(){
      this.dirty = true;
      this.setState('is-dirty', 'Änderungen…');
      this.schedule();
    },

    schedule(){
      clearTimeout(this.timer);
      this.timer = setTimeout(() => this.save(), 1200); // more professional (less spam)
    },
    ...
    async save(){
      ...
      this.saving = true;
      this.setState('is-saving', 'Speichern…');

      try {
        const fd = new FormData(form);
        fd.set('submit_mode', 'autosave');

        const inquiryId = window.INQUIRY_ID || 0;

        // If no inquiry id yet -> create draft via store action (expects JSON inquiry_id)
        const url = inquiryId > 0
          ? `/inquiries/${inquiryId}/autosave`
          : form.getAttribute('action');

        const method = inquiryId > 0 ? 'PATCH' : 'POST';
        ...
        // If draft created, store id
        if (!window.INQUIRY_ID && data?.inquiry_id) {
          window.INQUIRY_ID = parseInt(data.inquiry_id, 10) || 0;
          window.__CURRENT_INQUIRY_ID = window.INQUIRY_ID || null;
        }

        this.dirty = false;
        this.setState('is-saved', 'Gespeichert ✅');

        // ✅ SweetAlert on EACH autosave (toast)
        toast.fire({ icon: 'success', title: 'Autosave gespeichert' });

      } catch (err) {
        console.error(err);
        this.setState('is-failed', 'Fehler ❌');

        const msg = err?.message || 'Autosave fehlgeschlagen';
  ...
  window.InquiryAutosave = Autosave;

  // Mark dirty on ANY input/select/textarea changes (including Select2 change)
  document.addEventListener('input', (e) => {
    if (!e.target.closest('#inquiryForm')) return;
    Autosave.markDirty();
  }, true);
  ...
  // Protect against accidental leave when dirty
...
{{-- Discard draft (SweetAlert confirm) --}}
<script>
document.getElementById('discardDraftBtn')?.addEventListener('click', async () => {
  ...
  if (!inquiryId) {
    return Swal.fire({ icon:'info', title:'Kein Draft', text:'Es gibt noch keinen gespeicherten Draft.' });
  }

  const r = await Swal.fire({
    icon: 'warning',
    title: 'Draft verwerfen?',
    text: 'Diese Anfrage wird gelöscht.',
  ...
  if (res.ok) {
    await Swal.fire({ icon:'success', title:'Gelöscht', text:'Draft wurde verworfen.' });
    window.location.href = '/inquiry';
  } else {
    const data = await res.json().catch(()=>({}));
    Swal.fire({ icon:'error', title:'Fehler', text: data?.message || 'Löschen fehlgeschlagen.' });
  }
});
</script>
@endsection
