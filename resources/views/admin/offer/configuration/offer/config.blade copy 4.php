...
<head>
    ...
    <script>
        ...
        window.ServerUserPrefs = {!! auth()->user()->preference ?? 'null' !!};
    </script>
       <style>
    :root{
        --brand-color:#93c21c;
                    ...
                    <div class="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-300 mb-6">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <div>
                                ...
                                <span class="block text-[11px] text-slate-500">Kein Kunde/Objekt notwendig. Dokument wird nur als Template gespeichert.</span>
                            </div>
                        </label>
                    </div>
                    <div class="relative">
            ...
            <div class="flex flex-1 overflow-hidden relative">
                ...
                <main id="panel-templates" class="hidden flex-1 h-full overflow-y-auto scroller bg-slate-100/50 p-6">
                    <div class="max-w-7xl mx-auto space-y-4">
                        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                            <div class="p-4 border-b border-slate-200">
                                <div class="text-xs font-black text-slate-500 uppercase tracking-wider">Vorlagen</div>
    ...
    <div class="absolute top-1/2 left-1/2 w-[96vw] max-w-3xl -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="p-6 space-y-6">
            <div class="rounded-2xl border border-[#93c21c]/20 bg-[#f7fee7] p-4">
                ...
                <div id="template-apply-meta" class="text-sm text-slate-500 mt-1">—</div>
            </div>
            ...
            <div>
                ...
                <div id="template-customer-selected"
                    ...
                    <div class="min-w-0">
                        ...
                        <div id="template-sel-cust-addr" class="text-xs text-slate-500 truncate"></div>
                    </div>
...
<div id="user-prefs-modal" class="fixed inset-0 z-[300] hidden">
    ...
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl w-[500px] max-h-[90vh] flex flex-col animate-fadeIn">
        ...
        <div class="p-6 space-y-6 overflow-y-auto scroller">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Standard-Ansicht beim Start</label>
                <div class="flex gap-3">
                    <label class="flex-1 border border-slate-200 rounded-xl p-3 cursor-pointer hover:border-[#93c21c] flex items-center gap-2 bg-white has-[:checked]:border-[#93c21c] has-[:checked]:bg-[#f7fee7] transition-all">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Panel-Sichtbarkeit</label>
                <div class="space-y-2">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Sichtbare Spalten (Listenansicht)</label>
...
<script>
    ...
    window.App = {
        init: () => {
            ...
            document.addEventListener('click', e => {
                if (
                    !e.target.closest('#template-customer-search') &&
                    !e.target.closest('#template-customer-dropdown')
                ) {
           ...
           App.openSaveModal = () => {
                // 1. Prüfen, ob wir im Template-Modus gestartet sind
                ...
                const templateRadio = document.querySelector('input[name="save_mode"][value="template"]');
                const templateNameWrap = document.getElementById('template-name-wrap');
                
                // Die Container-Labels finden, um sie auszublenden
                const offerLabel = offerRadio.closest('label');

                if (isTemplateMode) {
                    // --- TEMPLATE-MODUS ---
                    offerLabel.style.display = 'none'; // Verstecke "Kunden-Angebot"
                    templateRadio.checked = true;      // Wähle "Als Vorlage" automatisch aus
                    templateNameWrap.classList.remove('hidden'); // Zeige Namensfeld sofort
                } else {
                    // --- NORMALER MODUS ---
                    offerLabel.style.display = 'flex'; // Zeige beide Optionen
                    offerRadio.checked = true;         // Standardmäßig "Angebot"
                    templateNameWrap.classList.add('hidden');
                }
            ...
            App.performSave = async () => {
                ...
                // If we aren't saving a template AND the wizard isn't in template mode, require customer data
                if (!isTemplate && !isWizardInTemplateMode) {
                ...
                const payload = {
                    ...
                    // ✅ BIOGRAPHY DATA (Sends local logs to be saved in DB)
                    biography: State.biographyItems, 

                    // Branding & Calculations
                    branding: {
                ...
                try {
                    ...
                    // 6. Post-Save Logic: Cleanup Biography State
                    // Mark all items as no longer "Local" because they are now stored in the database
                    State.biographyItems.forEach(item => {
                        if (item.isLocal) item.isLocal = false;
                    });

                    // Add a "Saved" entry to the local list
                    App.Bio.addEntry('Document Saved', 'All changes have been synced to the server.');

                    // 7. Handle UI Success Messaging
                    if (isTemplate) {
                        ...
                        if (result.folder_id) {
                            ...
                            document.getElementById('toast-confirm-cancel').style.display = '';
                        } else {
                    ...
                    // 8. Re-enable UI regardless of outcome
        ...
        handleItemAdd: async (sIdx, id, typeFromDrag = null) => {
            ...
            try {
                if (type === "master_set") {
                    ...
                    itemsArray.forEach((itemData) => {
                        if (itemData.type === 'component') {
                            ...
                            const distMeta = extractDistributorMeta(itemData);
                            const parentLine = buildBaseItem({
                                ...
                                supplier: distMeta.supplier,
                                distributor_name: distMeta.distributor_name,
                                distributor_article_no: distMeta.distributor_article_no,
                                distributor_id: distMeta.distributor_id,
                                distributor_price_id: distMeta.distributor_price_id,
                                skonto: distMeta.skonto,
                                payment_terms: distMeta.payment_terms,
                            ...
                            if (Array.isArray(itemData.children) && itemData.children.length > 0) {
                                itemData.children.forEach((childData) => {
                                    ...
                                    const childDistMeta = extractDistributorMeta(childData);
                                    const childLine = buildBaseItem({
                                        ...
                                        supplier: childDistMeta.supplier,
                                        distributor_name: childDistMeta.distributor_name,
                                        distributor_article_no: childDistMeta.distributor_article_no,
                                        distributor_id: childDistMeta.distributor_id,
                                        distributor_price_id: childDistMeta.distributor_price_id,
                                        skonto: childDistMeta.skonto,
                                        payment_terms: childDistMeta.payment_terms,
                ...
                // PRODUCT
                {
                   ...
                   const distMeta = extractDistributorMeta(p);
                    ...
                    pushItem(sIdx, item);
                    App.Bio.addEntry('Position hinzugefügt', `"${item.name}" wurde zu Sektion ${sIdx + 1} hinzugefügt.`);
                    safeRender();
                    return;
                }
            } catch (err) {
            ...
            try {
               if (typeFromDrag === 'master_set') {
                    ...
                    pushSub({
                        ...
                        margin: totalEk > 0 ? ((safeNum(data.total_price ?? data.price ?? totalSet) - totalEk) / totalEk) * 100 : 0,
                        marginPercent: totalEk > 0 ? ((safeNum(data.total_price ?? data.price ?? totalSet) - totalEk) / totalEk) * 100 : 0,
                        kind: 'article',
                ...
                if (typeFromDrag === 'product') {
                    ...
                    const data = resp?.data ?? resp ?? {};
                    const distMeta = extractDistributorMeta(data);
       ...
       startQuote: () => {
            // 1. Identify if we are in "Template Mode"
            const isTemplateMode = document.getElementById('wiz-template-mode')?.checked || false;

            // 2. Initialize Date and Document Type
            State.projectDate = document.getElementById('wiz-date').value;
            const typeRadios = document.getElementsByName('wiz-doc-type');
            typeRadios.forEach(t => { if (t.checked) State.docType = t.value; });

            // 3. Update Document Labels
            ...
            if (docTypeLabel) docTypeLabel.innerText = State.docType;
            if (docIdNameLabel) docIdNameLabel.innerText = State.docType === 'Angebot' ? 'Angebotsnummer' : 'KVA-Nummer';
            if (mainTitleEl) mainTitleEl.innerText = `Unverbindliches ${State.docType} für...`;

            // 4. Handle Customer Data (Conditional)
            ...
            if (isTemplateMode) {
                // --- TEMPLATE MODE: Use Placeholders ---
                if (custNameEl) custNameEl.innerText = "VORLAGE";
                if (custLastEl) custLastEl.innerText = "MUSTERMANN";
                if (custAddrEl) custAddrEl.innerText = "Musterstraße 1\n12345 Musterstadt";
                if (custIdInput) custIdInput.value = "TEMPLATE";
                
                // Ensure State objects aren't null to prevent calculation errors
                State.customer = { name: "Vorlage", lastname: "Mustermann", customer_no: "TEMPLATE" };
                State.object = State.object || { items: [] };
            } else {
                // --- NORMAL MODE: Use Selected Customer ---
                if (custNameEl) custNameEl.innerText = State.customer.name;
                if (custLastEl) custLastEl.innerText = State.customer.lastname || '';
                if (custAddrEl) {
                    custAddrEl.innerHTML = `${State.customer.street || ''}<br>${State.customer.postcode || ''} ${State.customer.city || ''}`;
                }
                if (custIdInput) custIdInput.value = State.customer.customer_no;
            }

            // 5. Update Date and Branding
            const dateLine = document.getElementById('doc-date-line');
            if (dateLine) {
                const formattedDate = new Date(State.projectDate).toLocaleDateString('de-DE');
                dateLine.innerText = `Wehrheim, ${formattedDate}`;
            }

            // Sync Company Names across UI
            ...
            if (teamNameEl) teamNameEl.innerText = `Ihr ${State.companyName}-Team`;

            const footerCoEl = document.getElementById('footer-company');
            if (footerCoEl) footerCoEl.innerText = `${State.companyName} GmbH`;

            const compHeaderEl = document.getElementById('doc-company-header');
            if (compHeaderEl) compHeaderEl.innerText = `${State.companyName} GmbH • Am Kappengraben 10 • 61273 Wehrheim`;

            // Apply specific branding (Colors/Logo images)
            App.applyBrandingToCover();

            // 6. Initialize Cover Letter Text
            const coverEl = document.getElementById('doc-cover-text');
            if (coverEl) {
            ...
            // 7. Prepare Content and Transition View
            ...
            // Automatically switch to List View for easier editing
        ...
        renderCalculationSidebar: () => {
            ...
            const renderCard = (isSub, sIdx, iIdx, subIdx, dataObj, prefix, hasChildren) => {
                ...
                // Added id="sb-item-..." to details and data-sb-focus to inputs
                return `
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-3 bg-slate-100/50 p-2 rounded text-xs">
                            ...
                            <div>
                                <div class="text-[9px] text-slate-400 mb-0.5 flex justify-between">
                                    <span>Marge</span>
                                    <select data-sb-focus="marginType:${focusKeyBase}" onchange="App.updatePosPriceCalc(${sIdx},${iIdx},${subIdxArg},'marginType',this.value)" ${isSelectReadonly}>
                                        <option value="percent" ${mType==='percent'?'selected':''}>%</option>
                                        <option value="fixed" ${mType==='fixed'?'selected':''}>€</option>
                                    </select>
                                </div>
                                <input data-sb-focus="margin:${focusKeyBase}" type="number" step="0.01" value="${mVal.toFixed(2)}" onchange="App.updatePosPriceCalc(${sIdx},${iIdx},${subIdxArg},'margin',this.value)" ${isVkReadonly}>
                            </div>
                            <div>
    ...
    App.TemplateTab = {
        async loadTemplates(q = '') {
            try {
                const res = await fetch(`/offers/templates?q=${encodeURIComponent(q)}`, {
                    headers: { Accept: 'application/json' },
                    credentials: 'same-origin'
                });
                ...
                const json = await res.json();
                State.templateItems = Array.isArray(json.items) ? json.items : [];
            } catch (e) {
        ...
        async render() {
                ...
                <div class="space-y-5">
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input type="text"
                                id="template-tab-search"
                                placeholder="Vorlagen durchsuchen..."
                                value="${App.escapeHtml(State.templateSearch || '')}"
                                oninput="App.TemplateTab.onSearch(this.value)"
                                class="w-full border border-slate-300 rounded-xl p-3 pl-10 text-sm outline-none focus:border-[#93c21c] bg-white">
                            <i class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"></i>
                        </div>

                        <button type="button"
                                onclick="App.TemplateTab.render()"
                                class="px-4 py-3 rounded-xl border border-slate-300 bg-white text-sm font-bold text-slate-700 hover:border-[#93c21c] hover:text-[#93c21c]">
                            <i class="fa-solid fa-rotate-right mr-2"></i>Aktualisieren
                        </button>
                    </div>

                    <div id="template-tab-list" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        <div class="text-sm text-slate-400">Lade Vorlagen...</div>
                    </div>
                </div>
            `;

            await App.TemplateTab.loadTemplates(State.templateSearch || '');

            const list = document.getElementById('template-tab-list');
            if (!list) return;

            if (!State.templateItems.length) {
                list.innerHTML = `
                    <div class="col-span-full border border-dashed border-slate-200 rounded-2xl p-8 text-center text-slate-400 bg-slate-50">
            ...
            list.innerHTML = State.templateItems.map(t => `
                <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-4 hover:border-[#93c21c] hover:-translate-y-[1px] transition">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                <span class="inline-block w-3 h-3 rounded-full border border-slate-200"
                                    style="background:${t.brand_color || '#93c21c'}"></span>
                                <div class="text-base font-black text-slate-800 truncate">${App.escapeHtml(t.name || 'Vorlage')}</div>
                            </div>

                            <div class="text-xs text-slate-500 mt-1">
                                ${App.escapeHtml(t.company_name || '—')}
                            </div>
                        </div>

                        <div class="text-right shrink-0">
                            <div class="text-[10px] uppercase tracking-wider font-black text-slate-400">Inhalt</div>
                            <div class="text-xs text-slate-600 mt-1">${t.section_count || 0} Sektionen</div>
                            <div class="text-xs text-slate-600">${t.item_count || 0} Positionen</div>
                        </div>
                    </div>

                    <div class="mt-3 text-[12px] text-slate-500 leading-relaxed min-h-[42px]">
                        ${App.escapeHtml(t.preview_text || 'Keine Beschreibung')}
                    </div>

                    <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                        <span>${App.escapeHtml(t.creator_name || '—')}</span>
                        <span>${App.escapeHtml(t.updated_at || '')}</span>
                    </div>

                    <div class="mt-4 flex items-center justify-between gap-2">
                        <button type="button"
                                onclick="App.TemplateTab.previewTemplate(${t.id})"
                                class="flex-1 px-3 py-2 rounded-xl border border-slate-300 bg-white text-sm font-bold text-slate-700 hover:border-[#93c21c] hover:text-[#93c21c]">
                            <i class="fa-solid fa-eye mr-2"></i>Vorschau
                        </button>

                        <button type="button"
                                onclick="App.TemplateTab.useTemplate(${t.id})"
                                class="flex-1 px-3 py-2 rounded-xl bg-[#93c21c] text-white text-sm font-black hover:brightness-105 shadow-sm">
                            <i class="fa-solid fa-check mr-2"></i>Verwenden
                        </button>
                    </div>
                </div>
            `).join('');
        ...
        async onSearch(val) {
            State.templateSearch = val || '';
            await App.TemplateTab.render();
        },

        async previewTemplate(templateId) {
            try {
                const res = await fetch(`/offers/templates/${templateId}`, {
                    headers: { Accept: 'application/json' },
                    credentials: 'same-origin'
                });
                ...
                const json = await res.json();
                const t = json.template || null;
                if (!t) throw new Error('Template missing');

                alert(
                    `Vorlage: ${t.name}\n` +
                    `Firma: ${t.company_name || '-'}\n` +
                    `Sektionen: ${(t.sections || []).length}\n` +
                    `Bilder: ${(t.placed_images || []).length}`
                );
            } catch (e) {
                console.error('previewTemplate failed', e);
        ...
        async useTemplate(templateId) {
            try {
                ...
                // already known from URL
                const prefill = State.prefill || {};
                const hasPrefillCustomer = !!prefill.customer_id;
                const hasPrefillObject = !!(prefill.alternative_id || prefill.product_id);

                // if customer + object already exist in URL, auto-apply
                if (hasPrefillCustomer && hasPrefillObject) {
                    await App.TemplateTab.applyTemplateUsingPrefill();
                    return;
                }

                // fallback: open modal only if something is missing
                App.TemplateTab.openApplyModal(t);
        ...
        async applyTemplateUsingPrefill() {
            const prefill = State.prefill || {};

            if (!prefill.customer_id) {
                App.TemplateTab.openApplyModal(State.selectedTemplate);
                return;
            }
            ...
            try {
                ...
                // 2) save into templateApply + main state
                State.templateApply = State.templateApply || {};
                State.templateApply.customer = customer;

                State.customer = customer;
                State.custId = customer.customer_no || '-';

                // 3) load objects for this customer
                const objRes = await fetch(`${API_BASE}/wizard/customers/${customer.id}/objects`, {
                    headers: { Accept: 'application/json' },
                    credentials: 'same-origin'
                });
                ...
                const products = Array.isArray(objData.products) ? objData.products : [];

                const matched = products
                    .filter(p => {
                        const altOk = prefill.alternative_id
                            ? Number(p.alternative_id) === Number(prefill.alternative_id)
                            : true;

                        const prodOk = prefill.product_id
                            ? Number(p.product_id) === Number(prefill.product_id)
                            : true;

                        return altOk && prodOk;
                    })
                    .map(p => ({
                        lead_product_id: parseInt(p.lead_product_id, 10),
                        alternative_id: p.alternative_id ? parseInt(p.alternative_id, 10) : null,
                        product_id: p.product_id ? parseInt(p.product_id, 10) : null,
                        label: p.label
                    }));

                if (!matched.length) {
                    throw new Error('No matching object found for prefill');
                }

                State.templateApply.objects = matched;
                State.object = {
                    items: matched,
                    name: matched.length === 1
                        ? matched[0].label
                        : `${matched.length} Produkte ausgewählt`
                };

                // 4) apply template
                App.TemplateTab.applyTemplateToState(State.selectedTemplate);

                // 5) sync cover/customer UI
                const nameEl = document.getElementById('doc-cust-name');
                const lastNameEl = document.getElementById('doc-cust-lastname');
                const addrEl = document.getElementById('doc-cust-addr');
                const custIdEl = document.getElementById('doc-cust-id');

                if (nameEl) nameEl.innerText = customer.display_name || customer.name || '';
                if (lastNameEl) lastNameEl.innerText = customer.lastname || '';
                if (addrEl) {
                    addrEl.innerHTML = `${customer.street || ''}<br>${customer.postcode || ''} ${customer.city || ''}`;
                }
                if (custIdEl) custIdEl.value = customer.customer_no || '-';

                App.renderQuotePage();

                App.toastConfirmShow({
                    title: 'Vorlage übernommen',
                    message: 'Die Vorlage wurde automatisch mit Kunde und Objekt aus dem Angebot geladen.',
                    okText: 'OK',
                    cancelText: '',
                    onOk: () => {}
                });
                ...
                console.error('applyTemplateUsingPrefill failed', e);

                // fallback to modal if auto-prefill fails
                App.TemplateTab.openApplyModal(State.selectedTemplate);
            }
        },
        applyTemplateToState(t) {
            State.sections = Array.isArray(t.sections) ? JSON.parse(JSON.stringify(t.sections)) : [];
            State.placedImages = Array.isArray(t.placed_images) ? JSON.parse(JSON.stringify(t.placed_images)) : [];

            State.brandColor = t.brand_color || '#93c21c';
            State.brandMode = t.brand_mode || 'text';
            State.brandLogoUrl = t.brand_logo_url || '';
            State.companyName = t.company_name || 'SOLAR ASPEKT';
            State.coverTextHtml = t.cover_text_html || '';
            ...
            if (logoSelect) logoSelect.value = State.brandLogoUrl;
        ...
        openApplyModal(template) {
            ...
            const prefill = State.prefill || {};
            if (prefill.customer_id) {
                App.TemplateTab.autoFillModalFromPrefill();
            }

            const titleEl = document.getElementById('template-apply-title');
            const nameEl = document.getElementById('template-apply-name');
            const metaEl = document.getElementById('template-apply-meta');
            const confirmBtn = document.getElementById('template-apply-confirm-btn');

            if (titleEl) titleEl.innerText = 'Vorlage übernehmen';
            if (titleEl) titleEl.innerText = 'Vorlage übernehmen';
            if (nameEl) nameEl.innerText = template?.name || '—';
            if (metaEl) {
                metaEl.innerText =
                    `${template?.company_name || '—'} • ${Array.isArray(template?.sections) ? template.sections.length : 0} Sektionen`;
            }
            ...
            const searchInput = document.getElementById('template-customer-search');
            const customerSelected = document.getElementById('template-customer-selected');
            const customerDropdown = document.getElementById('template-customer-dropdown');
            const objectStep = document.getElementById('template-object-step');
            const objectSelect = document.getElementById('template-object-select');
            const objectCount = document.getElementById('template-object-count');
            ...
            if (objectStep) objectStep.classList.add('opacity-50', 'pointer-events-none');
            if (objectSelect) objectSelect.innerHTML = '';
            if (objectCount) objectCount.innerText = '0';
        ...
        async autoFillModalFromPrefill() {
            const prefill = State.prefill || {};
            if (!prefill.customer_id) return;
            ...
            try {
                ...
                await App.TemplateTab.selectCustomer(customer);

                const objectSelect = document.getElementById('template-object-select');
                if (!objectSelect) return;

                const matchedValues = Array.from(objectSelect.options)
                    .filter(o => {
                        const altId = o.dataset.altId ? parseInt(o.dataset.altId, 10) : null;
                        const productId = o.dataset.productId ? parseInt(o.dataset.productId, 10) : null;

                        const altOk = prefill.alternative_id
                            ? altId === Number(prefill.alternative_id)
                            : true;

                        const prodOk = prefill.product_id
                            ? productId === Number(prefill.product_id)
                            : true;
                ...
                Array.from(objectSelect.options).forEach(o => {
                    o.selected = matchedValues.includes(o.value);
                });

                App.TemplateTab.selectObject();

            } catch (e) {
                console.error('autoFillModalFromPrefill failed', e);
            }
        },
        ...
        async filterCustomers() {
            const q = document.getElementById('template-customer-search')?.value?.trim() || '';
            const drop = document.getElementById('template-customer-dropdown');
            if (!drop) return;
            ...
            try {
                ...
                if (Array.isArray(data.items) && data.items.length) {
                    data.items.forEach(c => {
                        const div = document.createElement('div');
                        div.className = 'p-3 hover:bg-[#f7fee7] cursor-pointer border-b border-slate-100 last:border-0';
                        div.innerHTML = `
                            <div class="font-bold text-slate-800 text-sm">${App.escapeHtml(c.display_name || c.name || '')}</div>
                            <div class="text-xs text-slate-500">${App.escapeHtml((c.street || '') + ', ' + (c.city || ''))}</div>
                        `;
                        div.onclick = () => App.TemplateTab.selectCustomer(c);
                        drop.appendChild(div);
                    });
                    drop.classList.remove('hidden');
                } else {
                ...
                console.error('TemplateTab.filterCustomers failed', e);
            }
        },
        ...
        async selectCustomer(customer) {
            ...
            const selBox = document.getElementById('template-customer-selected');
            const nameEl = document.getElementById('template-sel-cust-name');
            const addrEl = document.getElementById('template-sel-cust-addr');
            const searchInput = document.getElementById('template-customer-search');
            const drop = document.getElementById('template-customer-dropdown');
            const objectStep = document.getElementById('template-object-step');
            const objectSelect = document.getElementById('template-object-select');
            ...
            if (nameEl) nameEl.innerText = customer.display_name || customer.name || '';
            if (addrEl) addrEl.innerText = `${customer.street || ''}, ${customer.city || ''}`;
            if (selBox) selBox.classList.remove('hidden');
            if (searchInput) searchInput.value = customer.display_name || customer.name || '';
            ...
            try {
                ...
                if (objectStep) objectStep.classList.remove('opacity-50', 'pointer-events-none');

            } catch (e) {
                console.error('TemplateTab.selectCustomer failed', e);
            }
        },

        clearCustomer() {
            State.templateApply = State.templateApply || {};
            State.templateApply.customer = null;
            State.templateApply.objects = [];

            const selBox = document.getElementById('template-customer-selected');
            const objectStep = document.getElementById('template-object-step');
            const objectSelect = document.getElementById('template-object-select');
            const countEl = document.getElementById('template-object-count');
            const confirmBtn = document.getElementById('template-apply-confirm-btn');
        ...
        selectObject() {
            const sel = document.getElementById('template-object-select');
            const countEl = document.getElementById('template-object-count');
            const confirmBtn = document.getElementById('template-apply-confirm-btn');
            ...
            State.templateApply = State.templateApply || {};
            State.templateApply.objects = picked;

            if (countEl) countEl.innerText = String(picked.length);

            const canApply = !!State.templateApply.customer && picked.length > 0;

            if (confirmBtn) {
                confirmBtn.disabled = !canApply;
                confirmBtn.classList.toggle('opacity-50', !canApply);
                confirmBtn.classList.toggle('cursor-not-allowed', !canApply);
            }
        },

        confirmApplyTemplate() {
            if (!State.selectedTemplate) return;
            if (!State.templateApply?.customer) {
                alert('Bitte zuerst einen Kunden auswählen.');
                return;
            }
            if (!Array.isArray(State.templateApply.objects) || !State.templateApply.objects.length) {
                alert('Bitte mindestens ein Objekt / Produkt auswählen.');
                return;
            }
            ...
            App.switchView('editor');
            App.Tabs.switch('list');
            App.TemplateTab.closeApplyModal();

            // also sync selected customer/object into main wizard/editor state
            State.customer = State.templateApply.customer;
            State.object = {
                items: State.templateApply.objects,
                name: State.templateApply.objects.length === 1
                    ? State.templateApply.objects[0].label
                    : `${State.templateApply.objects.length} Produkte ausgewählt`
            };

            // sync customer info into cover
            ...
            if (nameEl) nameEl.innerText = cust.display_name || cust.name || '';
            if (lastNameEl) lastNameEl.innerText = cust.lastname || '';
            if (addrEl) {
                addrEl.innerHTML = `${cust.street || ''}<br>${cust.postcode || ''} ${cust.city || ''}`;
            }
            if (custIdEl) custIdEl.value = cust.customer_no || '-';
            ...
            App.toastConfirmShow({
                title: 'Vorlage übernommen',
                message: 'Die Vorlage wurde mit dem ausgewählten Kunden und Objekt geladen.',
                okText: 'OK',
                cancelText: '',
                onOk: () => {}
            });
    ...
    App.ListView = {
        ...
        handleDropOnSection(ev, sIdx) {
            ...
            if (id && type && sIdx !== null && sIdx !== undefined) {
        ...
        getColDefs() {
            ...
            if (cols.pos) defs.push({ key: 'pos', label: 'Pos.', width: '88px', align: 'center' });
            ...
            if (cols.title) defs.push({ key: 'title', label: 'Produkttitel', width: '320px' });
            ...
            if (cols.actions) defs.push({ key: 'actions', label: 'Aktionen', width: '120px', align: 'right' });
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <details class="list-colpicker">
                            ...
                            <div class="list-colpicker-menu" style="width:260px; max-height:420px; overflow-y:auto;">
                                ...
                                ${pickerItem('Art.-Nr.', 'articleNumber')}
                                ${pickerItem('Produkttitel', 'title')}
                                ${pickerItem('Lieferant & Konditionen', 'supplier')}
                                ${pickerItem('Dokumente', 'dokumente')}
                                ${pickerItem('Typ', 'type')}
                                ${pickerItem('Status', 'status')}
                                ${pickerItem('Menge', 'qty')}
                                ${pickerItem('Einheit', 'unit')}
                                ${pickerItem('VPE', 'pe')}
                                ${pickerItem('EK / Einheit', 'ek')}
                                ${pickerItem('EK gesamt', 'ek_total')}
                                ${pickerItem('Marge', 'margin')}
                                ${pickerItem('VK / Einheit', 'vk')}
                                ${pickerItem('DB / Einheit', 'profit')}
                                ${pickerItem('VK gesamt', 'vk_total')}
                                ${pickerItem('DB gesamt', 'db_total')}
                                ${pickerItem('Gewichtung', 'weighting')}
        ...
        sectionHeadHtml(sec, sIdx, isVirtual = false) {
            ...
            const secTotal = (sec.items || []).reduce((sum, it) => {
                ...
                if ((it.lineType || 'standard') !== 'standard') return sum;
                if ((it.kind || '') === 'note') return sum;
            ...
            return `
                <div class="list-sec-head p-3 flex items-center justify-between border-y border-slate-200 bg-slate-50 mt-4 mb-1 rounded-lg mx-2 shadow-sm">
                    <div class="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        ${
                            isLocked
                                ? `<div class="bg-transparent outline-none font-black text-slate-800 w-full">${App.escapeHtml(sec.title || '')}</div>`
                                : `
                                    <input
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ${!isVirtual ? `
                            <button onclick="App.toggleSectionLock(${sIdx})" class="list-mini-btn" title="${isLocked ? 'Sektion entsperren' : 'Sektion sperren'}">
                                <i class="fa-solid ${isLocked ? 'fa-lock' : 'fa-unlock'}"></i>
                            </button>
                        ` : ``}

                        ${(!isLocked && !isVirtual) ? `
                            <button onclick="App.addNotePosition(${sIdx})" class="list-mini-btn" title="Hinweis hinzufügen">
                                <i class="fa-solid fa-note-sticky"></i>
                            </button>

                            <button onclick="App.addManualItem(${sIdx})" class="list-mini-btn" title="Position hinzufügen">
                                <i class="fa-solid fa-plus"></i>
                            </button>

                            <button onclick="App.removeSection(${sIdx})" class="list-action-btn danger" title="Sektion löschen">
                                <i class="fa-solid fa-trash"></i>
                            </button>
                        ` : ``}
                    </div>
        ...
        rowHtml(it, sIdx, iIdx, subIdx, level, posStr, defs, gridTemplate, extra = {}) {
            ...
            const isProtectedRow = isNote || hasLaborRows || currentKind === 'labor' || !!extra.isVirtualSection;
            ...
            const grandTotals = (App.getRenderableSections() || []).reduce((acc, sec) => {
                (sec.items || []).forEach(row => {
                    ...
                    const rowVK = App.calcItemGross(row);
                    const rowEK = App.calcItemCost(row);
            ...
            const dragAttrs = (!isLocked && !isProtectedRow) ? `
            ...
            defs.forEach((col) => {
                ...
                switch (col.key) {
                    case 'pos': {
                        cells += cellWrap(`
                            <div class="flex items-center gap-2">
                                ${(!isLocked && !isSub && !isProtectedRow) ? `
                                    <span class="drag-handle" draggable="true" ondragstart="App.dragStartPos(event, ${sIdx}, ${iIdx}, ${subArg})" title="Position verschieben">
                                        <i class="fa-solid fa-grip-vertical"></i>
                                    </span>
                                ` : ''}

                                ${(isSub && !isLocked && !isProtectedRow) ? `
                                    <span class="drag-handle !w-5 !h-5 !text-[10px]" draggable="true" ondragstart="App.dragStartPos(event, ${sIdx}, ${iIdx}, ${subArg})" title="Unterposition verschieben">
                                        <i class="fa-solid fa-grip-lines"></i>
                                    </span>
                                ` : ''}
                    ...
                    case 'articleNumber': {
                        cells += cellWrap(
                            isNote || currentKind === 'labor'
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center px-2 text-slate-400 w-full">—</div>`
                                : `
                                    <input
                                        value="${App.escapeHtml(it.article_no || it.articleNumber || '')}"
                                        onchange="${ctxText('article_no')}"
                                        class="mat-ctrl mat-input w-full"
                                        placeholder="Art.-Nr."
                                        ${isLocked ? 'readonly' : ''}
                                    >
                                `,
                    ...
                    case 'title': {
                        ...
                        const rawText = (it.desc || '').toString().replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
                        const preview = rawText ? App.escapeHtml(rawText).slice(0, 140) : '';
                        ...
                        cells += cellWrap(`
                            <div class="flex flex-col w-full" style="padding-left:${level * 20}px">
                                ...
                                <div class="mt-1 ml-2 ${level > 0 ? 'ml-6' : ''}">
                                    ${
                                        preview
                                            ? `<div class="text-[11px] text-slate-500 leading-relaxed break-words">${preview}</div>`
                                            : `<div class="text-[11px] text-slate-300 italic">Keine Beschreibung</div>`
                                    }

                                    <button type="button"
                                        onclick="App.openDescModal(${sIdx},${iIdx},${subArg})"
                                        class="text-[10px] text-[#93c21c] mt-1 text-left hover:underline"
                                        ${isLocked ? 'disabled' : ''}>
                                        <i class="fa-solid fa-pen-to-square"></i>
                                        Beschreibung bearbeiten ${it.desc_html || it.desc ? '(vorhanden)' : ''}
                                    </button>
                                </div>
                            </div>
                        `, alignClass);
                    ...
                    case 'supplier': {
                        const supplierName = it.distributor_name || it.supplier || '';
                        const supplierArticle = it.distributor_article_no || '';
                            ...
                            isNote || currentKind === 'labor'
                                ...
                                : `
                                    <div class="flex flex-col gap-2 w-full">
                                        <input
                                            value="${App.escapeHtml(supplierName)}"
                                            onchange="${ctxText('distributor_name')}"
                                        ...
                                        <input
                                            value="${App.escapeHtml(supplierArticle)}"
                                            onchange="${ctxText('distributor_article_no')}"
                                            placeholder="Lief.-Nr."
                                            class="mat-ctrl mat-input w-full"
                                            ${isLocked ? 'readonly' : ''}
                                        >
                                        ...
                                        <div class="grid grid-cols-2 gap-2 w-full">
                                            <div class="mat-addon-wrap">
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                    value="${skontoVal.toFixed(2)}"
                                                    onchange="${ctxText('skonto')}"
                                                    class="mat-addon-input text-right"
                                                    ${isLocked ? 'readonly' : ''}>
                                                <span class="mat-addon-text">Skonto %</span>
                                            </div>
                                            ...
                                            <div class="mat-addon-wrap">
                                                <input
                                                    ...
                                                    step="1"
                                                    value="${paymentDays}"
                                                    onchange="${ctxText('payment_terms')}"
                                                    class="mat-addon-input text-right"
                                                    ${isLocked ? 'readonly' : ''}>
                                                <span class="mat-addon-text">Tage</span>
                                            </div>
                                        </div>
                                    </div>
                                `,
                                    ...
                                    docs.length
                                        ? docs.map(doc => `
                                            <button type="button" class="mat-chip w-full justify-start gap-2 text-[#78b2ce] border-[#78b2ce]/20 bg-[#78b2ce]/10">
                                                ...
                                                <span class="truncate">${App.escapeHtml(doc?.name || 'Dokument')}</span>
                                            </button>
                                        `).join('')
                                        : `
                                            <span class="text-[10px] text-gray-400 italic flex items-center gap-1">
                                                <i class="fas fa-minus w-3 h-3"></i> Keine Dokumente
                                            </span>
                                        `
                                }
                            </div>
                    ...
                    case 'type': {
                        cells += cellWrap(
                            isNote
                                ? '-'
                                : hasLaborRows
                                    ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-center font-bold text-slate-500 w-full">Lohn</div>`
                                    : `
                                        <select onchange="App.updatePosConfig(${sIdx},${iIdx},${subArg},'kind',this.value)"
                                            class="mat-ctrl mat-input-center w-full"
                                            ${isLocked || isProtectedRow ? 'disabled' : ''}>
                    ...
                    case 'status': {
                        cells += cellWrap(
                            isNote
                                ...
                                : `
                                    <select
                                        onchange="App.updatePosStatus(${sIdx},${iIdx},${subArg},this.value)"
                                        class="mat-ctrl mat-input-center w-full"
                                        ${isLocked ? 'disabled' : ''}>
                    ...
                    case 'qty': {
                        const decFn = isSub
                            ? `App.updateSubItemDetails(${sIdx},${iIdx},${subArg},'qty', Math.max(1, ${qty} - 1))`
                            : `App.updateItemDetails(${sIdx},${iIdx},'qty', Math.max(1, ${qty} - 1))`;
                        const incFn = isSub
                            ? `App.updateSubItemDetails(${sIdx},${iIdx},${subArg},'qty', ${qty} + 1)`
                            : `App.updateItemDetails(${sIdx},${iIdx},'qty', ${qty} + 1)`;

                        cells += cellWrap(
                            isNote || hasLaborRows
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-center font-bold text-slate-500 w-full">${hasLaborRows ? qty.toFixed(2) : '-'}</div>`
                                : `
                                    <div class="flex items-center h-[34px] border border-[#e5e7eb] rounded-lg bg-white overflow-hidden focus-within:border-[#93c21c] focus-within:ring-2 focus-within:ring-[#93c21c]/20 w-full">
                                        <button type="button" ${isLocked ? 'disabled' : ''} onclick="${decFn}" class="w-8 h-full flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-[#93c21c] transition-colors shrink-0" title="Verringern">
                                        ...
                                        <input
                                            ...
                                            class="flex-1 w-full h-full text-center text-xs font-bold text-slate-800 outline-none bg-transparent"
                                            ${isLocked ? 'readonly' : ''}
                                        >
                                        <button type="button" ${isLocked ? 'disabled' : ''} onclick="${incFn}" class="w-8 h-full flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-[#93c21c] transition-colors shrink-0" title="Erhöhen">
                    ...
                    case 'pe': {
                        cells += cellWrap(
                            isNote || hasLaborRows
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-center font-bold text-slate-500 w-full">—</div>`
                                : `
                                    <input
                                        type="number"
                                        step="1"
                                        min="1"
                                        value="${pPe}"
                                        onchange="${ctxText('price_unit_value')}"
                                        class="mat-ctrl mat-input-center w-full"
                                        ${isLocked ? 'readonly' : ''}
                                    >
                                `,
                    ...
                    case 'ek': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">-</div>`
                                : hasLaborRows
                                    ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">${App.money(ek)} €</div>`
                                    : `
                    ...
                    case 'ek_total': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="font-mono text-slate-400 font-bold">-</div>`
                                : `<div class="font-mono text-slate-800 font-bold">${App.money(totalEK)} €</div>`,
                            alignClass
                        );
                        break;
                    }
                    ...
                    case 'margin': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">-</div>`
                                : hasLaborRows
                                    ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">${margin.toFixed(1)} %</div>`
                                    : `
                                        <div class="mat-addon-wrap w-full">
                                            <input
                    ...
                    case 'vk': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">-</div>`
                                : hasLaborRows
                                    ? `<div class="mat-ctrl bg-slate-50 flex items-center justify-end px-2 font-mono font-bold text-slate-500 w-full">${App.money(vk)} €</div>`
                                    : `
                                        <div class="mat-addon-wrap w-full">
                                            <input
                    ...
                    case 'profit': {
                        ...
                        cells += cellWrap(
                            isNote
                                ? `<div class="font-mono text-slate-400 font-bold">-</div>`
                                : `<div class="font-mono text-[#78b2ce] font-bold">${App.money(profitPerPiece)} €</div>`,
                            alignClass
                        );
                        break;
                    }
                    ...
                    case 'vk_total': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="font-mono text-slate-400 font-bold">-</div>`
                                : `<div class="font-mono text-slate-800 font-bold">${App.money(totalVK)} €</div>`,
                            alignClass
                        );
                        break;
                    }
                    ...
                    case 'db_total': {
                        cells += cellWrap(
                            isNote
                                ? `<div class="font-mono text-slate-400 font-bold">-</div>`
                                : `<div class="font-mono text-slate-800 font-bold">${App.money(db1)} €</div>`,
                            alignClass
                        );
                        break;
                    }
                                    ...
                                    <div class="flex flex-col gap-2 w-full">
                                        ...
                                        <div class="flex flex-col gap-0.5">
                                            <div class="flex justify-between items-center text-[10px]">
                                                ...
                                                <span class="font-bold text-[#78b2ce]">${weightDBText}%</span>
                                            </div>
                                            <div class="w-full h-1.5 bg-[#78b2ce]/10 rounded-full overflow-hidden">
                                                <div class="h-full bg-[#78b2ce] rounded-full" style="width:${safeWeightDB}%"></div>
                                            </div>
                                        </div>
                                    </div>
                                `,
                    ...
                    case 'actions': {
                        cells += cellWrap(`
                            <div class="flex items-center gap-1 justify-end w-full">
                                ${(!isLocked && !isSub && !isNote && !hasLaborRows && !extra.isVirtualSection) ? `
                                    <button onclick="App.addSubItem(${sIdx},${iIdx})" class="mat-btn-icon" title="Unterposition hinzufügen">
                                        <i class="fa-solid fa-level-down-alt"></i>
                                    </button>
                                ` : ''}

                                ${!extra.isVirtualSection ? `
                                    <button onclick="App.openPosSettings(${sIdx},${iIdx},${subArg})" class="mat-btn-icon" title="Einstellungen">
                                        <i class="fa-solid fa-sliders"></i>
                                    </button>
                                ` : ''}

                                ${(!isLocked && !extra.isVirtualSection) ? `
                    ...
                    <div class="p-4 bg-slate-50/80 shadow-inner border-t border-slate-200">
                        <div class="flex items-center justify-between mb-3">
                            <div class="text-xs font-black uppercase tracking-wider text-slate-600">
                                <i class="fa-solid fa-users text-brand-primary"></i> Arbeitsleistungsdetails
                            </div>
                ...
                (sec.items || []).forEach(row => {
                    ...
                    if (hasSubItems) {
                        row.subItems.forEach(sub => {
                            if (!sub || sub.active === false) return;
                            if ((sub.kind || '') === 'note') return;
                            if ((sub.lineType || sub.status || 'standard') !== 'standard' && (sub.status || 'normal') !== 'normal') return;
                    ...
                    case 'ek_total':
                        cells += cellWrap(`
                            <div class="w-full flex flex-col items-end">
                                ...
                                <div class="text-sm font-black text-slate-800">${App.money(totals.ek)} €</div>
                            </div>
                    ...
                    case 'total':
                        cells += cellWrap(`
                            <div class="w-full flex flex-col items-end">
                                ...
                                <div class="text-sm font-black text-[#4f8aa7]">${App.money(totals.vk)} €</div>
                            </div>
                        `, 'mat-grid-cell-right mat-total-cell');
                        break;
                    ...
                    case 'db_total':
                        cells += cellWrap(`
                            <div class="w-full flex flex-col items-end">
                                ...
                                <div class="text-sm font-black text-emerald-700">${App.money(totals.db)} €</div>
                            </div>
                        `, 'mat-grid-cell-right mat-total-cell');
                        break;
                ...
                sectionEntries.forEach((entry, visualSectionIdx) => {
                    ...
                    (sec.items || []).forEach((it, iIdx) => {
                                ...
                                <div class="mx-4 mb-2 px-3 py-2 text-[10px] text-slate-400 border border-dashed border-slate-200 rounded bg-slate-50 text-center transition-colors"
                                    ...
                                    ondrop="
                                        ...
                                        if (App.getDragState()?.type === 'pos') {
                                            App.moveDraggedNode(
                                                App.dragState,
                                                { mode: 'to-sub', sIdx: ${realSIdx}, iIdx: ${iIdx}, depth: 1 }
                                            );
                                            App.clearDragMode();
                                        } else if (App.isLibraryDrag()) {
                                            App.ListView.handleDropOnPosition(event, ${realSIdx}, ${iIdx});
                                        }
                                    ">
                        ...
                        if (!hasTree || App.ListView.isOpen(mainKey, true)) {
                            html += App.ListView.renderSubTree(
                                it,
                                realSIdx,
                                iIdx,
                                defs,
                        ...
                        html += `
                            <div class="list-section-drop"
                                ...
                                ondrop="
                                    ...
                                    if (App.isLibraryDrag()) {
                                        App.ListView.handleDropOnSection(event, ${realSIdx});
                                        return;
                                    }
                                    ...
                                    if (App.getDragState()?.type === 'pos') {
                                        App.moveDraggedNode(
                                            App.dragState,
                                            { mode: 'to-main', sIdx: ${realSIdx}, iIdx: (State.sections[${realSIdx}]?.items?.length || 0) }
                                        );
                                        App.clearDragMode();
                                    }
                                ">
  ...
  App.applyGlobalMarginUpdate = function(kind, newMargin) {
    ...
    App.addPositionQuick = function () {
        ...
        App.Bio.addEntry('Neue Sektion', `Sektion "${t || 'Neue Sektion'}" erstellt.`);
        const defaultMargin = App.getDefaultMargin('article');
            State.sections[sIdx].items.push({
                name: 'Neue Position',
                desc: 'Beschreibung',
   ...
   App.vkFromEkMargin = function(ek, marginPercent){
    ...
    data: {
        ...
        showThumbnails: false,
        showCalcSidebar: false,
        columns: null // Gets merged with App.ListView.getDefaults().cols
    },

    // Load from injected blade variable on startup
    load() {
        const dbPrefs = window.ServerUserPrefs;
        
        if (dbPrefs) {
            this.data.defaultTab = dbPrefs.default_tab || 'list';
            this.data.showThumbnails = !!dbPrefs.show_thumbnails;
            this.data.showCalcSidebar = !!dbPrefs.show_calc_sidebar;
            
            if (dbPrefs.list_columns) {
                this.data.columns = dbPrefs.list_columns;
                
                // Inject to ListView state so rendering uses it
                App.ListView.ensureStore();
        ...
        if (this.data.showThumbnails) App.toggleThumbnails(true);
        if (this.data.showCalcSidebar) App.toggleSidebar('right', true);
    },

    // Save to database via AJAX
    async saveToDb() {
        const payload = {
            default_tab: this.data.defaultTab,
            show_thumbnails: this.data.showThumbnails,
            show_calc_sidebar: this.data.showCalcSidebar,
            list_columns: App.ListView.cols() // Fetch live columns
            };
    ...
    App.UserPrefsModal = {
        colMap: {
            ...
            status: 'Status', qty: 'Menge', unit: 'Einheit', pe: 'VPE', ek: 'EK / Einheit', 
...
<script>
   App.UNIT_OPTIONS = [
        'Pcs', 'm', 'rm', 'm²', 'm³', 'kg', 'g', 't', 'l', 'ml',
        'hrs', 'min', 'Day', 'Week', 'Month', 'Flat', 'Set'
    ];
    ...
    App.renderLaborUnitOptions = function(selected) {
        const val = (selected || 'hrs').toString();
        // Only time/labor based units
        const opts = ['hrs', 'min', 'Day', 'Flat'];
        return opts.map(u => 
            `<option value="${u}" ${u === val ? 'selected' : ''}>${u}</option>`
        ).join('');
    };
