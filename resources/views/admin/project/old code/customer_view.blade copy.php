    ...
    <div class="content-overlay"></div>
        ...
        <div class="task-list">
            ...
            <div style="display: flex; gap: 20px; margin-bottom: 15px;">
                <button onclick="showTab('board')" id="tab-board" class="tab-button active">📋 Board</button>
                <button onclick="showTab('list')" id="tab-list" class="tab-button">📄 Tasks</button>
            </div>
            ...
            <div id="board-view" class="tab-view">
                <div style="display: flex; gap: 20px;">
                    <!-- Not started column -->
                    <div style="flex: 1; background: #2a2a2a; border-radius: 8px; padding: 10px;">
                    ...
                    <!-- In progress column -->
                    <div style="flex: 1; background: #2a2a2a; border-radius: 8px; padding: 10px;">
                        <strong class="badge in-progress">In progress</strong>
                        <div class="task">T</div>
                    </div>
                    <!-- Done column -->
                    <div style="flex: 1; background: #2a2a2a; border-radius: 8px; padding: 10px;">
                        <strong class="badge done">Done</strong>
                        <div class="task">Task 3<br><small>Test Sadid<br>19. März 2025</small></div>
                    </div>
                </div>
            </div>
   ...
   function loadProjectTasks(productId) {
    ...
    function addMainTask() {
      ...
      function addSubTask() {
        ...
        statuses.forEach(status => {
            ...
            column.style.flex = "1";
            column.style.background = "#2a2a2a";
    ...
    function renderTaskAccordion(phases) {
        ...
        phases.forEach((phase, index) => {
            ...
            const contentId = `phase-content-${index}`;
            ...
            header.innerHTML = `
                ...
                <div class="progress" style="width: 200px; height: 10px;">
                    <div id="progress-${phaseId}" class="progress-bar bg-success" 
            ...
            phase.activities.forEach((task) => {
                ...
                // ✅ Set all required attributes directly here
                taskEl.setAttribute("data-activity-id", task.activity_id || task.id || null);
                ...
                taskEl.innerHTML = `
                    <div>
                        <input type="checkbox" class="task-checkbox" data-phase="${phaseId}" ${isChecked ? "checked" : ""}>
                        ${task.title_activity}
                    </div>
                    <div>
            ...
            // Bind checkbox progress updates
            setTimeout(() => {
                ...
                checkboxes.forEach(cb => {
                    cb.addEventListener("change", () => {
                        ...
                        updatePhaseProgress(phaseId, total, checked);
                    });
                });
            }, 0);
        });
    ...
    function visitProfile(button) {
        ...
        fetch(url)
            ...
            .then(taskData => {
                ...
                loadAllChecklists(selectedProductId); // fill dropdown
            })
            .catch(() => {
                ...
                fetch(`/project/checklist/${productId}`)
                    .then(res => {
                        if (!res.ok) throw new Error("Kein Default-Checklist gefunden");
                        return res.json();
                    })
                    .then(phaseData => {
                        if (phaseData.phases && Array.isArray(phaseData.phases)) {
                            ...
                            Swal.fire("Achtung", "Diese Standard-Checkliste enthält keine Phasen.", "warning");
                        }
                    })
                    .catch(err => {
    ...
    function saveCurrentChecklistToProjectTasks() {
        ...
        if (!checklistId || !customerId || !alternativeId || !productId) {
            Swal.fire({
                title: "Fehlende Angaben",
                html: `Bitte wähle zuerst:<br>
                    ${!checklistId ? "<strong>Checkliste</strong><br>" : ""}
                    ${!customerId ? "<strong>Kunde</strong><br>" : ""}
                    ${!alternativeId ? "<strong>Alternative</strong><br>" : ""}
                    ${!productId ? "<strong>Produkt</strong><br>" : ""}`,
                icon: "warning"
            });
            return;
        }
