                        ...
                        <div class="card-content">
                            <div class="card-body"> 
                                <div class="nav-vertical">
                                    <ul class="nav nav-tabs nav-right flex-column" role="tablist" style="height: 77px;">
                                        <li class="nav-item">
                                            <a class="nav-link active" id="baseVerticalRight-tab1" data-toggle="tab" aria-controls="tabVerticalRight1" href="#tabVerticalRight1" role="tab" aria-selected="false">Coins</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link" id="baseVerticalRight-tab2" data-toggle="tab" aria-controls="tabVerticalRight2" href="#tabVerticalRight2" role="tab" aria-selected="false">Feedbacks</a>
                                        </li>
                                        <li class="nav-item">
                                            <a class="nav-link " id="baseVerticalRight-tab3" data-toggle="tab" aria-controls="tabVerticalRight3" href="#tabVerticalRight3" role="tab" aria-selected="true">Charts </a>
                                        </li>
                                    </ul>
                                    <div class="tab-content border rounded bg-white shadow-sm p-3" style="min-height: 300px;">
                                        <!-- Coins Tab -->
                                        <div class="tab-pane fade show active" id="tabVerticalRight1" role="tabpanel" aria-labelledby="baseVerticalRight-tab1"> 
                                            <div class="d-flex justify-content-between align-items-center mb-2">
                                                <h5 class="text-success"><i class="feather icon-award"></i> Projekt-Auszeichnung</h5>
                                                <button class="btn btn-sm btn-outline-primary" onclick="openAwardModal()">
                                                    <i class="feather icon-edit"></i> Auszeichnung bearbeiten
                                                </button>
                                            </div>

                                            <div id="project_award_area" class="mt-2">
                                                🔄 Lädt Auszeichnungsstatus...
                                            </div> 
                                        </div>

                                        <!-- Feedbacks Tab -->
                                        <div class="tab-pane fade" id="tabVerticalRight2" role="tabpanel" aria-labelledby="baseVerticalRight-tab2">
                                            <div class="d-flex justify-between align-items-center mb-2">
                                                <h5 class="text-primary"><i class="feather icon-message-circle"></i> Feedbackübersicht</h5>
                                                <button class="btn btn-sm btn-outline-primary" onclick="openFeedbackModal('create')">
                                                    <i class="feather icon-plus"></i> Feedback hinzufügen
                                                </button>
                                            </div>
                                            <div id="project_feedback_area">🔄 Feedback wird geladen...</div>
                                        </div>

                                        <!-- Charts Tab -->
                                        <div class="tab-pane fade" id="tabVerticalRight3" role="tabpanel" aria-labelledby="baseVerticalRight-tab3">
                                            <h5><i class="feather icon-pie-chart"></i> Projektstatistik</h5>
                                            <div>Charts kommen bald...</div>
                                        </div>
                                    </div>
...
<!-- END: Content-->
            ...
            <div class="modal fade text-left" id="controllerModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel160" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        <div class="modal-header bg-primary white">
                            <h5 class="modal-title" id="myModalLabel160">Mitarbeiter hinzufügen</h5>
                        ...
                        <form  method="post" id="add_controller_form" action="{{ route('add.controller.to.project') }}">
                            @csrf
                            <input type="hidden" name="project_id" id="controller_project_id" value="">
                            <input type="hidden" name="phase_id" id="controller_phase_id" value="">
                            <input type="hidden" name="activity_id" id="controller_activity_id" value="">

                            <div class="modal-body">
                                <label for="employee_id">Mitarbeiter auswählen</label>
                                <select name="employee_id[]" id="controller_employee_id_select" class="form-control employee" style="width: 100%;" multiple>
                                     @foreach ($employees as $emp)
                                        <option value="{{$emp->id}}" 
                                                data-image="{{asset('images/employee/'.$emp->image)}}">
                                            {{$emp->name}} {{$emp->lastname}}
           ...
           <!-- Project Leader Modal -->
            ...
            <!-- Comment Panel -->
            <div id="comment-panel" class="fixed top-0 right-0 w-[65vw] h-full bg-white shadow-2xl z-[9999] p-6 overflow-y-auto hidden border-l border-gray-300 transition-all duration-300 ease-in-out">
          ...
          <!-- File Manager Panel -->
            ...
            <div id="timeline-log-panel" class="timeline-log-panel ">
                <!-- Header -->
                <div class="timeline-header d-flex justify-content-between align-items-center px-4 py-1">
                    <h5 class="mb-0 text-white text-2xl">Timeline Aktivitäten</h5>
                ...
                <!-- Body -->
                <div id="timeline-log-content" class="timeline-body px-4 pt-0 pb-4" style="overflow-y:auto; flex-grow: 1;"></div>
            </div>
            ...
            <!-- Assign Award Modal -->
                <div class="modal fade" id="assignAwardModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <form id="awardForm" action="javascript:void(0)">
                        @csrf
                        <input type="hidden" id="award_id" name="award_id">
                        <input type="hidden" id="award_project_id" name="project_id">
                        <input type="hidden" id="award_phase_id" name="phase_id">
                        <input type="hidden" id="award_activity_id" name="activity_id">
                        <div class="modal-content">
                            <div class="modal-header bg-success text-white">
                            <h5 class="modal-title">Auszeichnung</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                            <div class="mb-2">
                                <label>Coins</label>
                                <input type="number" id="award_coins" name="coins_awarded" class="form-control" required>
                            </div>
                            <div class="mb-2">
                                <label>Datum</label>
                                <input type="date" id="restricted_day" name="restricted_day" class="form-control">
                            </div>
                            <div class="mb-2">
                                <label>Uhrzeit</label>
                                <input type="time" id="restricted_time" name="restricted_time" class="form-control">
                            </div>
                            <div class="mb-2">
                ...
                <!-- Modal -->
                <div class="modal fade" id="awardFeedbackModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog">
                    <form id="awardFeedbackForm">
                    @csrf
                    ...
                    <div class="modal-content">
                        <div class="modal-header bg-success text-white">
                        <h5 class="modal-title">Feedback & Coins</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                        <div class="mb-2">
                            ...
                            <option value="final">Abschluss</option>
                            </select>
                        </div>
                        <div class="mb-2">
                        ...
                        <div class="form-check mb-2">
                            <input type="checkbox" class="form-check-input" name="controller_feedback" value="1">
                            <label class="form-check-label">Controller Feedback</label>
                        </div>
                        <div class="form-check mb-2">
                            <input type="checkbox" class="form-check-input" name="qualified" value="1">
                            <label class="form-check-label">Qualifiziert für Belohnung</label>
                        </div>
                        </div>
                        <div class="modal-footer">
                        <button type="submit" class="btn btn-success">Speichern & Coins verteilen</button>
                        </div>
                    </div>
                    </form>
                </div>
...
<script> 
            ...
            function searchProjectList(query) {
                fetch(`/project/search/status?search=${encodeURIComponent(query)}`)
                    .then(res => res.json())
                    .then(data => renderProjectList(data, highlightId))
            ...
            function loadProjectTasks(productId) {
                fetch(`/project/checklist/${productId}`)
                    ...
                    .then(data => {
                        ...
                        selectedChecklistId = data.project_montage_id ?? null;
            ...
            async function renderTaskAccordion(phases) {
                ...
                container.innerHTML = "<h3 class='text-xl font-semibold mb-4'>Aufgabe</h3>";
                ...
                for (const phase of phases) {
                    ...
                    const awardContainerId = `award-phase-${phase.phase_id}`;
                    ...
                    header.innerHTML = `
                        <div class="relative w-full p-2 rounded-lg hover:bg-blue-100 transition" style="background:#2c3e50 !important">
                            <div class="absolute top-2 right-2 z-50">
                                <button class="text-white hover:text-blue-600 text-2xl" onclick="toggleMenu(this)">
                                ...
                                <div class="menu-dropdown hidden absolute right-0 mt-2 w-44 bg-white border rounded shadow-lg">
                                    ...
                                    <button onclick="event.stopPropagation(); opentaskController(${selectedProjectId}, ${phase.phase_id})" class="w-full text-left px-2 text-sm hover:bg-blue-50">
                                        <i class="feather icon-users"></i> Kontrolleur
                                    </button>
                                    <button  onclick="event.stopPropagation(); award(${selectedProjectId}, ${phase.phase_id})" class="w-full text-left px-2 text-sm hover:bg-blue-50">
                                       <i class="feather icon-star"></i> Coins  
                                    </button> 

                                    <button onclick="event.stopPropagation(); unverifyModal(${selectedProjectId}, ${phase.phase_id})" class="w-full text-left px-2 text-sm hover:bg-blue-50">
                                        <i class="feather icon-x-circle text-danger"></i> Rückgängig
                                    </button>

                                    <button  onclick="event.stopPropagation(); verifyModal(${selectedProjectId}, ${phase.phase_id})" class="w-full text-left px-2 text-sm hover:bg-blue-50">
                                       <i class="feather icon-check"></i> Verifiziert
                                    </button> 
                            ...
                            <div onclick="document.getElementById('${contentId}').style.display = 
                                ...
                                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-1">
                                    <div class="phase-header-title flex flex-col text-white text-base font-semibold">
                                       <div class="flex items-center gap-2 text-white text-base font-semibold mb-1 phase-icon-line">
                                            <i class="feather icon-folder"></i>
                                            ${getTaskStatusIcon(phase.activities)}
                                            <span id="verify-badge-${phase.phase_id}" class="verified-badge ml-2"></span>
                                         </div>

                                        <div class="flex items-center gap-2 phase-header-title">
                                            ${phase.phase_name}
                                            <label class="flex items-center gap-1 text-sm text-green-200">
                                                <input type="checkbox" 
                                                    data-phase-id="${phase.phase_id}" 
                                                    onchange="handleVerifyCheckbox(this)" 
                                                    ${phase.verify === 'verified' ? 'checked disabled' : ''}>
                                                Verifiziert
                                            </label>
                                        </div> 
                                ...
                                <div class="flex justify-between items-center mt-0 gap-2 flex-wrap">
                                    <!-- Left: Users -->
                                    <ul class="users-list flex flex-wrap gap-1 mt-1"></ul>

                                    <!-- Right: Awards -->
                                    <div class="award-info text-sm text-yellow-400 flex items-center gap-2" id="${awardContainerId}"></div>
                                </div> 
                                 <div id="controllers-phase-${phase.phase_id}" class="mt-0 ml-2"></div>
                    ...
                    if (phase.verified_by_name && phase.verified_at) {
                        const badgeSpan = header.querySelector(`#verify-badge-${phase.phase_id}`);
                        if (badgeSpan) {
                            badgeSpan.innerHTML = `<i class="feather icon-check-circle"></i>`;
                            badgeSpan.title = `Verifiziert von: ${phase.verified_by_name} am ${phase.verified_at}`;
                            badgeSpan.classList.add('text-green-400', 'cursor-pointer');
                            badgeSpan.onclick = () => {
                                Swal.fire({
                                    icon: 'info',
                                    title: 'Verifiziert',
                                    html: `<b>${phase.verified_by_name}</b><br>am <b>${phase.verified_at}</b>`
                                });
                            };
                        }
                    }
                        ...
                        taskEl.innerHTML = `
                            <div class="mb-2">
                                <div class="flex justify-between items-start gap-4">
                                    <div class="flex-1">
                                        <strong class="block text-base">${task.title_activity}</strong>
                                        <small class="text-gray-500">(${task.duration || 0} Std)</small>
                                        <div class="text-gray-500 text-sm mt-1">
                                            <i class="feather icon-chevron-right"></i> ${task.description || 'Keine Beschreibung'}
                                        </div>
                                        <p class="task-date mt-1 text-sm text-gray-500">
                                            <i class="feather icon-calendar"></i>
                                            <strong>Start:</strong> ${timeline?.start_date || 'N/A'} |
                                            <strong>Fällig:</strong> ${timeline?.due_date || 'N/A'}
                                        </p>
                                    </div>

                                    <div class="flex items-center gap-4">
                                        <div class="feedback-star" id="feedback-activity-${selectedProjectId}-${phase.phase_id}-${activityId}" >
                                       
                                        </div>
                                        <div onclick="openCommentLayout(${selectedProjectId}, ${phase.phase_id}, ${activityId})" class="relative cursor-pointer" title="Kommentare">
                                            <i class="feather icon-message-square text-primary text-xl"></i>
                                            <span id="comment-badge-${activityId}" class="badge badge-pill badge-primary badge-up">0</span>
                                        </div>
                                        <div onclick="openFilePanel(${selectedProjectId}, ${phase.phase_id}, ${activityId})" class="relative cursor-pointer" title="Dateien">
                                            <i class="feather icon-paperclip text-warning text-xl"></i>
                                            <span class="badge badge-pill badge-warning badge-up file-count-badge" data-activity-id="${activityId}">0</span>
                                        </div>
                                        <div onclick="openLogPanel(${selectedProjectId}, ${phase.phase_id}, ${activityId})" class="relative cursor-pointer" title="Log">
                                            <i class="feather icon-server text-warning text-xl"></i>
                                            
                                        </div>

                                        <div class="relative">
                                            <button onclick="toggleTaskMenu(this)" class="text-gray-600 hover:text-black text-lg">
                                                <i class="feather icon-more-horizontal"></i>
                                            </button>
                                            <div class="task-menu-dropdown hidden absolute right-0 top-full mt-1 w-40 bg-white border rounded shadow z-50">
                                                <button onclick="event.stopPropagation(); changeTaskDate(${phase.phase_id}, ${activityId})"
                                                    class="block w-full px-0 py-1 text-left text-sm hover:bg-gray-100">
                                                    <i class="feather icon-calendar"></i> Fälligkeitsdatum
                                                </button>
                    ...
                    $.get(`/project/controllers/${selectedProjectId}/${phase.phase_id}`, function (html) {
                        $(`#controllers-phase-${phase.phase_id}`).html(html);
                    });

                    $.get(`/project-awards/${selectedProjectId}/${phase.phase_id}`, function (groups) {
                        const container = document.getElementById(`award-phase-${phase.phase_id}`);
                        if (!container) return;
                        container.innerHTML = '';

                        groups.forEach(group => {
                            const groupWrapper = document.createElement('div');
                            groupWrapper.className = 'border px-2 py-1 rounded bg-yellow-50 text-yellow-800 text-xs mb-1';
                            groupWrapper.innerHTML = `<b>${group.employee}</b> <span class="text-xs text-yellow-600">(${group.awards.reduce((sum, a) => sum + a.coins, 0)} Coins)</span>`;

                            
                            group.awards.forEach(a => {
                                const badge = document.createElement('div');
                                badge.className = 'flex items-center gap-1 justify-between px-1 py-0.5 bg-yellow-100 rounded mt-1';
                                badge.innerHTML = `
                                    <div class="flex items-center gap-2">
                                        <i class="feather icon-award"></i>
                                        <span>${a.coins} Coins</span>
                                        ${a.date ? `<span class="text-xs text-gray-600">(${a.date}${a.time ? ' ' + a.time + ':00' : ''})</span>` : ''}
                                        ${a.reason ? `<i class="feather icon-info text-blue-600 cursor-pointer" onclick="showAwardInfo('${escapeHtml(a.reason)}')"></i>` : ''}
                                    </div>
                                    <div class="flex items-center gap-2 text-xs">
                                        <button onclick="editAward(${a.id})" class="text-green-600 hover:text-green-800"><i class="feather icon-edit-2"></i></button>
                                        <button onclick="deleteAward(${a.id}, ${phaseId})" class="text-red-600 hover:text-red-800"><i class="feather icon-trash-2"></i></button>
                                    </div>
                                `;
                                ...
                                controllerBtn.className = 'btn btn-sm btn-outline-info mt-1';
                                controllerBtn.innerHTML = `<i class="feather icon-shield-check"></i> Controller bestätigen`;
                                ...
                                groupWrapper.appendChild(controllerBtn);

                                // ✅ Feedback & Coins vergeben button
                                if (a.qualified && a.project_task_id && a.date) {
                                    const btn = document.createElement('button');
                                    btn.className = 'btn btn-sm btn-outline-success mt-2';
                                    btn.innerHTML = `<i class="feather icon-thumbs-up"></i> Feedback & Coins`;
                                    btn.onclick = () => {
                                        openAwardFeedbackModal(
                                            a.id,
                                            selectedProjectId,
                                            phaseId,
                                            a.activity_id ?? null,
                                            a.project_task_id ?? null
                                        );
                                    };
                                    groupWrapper.appendChild(btn);
                                }

                                $.post('/project-awards/check-controller', {
                                    ...
                                    if (res.isController) {
                                        ...
                                        controllerBtn.innerHTML = `<i class="feather icon-shield-check"></i> Controller bestätigen`;
                                        controllerBtn.onclick = () => {
                                            markControllerCheck(selectedProjectId, phaseId, a.activity_id ?? null);
                                        };
                                        groupWrapper.appendChild(controllerBtn);
                                    }
                                });
                    ...
                    refreshAwardPhase(phase.phase_id);
                    loadVerifyStatus(selectedProjectId, phase.phase_id);
            ...
            function refreshAwardPhase(phaseId) {
                $.get(`/project-awards/${selectedProjectId}/${phaseId}`, function (groups) {
                    const container = document.getElementById(`award-phase-${phaseId}`);
                    if (!container) return;
                    container.innerHTML = '';
                    ...
                    groups.forEach(group => {
                        const groupWrapper = document.createElement('div');
                        groupWrapper.className = 'border px-2 py-1 rounded bg-yellow-50 text-yellow-800 text-xs mb-1';
                        groupWrapper.innerHTML = `<b>${group.employee}</b>`;
            ...
            function verifyModal(projectId, phaseId) {
                Swal.fire({
                    title: 'Verifizieren bestätigen?',
                    text: 'Möchten Sie diesen Phase verifizieren?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    $.post('/project-tasks/verify', {
                        ...
                        if (response.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Verifiziert!',
                                text: `Verifiziert von: ${response.verified_by}`,
                            });

                            renderTaskAccordionFromCurrentState();
                        } else if (response.status === 'forbidden') {
                            Swal.fire({
                                icon: 'error',
                                title: 'Zugriff verweigert!',
                                text: response.message || 'Sie sind nicht als Controller eingetragen.',
                            });
                        }
                    }).fail(() => {
                        Swal.fire({
                            icon: 'error',
                            title: 'Fehler!',
                            text: 'Serverfehler beim Verifizieren.',
                        });
                    });
                });
            }
            ...
            function renderTaskAccordionFromCurrentState() {
                fetch(`/customer/project/phase/get/${selectedCustomerId}/${selectedAlternativeId}/${selectedProductId}`)
                    .then(res => res.json())
                    .then(data => {
                        renderTaskAccordion(data.phases);
                    });
            ...
            function unverifyModal(projectId, phaseId) {
                Swal.fire({
                    title: 'Verifizierung entfernen?',
                    text: 'Möchten Sie die Verifizierung dieser Phase zurücksetzen?',
                    icon: 'warning',
                    showCancelButton: true,
                    ...
                    $.post('/project-tasks/unverify', {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        project_id: projectId,
                        phase_id: phaseId
                    }, function (response) {
                        if (response.status === 'success') {
                            Swal.fire('Zurückgesetzt!', 'Die Phase ist nicht mehr verifiziert.', 'success');
                            renderTaskAccordionFromCurrentState();
                        } else {
            ...
            function loadVerifyStatus(projectId, phaseId) {
                $.get(`/project/get/task/status/verify/${projectId}/${phaseId}`, function (res) {
                    if (Array.isArray(res) && res.length > 0 && res[0].verify === 'verified') {
                        const checkbox = document.querySelector(`input[type="checkbox"][data-phase-id="${phaseId}"]`);
                        if (checkbox) {
                            checkbox.checked = true;
                            checkbox.disabled = true; // Prevent user from toggling it again
                        }

                        const phaseHeader = document.querySelector(`.accordion-header[data-phase-id="${phaseId}"] .phase-header-title`);
                        if (phaseHeader && !phaseHeader.querySelector('.verified-badge')) {
                            const badge = document.createElement("span");
                            badge.className = "verified-badge text-green-500 text-sm ml-2 cursor-pointer";
                            badge.innerHTML = `<i class="feather icon-check-circle"></i>`;
                            badge.title = `Verifiziert von: ${res[0].name} ${res[0].lastname} am ${res[0].updated_at}`;
                            phaseHeader.appendChild(badge);
                        }
                    }
                }).fail(() => {
                    console.warn(`❌ Fehler beim Laden des Verifizierungsstatus für Phase ID: ${phaseId}`);
                });
            }
            ...
            function handleVerifyCheckbox(checkbox) {
                const phaseId = checkbox.getAttribute("data-phase-id");
                const isChecked = checkbox.checked;

                Swal.fire({
                    title: isChecked ? 'Phase verifizieren?' : 'Verifizierung entfernen?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    $.post("/project-tasks/verify", {
                        ...
                        project_id: selectedProjectId,
                        phase_id: phaseId,
                        status: isChecked ? 'verified' : null
                    }, function (res) {
                        if (res.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                text: isChecked 
                                    ? `Verifiziert von: ${res.verified_by} am ${res.verified_at}`
            ...
            // Project Controller CRUD : start 
            function opentaskController(projectId, phaseId) {
                $('#controller_project_id').val(projectId);
                $('#controller_phase_id').val(phaseId);
                $('#controller_activity_id').val(''); // set if needed
                $('#controllerModal').modal('show');
            }



                function loadControllers(projectId, phaseId) {
                    $.get(`/project/controllers/${projectId}/${phaseId}`, function (html) {
                        $(`#controllers-phase-${phaseId}`).html(html);
                    });
                }

                function deleteController(id, projectId, phaseId) {
                    Swal.fire({
                        title: 'Löschen bestätigen',
                        text: "Möchten Sie diesen Kontrolleur wirklich entfernen?",
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonText: 'Ja, löschen',
                        cancelButtonText: 'Abbrechen'
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: `/project/controllers/delete/${id}`,
                                type: 'DELETE',
                                data: {
                                    _token: document.querySelector('meta[name="csrf-token"]').content
                                },
                                success: function () {
                                    Swal.fire('Gelöscht!', 'Kontrolleur wurde entfernt.', 'success');
                                    $(`#controllers-phase-${phaseId}`).html(""); // Clear first
                                    $.get(`/project/controllers/${projectId}/${phaseId}`, function (html) {
                                        $(`#controllers-phase-${phaseId}`).html(html); // Reload updated list
                                    });
                                },
                                error: function () {
            ...
            // Project Controller CRUD : End 


            function getTaskStatusIcon(activities) {
                let hasLateTask = false;
                let hasCompleteOnTime = false;

                const today = new Date();

                for (const task of activities) {
                    const doneRange = task.done_range || 0;
                    const due = new Date(task.due_date);
                    const done = new Date(task.done_date);
                    const start = new Date(task.start_date);

                    if (doneRange < 100 && due < today) {
                        hasLateTask = true;
                    }

                    if (doneRange === 100 && start <= task.restricted_day && done <= task.restricted_day) {
                        hasCompleteOnTime = true;
                    }
                }

                if (hasLateTask) {
                    return `<i class="feather icon-alert-triangle text-danger ms-1" title="Fälligkeitsdatum überschritten!"></i>`;
                } else if (hasCompleteOnTime) {
                    return `<i class="feather icon-check-circle text-success ms-1" title="Aufgaben abgeschlossen & pünktlich!"></i>`;
                }
            ...
            // Award CRUD : end
             ...
             // Menu bar of task and sub task (Date and Employee Drop down) :start
            ...
            function toggleTaskMenu(button) {
             ...
             // Menu bar of task and sub task (Date and Employee Drop down) :end 


            async function changeTaskDate(phaseId, activityId) {
                try {
                    // 1. Get existing dates
                    const response = await $.get(`/project-timeline/get-dates/${selectedProjectId}/${phaseId}/${activityId}`);
                    const { start_date, due_date } = response;

                    // 2. Show SweetAlert form
                    const { value: formValues } = await Swal.fire({
                        title: 'Datum aktualisieren',
                        html: `
                            <label for="start-date" class="d-block text-left">Startdatum</label>
                            <input id="start-date" type="date" class="swal2-input" value="${start_date || ''}">
                            <label for="due-date" class="d-block text-left">Fälligkeitsdatum</label>
                            <input id="due-date" type="date" class="swal2-input" value="${due_date || ''}">
                        `,
                        confirmButtonText: 'Speichern',
                        cancelButtonText: 'Abbrechen',
                        showCancelButton: true,
                        ...
                        preConfirm: () => {
                            ...
                            if (!start || !due) {
                                Swal.showValidationMessage('Beide Felder sind erforderlich');
                            }
                    ...
                    // 3. Submit updates
                    if (formValues) {
                        await $.post('/project-timeline/update-dates', {
                            _token: document.querySelector('meta[name="csrf-token"]').content,
                            project_id: selectedProjectId,
                            phase_id: phaseId,
                            activity_id: activityId,
                        ...
                        const taskElement = document.querySelector(`[data-activity-id="${activityId}"]`);
                        if (taskElement) {
                            const datePara = taskElement.querySelector('p.mt-1');
                            if (datePara) {
                                datePara.innerHTML = `<i class="feather icon-calendar"></i> <strong>Start:</strong> ${formValues.start_date} | <strong>Fällig:</strong> ${formValues.due_date}`;
                            }
                        }
                    ...
                    console.error(error);
                    Swal.fire('Fehler', 'Etwas ist schiefgelaufen.', 'error');
                }
            }
            ...
            function updatePhaseProgress(phaseId) {
                ...
                console.log(`Found ${taskFills.length} task progress bars for phase ${phaseId}`);
                ...
                taskFills.forEach(fill => {
                    ...
                    console.log(`Task fill: ${fill.id}, width: ${width}`);
                    total += width;
                    count++;
                });
           ...
           // ✅ Safe Phase Progress Bar Updater 
                            ...
                            employees.forEach(emp => {
                                usersList.append(`
                                    <li class="avatar pull-up delete_user" title="${emp.name} ${emp.lastname}" 
                                        ...
                                        <img class="media-object rounded-circle" src="/images/employee/${emp.image}" alt="${emp.name}" height="30" width="30">
                                    </li>
                                `);
                            });
            ...
            function renderProjectKanban(data) {
                ...
                data.forEach(project => {
                    ...
                    let employee = {
                        employee_id: project.employee?.employee_id ?? 0,
            ...
            function renderProjectList(data, highlightId = null) {
                ...
                data.forEach(project => {
                    ...
                    const serviceEscaped = project.service ? project.service.replace(/'/g, "\\'") : "";

                    const departmentName = project.department_name || "-";
                    const serviceName = project.service_name || "-";
                    ...
                    const progressHtml = `
                        <div class="custom-progress mt-1" title="${completedTasks} von ${totalTasks} Aufgaben erledigt">
                            <div class="custom-progress-bar" style="width: ${progress}%; background: ${progressColor}">
                                <span class="text-white ps-1">${progress}%</span>
                            </div>
                        </div>
                    `;

                    // Optional main employee (if exists)
                    const employee = project.employee || {};
                    const empHtml = employee.image
                        ? `<ul class="list-unstyled users-list m-0 d-flex align-items-center">
                                <li class="avatar pull-up" title="${employee.name} ${employee.lastname}">
                                    <img class="rounded-circle" src="${emp_src}/${employee.image}" alt="${employee.name}" height="25" width="25">
                                </li>
                            </ul>`
                        : `<small class="text-muted">Kein Mitarbeiter</small>`;

                    const row = document.createElement("tr");
                    if (highlightId && project.project_id == highlightId) {
                    ...
                    row.innerHTML = ` 
                        <td class="small">
                            <strong>${project.customer_name} ${project.customer_lastname}</strong>
                            <div><i class="feather icon-mail"></i> ${project.email}</div>
                            <div><i class="feather icon-map-pin"></i> ${project.street}, ${project.postcode}, ${project.city}</div>
                            <div><i class="feather icon-calendar"></i> ${updatedDate}</div>
                            <div><strong>Produkt:</strong> ${project.initial || '-'}</div>
                            <div><strong>Leistung:</strong> ${serviceName}</div>
                            <div><strong>Abteilung:</strong> ${departmentName}</div>
                            <div><strong>Fortschritt:</strong> ${progressHtml}</div>
                        </td> 
                        <td>${empHtml}</td>
                        <td>Projectlieter</td>
                        <td>Kontroller</td>

                        <td id="employee-list-${project.project_id}"><small class="text-muted">Lade...</small></td>
                        <td><span class="badge badge-info">${stageLabel}</span></td>
                        <td>
                            <button class="btn btn-sm btn-primary"
                                onclick="visitProfileFromList(
                                    ${project.project_id},
                                    ${project.customer_id},
                                    ${project.alternative_id},
                                    ${project.product_id},
                    ...
                    // 🔁 AJAX call for associated employees
                    $.get(`/project/${project.project_id}/employees`, function (employees) {
                        const seen = new Set();
                        const unique = [];
                        ...
                        const html = unique.length
                            ? unique.map(emp => {
                                ...
                                return `
                                    <div class="avatar d-inline-block me-1" title="${emp.name} ${emp.lastname}">
                                        <img src="${img}" alt="${emp.name}" height="25" width="25" class="rounded-circle">
                                    </div>
                                `;
                            }).join('')
                            : `<small class="text-muted">Keine zugewiesenen Mitarbeiter</small>`;

                        document.getElementById(`employee-list-${project.project_id}`).innerHTML = html;
                    });
                });
            }


            // ✅ Must be in global script (not inside a function or DOMContentLoaded)
            function visitProfileFromList(projectId, customerId, alternativeId, productId, service) {
                ...
                // loadProjectFeedbackAward(projectId);  

                visitProfile(fakeBtn); // Call your main logic
            }
 
 
            function addCard(
                    columnId, product, customerName, customerDetails, date, address,
                    customerId, alternativeId, productId, service, employee, projectId,
                    serviceId, departmentId, employeeId,
                    progress, priority, projectStatus, color, montageStart, endDate,
                    departmentName, serviceName, totalTasks, completedTasks
                ) 
                {
                    ...
                    let progressColor = progress >= 80 ? '#4caf50' : progress >= 50 ? '#ffc107' : '#f44336';
                    ...
                    card.innerHTML = `
                        ...
                        <div class="p-1 small">
                            <div>${customerDetails}<br>${date}<br>${address}</div>
                            <div><strong>Leistung:</strong> ${serviceName}</div>
                            <div><strong>Abteilung:</strong> ${departmentName}</div>   
                            <div><strong>Fortschritt:</strong>
                                <div class="custom-progress" title="${completedTasks} von ${totalTasks} Aufgaben erledigt">
                                    <div class="custom-progress-bar" style="width: ${progress}%; background: ${progressColor}">
                                        <span>${progress}%</span>
                                    </div>
                                </div>
            ...
            function visitProfile(button) {
                // Highlight selected card
                     ...
                     // Highlight selected card if it exists
                        ...
                        // Highlight selected table row if it exists
                ...
                fetch(customerInfoUrl)
                    ...
                    .then(data => {
                        ...
                        // Beteiligte Personen (correct deduplication)
                        const employeeText = document.getElementById("project-employee");
                            if (employeeText && Array.isArray(data.employees)) {
                                console.log("👥 Raw employee list:", data.employees);

                                const seenIds = new Set();
                                const uniqueEmployees = [];

                                data.employees.forEach(emp => {
                                    const empId = emp.id || `${emp.name}-${emp.lastname}`; // fallback key if id is missing
                                ...
                                const html = uniqueEmployees.map(emp => {
                                    const img = emp.image ? `/images/employee/${emp.image}` : '/images/gender/male.png';
                                    return `
                                        <div class="avatar" title="${emp.name} ${emp.lastname}">
                    ...
                    .then(taskData => {
                        ...
                        if (taskData && Array.isArray(taskData.phases) && taskData.phases.length > 0) {
                            ...
                            renderTaskAccordion(taskData.phases);
                              ...
                              // 🟢 Rufe für jede Phase die Award-Ladefunktion auf:
                            taskData.phases.forEach(phase => {
                                refreshAwardPhase(phase.phase_id);
                                loadVerifyStatus(selectedProjectId, phase.phase_id); // ✅ Add here
...
<script>
        ...
        $(document).ready(function () {
            $('#awardForm').on('submit', function (e) {
                ...
                const url = awardId ? `/project-awards/update/${awardId}` : `/project-awards/create`;

                const data = {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    award_id: awardId,
                    project_id: $('#award_project_id').val(),
                    phase_id: $('#award_phase_id').val(),
                    activity_id: $('#award_activity_id').val(),
                    coins_awarded: $('#award_coins').val(),
                    restricted_day: $('#restricted_day').val(),
                    restricted_time: $('#restricted_time').val(),
                    reason: $('#reason').val()
                };
                ...
                $.post(url, data)
                    .done(() => {
                        ...
                        Swal.fire('Gespeichert!', 'Die Auszeichnung wurde gespeichert.', 'success');
                        refreshAwardPhase(data.phase_id);
                    })
                    .fail(xhr => {
                        console.error(xhr.responseText);
            ...
            function award(projectId, phaseId, activityId = null) {
                $('#award_id').val('');
                $('#award_project_id').val(projectId);
                $('#award_phase_id').val(phaseId);
                $('#award_activity_id').val(activityId);
                $('#coins_awarded').val('');
                $('#restricted_day').val('');
                $('#restricted_time').val('');
                $('#reason').val('');
            ...
            function showAwardInfo(reason) {
                ...
                function editAward(id) {
                    $.get(`/project-awards/${id}`, function (data) {
                        $('#award_id').val(data.id); // ⬅️ set the ID
                        $('#award_project_id').val(data.project_id);
                        $('#award_phase_id').val(data.phase_id);
                        $('#award_activity_id').val(data.activity_id);
                        $('#coins_awarded').val(data.coins_awarded);
                        $('#restricted_day').val(data.restricted_day);
                        $('#restricted_time').val(data.restricted_time);
                ...
                function deleteAward(id, phaseId) {
                    Swal.fire({
                        title: 'Löschen bestätigen',
                        text: 'Willst du diese Auszeichnung wirklich löschen?',
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonText: 'Ja, löschen!',
                    }).then((result) => {
                        if (result.isConfirmed) {
                            $.ajax({
                                url: `/project-awards/${id}`,
                                type: 'DELETE',
                                data: { _token: $('meta[name="csrf-token"]').attr('content') },
                                success: () => {
                                    refreshAwardPhase(phaseId);
                                    Swal.fire('Gelöscht!', 'Die Auszeichnung wurde entfernt.', 'success');
                                }
                            });
                        }
                    });
                ...
                function openAwardFeedbackModal(awardId, projectId, phaseId, activityId, taskId) {
                    $('#award_id').val(awardId);
                    $('#project_id').val(projectId);
                    $('#phase_id').val(phaseId);
                    $('#activity_id').val(activityId || '');
                    $('#project_task_id').val(taskId || '');
                    $('#awardFeedbackModal').modal('show');
                }


                $('#awardFeedbackForm').on('submit', function (e) {
                    e.preventDefault();

                    const data = $(this).serialize();

                    $.post('/project-feedback/store', data, function () {
                        $('#awardFeedbackModal').modal('hide');
                        Swal.fire('Erfolg!', 'Feedback gespeichert & Coins vergeben.', 'success');
                        refreshAwardPhase($('#phase_id').val());
                    }).fail((xhr) => {
                        console.error(xhr.responseText);
                        Swal.fire('Fehler!', 'Feedback konnte nicht gespeichert werden.', 'error');
                    });
                });


                function markControllerCheck(projectId, phaseId, activityId) {
                    $.post('/project-feedback/controller-check', {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        project_id: projectId,
                        phase_id: phaseId,
                        activity_id: activityId
                    }, function () {
                        Swal.fire('Bestätigt!', 'Controller hat den Task bestätigt.', 'success');
                        refreshAwardPhase(phaseId);
                    }).fail(() => {
                        Swal.fire('Fehler!', 'Konnte nicht bestätigen.', 'error');
                    });
                }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    $('#add_controller_form').on('submit', function (e) {
        e.preventDefault(); // ✅ Prevent page reload

        let form = $(this);
        let formData = new FormData(this);
        let projectId = formData.get('project_id');
        let phaseId = formData.get('phase_id');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    // ✅ Reload controller section in accordion
                    $(`#controllers-phase-${phaseId}`).html("");
                    $.get(`/project/controllers/${projectId}/${phaseId}`, function (html) {
                        $(`#controllers-phase-${phaseId}`).html(html);
                    });
                }
            ...
            error: function (xhr) {
                Swal.fire('Fehler', 'Ein Fehler ist aufgetreten. Bitte überprüfen Sie Ihre Eingaben.', 'error');
                console.error(xhr.responseText);
            }
        });
    });
