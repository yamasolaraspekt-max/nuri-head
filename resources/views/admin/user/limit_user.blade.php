{{-- resources/views/admin/user/limit_user.blade.php --}}
    ...
    @push('style')
        <style>
          ...
          @media(max-width:1100px){
            .ua-wrap{max-width:100%;padding:18px 14px}
            .ua-titlebar{margin-top:76px}
            ...
            .ua-actions{justify-content:flex-start}
            .ua-input{min-width:100%}
          }
...
@section('content')
    <div class="ua-wrap" id="ua-root">
      <div class="ua-titlebar">
        <div>
          <div class="ua-title">Limited Users</div> 
        </div>
    ...
    <div class="ua-bd" id="ua-modal" aria-hidden="true">
      <div class="ua-modal" role="dialog" aria-modal="true" aria-labelledby="ua-modal-title">
        <div class="ua-mh">
          <div>
            <h3 class="ua-mt" id="ua-modal-title">Add Limited User</h3>
        ...
        <div class="ua-mb">
          <div class="ua-field">
            ...
            <div style="font-size:12px;color:var(--text-muted)">
              Saved into <b>users.name</b> as employee id.
            </div>
          </div>
    ...
    @once
        @push('scripts')
            ...
            (() => {
              ...
              const CAN_ADD = @json((bool) $canAdd);
              const CAN_UPDATE = @json((bool) $canUpdate);
              const CAN_DELETE = @json((bool) $canDelete);
              const IS_MAIN_ADMIN = @json((bool) $isMainAdmin);
              ...
              function renderStats(s){
                const conf = [
                  { k:"total",    l:"Total Limited", c:"var(--blue)" },
                ...
                DOM.stats.innerHTML = conf.map(x => `
                  <div class="ua-stat">
                    <div class="ua-stat-l">
                      <span class="ua-dot" style="background:${x.c}"></span>
                      ${x.l}
                    </div>
              ...
              function renderList(items){
                if (!items?.length){
                  DOM.list.innerHTML = `<div style="text-align:center;padding:60px;color:var(--text-muted)">No limited users found.</div>`;
                  return;
                }
                  ...
                  return `
                    <div class="ua-item" data-id="${id}">
                      ...
                      <div class="ua-main">
                        ...
                        <div class="ua-subt">
                          Employee ID stored in users.name:
                          <b>${esc(i.employee_id || i.name || "—")}</b>
                        </div>
                      </div>
              ...
              async function load(){
                ...
                try {
                  ...
                  DOM.list.innerHTML = `<div style="text-align:center;padding:60px;color:var(--red)">Failed to load users.</div>`;
                }
              }
              ...
              function openCreate(){
                ...
                DOM.modalTitle.textContent = "Add Limited User";
                DOM.modalSub.textContent = "Create a new limited user";
              ...
              function openEdit(id){
                ...
                DOM.modalTitle.textContent = "Edit Limited User";
                DOM.modalSub.textContent = `${row.emp_name || row.name || "—"} ${row.emp_lastname || ""}`.trim();

                DOM.fPassWrap.style.display = "none";
                DOM.fEmail.value = row.email || "";
                DOM.fEmployee.value = String(row.employee_id || row.name || "");
                DOM.fRole.value = String(row.is_admin || "2");
              ...
              function openDelete(id){
                ...
                state.delId = id;
                DOM.delSub.textContent = row
                  ? `${row.emp_name || row.name || "—"} ${row.emp_lastname || ""}`.trim()
                  : `User #${id}`;
              ...
              [DOM.modalX, DOM.modalCancel].forEach(x => {
                x.addEventListener("click", () => closeBD(DOM.modalBD));
              });

              DOM.modalBD.addEventListener("click", (e) => {
                if (e.target === DOM.modalBD) closeBD(DOM.modalBD);
              });

              [DOM.delX, DOM.delCancel].forEach(x => {
                x.addEventListener("click", () => closeBD(DOM.delBD));
              });

              DOM.delBD.addEventListener("click", (e) => {
                if (e.target === DOM.delBD) closeBD(DOM.delBD);
              });

              [DOM.passX, DOM.passCancel].forEach(x => {
                x.addEventListener("click", () => closeBD(DOM.passBD));
              });

              DOM.passBD.addEventListener("click", (e) => {
                if (e.target === DOM.passBD) closeBD(DOM.passBD);
              });
              ...
              DOM.modalSave.addEventListener("click", async () => {
                ...
                const payload = {
                  ...
                  is_admin: String(DOM.fRole.value || "2"),
                  is_active: DOM.fActive.checked ? 1 : 0,
                };

                if (!payload.employee_id) {
                  return showErr(DOM.modalErr, "Employee is required.");
                }

                if (!payload.email) {
                  return showErr(DOM.modalErr, "Email is required.");
                }
                ...
                try {
                  if (state.mode === "create") {
                    ...
                    if (pass.length < 8) {
                      return showErr(DOM.modalErr, "Password must be at least 8 characters.");
                    }
              ...
              DOM.passSave.addEventListener("click", async () => {
                ...
                if (p1.length < 8) {
                  return showErr(DOM.passErr, "Password must be at least 8 characters.");
                }

                if (p1 !== p2) {
                  return showErr(DOM.passErr, "Passwords do not match.");
                }
