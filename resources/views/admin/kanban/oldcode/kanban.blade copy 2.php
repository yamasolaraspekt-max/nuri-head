...
<style>
    ...
    /* Modal Styles for Team Selection */
    #swal-team-form .team-row {
        display: flex;
        gap: 5px;
        margin-bottom: 8px;
        align-items: center;
    }
    #swal-team-form .btn-remove-row {
        color: #dc3545;
        cursor: pointer;
        font-size: 18px;
    }
...
(function () {
  ...
  function cardHTML(item, stageKey) {
    const fullName = `${item.customer_name ?? ""} ${item.customer_lastname ?? ""}`.trim() || "Unbekannt";
    ...
    const fieldEmployee = item.field_employee && item.field_employee.employee_id ? item.field_employee : null;
    
    const empList = [];
    if (employee) empList.push({ title: ((employee.lastname ?? "") + " " + (employee.name ?? "")).trim() || "Innendienst", image: employee.image || "" });
    if (fieldEmployee) empList.push({ title: ((fieldEmployee.lastname ?? "") + " " + (fieldEmployee.name ?? "")).trim() || "Außendienst", image: fieldEmployee.image || "" });

    const empHTML = empList.length > 0
      ? `<ul class="list-unstyled users-list m-0 d-flex align-items-center">${empList.map(e => `<li class="avatar pull-up" title="${e.title}"><img class="media-object rounded-circle" src="${APP.EMP_SRC}/${e.image}" height="30" width="30" alt=""></li>`).join("")}</ul>`
      : "<small>&ndash;</small>";

    const hideJunk = stageRank(canonicalStage(stageKey)) >= stageRank("deal");
    const branchName = (item.branch_name ?? item.branch ?? item.branch_title ?? item.department_name ?? item.department ?? "").toString().trim();

    return `
      <div class="card-status-overlay" aria-hidden="true"></div>
      <div class="kb-menu kb-menu--card" aria-label="Kartenmenü">
        <button type="button" class="btn-icon kb-menu-toggle" data-act="custom-menu-toggle" title="Menü" aria-haspopup="menu" aria-expanded="false"><i class="feather icon-more-vertical" aria-hidden="true"></i></button>
        <div class="kb-menu-dropdown" role="menu" aria-label="Menü" hidden>
          ...
          <button type="button" class="kb-menu-item" data-menu="ticket" role="menuitem"><i class="feather icon-alert-circle mr-50"></i> Ticket</button>
          <button type="button" class="kb-menu-item" data-menu="wartung" role="menuitem"><i class="feather icon-tool mr-50"></i> Wartung</button>
          <button type="button" class="kb-menu-item" data-menu="termin" role="menuitem"><i class="feather icon-calendar mr-50"></i> Termin <span class="kb-menu-pill kb-menu-pill--ap" data-ap-count style="display:none;">0</span></button>
          <button type="button" class="kb-menu-item" data-menu="aufgabe" role="menuitem"><i class="feather icon-check-square mr-50"></i> Aufgabe <span class="kb-menu-pill kb-menu-pill--pt" data-pt-count style="display:none;">0</span></button>
        </div>
      </div>
      <div class="card-header card-header--kb">
        <div class="card-title"><strong class="card-name">${escapeHTML(fullName)}</strong><div class="circle product_circle" aria-hidden="true">${escapeHTML(item.initial ?? "")}</div></div>
      </div>
      <div class="kb-card-meta">
        <div class="kb-meta-row">
          <span class="kb-meta-item"><i class="feather icon-calendar"></i><span>${escapeHTML(updated)}</span></span>
          ${branchName ? `<span class="kb-meta-sep" aria-hidden="true">•</span><span class="kb-meta-item kb-branch" title="${escapeHTML(branchName)}">${branchSVG(14)}<span class="kb-branch-name">${escapeHTML(branchName)}</span></span>` : ``}
        </div>
        <small class="kb-meta-address">${escapeHTML(address)}</small>
      </div>
      ...
      <div class="live-feed-bar card-live-feed" data-feed-root data-feed-count="0" style="display:none;">
        <div class="live-feed-left"><div class="live-feed-icon"><i class="feather icon-zap"></i></div></div>
        <div class="live-feed-body">
          <div class="live-feed-line d-none" data-feed-empty><span class="live-feed-title">Keine Aktivitäten</span><span class="live-feed-dot">•</span><span class="live-feed-text">Noch keine Termine oder Aufgaben.</span></div>
          <div class="live-feed-line d-none" data-feed-line><span class="live-feed-title" data-feed-title>Aktivität</span><span class="live-feed-dot">•</span><span class="live-feed-text" data-feed-text>Details…</span></div>
          <div class="live-feed-meta"><span class="live-feed-pill" data-feed-pill>Info</span><span class="live-feed-time"><i class="feather icon-clock mr-25"></i><span data-feed-time>–</span></span><span class="live-feed-counter" data-feed-counter></span></div>
        </div>
        <div class="live-feed-controls">
          ...
          <button type="button" class="live-feed-btn" title="Pause / Abspielen" data-feed-toggle><i class="feather icon-pause" data-feed-icon-pause></i><i class="feather icon-play d-none" data-feed-icon-play></i></button>
      ...
      <div class="card-actions" role="group" aria-label="Aktionen">
        ...
        <div class="right-actions">
          ...
          <a href="/new_lead_profile/${item.customer_id}" class="btn-icon" title="Profil"><i class="feather icon-eye"></i></a>
  ...
  async function fetchJunkTab(qsStr) {
    ...
    if (!pane) return;
    try {
      const res = await fetch(`${APP.endpoints.junk}${qsStr ? `?${qsStr}` : ""}`, { headers: { Accept: "text/html", "X-Requested-With": "XMLHttpRequest" }, credentials: "same-origin" });
      pane.querySelector("#junkInner").innerHTML = await res.text();
    } catch (e) {}
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (!target) { Swal.fire("Hinweis", "Bitte Zielphase wählen.", "info"); return; }
    const { value: reason, isConfirmed } = await Swal.fire({ title: "Grund", input: "textarea", showCancelButton: true, confirmButtonText: "Wiederherstellen" });
    if (!isConfirmed) return;
    try {
      const res = await postJSON(`${APP.endpoints.changeStage}/${encodeURIComponent(row.dataset.customerId)}/${encodeURIComponent(row.dataset.alternativeId)}/${encodeURIComponent(row.dataset.productId)}`, { lead_product_id: Number(btn.dataset.id), stage: target, description: reason, source: "junk" });
...
(function () {
  ...
  function buildRowHTML(lead) {
    const s = utils.canonicalStage(lead.stage);
    const badgeMap = { lead: 'warning', offer: 'warning', deal: 'success', project: 'success', completed: 'success', archive: 'secondary' };
    const tone = badgeMap[s] || 'danger';
    
    const ws = (lead.work_status || 'playing').toLowerCase();
    ...
    return `
      <tr id="row-${lpId}" class="list-row-item" data-customer-id="${cId}" data-alternative-id="${aId}" data-product-id="${pId}" data-stage="${s}">
        <td>${lead.created_at ? new Date(lead.created_at).toLocaleDateString("de-DE") : "-"}</td>
        <td>
          <a href="/new_lead_profile/${cId}" class="customer-link">${lead.customer_lastname ?? ""} ${lead.customer_name ?? ""}</a>
          <div class="list-action-bar">
             ...
             <button type="button" class="btn-list-icon note" data-open-notes data-customer="${cId}" data-alt="${aId}" data-product="${pId}">
                <i class="feather icon-message-square"></i>
                <span class="badge-notes" data-count="0" style="display:none">0</span>
             </button>
             <a href="/lead/process/history/${cId}/${aId}/${pId}" class="btn-list-icon history" data-lh-history><i class="feather icon-activity"></i></a>
          </div>
        </td>
        <td>${lead.city ?? ""}</td>
        <td>${lead.initial ?? ""}</td>
        <td>${lead.employee?.lastname ?? "-"}</td>
        <td><span class="badge bg-${tone}">${APP.stageNames[s] || s}</span></td>
        <td>
            <select class="form-control stage-select" data-id="${lpId}">
                ${Object.entries(APP.stageNames).map(([k,l]) => `<option value="${k}" ${s===k?'selected':''}>${l}</option>`).join('')}
            </select>
        </td>
      </tr>`;
  }
...
(function () {
  ...
  /* ========================================================================== */
  /* UNIVERSAL ACTION HANDLER (Works for Kanban Cards AND List Rows)            */
  /* ========================================================================== */
  document.addEventListener("click", (e) => {
    ...
    // Find Parent: Could be a Kanban Card (.card) OR a Table Row (tr)
    const card = actBtn.closest(".card");
    const row  = actBtn.closest("tr");
    
    // We only need special handling here if it's a ROW (List View).
    // The previous script block handles Kanban Card logic via `card` references.
    if (card) return; 
    if (!row) return;
    ...
    // --- LOGIC FOR PLAY / PAUSE / STOP ---
    if (actBtn.dataset.run) {
        return (async () => {
            const state = actBtn.dataset.run;
            const { value: reason, isConfirmed } = await Swal.fire({
                title: `Grund für ${state === "playing" ? "Start" : state === "paused" ? "Pause" : "Stopp"}`,
                input: "textarea",
            ...
            try {
                const res = await window.LeadUI.net.postJSON(
                    `${window.LeadUI.APP.endpoints.progress}/${data.leadProductId}/${state}`,
                    { reason }
                );
                if (!res || res.success === false) throw new Error(res?.message || "Fehler");

                // Refresh UI (Silent refresh to update colors/icons)
    ...
    // --- LOGIC FOR NOTES IN LIST VIEW ---
    if (actBtn.dataset.act === "notes") {
        e.stopPropagation();
        
        let name = "Kunde";
        // Try to get name from our specific class first
        const linkEl = row.querySelector(".customer-link");
        if (linkEl) {
            name = linkEl.textContent.trim();
        } else {
            // Fallback: Clone cell to remove icon text
            const cell = row.querySelector("td:nth-child(2)");
            if(cell) {
                 const clone = cell.cloneNode(true);
                 const icons = clone.querySelector(".list-action-bar");
                 if(icons) icons.remove();
                 name = clone.textContent.trim();
            }
        }

        window.LeadUI.notes.openNotesDrawerFor(
          data.customerId,
          data.alternativeId,
          data.productId,
          `Notizen • ${name}`
  ...
  partials.fetchTicketsTab = async function (qsStr = "") {
    ...
    try {
      ...
      const total = totalNode ? Number(totalNode.getAttribute("data-ticket-total") || totalNode.getAttribute("data-total") || totalNode.dataset.ticketTotal || totalNode.dataset.total || 0) : 0;
      const badge = document.querySelector("#tabCountTicket");
      if (badge) badge.textContent = String(total);
    } catch (e) { console.error("Ticket partial load failed:", e); }
  };
  ...
  partials.fetchInvestmentTab = async function (qsStr = "") {
    ...
    try {
      ...
      const total = inner?.getAttribute("data-investment-total") || inner?.dataset.investmentTotal || 0;
      const badge = document.querySelector("#tabCountInvestment");
      if (badge) badge.textContent = String(total);
    } catch (e) { console.error("investment tab load failed", e); }
  };
  ...
  async function confirmStageChange(newStage, currentStage) {
    ...
    const step1 = await Swal.fire({ title: "Phase ändern?", text: `Von „${labelCur}“ zu „${labelNew}“ wechseln?`, icon: "question", showCancelButton: true, confirmButtonText: "Ja" });
    if (!step1.isConfirmed) return { ok: false };
    if (currentStage && isBackward(currentStage, newStage)) {
      const warn = await Swal.fire({ title: "Achtung – Rückwärtswechsel", html: "Bitte stornieren/prüfen Sie Details (Positionen, Termine, Aufträge) vor dem Rücksprung.", icon: "warning", showCancelButton: true, confirmButtonText: "Weiter" });
      if (!warn.isConfirmed) return { ok: false };
      const reasonPrompt = await Swal.fire({ title: "Grund erforderlich", input: "textarea", inputPlaceholder: "Grund für Rückwärtswechsel…", inputAttributes: { "aria-label": "Grund" }, inputValidator: (v) => !v?.trim() ? "Bitte Grund eingeben" : undefined, showCancelButton: true, confirmButtonText: "Speichern" });
      if (!reasonPrompt.isConfirmed) return { ok: false };
      return { ok: true, reasonHTML: (reasonPrompt.value || "").replace(/\n/g, "<br>"), backward: true };
    }
  ...
  function setTabCount(selector, n) { const el = document.querySelector(selector); if (el) el.textContent = String(Number(n || 0)); }
  function syncTabCountsFromListPayload(payload) {
    const total = payload?.pagination?.total || payload?.meta?.total || (Array.isArray(payload?.leads) ? payload.leads.length : 0);
    setTabCount("#tabCountList", total); setTabCount("#tabCountKanban", total);
  }
  function syncTabCountsFromKanban(leads) { if (Array.isArray(leads)) setTabCount("#tabCountKanban", leads.length); }
  function refreshArchiveAndJunk(qsStr) { partials.fetchJunkTab(qsStr); window.LeadUI?.partials?.fetchTicketsTab?.(qsStr); }

  function quillSwalOpts(id) {
    return { title: "Notiz (optional)", html: `<div id="${id}" style="height:170px;"></div>`, showCancelButton: true, confirmButtonText: "Speichern", focusConfirm: false, allowEnterKey: false, zIndex: 200000, didOpen: () => { if (window.Quill) { const q = new Quill(`#${id}`, { theme: "snow" }); window[id] = q; setTimeout(() => q.focus(), 0); } }, preConfirm: () => window[id]?.root?.innerHTML || "" };
  }
  async function promptOptionalNote(id) { const r = await Swal.fire(quillSwalOpts(id)); return r.isConfirmed ? r.value || "" : null; }

  async function applyStageChange({ customerId, alternativeId, productId, leadProductId, newStage, noteHTML }) {
    ...
    const payload = { stage: newStage, description: noteHTML || "", lead_product_id: Number(leadProductId || 0) || undefined };
  ...
  function refreshCardStatus(card, overrides = {}) {
    ...
    const ws = (overrides.work_status || card.dataset.runState || "playing").toLowerCase();
    ...
    if (old) old.outerHTML = buildStatusBlock({ stage: s, work_status: ws, latest_phase: overrides.latest_phase ?? card.dataset.latestPhase ?? "-", latest_activity: overrides.latest_activity ?? card.dataset.latestActivity ?? "-", updated_at: stamp, done_date: stamp });
    applyRunStateUI(card, ["playing", "paused", "stopped"].includes(ws) ? ws : "playing");
    featherRefreshSoon();
  }
  ...
  function moveOrRefreshKanbanCard({ newStage, cardFromDOM }) {
    ...
    if (!card) return;
    if (stageFilterExcludes(newStage)) { card.remove(); } else {
  ...
  async function onDrop(e) {
    ...
    try {
      ...
      await applyStageChange({ customerId: card.dataset.customerId, alternativeId: card.dataset.alternativeId, productId: card.dataset.productId, leadProductId: card.dataset.leadProductId, newStage, noteHTML: confirm.reasonHTML });
      moveOrRefreshKanbanCard({ newStage, cardFromDOM: card });
      enforceActionVisibility(card);
      silentRefreshBoth();
      Swal.fire("OK", "Phase aktualisiert.", "success");
    } catch (e) { Swal.fire("Fehler", e.message || "Serverfehler.", "error"); }
  }
  document.addEventListener("dragover", (e) => { const col = e.target.closest(".column"); if (col) e.preventDefault(); });
  document.addEventListener("drop", (e) => { const col = e.target.closest(".column"); if (!col) return; onDrop(e); });

  /* ========================================================================== */
  /* List rendering (WITH Live Feed own row under each row)                    */
  /* ========================================================================== */

  const interestIcons = { interest: { icon: "kaufinteresse.svg", label: "Kaufinteresse" }, intent: { icon: "kaufabsicht.svg", label: "Kaufabsicht" }, option: { icon: "kaufoption.svg", label: "Kaufoption" } };
  const servicesMap = { complete: "Komplett", montage: "Montage", product: "Produkt", plan: "Planung", maintenance: "Wartung", repair: "Reparatur", emergency: "Notdienst", others: "Sonstiges" };
  ...
  function employeeCellHTML(lead) {
    ...
    if (!office && !field) return "<small>&ndash;</small>";
    const chunks = [];
    if (office && (office.name || office.lastname)) chunks.push(`<div class="d-flex align-items-center mb-1"><img src="/images/employee/${office.image || ""}" width="30" height="30" class="rounded-circle mr-1" alt=""><div><div><strong>${office.lastname || ""}</strong> ${office.name || ""}</div><small class="text-muted">Innendienst</small></div></div>`);
    if (field && (field.name || field.lastname)) chunks.push(`<div class="d-flex align-items-center"><img src="/images/employee/${field.image || ""}" width="26" height="26" class="rounded-circle mr-1" alt=""><div><div><strong>${field.lastname || ""}</strong> ${field.name || ""}</div><small class="text-muted">Außendienst</small></div></div>`);
    return chunks.join("");
  }

  function listFeedHTML() {
    return `<div class="live-feed-bar list-live-feed card-live-feed" data-feed-root data-feed-count="0" style="display:none; margin-top:0.4rem;"><div class="live-feed-left"><div class="live-feed-icon"><i class="feather icon-zap"></i></div></div><div class="live-feed-body"><div class="live-feed-line d-none" data-feed-empty><span class="live-feed-title">Keine Aktivitäten</span><span class="live-feed-dot">•</span><span class="live-feed-text">Noch keine Termine oder Aufgaben.</span></div><div class="live-feed-line d-none" data-feed-line><span class="live-feed-title" data-feed-title>Aktivität</span><span class="live-feed-dot">•</span><span class="live-feed-text" data-feed-text>Details…</span></div><div class="live-feed-meta"><span class="live-feed-pill" data-feed-pill>Info</span><span class="live-feed-time"><i class="feather icon-clock mr-25"></i><span data-feed-time>–</span></span><span class="live-feed-counter" data-feed-counter></span></div></div><div class="live-feed-controls"><button type="button" class="live-feed-btn" title="Zurück" data-feed-prev><i class="feather icon-skip-back"></i></button><button type="button" class="live-feed-btn" title="Pause / Abspielen" data-feed-toggle><i class="feather icon-pause" data-feed-icon-pause></i><i class="feather icon-play d-none" data-feed-icon-play></i></button><button type="button" class="live-feed-btn" title="Weiter" data-feed-next><i class="feather icon-skip-forward"></i></button></div></div>`;
  }
  ...
  function buildRowHTML(lead) {
      ...
      const created = lead.created_at ? new Date(lead.created_at).toLocaleDateString("de-DE") : "-";
      const interest = interestIcons[lead.interest] || null;
      const translatedPhase = servicesMap[lead.phase_section_title] ?? servicesMap[lead.service] ?? null;
      ...
      const stageOptions = Object.entries(APP.stageNames)
        .map(([k, l]) => `<option value="${k}" ${s === k ? "selected" : ""}>${l}</option>`)
        .join("");
      ...
      return `
        <tr id="row-${lpId}"
          ...
          <td>
            <a href="/new_lead_profile/${cId}" class="customer-link">
              ${lead.customer_lastname ?? ""} ${lead.customer_name ?? ""}
            </a>

            <div class="list-action-bar">
              <button type="button" class="btn-list-icon play ${playColor}" data-run="playing" title="Start"><i class="feather icon-play"></i></button>
              <button type="button" class="btn-list-icon pause ${pauseColor}" data-run="paused" title="Pause"><i class="feather icon-pause"></i></button>
              <button type="button" class="btn-list-icon stop ${stopColor}" data-run="stopped" title="Stopp"><i class="feather icon-square"></i></button>
              ...
              <a href="/lead/process/history/${cId}/${aId}/${pId}" 
          ...
          <td>
            <div class="d-flex align-items-center gap-2 flex-wrap">
              ...
              ${lead.department_name ? `<span class="tooltip-trigger position-relative"><img src="/images/icons/abteilung.svg" style="width:30px" alt=""><span class="custom-tooltip">${lead.department_name}</span></span>` : ""}
              ${translatedPhase ? `<span class="tooltip-trigger position-relative"><img src="/images/icons/dienstleistung.svg" style="width:33px" alt=""><span class="custom-tooltip">${translatedPhase}</span></span>` : ""}
              ${interest ? `<span class="tooltip-trigger position-relative"><img src="/images/icons/${interest.icon}" style="width:20px" alt=""><span class="custom-tooltip">${interest.label}</span></span>` : ""}
            </div>
          </td>
          <td>${employeeCellHTML(lead)}</td>
          <td>
            <div><span class="badge bg-${tone} ${extra}">${txt}</span></div>
            ${lead.latest_phase || lead.latest_activity ? 
              `<div class="small mt-1 text-muted">
                ${lead.latest_phase ? `<i class="feather icon-box"></i> ${lead.latest_phase}<br>` : ''}
                ${lead.latest_activity ? `<i class="feather icon-check-circle"></i> ${lead.latest_activity}` : ''}
              </div>` : ""}
          </td>
          <td>
            <select class="form-control stage-select" data-id="${lpId}">
        ...
        <tr class="list-feed-row" data-customer-id="${cId}" data-alternative-id="${aId}" data-product-id="${pId}" data-lead-product-id="${lpId}">
          <td colspan="7">${listFeedHTML()}</td>
        </tr>
      `;
    }
  function runIdle(fn) {
  ...
  function normalizePaginationMeta(input) {
    ...
    const cp = Number(direct.current_page ?? direct.currentPage ?? direct.page ?? 1);
    const lp = Number(direct.last_page ?? direct.lastPage ?? (direct.total && direct.per_page ? Math.ceil(direct.total / direct.per_page) : 1));
  ...
  function renderPagination(metaLike) {
    ...
    const add = (p, l, dis = false, act = false) => {
      if (dis) html += `<li class="page-item disabled"><span class="page-link">${l}</span></li>`;
      else if (act) html += `<li class="page-item active"><span class="page-link">${l}</span></li>`;
      else html += `<li class="page-item"><a class="page-link" href="#" data-page="${p}">${l}</a></li>`;
    };
    add(current_page - 1, "«", current_page === 1);
    const win = 2; const st = Math.max(1, current_page - win); const en = Math.min(last_page, current_page + win);
    if (st > 1) { add(1, "1", false, current_page === 1); if (st > 2) html += '<li class="page-item disabled"><span class="page-link">…</span></li>'; }
    for (let p = st; p <= en; p++) { add(p, String(p), false, p === current_page); }
    if (en < last_page) { if (en < last_page - 1) html += '<li class="page-item disabled"><span class="page-link">…</span></li>'; add(last_page, String(last_page), false, current_page === last_page); }
    add(current_page + 1, "»", current_page === last_page);
    wrap.innerHTML = html + "</ul></nav>";
  }

  function syncSummary(data) {
    const setTxt = (sel, v) => { const el = qs(sel); if (el) el.textContent = String(v ?? ""); };
  ...
  function normalizeLead(raw) {
    const pick = (obj, ...keys) => { for (const k of keys) { const v = obj?.[k]; if (v !== undefined && v !== null && v !== "") return v; } return null; };
  ...
  function ensureLoadedMap() {
    ...
    else { if (!("kanban" in State.loaded)) State.loaded.kanban = false; if (!("list" in State.loaded)) State.loaded.list = false; }
  }
  ...
  function fetchKanbanView(qsStr) {
    ...
    return safeFetchJSON(`${APP.endpoints.kanbanSearch}${qsStr ? `?${qsStr}` : ""}`, { signal, retries: 0 })
      .then((payload) => {
        const arr = Array.isArray(payload?.leads) ? payload.leads : Array.isArray(payload?.data) ? payload.data : Array.isArray(payload) ? payload : [];
        State.lastKanbanData = arr.map(normalizeLead);
        if (!State.loaded.kanban) {
          renderKanbanIncremental(State.lastKanbanData, autoChunk(), () => {
            ensureLoadedMap(); State.loaded.kanban = true; syncTabCountsFromKanban(State.lastKanbanData);
          });
        } else {
          renderKanbanDiff(State.lastKanbanData); syncTabCountsFromKanban(State.lastKanbanData);
        }
      }).catch((e) => { if (e.name !== "AbortError") Swal.fire("Fehler", e.message, "error"); });
  }

  function fetchListView(qsStr) {
    const signal = cancel("list");
    const tbody = qs("#kanbanTableBody");
    if (tbody && !State.loaded.list) tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted">Lade Liste…</td></tr>';
    return safeFetchJSON(`${APP.endpoints.listSearch}${qsStr ? `?${qsStr}` : ""}`, { signal, retries: 0 })
      .then((payload) => {
        ensureLoadedMap(); State.loaded.list = true;
      ...
      }).catch((e) => { if (e.name !== "AbortError") { Swal.fire("Fehler", e.message, "error"); updateListView([], {}); renderPagination(null); } });
  }
  ...
  document.addEventListener("change", async (e) => {
    ...
    const prevIndex = [...sel.options].findIndex((o) => o.defaultSelected);
    const oldStage = prevIndex >= 0 ? canonicalStage(sel.options[prevIndex].value) : null;
    const customerId = row?.dataset.customerId || sel.dataset.customerId;
    const alternativeId = row?.dataset.alternativeId || sel.dataset.alternativeId;
    const productId = row?.dataset.productId || sel.dataset.productId;
    const leadProductId = sel.dataset.id || row?.id?.split("-")[1];

    try {
      const confirm = await confirmStageChange(newStage, oldStage);
      if (!confirm.ok) { sel.selectedIndex = prevIndex; return; }
      await applyStageChange({ customerId, alternativeId, productId, leadProductId, newStage, noteHTML: confirm.reasonHTML });
      const card = document.getElementById(`card-${leadProductId}`) || document.querySelector(`.card[data-lead-product-id="${leadProductId}"]`);
      if (card) { moveOrRefreshKanbanCard({ newStage, cardFromDOM: card }); enforceActionVisibility(card); }
      ...
      silentRefreshBoth();
      Swal.fire("OK", "Phase aktualisiert.", "success");
    } catch (err) { sel.selectedIndex = prevIndex; Swal.fire("Fehler", err.message || "Serverfehler.", "error"); }
  });
  ...
  /* ========================================================================== */
  document.addEventListener("click", (e) => {
    const actBtn = e.target.closest(".card .card-actions [data-act],[data-run]");
    if (!actBtn) return;
    const card = actBtn.closest(".card");
    if (!card) return;
    if (actBtn.dataset.run) return (async () => {
      const state = actBtn.dataset.run;
      const { value: reason, isConfirmed } = await Swal.fire({ title: `Grund für ${state === "playing" ? "Start" : state === "paused" ? "Pause" : "Stopp"}`, input: "textarea", showCancelButton: true, confirmButtonText: "Speichern", inputValidator: (v) => !v?.trim() ? "Bitte Grund eingeben" : undefined });
      if (!isConfirmed) return;
      const prev = card.dataset.runState || "playing";
      applyRunStateUI(card, state);
      try {
        const res = await postJSON(`${APP.endpoints.progress}/${card.dataset.leadProductId}/${state}`, { reason });
        if (!res || res.success === false) throw new Error(res?.message || "Fehler");
        refreshCardStatus(card, { work_status: state, updated_at: new Date().toISOString() });
        silentRefreshBoth();
      } catch (err) { applyRunStateUI(card, prev); Swal.fire("Fehler", err.message || "Speichern fehlgeschlagen.", "error"); }
    })();

    const act = actBtn.dataset.act;
    if (act === "profile") return window.location.assign(`/new_lead_profile/${card.dataset.customerId}`);
    if (act === "edit") return Swal.fire({ title: "Lead bearbeiten?", icon: "question", showCancelButton: true }).then((r) => { if (r.isConfirmed) window.location.assign(`/new_lead_edit/${card.dataset.customerId}/${card.dataset.alternativeId}`); });
    if (act === "notes") return notes.openNotesDrawerFor(card.dataset.customerId, card.dataset.alternativeId, card.dataset.productId, card.querySelector(".card-header strong")?.textContent?.trim());
    if (act === "delete") return (async () => {
      if (stageRank(canonicalStage(card.dataset.stage)) >= stageRank("deal")) return Swal.fire("Gesperrt", "Leads ab „Auftrag“ können nicht in Junk verschoben werden.", "info");
      const ok = await Swal.fire({ title: "In Junk verschieben?", text: "Dieser Lead wird in den Junk-Bereich verschoben (nicht gelöscht).", icon: "warning", showCancelButton: true, confirmButtonText: "Ja, verschieben" });
      if (!ok.isConfirmed) return;
      const note = await Swal.fire(quillSwalOpts("quillJunk"));
      try {
        const url = `${APP.endpoints.changeStage}/${encodeURIComponent(card.dataset.customerId)}/${encodeURIComponent(card.dataset.alternativeId)}/${encodeURIComponent(card.dataset.productId)}`;
        const res = await postJSON(url, { stage: "junk", description: note.isConfirmed ? note.value || "" : "", lead_product_id: Number(card.dataset.leadProductId || 0) || undefined });
        if (!res?.success) throw new Error(res?.message || "Fehler");
        card.remove(); updateCounts(); silentRefreshBoth(); Swal.fire("Verschoben", "Lead liegt jetzt im Junk.", "success");
      } catch (err) { Swal.fire("Fehler", err.message || "Verschieben fehlgeschlagen.", "error"); }
    })();
    if (act === "archive") return (async () => {
      ...
      try {
        const url = `${APP.endpoints.changeStage}/${encodeURIComponent(card.dataset.customerId)}/${encodeURIComponent(card.dataset.alternativeId)}/${encodeURIComponent(card.dataset.productId)}`;
        const data = await postJSON(url, { stage: "archive", description: note.isConfirmed ? note.value : "", lead_product_id: Number(card.dataset.leadProductId || 0) || undefined });
        if (!data.success) throw new Error(data.message || "Fehler");
        card.remove(); updateCounts(); silentRefreshBoth(); Swal.fire("Archiviert", "Lead verschoben.", "success");
      } catch (err) { Swal.fire("Fehler", err.message || "Archivieren fehlgeschlagen.", "error"); }
    })();
  });

  function addPage(qsStr, page) { const p = new URLSearchParams(qsStr || ""); p.set("page", String(page)); return p.toString(); }
  function isKanbanActive() { return qs("#home")?.classList.contains("active"); }
  ...
  document.addEventListener("click", (e) => {
    ...
    State.page = 1; State.filtersQS = filters.buildFilterQS(); saveToLocal(); syncURL();
  ...
  document.addEventListener("click", (e) => {
    ...
    State.page = p; saveToLocal(); syncURL();
    fetchListView(addPage(State.filtersQS, State.page));
  });

  if (window.jQuery) {
    jQuery('a[data-toggle="tab"][href="#home"]').on("shown.bs.tab", function () { ensureColumns(); renderKanbanDiff(State.lastKanbanData); featherRefreshSoon(); enforceActionVisibility(); });
    jQuery('a[data-toggle="tab"][href="#junk"]').on("shown.bs.tab", function () { partials.fetchJunkTab(State.filtersQS); });
    jQuery('a[data-toggle="tab"][href="#investment"]').on("shown.bs.tab", function () { partials.fetchInvestmentTab(State.filtersQS); });
  }

  document.addEventListener("shown.bs.tab", (e) => {
    const trg = e.target?.getAttribute("href") || "";
    if (trg === "#ticket") { const qsStr = window.LeadUI.filters.buildFilterQS(); window.LeadUI.partials.fetchTicketsTab(qsStr); }
  });

  function setSummaryActive(id) { qsa(".summary-card").forEach((c) => c.classList.remove("active")); if (id) qs("#" + id)?.classList.add("active"); }
  function applyStatusGroup(g, cardId) {
    State.statusGroup = g; State.page = 1; State.filtersQS = filters.buildFilterQS(); saveToLocal(); syncURL();
    const qsStrWithPage = addPage(State.filtersQS, State.page);
    fetchListView(qsStrWithPage); fetchKanbanView(State.filtersQS); refreshArchiveAndJunk(State.filtersQS); setSummaryActive(cardId || null); filters.updateFilterBadges();
  }
  ...
  qs("#btnApplyFilters")?.addEventListener("click", () => {
    State.page = 1; State.filtersQS = filters.buildFilterQS(); window.LeadUI.partials.fetchTicketsTab(State.filtersQS); window.LeadUI.partials.fetchInvestmentTab(State.filtersQS); State.lastAppliedQS = State.filtersQS; saveToLocal(); syncURL();
    const withPage = addPage(State.filtersQS, State.page);
    fetchListView(withPage); fetchKanbanView(State.filtersQS); refreshArchiveAndJunk(State.filtersQS); closeOverlays();
  });
  qs("#btnClearFilters")?.addEventListener("click", () => {
    const form = qs("#kanbanFilterForm"); if (!form) return;
    form.reset(); if (window.jQuery) jQuery(form).find(".select2").val(null).trigger("change");
    State.statusGroup = null; setSummaryActive(null);
    State.page = 1; State.filtersQS = filters.buildFilterQS(); saveToLocal(); syncURL(); filters.updateFilterBadges();
    const withPage = addPage(State.filtersQS, State.page);
    fetchListView(withPage); fetchKanbanView(State.filtersQS); refreshArchiveAndJunk(State.filtersQS); window.LeadUI.partials.fetchInvestmentTab(State.filtersQS); window.LeadUI.partials.fetchTicketsTab(State.filtersQS);
  });
  ...
  document.addEventListener("keydown", (e) => {
    if (e.ctrlKey && e.key.toLowerCase() === "f") { e.preventDefault(); qs("#btnOpenDrawer")?.click(); }
    if (e.key === "Escape") closeOverlays();
  });

  function silentRefreshBoth() {
    const qsStr = State.filtersQS; fetchListView(addPage(qsStr, State.page)); fetchKanbanView(qsStr); window.LeadUI.partials.fetchTicketsTab(qsStr);
  }
  window.LeadUI.silentRefreshBoth = silentRefreshBoth;

  document.addEventListener("DOMContentLoaded", () => {
    featherRefreshSoon(); filters.initSelect2(); filters.updateFilterBadges();
    initFromURL(); if (!location.search) restoreFromLocal();
    State.filtersQS = filters.buildFilterQS(); saveToLocal(); syncURL();
    ensureLoadedMap(); State.loaded.kanban = false; State.loaded.list = false;
    fetchListView(addPage(State.filtersQS, State.page)); fetchKanbanView(State.filtersQS);
  });
})();
</script>
<script>
...
(function () {
  ...
  async function onDrop(e) {
    ...
    try {
      ...
      await applyStageChange({
        ...
        leadProductId: card.dataset.leadProductId,
        newStage,
 ...
 function buildRowHTML(lead) {
    ...
    // Determine active run state for coloring
    const ws = (lead.work_status || 'playing').toLowerCase();
    const playColor = ws === 'playing' ? 'text-success' : ''; 
    const pauseColor = ws === 'paused' ? 'text-warning' : '';
    const stopColor = ws === 'stopped' ? 'text-danger' : '';
    ...
    return `
      <tr id="row-${lead.lead_product_id}"
        ...
        <td>
          <a href="/new_lead_profile/${lead.customer_id}" class="customer-link">
             ${lead.customer_lastname ?? ""} ${lead.customer_name ?? ""}
          </a>
  ...
  /* ========================================================================== */
  /* Sorting / paging (LIST) - UNIFIED DROPDOWN VERSION                         */
  /* ========================================================================== */

  // 🟢 RESTORED: This function is required by silentRefreshBoth()
  ...
  // 1. Handle Unified Dropdown Change
  document.addEventListener("change", (e) => {
      if (e.target && e.target.id === "listSortSelect") {
          
          const rawValue = e.target.value; // e.g. "created_at|desc"
  ...
  // Helper: Perform the fetch and sync URL
  function refreshListWithSort() {
      State.page = 1; // Reset to page 1 on sort change
      
      // Save to local storage & URL
      State.filtersQS = window.LeadUI.filters.buildFilterQS();
      window.LeadUI.utils.saveToLocal();
      window.LeadUI.utils.syncURL();

      // Trigger Fetch using the global refresh function
      window.LeadUI.silentRefreshBoth(); 
  }

  // 2. Initialize Dropdown on Load (Restore State)
  document.addEventListener("DOMContentLoaded", () => {
      const sel = document.getElementById("listSortSelect");
      if (sel && State.sort.key && State.sort.dir) {
          // Construct the value to look for, e.g. "city|asc"
          const combinedVal = `${State.sort.key}|${State.sort.dir}`;
          
          // Check if option exists before setting it
          const optionExists = [...sel.options].some(o => o.value === combinedVal);
          if (optionExists) {
              sel.value = combinedVal;
          }
      }
  });
  /* ========================================================================== */
