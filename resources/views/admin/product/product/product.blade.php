...
<style>
    ...
    .products-filters-shell{ margin-bottom:1rem; }
    ...
    #product-pagination{ margin-top:.75rem; }
    ...
    .product-menu-float.drop-up{ transform-origin:bottom right; }
    ...
    .product-cart-backdrop { z-index: 1090; }
    .product-cart-drawer { z-index: 1095; }
    .swal2-container{
    ...
    .product-cart-btn-primary:hover{ background:#7baa18; }
    ...
    .product-cart-btn-soft:hover{ background:#f9fafb; }
                ...
                <div class="products-header">
                    ...
                    <div class="products-header-actions">
                       ...
                       <div class="view-toggle-group">
                        ...
                        <button type="button" id="product-cart-fab" class="btn btn-outline-success position-relative">
    ...
    <div class="product-cart-group">
        <div class="d-flex align-items-center justify-content-between mb-1">
            ...
            <div class="d-flex" style="gap:.5rem;">
                <button type="button" id="ms-cart-back-btn" class="btn btn-sm btn-outline-secondary" style="display:none;">
                        ...
                        <div class="product-image-right">
                            <div class="product-image-form-card">
                                ...
                                <div class="product-image-form-actions">
                                    ...
                                    <button type="button" class="btn btn-outline-secondary" id="productImageModalCancel">
                                        Abbrechen
                                    </button>
                                </div>
                            </div>
...
(function ($) {
        ...
        if (!list.length) {
            ...
            list.forEach(function (dist) {
                ...
                const priceHtml = dist.display_price !== null && dist.display_price !== undefined
                    ? Number(dist.display_price).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' €'
                    : '–';
                ...
                html += `
                    <div class="product-modern-distributor-item">
                        <div class="product-modern-distributor-top">
                            ...
                            <div class="product-modern-distributor-name">${escapeHtml(dist.distributor_name || '')}</div>
                        </div>
   ...
   function saveProductImage() {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                ...
                console.log('UPLOAD ERROR RESPONSE:', xhr.responseJSON);
                console.log('UPLOAD ERROR TEXT:', xhr.responseText);
    ...
    function setCartStep(step) {
        ...
        if ($el.cartStepProductsWrap().length) $el.cartStepProductsWrap().toggle(isProducts);
        if ($el.cartStepConfigWrap().length) $el.cartStepConfigWrap().toggle(!isProducts);

        if ($el.cartNextBtn().length) $el.cartNextBtn().toggle(isProducts);
        if ($el.cartBackBtn().length) $el.cartBackBtn().toggle(!isProducts);

        if ($el.cartStepProductsBadge().length) {
            $el.cartStepProductsBadge()
                .toggleClass('badge-success', isProducts)
                .toggleClass('badge-light', !isProducts);
        }

        if ($el.cartStepConfigBadge().length) {
   ...
   function loadProducts(pageUrl = null, highlightId = null) {
    ...
    // remember current list state/url
    if (pageUrl) {
    ...
    function applyBulkAction() {
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                error: (xhr) => {
                    const msg = xhr.responseJSON?.message || 'Aktion fehlgeschlagen.';
                    toastError(msg);
                }
            });
        });
            ...
            success: (res) => {
                ...
                if (!lists.length) {
                    ...
                    lists.forEach((row) => {
                        ...
                        $select.append(
                            $('<option>', { value: row.id, text: label }).attr('data-attached', row.is_attached ? 1 : 0)
                        );
                    });
                }
    ...
    function renderCartSummary() {
        ...
        if ($el.cartCountTotal().length) $el.cartCountTotal().text(count);
        ...
        if ($el.cartSubTotalConfig().length) $el.cartSubTotalConfig().text(moneyFormat(cartState.cart?.sub_total || 0));
        if ($el.cartTotalConfig().length) $el.cartTotalConfig().text(moneyFormat(cartState.cart?.total || 0));
                    ...
                    <div class="product-cart-item-left w-100">
                        <div class="product-cart-product">
                            ...
                            <div class="product-cart-product-content">
                                ...
                                ${item.description
                                    ? `<div class="product-cart-item-desc">${escapeHtml(item.description)}</div>`
                                    : ''
                                }
    ...
    function renderCartSections() {
        ...
        const html = cartState.sections
            ...
            .map(section => {
                ...
                return `
                    <div class="product-cart-section" data-section-id="${section.id}">
                        ...
                        <div class="product-cart-items">
                            <div class="product-cart-root-dropzone" data-root-dropzone="1" data-section-id="${section.id}">
    ...
    function addBulkProductsToCart(productIds) {
        if (!Array.isArray(productIds) || !productIds.length) {
            return toastInfo('Keine Produkte ausgewählt.');
        }
...
function convertCartToMasterSet() {
                        ...
                        Swal.fire({
                            ...
                            html: `
                                <div style="font-size:14px; line-height:1.6;">
                                    ...
                                    <div style="margin-top:6px; color:#64748b;">ID: ${res.master_set_id || '-'}</div>
                                </div>
                            `,
                            ...
                            if ($el.cartId().length) $el.cartId().val('');
                            if ($el.cartName().length) $el.cartName().val('');
                            if ($el.cartDescription().length) $el.cartDescription().val('');
                            if ($el.cartArticleGroup().length) $el.cartArticleGroup().val('').trigger('change');
                            if ($el.cartMasterSet().length) $el.cartMasterSet().val('').trigger('change');
                            if ($el.cartSectionName().length) $el.cartSectionName().val('');
                            if ($el.cartTargetSection().length) {
    ...
    function bindEvents() {
        ...
        $('#filter_article_group, #filter_distributor, #filter_status, #filter_category, #filter_sort, #filter_per_page')
            .on('change', function () {
                loadProducts();
            });
        ...
        $el.resetBtn().on('click', function () {
            ...
            loadProductImages(productId, imageUrl, productName);
        });

        $('#productImageModalClose, #productImageModalCancel, #productImageModalBackdrop').on('click', function () {
            closeProductImageModal();
        });
        ...
        if ($el.cartNextBtn().length) {
            $el.cartNextBtn().on('click', function () {
                goToConfigStep();
            });
        }

        if ($el.cartBackBtn().length) {
            $el.cartBackBtn().on('click', function () {
                goToProductsStep();
            });
        }
