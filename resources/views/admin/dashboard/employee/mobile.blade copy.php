...
@section('script')
          ...
          async function deleteShortcut(id) {
            ...
            async function loadDepartmentOptions() {
                ...
                try {
                    ...
                    if (departmentDashboardState.currentDepartmentId) {
                        select.value = departmentDashboardState.currentDepartmentId;
                    }
                } catch (error) {
                    console.error('Department options failed:', error);
                    select.innerHTML = '<option value="">Keine Abteilung gefunden</option>';
                    showToast?.('Abteilungen konnten nicht geladen werden');
                }
            }
            ...
            async function loadDepartmentOverview() {
                const routes = window.DASHBOARD_DEPARTMENT_ROUTES || {};
                const select = document.getElementById('departmentSelect');
                const info = document.getElementById('deptInfoNote');

                if (!routes.overview) {
                    showToast?.('Department Overview Route fehlt');
                    return;
                }

                const departmentId = select?.value || departmentDashboardState.currentDepartmentId;
                const from = document.getElementById('departmentFromDate')?.value || '';
                const to = document.getElementById('departmentToDate')?.value || '';
                ...
                if (departmentDashboardState.loading) {
                    return;
                }
                ...
                try {
                  ...
                  setTimeout(() => {
                    ...
                    showToast?.(error?.message || 'Abteilungsdaten konnten nicht geladen werden');
            ...
            function renderDepartmentOverview(result) {
                ...
                setDeptText(
                    'deptSubtitle',
                    subtitle || 'Team, Aufgaben, Termine, Leads, Angebote, Aufträge und Rechnungen'
                );

                setDeptText('deptTeamMembers', kpis.team_members ?? 0);
                setDeptText('deptLeads', kpis.leads ?? 0);
                setDeptText('deptOffers', kpis.offers ?? 0);
                setDeptText('deptDeals', kpis.deals ?? 0);
                setDeptText('deptTickets', kpis.tickets ?? 0);
                setDeptText('deptAppointments', kpis.appointments ?? 0);
                setDeptText('deptTasks', kpis.tasks ?? 0);
                setDeptText('deptInvoiceTotal', formatDeptMoney(kpis.invoice_total || 0));
            }

            function renderDepartmentTeam(team) {
                const list = document.getElementById('deptTeamList');
                ...
                if (!team.length) {
                    list.innerHTML = '<div class="empty-state">Keine Mitarbeiter gefunden.</div>';
                    return;
                }
                ...
                list.innerHTML = team.map(emp => `
                    <div class="dept-team-row">
                        <img class="dept-avatar"
                             src="${escapeHtml(emp.image_url || '/images/gender/male.png')}"
                             alt="${escapeHtml(emp.name || 'Mitarbeiter')}">

                        <div>
                            <div class="dept-row-title">
                                ${escapeHtml(emp.name || 'Mitarbeiter')}
                            </div>

                            <div class="dept-row-meta">
                                ${escapeHtml(emp.position || 'Ohne Position')}
                                ${emp.qualification ? ' · ' + escapeHtml(emp.qualification) : ''}
                                · Office ${deptSafeNumber(emp.office_percent)}%
                                · Montage ${deptSafeNumber(emp.montage_percent)}%
                            </div>
                        </div>

                        <span class="dept-mini-pill">
                            ${emp.main ? 'Haupt' : `${deptSafeNumber(emp.percent)}%`}
                        </span>
                    </div>
                `).join('');
            }
            ...
            function deptFlowPersonList(item) {
                ...
                [
                    ...
                    if (person && (person.name || person.id || person.image_url)) {
                        list.push(person);
                    }
                });
                ...
                return list.filter(person => {
                    const key = person.id ? `id:${person.id}` : `name:${person.name || person.image_url}`;
            ...
            function deptFlowMainPerson(item) {
                return deptFlowPersonList(item)[0] || null;
            }
            ...
            function deptFlowAmount(item, type) {
                if (type === 'invoices') {
                    if (Number(item.open || 0) > 0) {
                        return deptFlowMoney(item.open);
                    }

                    if (Number(item.total || 0) > 0) {
                        return deptFlowMoney(item.total);
                    }
                }

                if (type === 'offers' && Number(item.total_gross || 0) > 0) {
                    return deptFlowMoney(item.total_gross);
                }

                if (type === 'deals' && Number(item.price || 0) > 0) {
                    return deptFlowMoney(item.price);
                }
            ...
            function deptFlowChip(icon, value, extraClass = '') {
                ...
                return `
                    <span class="dept-flow-chip ${escapeHtml(extraClass)}" title="${escapeHtml(clean)}">
                        <i data-lucide="${escapeHtml(icon)}"></i>
            ...
            function deptFlowPeopleHtml(people) {
                ...
                const avatars = people.slice(0, 3).map(person => {
                    const image = person.image_url || '/images/gender/male.png';
                ...
                return `
                    <span class="dept-flow-people">
                        ${avatars}
                        ${names ? `<span class="dept-flow-people-name">${escapeHtml(names)}</span>` : ''}
                    </span>
                `;
            }

            function renderDepartmentRecent() {
                const list = document.getElementById('deptRecentList');
                const data = window.departmentDashboardState?.data;
                const type = window.departmentDashboardState?.recentType || 'leads';
                ...
                list.innerHTML = items.map(item => {
                    ...
                    const mainPerson = deptFlowMainPerson(item);
                    const avatar = mainPerson?.image_url || '/images/gender/male.png';
                        ...
                        <div class="dept-flow-card">
                            <div class="dept-flow-avatar-box">
                                <img class="dept-flow-avatar"
                                     ...
                                     alt="${escapeHtml(mainPerson?.name || typeLabel)}">

                                <span class="dept-flow-type-icon">
                                    <i data-lucide="${escapeHtml(typeIcon)}"></i>
                                </span>
                            </div>

                            <div class="dept-flow-body">
                                <div class="dept-flow-head">
                                    <span class="dept-flow-type-pill">${escapeHtml(typeLabel)}</span>
                                    <span class="dept-flow-customer" title="${escapeHtml(customerName)}">
                                        ${escapeHtml(customerName)}
                                    </span>
                                </div>

                                <div class="dept-flow-line">
                                    ${deptFlowChip('hash', customerNo, 'customer-no')}
                                    ${deptFlowChip('box', product, 'product')}
                                    ${deptFlowChip('radio', source, 'source')}
                                </div>

                                <div class="dept-flow-line">
                                    ${deptFlowChip('map-pin', address, 'address')}
            ...
            function renderDepartmentChanges(changes) {
                const list = document.getElementById('deptChangeList');
                ...
                if (!changes.length) {
                    list.innerHTML = '<div class="empty-state">Keine Änderungen gefunden.</div>';
                    return;
                }
                ...
                list.innerHTML = changes.map(change => `
                    <div class="dept-change-row">
                        <span class="dept-kpi-icon blue" style="width:38px;height:38px;">
                            <i data-lucide="${historyIcon(change.event_type)}"></i>
                        </span>

                        <div>
                            <div class="dept-row-title">
                                ${escapeHtml(historyLabel(change.event_type))} · ${escapeHtml(change.model_type || '')}
                            </div>
            ...
            function renderDepartmentCharts(charts) {
                ...
                renderDepartmentProjectPie(charts.project_status || {});
                renderDepartmentRevenueBar(charts.monthly_revenue || {});
            ...
            function renderDepartmentProjectPie(projectStatus) {
                const labels = projectStatus.labels || ['Auftrag', 'Lead', 'Montage', 'Angebot'];
                const values = projectStatus.values || [0, 0, 0, 0];

                const total = values.reduce((sum, value) => sum + deptSafeNumber(value), 0);

                const totalEl = document.getElementById('deptPieTotal');
                const periodEl = document.getElementById('deptPiePeriod');

                if (totalEl) {
                    totalEl.textContent = `${total} Vorgänge`;
                }
            ...
            function renderDepartmentRevenueBar(monthlyRevenue) {
                const labels = monthlyRevenue.labels || [];
                const values = monthlyRevenue.values || [];

                const total = deptSafeNumber(
                    monthlyRevenue.total || values.reduce((sum, value) => sum + deptSafeNumber(value), 0)
                );

                const totalEl = document.getElementById('deptBarTotal');
                const periodEl = document.getElementById('deptBarPeriod');
            ...
            function renderDepartmentHistory(changes) {
                const list = document.getElementById('deptHistoryList');
                ...
                list.innerHTML = changes.map(change => {
                    const eventType = change.event_type || 'updated';
                    const modelType = change.model_type || 'Eintrag';
                    const userName = change.user_name || 'Unbekannt';
                    const date = change.date || '';
                    const modelId = change.model_id || '';

                    return `
                        <div class="dept-history-row">
                            <span class="dept-history-icon">
                                <i data-lucide="${historyIcon(eventType)}"></i>
                            </span>

                            <div>
                                <div class="dept-history-title">
                                    ${escapeHtml(historyLabel(eventType))} · ${escapeHtml(modelType)}
                                </div>

                                <div class="dept-history-meta">
                                    ${escapeHtml(userName)} · ${escapeHtml(date)}
                                </div>
                            </div>

                            <span class="dept-history-pill">
                                #${escapeHtml(modelId)}
                            </span>
                        </div>
                    `;
            ...
            function historyIcon(type) {
                ...
                if (normalized.includes('created')) return 'plus-circle';
                if (normalized.includes('deleted')) return 'trash-2';
                if (normalized.includes('updated')) return 'pencil';
                if (normalized.includes('status')) return 'refresh-cw';
            ...
            function historyLabel(type) {
                ...
                if (normalized.includes('created')) return 'Erstellt';
                if (normalized.includes('deleted')) return 'Gelöscht';
                if (normalized.includes('updated')) return 'Aktualisiert';
                if (normalized.includes('status')) return 'Status geändert';

                return type || 'Änderung';
            }
              ...
              function renderDeptPieChart(canvasId, labels, values, chartKey) {
                  ...
                  if (!canvas || !window.Chart) {
                    console.warn('Chart canvas missing:', canvasId);
                    return;
                  }

                  const wrapper = canvas.closest('.chart-wrap');

                  if (wrapper && wrapper.offsetHeight < 80) {
                  ...
                  if (!ctx) {
                    console.warn('Chart context missing:', canvasId);
                    return;
                  }
                  ...
                  const safeValues = Array.isArray(values) && values.some(v => Number(v) > 0)
                    ? values
                    : [1];
              ...
              function renderDeptBarChart(canvasId, labels, datasets, chartKey) {
                  ...
                  const safeDatasets = Array.isArray(datasets) && datasets.length
                    ? datasets.map(dataset => ({
                      ...dataset,
                      data: Array.isArray(dataset.data) && dataset.data.length
                        ? dataset.data
                        : [0]
                    }))
                    : [
            ...
            async function initDepartmentDashboard() {
                ...
                const fromInput = document.getElementById('departmentFromDate');
                const toInput = document.getElementById('departmentToDate');
                ...
                if (!departmentDashboardState.bound) {
                    document.getElementById('departmentSelect')?.addEventListener('change', loadDepartmentOverview);
                    document.getElementById('departmentFromDate')?.addEventListener('change', loadDepartmentOverview);
                    document.getElementById('departmentToDate')?.addEventListener('change', loadDepartmentOverview);
                    document.getElementById('refreshDepartmentDashboardBtn')?.addEventListener('click', loadDepartmentOverview);

                    document.addEventListener('click', function (event) {
                        const tab = event.target.closest('#deptRecentTabs button');

                        if (!tab) {
                            return;
                        }

                        document.querySelectorAll('#deptRecentTabs button').forEach(item => {
                            item.classList.remove('active');
                        });
