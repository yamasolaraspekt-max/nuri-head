...
<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
  ...
  <style>
    ...
    * {
    ...
    .quick-menu-wrapper:hover .btn-quick-menu {
      background: #f8fafc;
      color: var(--primary);
      border-color: #f1f5f9;
    }
    ...
    .pill-blue {
      background: #f0f7ff;
      border: 1px solid rgba(192, 216, 234, 0.5);
      color: var(--text-main);
    }
    ...
    .pill-green {
      background: rgba(147, 194, 28, 0.1);
      border: 1px solid rgba(147, 194, 28, 0.2);
      color: var(--text-main);
    }
    ...
    .labor-btn-avatar:hover {
      transform: scale(1.1);
      border-color: var(--accent);
    }

    .labor-btn-avatar img {
      width: 100%;
      height: 100%;
      object-fit: cover;
    }
    ...
    .wiz-section-btn:hover {
      background: #f0f7ff;
      color: var(--primary);
    }

    .wiz-section-btn.active {
      background: var(--primary);
      color: white;
    }
    ...
    .card-group {
      ...
      overflow: hidden;
      /* Keeps the button from spilling out if rounded */
    }
    ...
    .phase-card.is-collapsed .phase-footer {
      display: none !important;
    }

    /* Optional: make header look tighter when collapsed */
    .phase-card.is-collapsed .phase-header {
      border-bottom: 0;
    }
  ...
  <style>
    ...
    .form-control:focus {
      border-color: var(--primary);
      box-shadow: 0 0 0 3px rgba(116, 178, 212, .15);
    }
...
@section('content')
  ...
  <main id="app-container" class="containers">
    <!-- Dynamic Content -->
  </main>
    ...
    <div class="modal-window fade-in" style="max-width: 400px; height: auto; overflow: visible;">
      ...
      <div style="padding: 1.5rem;">
        <label id="prompt-label"
          style="display:block; margin-bottom:0.5rem; font-weight:700; color:#64748b; font-size:0.875rem;">Name</label>
        <input type="text" id="prompt-input" class="search-input"
          style="width:100%; border:2px solid var(--primary-light);" autocomplete="off">
    ...
    <div class="modal-window pdf-preview-window fade-in">
      <div class="modal-header">
        ...
        <div style="display:flex; gap:1rem;">
          <button onclick="ui.downloadPDF()" class="btn btn-primary"><i class="fas fa-download"></i> DOWNLOAD
            PDF</button>
          <button onclick="document.getElementById('pdf-modal-container').classList.add('hidden')"
            class="btn btn-icon"><i class="fas fa-times"></i></button>
        </div>
      </div>
      <div class="modal-body custom-scrollbar"
        style="background:#e2e8f0; padding: 2rem; display:flex; justify-content:center;">
        <!-- The Report Paper -->
      ...
      <div style="padding: 1.5rem; background: rgba(248, 250, 252, 0.3);">
        ...
        <div style="display:flex; gap:.75rem; align-items:center;">
          <input id="link-input" type="text" class="search-input"
            style="width:100%; border:2px solid var(--primary-light);" placeholder="https://..." autocomplete="off">
  ...
  <div id="taskWizardModal" class="twiz-backdrop d-none">
    <div class="twiz-modal">
      ...
      <div class="twiz-steps">
        ...
        <div class="twiz-step" data-step="2">2) Lead-Phasen</div>
      ...
      <div class="twiz-body">
        ...
        <div class="twiz-pane" data-pane="1">
          ...
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <button type="button" class="btn btn-sm btn-outline-primary" onclick="TaskWizard.addActivityRow()">+
        ...
        <div class="twiz-pane d-none" data-pane="2">
          <label>Lead-Phase(n) auswählen *</label>
          <div class="twiz-hint">Aktive Lead-Phasen aus lead_stages werden angezeigt.</div>

          <div id="tw_stages" class="twiz-grid">
            @php
// ✅ Lead stages only: this view no longer reads from the legacy `stages` table.
$leadStages = \App\Models\LeadStage::query()
...
$secLabels = [
  ...
  'product' => 'Verkauf',
        ...
        <div class="twiz-pane d-none" data-pane="4">
          ...
          <input class="form-control mb-2" id="tw_product_search" placeholder="Produkte suchen..."
            oninput="TaskWizard.filterProducts(this.value)" />
      ...
      <div style="padding:1.25rem;">
        ...
        <div style="display:flex; justify-content:flex-end; gap:.5rem; margin-top:1rem;">
          <button type="button" class="btn btn-outline-secondary"
            onclick="wizard.closeActivityModal()">Abbrechen</button>
  ...
  <div id="desc-modal" class="modal-backdrop hidden" style="z-index: 360;">
    <div class="modal-window fade-in" style="max-width: 980px; height: 90vh;">
      ...
      <div
        style="padding: 1rem; border-bottom:1px solid var(--border-color); display:flex; gap:1rem; align-items:center;">
        <div style="flex:1;">
          <label
            style="display:block; font-weight:900; font-size:12px; color:#64748b; margin-bottom:6px;">Kontext</label>
      ...
      <div class="modal-body" style="display:grid; grid-template-columns: 320px 1fr; gap:1rem; height:100%;">
        <!-- left: variants list -->
        <div
          style="background:#fff; border:1px solid var(--border-color); border-radius:16px; padding:12px; overflow:auto;">
        ...
        <!-- right: quill editor -->
        <div
          style="background:#fff; border:1px solid var(--border-color); border-radius:16px; overflow:hidden; display:flex; flex-direction:column;">
          <div style="padding:10px; border-bottom:1px solid #f1f5f9; display:flex; gap:.75rem; align-items:center;">
            <input id="desc-variant-title" class="form-control" placeholder="Titel (z.B. Standard / Kurz / Technisch)"
              style="flex:1;">
  ...
  <div id="task-qual-modal" class="modal-backdrop hidden" style="z-index: 400;">
    <div class="modal-window fade-in"
      style="max-width: 500px; height: auto; max-height: 85vh; display:flex; flex-direction:column;">
      <div class="modal-header">
      ...
      <div class="modal-search">
        <div class="search-wrapper">
          <i class="fas fa-search search-icon"></i>
          <input type="text" id="tq-search" class="search-input" style="width:100%"
            placeholder="Qualifikation suchen..." oninput="ui.filterTaskQuals(this.value)">
        </div>
      </div>
      ...
      <div id="tq-footer" class="hidden"
        style="padding:1.5rem; border-top:1px solid var(--border-color); background:white;">
        <div style="margin-bottom:1rem; font-weight:900; color:var(--text-main);" id="tq-selected-name"></div>

        <div style="display:flex; gap:1rem;">
          <div style="flex:1;">
            <label
              style="font-size:0.75rem; font-weight:800; color:#94a3b8; display:block; margin-bottom:4px;">Stunden</label>
            <input type="number" id="tq-hours" class="form-control" value="1" step="0.25">
          </div>
          <div style="flex:1;">
            <label style="font-size:0.75rem; font-weight:800; color:#94a3b8; display:block; margin-bottom:4px;">Rate
              (€/h)</label>
          ...
          <div style="background:#fff; border:1px solid var(--border-color); border-radius:16px; padding:1rem;">
            ...
            <div style="margin-top:1rem; padding-top:1rem; border-top:1px solid #f1f5f9;">
              <div style="display:flex; justify-content:space-between; font-weight:900;">
                <span>Quelle:</span>
                <span style="color:#64748b;">Manuell + Aufgaben → Qualifikationen</span>
              </div>
              <div class="text-muted" style="margin-top:6px;">
                Manuelle Personalpositionen bleiben erhalten; Aufgaben-Personal wird zusätzlich aus task_labor summiert.
              </div>
            </div>
          ...
          <div style="background:#fff; border:1px solid var(--border-color); border-radius:16px; padding:1rem;">
            ...
            <div style="margin-top:1rem; padding-top:1rem; border-top:1px solid #f1f5f9;">
              <div style="display:flex; justify-content:space-between; align-items:flex-end;">
                <div>
                  <div style="font-size:10px; font-weight:900; color:#94a3b8; text-transform:uppercase;">Gesamt (VK)
                  </div>
  ...
  <div id="duplicate-modal" class="modal-backdrop hidden" style="z-index: 420;">
    <div class="modal-window fade-in"
      style="max-width: 760px; height:auto; max-height:90vh; display:flex; flex-direction:column;">
      <div class="modal-header">
        <h3 class="modal-title" style="display:flex; align-items:center; gap:.75rem;">
      ...
      <div class="modal-header"
        style="border-top:1px solid var(--border-color); border-bottom:none; justify-content:flex-end; gap:.75rem;">
  ...
  <div id="distributor-compare-modal" class="modal-backdrop hidden" style="z-index: 430;">
    <div class="modal-window fade-in" style="max-width: 1100px; height: 88vh; display:flex; flex-direction:column;">
      ...
      <div class="modal-body custom-scrollbar"
        style="background:rgba(248,250,252,.35); display:grid; grid-template-columns: 1.1fr .9fr; gap:1rem;">
        <div>
        ...
        <div>
          <div
            style="background:white; border:1px solid var(--border-color); border-radius:16px; padding:1rem; height:100%;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:.75rem;">
  ...
  <div id="locked-warning-modal" class="modal-backdrop hidden" style="z-index: 9999;">
    <div class="modal-window fade-in" style="max-width: 400px; height: auto; padding: 2rem; text-align: center;">
      ...
      <h3 style="font-size: 1.25rem; font-weight: 900; margin-bottom: 0.5rem; color: var(--text-main);">Set ist gesperrt
      </h3>
      <p style="color: var(--text-light); font-size: 0.875rem; margin-bottom: 1.5rem;">
        Dieses MasterSet ist schreibgeschützt. Änderungen können nicht vorgenommen und nicht gespeichert werden. Bitte
        entsperren Sie das Set zuerst oben rechts.
      </p>
      <button onclick="document.getElementById('locked-warning-modal').classList.add('hidden')"
        class="btn btn-secondary" style="width: 100%;">Verstanden</button>
    </div>
  </div>
  @endsection
...
@push('scripts')
  ...
  <script>
    /**
     * MasterSet PRO-Manager — Full Script (Material + Labor + Tasks by Lead-Phases)
     * - Keeps your existing endpoints + UI behavior
     * - Adds full "Aufgabe" tab with:
     *   - Lead-Phase filter + search
    ...
    (() => {
      ...
      const normalizeSet = (set) => {
        ...
        s.tasks.forEach((t, i) => {
          normalizeTaskStage(t);
          t.hours = toNum(t.hours, 0);
          t.sort_order = Number.isFinite(+t.sort_order) ? +t.sort_order : i;
          t.task_labor = Array.isArray(t.task_labor) ? t.task_labor : [];
          t.task_labor.forEach((tl) => {
            tl.qualification_id = tl.qualification_id ?? null;
            tl.name = tl.name || '—';
            tl.rate = toNum(tl.rate ?? tl.hourly_rate, 0);
        ...
        s.tasks.forEach((t) => {
          (t.task_labor || []).forEach((tl) => {
            ...
            if (!qid) return;
            if (!taskLaborTotals[qid]) {
              taskLaborTotals[qid] = { hours: 0, rate: toNum(tl.rate, 0) };
            }
            taskLaborTotals[qid].hours += toNum(tl.hours, 0);
            taskLaborTotals[qid].rate = toNum(tl.rate, taskLaborTotals[qid].rate);
          });
        });

        s.labor.forEach((l, i) => {
          l.hours = toNum(l.hours, 0);
          l.rate = toNum(l.rate ?? l.hourly_rate, 0);
          l.sort_order = Number.isFinite(+l.sort_order) ? +l.sort_order : i;

          const explicitSource = l.source || l.labor_source || null;
          const qid = String(l.qualification_id || '');
          const taskTotal = qid ? taskLaborTotals[qid] : null;

          if (explicitSource === 'tasks' || explicitSource === 'task' || explicitSource === 'auto') {
            l.source = 'tasks';
            l.is_auto = true;
            l.is_manual = false;
          } else if (explicitSource === 'manual') {
            l.source = 'manual';
            l.is_auto = false;
            l.is_manual = true;
          } else if (taskTotal && Math.abs(toNum(l.hours, 0) - toNum(taskTotal.hours, 0)) < 0.01) {
            // Old saved rows that were generated from task_labor are auto rows.
            l.source = 'tasks';
            l.is_auto = true;
            l.is_manual = false;
          } else {
      ...
      const getTaskLeadStageId = (task) => {
        const value = task?.lead_stage_id ?? task?.stage_id ?? null;
        return value === '' || value === undefined ? null : value;
      };

      const normalizeTaskStage = (task) => {
        if (!task) return task;

        const leadStageId = task.lead_stage_id ?? task.stage_id ?? null;

        task.lead_stage_id = leadStageId;
        task.stage_id = leadStageId; // frontend compatibility alias

        if (!task.stage_name && task.lead_stage_name) {
          task.stage_name = task.lead_stage_name;
        }
        if (!task.lead_stage_name && task.stage_name) {
          task.lead_stage_name = task.stage_name;
        }
      ...
      const findStageById = (stageId) => {
        ...
        return (state.taskOptions || []).find((s) => {
          const id = s.id ?? s.lead_stage_id ?? s.stage_id;
          return String(id) === String(stageId);
        }) || null;
      };

      const getStageDisplayName = (stage) => {
        return stage?.name || stage?.stage || stage?.title || stage?.key || 'Lead-Phase';
      };

      const ensureSelectedStage = () => {
        if (!state.selectedStageId && Array.isArray(state.taskOptions) && state.taskOptions.length) {
      ...
      const api = {
        ...
        async loadTaskOptions(q = '') {
          ...
          const leadStageId = state.selectedStageId || '';

          const url =
            `/tasks/options?article_group_id=${encodeURIComponent(groupId)}` +
            `&q=${encodeURIComponent(q || '')}` +
            `&section=${encodeURIComponent(section)}` +
            `&lead_stage_id=${encodeURIComponent(leadStageId)}` +
            `&stage_id=${encodeURIComponent(leadStageId)}`;

          const res = await this.request(url);
          if (!res) return;

          state.taskOptions = (res.data || []).map((stage) => {
            const id = stage.id ?? stage.lead_stage_id ?? stage.stage_id;
            ...
            return {
              ...
              lead_stage_id: id,
              stage_id: id,
              name: stage.name || stage.stage || stage.title || stage.key || ('Lead-Phase #' + id),
              stage: stage.stage || stage.name || stage.title || stage.key || ('Lead-Phase #' + id),
              phases: Array.isArray(stage.phases)
                ? stage.phases.map((phase) => ({
                  ...phase,
                  lead_stage_id: phase.lead_stage_id ?? phase.stage_id ?? id,
                  stage_id: phase.lead_stage_id ?? phase.stage_id ?? id,
                  activities: Array.isArray(phase.activities)
                    ? phase.activities.map((activity) => ({
                      ...activity,
                      lead_stage_id: activity.lead_stage_id ?? activity.stage_id ?? phase.lead_stage_id ?? id,
                      stage_id: activity.lead_stage_id ?? activity.stage_id ?? phase.lead_stage_id ?? id,
                    }))
                    : [],
                }))
                : [],
          ...
          if (state.pickerContext?.type === 'task') {
            ...
            const modalStageSelect = document.getElementById('modal-task-stage');
            if (modalStageSelect) {
              modalStageSelect.innerHTML = `
            <option value="">Alle Lead-Phasen</option>
            ${(state.taskOptions || []).map(st => `
              <option value="${escapeHtml(st.id)}" ${String(st.id) === String(state.selectedStageId || '') ? 'selected' : ''}>
                ${escapeHtml(getStageDisplayName(st))}
              </option>
            `).join('')}
          `;
            }
        ...
        async saveSet(isAutoSave = false, needsReload = false) {
          ...
          const tasks = (s.tasks || [])
            .map((t, i) => normalizeTaskStage({ ...t, sort_order: Number.isFinite(+t.sort_order) ? +t.sort_order : i }))
            .sort((a, b) => {
              const sa = String(getTaskLeadStageId(a) ?? '');
              const sb = String(getTaskLeadStageId(b) ?? '');
          ...
          const payload = {
            ...
            labor: (s.labor || []).map((l, i) => ({
              qualification_id: l.qualification_id || null,
              department_position_id: l.department_position_id || null,
              hours: toNum(l.hours, 0),
              hourly_rate: toNum(l.rate ?? l.hourly_rate, 0),
              source: l.source || (l.is_auto ? 'tasks' : 'manual'),
              is_manual: (l.source || (l.is_auto ? 'tasks' : 'manual')) === 'manual',
              sort_order: Number.isFinite(+l.sort_order) ? +l.sort_order : i
            })),

            tasks: tasks.map((rawTask, i) => {
              const t = normalizeTaskStage(rawTask);
              const leadStageId = getTaskLeadStageId(t);

              return {
                stage_id: leadStageId,
                lead_stage_id: leadStageId,
                lead_sub_stage_id: t.lead_sub_stage_id ?? null,
                ...
                stage_name: t.stage_name ?? t.lead_stage_name ?? null,
                lead_stage_name: t.lead_stage_name ?? t.stage_name ?? null,
    ...
    `);
      ...
      <div class="text-muted">Aufgaben: <strong>${acts.length}</strong></div>
      <div class="text-muted">Lead-Phasen: <strong>${stages.length}</strong></div>
    ...
    `;
      ...
      const wizard = {
        ...
        setPhaseAufgaben(phase, activities) {
          if (!phase) return;
        ...
        removeActivityLocal(activityId) {
          ...
          for (const ph of phases) {
            const acts = this.getPhaseAufgaben(ph);
        ...
        renderPhases() {
          ...
          container.innerHTML = phases.map(phase => {
            const stageName =
              ...
              phase.stage?.name ||
              phase.stage ||
              (phase.stage_id ? ('Lead-Phase #' + phase.stage_id) : '—');
            ...
            <div class="phase-card ${collapsed ? 'is-collapsed' : ''}" data-id="${escapeHtml(phase.id)}">
              <div class="phase-header">
                <div style="display:flex; align-items:center; gap:0.5rem; width:100%; min-width:0;">
                  ...
                  <span class="stage-badge" title="Lead-Phase" style="flex:0 0 auto;">
        ...
        initSortables() {
          ...
          const patchLocalAufgabenMove = (activityId, fromPhaseId, toPhaseId, toIndex) => {
            const section = getSection();
            const phases = getPhases(section);
          ...
          // 2) Aufgaben sortable (skip collapsed)
          const section = getSection();
          const phases = getPhases(section);
          ...
          phases.forEach(phase => {
            ...
            this._activitySortables[phaseId] = new Sortable(body, {
              ...
              onEnd: async (evt) => {
                ...
                if (movedId) {
                  patchLocalAufgabenMove(movedId, fromPhaseId, toPhaseId, evt.newIndex);
                }
      ...
      const ui = {
        ...
        removeLaborWithConfirm(idx) {
          ...
          const source = laborItem.source || (laborItem.is_auto ? 'tasks' : 'manual');
          const isAuto = source === 'tasks' || source === 'task' || source === 'auto';

          const message = isAuto
            ? `Möchten Sie "${label}" wirklich entfernen?\n\nDiese Qualifikation wird dann aus allen Aufgaben im Tab "Aufgabe" gelöscht.`
            : `Möchten Sie "${label}" wirklich entfernen?\n\nDiese manuelle Personalposition wird entfernt. Aufgaben bleiben unverändert.`;
  ...
  `;
        ...
        costingRecalcPreview() {
          ...
          // Use the Personal table as source of truth: manual rows + auto rows from tasks.
          const totals = {}; // key => {name, rate, hours}

          labor.forEach((l, index) => {
            const qid = String(l.qualification_id || `manual-${index}`);
            const source = l.source || (l.is_auto ? 'tasks' : 'manual');
            const key = `${source}:${qid}:${index}`;
            const hours = toNum(l.hours, 0);
            if (hours <= 0) return;

            totals[key] = {
              name: l.name || l.position_name || l.employee_name || '—',
              rate: toNum(l.rate ?? l.hourly_rate, 0),
              hours,
              source,
            };
          });

          // Fallback for older unsynced sets: build from tasks only if labor is empty.
          if (Object.keys(totals).length === 0) {
            tasks.forEach(t => {
              (t.task_labor || []).forEach(tl => {
                ...
                if (!qid) return;
                const key = `tasks:${qid}`;
                if (!totals[key]) totals[key] = { name: tl.name || '—', rate: toNum(tl.rate, 0), hours: 0, source: 'tasks' };
                totals[key].hours += toNum(tl.hours, 0);
                totals[key].rate = toNum(tl.rate, totals[key].rate);
              });
            });
          }
  ...
  `;
        ...
        async openCustomTaskModal() {
          ...
          try {
            ...
            fullOptions.forEach(stage => {
              ...
              phases.forEach(p => {
                const stageName = getStageDisplayName(stage) || 'Ohne Lead-Phase';
                const phaseName = p.phase_name || p.name || 'Unbenannt';
                const key = `${stageName}__${phaseName}`;

                if (!uniquePhases.has(key)) {
                  uniquePhases.set(key, {
                    val: `${stage.id}|${p.id}|${stageName}|${phaseName}`, // stage.id is lead_stages.id 
                    label: phaseName,
                    group: stageName
                  });
                }
        ...
        async saveCustomTask() {
          ...
          try {
            ...
            const newTask = {
              ...
              lead_stage_id: parseInt(stageId),
        ...
        addPhaseFromEncoded(encoded) {
          ...
          if (!Array.isArray(state.editingSet.tasks)) {
            state.editingSet.tasks = [];
          }

          let addedCount = 0;
          const leadStageId = payload.lead_stage_id ?? payload.stage_id ?? null;

          payload.activities.forEach(act => {
            const activityId = act.phase_activity_id ?? act.id;
            const exists = state.editingSet.tasks.some(
              (t) => String(t.phase_activity_id) === String(activityId)
            );

            if (exists) return;

            state.editingSet.tasks.push(normalizeTaskStage({
              stage_id: leadStageId,
              lead_stage_id: leadStageId,
              stage_name: payload.stage_name ?? payload.lead_stage_name ?? null,
              lead_stage_name: payload.lead_stage_name ?? payload.stage_name ?? null,
              task_phase_id: payload.task_phase_id ?? null,
              phase_name: payload.phase_name ?? null,
              phase_activity_id: activityId,
          ...
          if (addedCount > 0) {
            ui.showStatus(`${addedCount} AUFGABEN IMPORTIERT`);
            ui.renderTasksTab();
            app.autoSave();
          } else {
        ...
        // 5. THE AGGREGATOR: Calculates task labor totals, but preserves manual labor rows
        syncGlobalLaborFromTasks() {
          if (!state.editingSet) return;

          const tasks = Array.isArray(state.editingSet.tasks) ? state.editingSet.tasks : [];
          const currentLabor = Array.isArray(state.editingSet.labor) ? state.editingSet.labor : [];
          const totals = {};
          ...
          tasks.forEach(t => {
            ...
            t.task_labor.forEach(tl => {
              ...
              const hours = toNum(tl.hours, 0);
              const rate = toNum(tl.rate ?? tl.hourly_rate, 0);
              const name = tl.name || '—';
              ...
              totals[qid].rate = rate;
              totals[qid].hourly_rate = rate;
          ...
          // Keep manual rows. Remove previous auto rows and rebuild them from current tasks.
          const manualRows = currentLabor
            .filter(row => {
              const source = row.source || (row.is_auto ? 'tasks' : 'manual');
              return source !== 'tasks' && source !== 'task' && source !== 'auto';
            })
            .map((row, i) => ({
              ...row,
              hours: toNum(row.hours, 0),
              rate: toNum(row.rate ?? row.hourly_rate, 0),
              hourly_rate: toNum(row.rate ?? row.hourly_rate, 0),
              source: 'manual',
              is_manual: true,
              is_auto: false,
              sort_order: Number.isFinite(+row.sort_order) ? +row.sort_order : i,
            }));

          const autoRows = Object.values(totals)
            .filter(row => toNum(row.hours, 0) > 0)
            .map((row, i) => ({
              ...row,
              sort_order: manualRows.length + i,
            }));

          state.editingSet.labor = [...manualRows, ...autoRows];
              ...
              tabContainer.innerHTML = `
                        <section class="tw-section">
                          <header class="tw-header">
                            ...
                            <div class="tw-actions" style="display:flex; gap:.5rem; flex-wrap:wrap; justify-content:flex-end;">
                              <button type="button" class="btn btn-accent" onclick="ui.openLaborPicker()" title="Personal manuell hinzufügen">
                                <i class="fas fa-plus"></i> PERSONAL MANUELL
                              </button>
                              <button type="button" class="btn btn-secondary" onclick="ui.syncGlobalLaborFromTasks()" title="Personal aus Aufgaben neu berechnen">
                                <i class="fas fa-link"></i> AUS AUFGABEN AKTUALISIEREN
                              </button>
                            </div>
                          </header>
  ...
  `;
    ...
    `;
          ...
          `);
            ...
            <div class="flex gap-3 w-full">
              <div class="shrink-0 w-14 h-14 bg-slate-50 border border-slate-200 rounded-lg overflow-hidden flex items-center justify-center mt-1">
                ${data.image_url
                      ? `<img src="${esc(data.image_url)}" alt="img" class="w-full h-full object-cover" />`
    ...
    `;
            ...
            comps.forEach((c, idx) => {
              ...
              // Always render the container so it exists as a drop zone for Drag & Drop
              const displayStyle = c.isExpanded ? '' : 'display:none;';
              bodyHtml += `<div class="flex flex-col border-t border-gray-100 bg-slate-50/30 min-h-[14px] pb-1" data-parent-index="${idx}" id="sub-list-${idx}" style="${displayStyle}">`;
  ...
  `;
        ...
        initMaterialSortables() {
          ...
          // ==========================================
          // 1. MAIN SORTABLE (Top Level Articles)
          // ==========================================
          this._matSortables.main = new Sortable(mainBody, {
            group: 'material-comps', // THIS ALLOWS DRAGGING TO SUBLISTS
            animation: 150,
            handle: '.handle, .sub-handle',
            draggable: '.group-row, .is-sub-row',
            ...
            onAdd: (evt) => {
              stopAutoScroll();
              // Sub-item dragged back out to main list
              const fromParentIdx = evt.from.getAttribute('data-parent-index');
              if (fromParentIdx != null) {
                const oldIndex = evt.oldDraggableIndex;
                const newIndex = evt.newDraggableIndex;

                const parentComp = state.editingSet.components[fromParentIdx];
                if (!parentComp) return;

                const movedSub = parentComp.subComponents.splice(oldIndex, 1)[0];
                if (!movedSub) return;

                // Convert back to a main article
                movedSub.type = 'haupt';
                movedSub.subComponents = [];

                state.editingSet.components.splice(newIndex, 0, movedSub);
            ...
            onEnd: (evt) => {
              ...
              if (evt.to !== evt.from) return; // Handled by target's onAdd
              ...
              if (oldIndex < 0 || newIndex < 0 || oldIndex >= comps.length || newIndex >= comps.length) return;

              const moved = comps.splice(oldIndex, 1)[0];
              if (!moved) return;

              comps.splice(newIndex, 0, moved);
              comps.forEach((c, i) => { if (c) c.pos = i + 1; });
          ...
          // ==========================================
          // 2. SUB SORTABLES (Nested Articles)
          // ==========================================
          const comps = Array.isArray(state.editingSet?.components) ? state.editingSet.components : [];
          comps.forEach((c, mIdx) => {
            ...
            this._matSortables.subs[mIdx] = new Sortable(subWrap, {
              group: 'material-comps', // THIS MATCHES THE MAIN LIST
              animation: 150,
              handle: '.sub-handle, .handle',
              draggable: '.is-sub-row, .group-row',
              ghostClass: 'drag-ghost',
              chosenClass: 'drag-chosen',
              dragClass: 'drag-dragging',
              forceFallback: true,
              ...
              onAdd: (evt) => {
                stopAutoScroll();
                const fromMain = evt.from.id === 'material-main-body';
                const oldIndex = evt.oldDraggableIndex;
                const newIndex = evt.newDraggableIndex;

                if (fromMain) {
                  // Main article was dragged into this sub-article list
                  const movedComp = state.editingSet.components.splice(oldIndex, 1)[0];
                  if (!movedComp) return;

                  movedComp.type = 'zubehoer'; // Change to accessory

                  // Flatten any existing sub-items of the dragged item
                  const subsToMove = Array.isArray(movedComp.subComponents) ? movedComp.subComponents : [];
                  movedComp.subComponents = [];
                  subsToMove.forEach(s => s.type = 'zubehoer');

                  if (!Array.isArray(c.subComponents)) c.subComponents = [];
                  c.subComponents.splice(newIndex, 0, movedComp, ...subsToMove);

                  ui.renderComponentItems();
                  app.autoSave();
                } else {
                  // Sub-article was moved from another article's sub-list
                  const fromParentIdx = evt.from.getAttribute('data-parent-index');
                  const fromParent = state.editingSet.components[fromParentIdx];
                  if (!fromParent) return;

                  const movedSub = fromParent.subComponents.splice(oldIndex, 1)[0];
                  if (!movedSub) return;

                  if (!Array.isArray(c.subComponents)) c.subComponents = [];
                  c.subComponents.splice(newIndex, 0, movedSub);
        ...
        <div style="padding:1rem; display:flex; flex-direction:column; gap:.75rem;">
          <div style="display:flex; gap:1rem; align-items:flex-end;">
            <div style="display:flex; flex-direction:column; gap:4px;">
              <label style="font-size:.7rem; font-weight:800; color:#94a3b8; text-transform:uppercase; margin:0;">Dienstleistung</label>
              <select id="modal-task-section" class="search-input" style="width:175px; padding:.5rem; font-size:.85rem;">
                ${sections.map(s => `<option value="${s.v}" ${state.taskSectionFilter === s.v ? 'selected' : ''}>${s.l}</option>`).join('')}
              </select>
            </div>

            <div style="display:flex; flex-direction:column; gap:4px;">
              <label style="font-size:.7rem; font-weight:800; color:#94a3b8; text-transform:uppercase; margin:0;">Lead-Phase</label>
              <select id="modal-task-stage" class="search-input" style="width:190px; padding:.5rem; font-size:.85rem;">
                <option value="">Alle Lead-Phasen</option>
              </select>
            </div>

            <div style="flex:1; display:flex; flex-direction:column; gap:4px;">
              <label style="font-size:.7rem; font-weight:800; color:#94a3b8; text-transform:uppercase; margin:0;">Suche</label>
      ...
      `;
          ...
          contentEl.innerHTML = `
        <div style="padding:2rem; text-align:center; color:#94a3b8; font-weight:900;">
          <i class="fas fa-spinner fa-spin"></i> Aufgaben werden geladen...
        </div>
      `;

          await api.loadTaskOptions(state.taskSearch || '');
          ...
          const triggerLoad = () => {
            ...
            clearTimeout(ui._modalTaskSearchTimer);
            ui._modalTaskSearchTimer = setTimeout(() => {
              api.loadTaskOptions(state.taskSearch);
            }, 250);
          };

          if (sectionInput) sectionInput.onchange = triggerLoad;
          if (stageInput) stageInput.onchange = triggerLoad;
          if (textInput) textInput.oninput = triggerLoad;
      ...
      `;
        ...
        `;
          ...
          const listStages = mode === 'modal'
            ? stages
            : (stage ? [stage] : [stages[0]]);

          return listStages.map((st) => {
            const leadStageId = st.lead_stage_id ?? st.stage_id ?? st.id;
            const stName = getStageDisplayName(st);
            const stColor = st.color || 'var(--primary)';
            ...
            return `
          <div id="taskopt-stage-${escapeHtml(leadStageId)}" class="catalog-item" style="margin-bottom:1rem;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:.75rem;">
              <div style="font-weight:900; color:var(--text-main); display:flex; align-items:center; gap:.5rem;">
                <span style="width:10px; height:10px; border-radius:999px; background:${escapeHtml(stColor)}; display:inline-block;"></span>
                ${escapeHtml(stName)}
              </div>
              <span style="font-size:.625rem; font-weight:900; color:#cbd5e1; text-transform:uppercase;">
            ...
            ${phases.length
                ? phases.map((ph) => {
                  ...
                  const phLeadStageId = ph.lead_stage_id ?? ph.stage_id ?? leadStageId;
                  ...
                  const phasePayload = {
                    stage_id: phLeadStageId,
                    lead_stage_id: phLeadStageId,
                    stage_name: stName,
                    lead_stage_name: stName,
                    lead_sub_stage_id: ph.lead_sub_stage_id ?? null,
                    task_phase_id: ph.id,
                    phase_name: phName,
                    activities: acts.map(a => ({
                      phase_activity_id: a.id ?? a.phase_activity_id,
                      lead_stage_id: a.lead_stage_id ?? a.stage_id ?? phLeadStageId,
                      stage_id: a.lead_stage_id ?? a.stage_id ?? phLeadStageId,
                      lead_sub_stage_id: a.lead_sub_stage_id ?? ph.lead_sub_stage_id ?? null,
                  ...
                  return `
                    ...
                    const title = act.title || act.name || ('Aufgabe #' + actId);
                    ...
                    const activityLeadStageId = act.lead_stage_id ?? act.stage_id ?? phLeadStageId;

                    const payload = {
                      stage_id: activityLeadStageId,
                      lead_stage_id: activityLeadStageId,
                      lead_sub_stage_id: act.lead_sub_stage_id ?? ph.lead_sub_stage_id ?? null,
                      stage_name: stName,
                      lead_stage_name: stName,
                      task_phase_id: ph.id,
                      phase_name: phName,
                    ...
                    return `
                            <button
                              ...
                              title="Hinzufügen">
                              <span style="font-size:.75rem; font-weight:900;">${escapeHtml(title)}</span>
                              <span style="font-size:.625rem; font-weight:700; color:#94a3b8;">
                                ${desc ? escapeHtml(desc).slice(0, 80) : '—'}
                              </span>
                              <span style="font-size:.625rem; font-weight:900; color:#94a3b8; text-transform:uppercase;">
                ...
                : `
                <div style="padding:1rem; color:#cbd5e1; font-weight:900; text-align:center;">
                  Keine Phasen / Aufgaben in dieser Lead-Phase
                </div>
              `
              }
          </div>
        ...
        addTaskFromEncoded(encoded) {
          ...
          state.editingSet.tasks.push(normalizeTaskStage({
            stage_id: leadStageId,
            lead_stage_id: leadStageId,
            lead_sub_stage_id: payload.lead_sub_stage_id ?? null,
            stage_name: payload.stage_name ?? payload.lead_stage_name ?? null,
            lead_stage_name: payload.lead_stage_name ?? payload.stage_name ?? null,
            task_phase_id: payload.task_phase_id ?? null,
            phase_name: payload.phase_name ?? null,
          ...
          ui.showStatus('AUFGABE HINZUGEFÜGT');
          ui.renderTasksTab();
          app.autoSave();
        },
    ...
    `;
          ...
          const stageLabel = (stageKey) => {
            if (stageKey === 'null') return 'Ohne Lead-Phase';
            const stage = findStageById(stageKey);
            const fallback = tasks.find(x => String(getTaskLeadStageId(x)) === String(stageKey))?.stage_name;
            return getStageDisplayName(stage) || fallback || ('Lead-Phase #' + stageKey);
          };
  ...
  `;
      ...
      <div class="catalog-item task-row" data-task-index="${idx}" style="margin:0; border-radius:var(--radius-xl); display:flex; flex-direction:column; gap:1rem;">
        <div style="display:flex; justify-content:space-between; gap:1rem; align-items:flex-start;">
          ...
          <div style="display:flex; gap:0.75rem; align-items:flex-start; flex-shrink:0;">
            ...
            >
              <option value="">Ohne Lead-Phase</option>
              ${(state.taskOptions || []).map(st => `
                <option value="${escapeHtml(st.id)}" ${String(st.id) === String(getTaskLeadStageId(t)) ? 'selected' : ''}>
                  ${escapeHtml(getStageDisplayName(st))}
                </option>
              `).join('')}
            </select>
    ...
    `;
          ...
          // Group by lead_stage_id; stage_id remains only a frontend alias
          const byStage = new Map();
          tasks.forEach((t, idx) => {
            normalizeTaskStage(t);
            const key = String(getTaskLeadStageId(t) ?? 'null');
    ...
    `;
        ...
        changeTaskStage(taskIndex, stageId) {
          ...
          if (!stageId) {
            t.stage_id = null;
            t.lead_stage_id = null;
            t.stage_name = null;
            t.lead_stage_name = null;
          } else {
            const st = findStageById(stageId);
            const name = getStageDisplayName(st);

            t.stage_id = stageId;
            t.lead_stage_id = stageId;
            t.stage_name = name;
            t.lead_stage_name = name;
          }
        ...
        // 3. Add to Set (manual labor, independent from tasks)
        addLaborFromJson(encodedOrJson) {
          ...
          try {
            ...
            console.error(e);
            ui.showStatus('PERSONAL PAYLOAD ERROR', true);
            return;
          }

          if (!state.editingSet) return;
          if (!Array.isArray(state.editingSet.labor)) state.editingSet.labor = [];

          const qid = String(l.qualification_id || '');
          const existingManual = state.editingSet.labor.find(row =>
            String(row.qualification_id || '') === qid &&
            (row.source || (row.is_auto ? 'tasks' : 'manual')) === 'manual'
          );

          if (existingManual) {
            existingManual.hours = toNum(existingManual.hours, 0) + 1;
            existingManual.rate = toNum(existingManual.rate ?? existingManual.hourly_rate, toNum(l.hourly_rate, 0));
            existingManual.hourly_rate = existingManual.rate;
            existingManual.source = 'manual';
            existingManual.is_manual = true;
            existingManual.is_auto = false;
          } else {
            state.editingSet.labor.push({
              qualification_id: l.qualification_id,
              name: l.name,
              rate: toNum(l.hourly_rate ?? l.default_price, 0),
              hourly_rate: toNum(l.hourly_rate ?? l.default_price, 0),
              hours: 1,
              source: 'manual',
              is_manual: true,
              is_auto: false,
              ...
              employee_name: null,
              position_name: null,
              sort_order: state.editingSet.labor.length,
            });
          }

          this.closeModal();
          this.recalculateLocalStats();
          if (state.editorTab === 'personal') this.renderLaborItems();
          else this.render();
          app.autoSave();
          this.showStatus('PERSONAL MANUELL HINZUGEFÜGT');
        },

        // 4. Render Table Rows (manual rows editable, task rows readonly)
        renderLaborItems() {
          const body = document.getElementById('labor-body');
          if (!body) return;

          const laborRows = Array.isArray(state.editingSet?.labor) ? state.editingSet.labor : [];

          if (!laborRows.length) {
            body.innerHTML = `
          <tr>
            <td colspan="6" style="padding:2rem; text-align:center; color:#94a3b8; font-weight:800;">
              Noch kein Personal vorhanden. Füge Personal manuell hinzu oder ordne Personal in Aufgaben zu.
            </td>
          </tr>
        `;
            return;
          ...
          body.innerHTML = laborRows.map((l, idx) => {
            const source = l.source || (l.is_auto ? 'tasks' : 'manual');
            const isAuto = source === 'tasks' || source === 'task' || source === 'auto';
            const title = l.name || l.position_name || 'Unbekannt';
            const subTitle = isAuto
              ? 'Auto aus Aufgaben'
              : (l.qualification_id ? 'Manuelle Qualifikation' : (l.employee_name || 'Manuelles Personal'));

            const rate = toNum(l.rate ?? l.hourly_rate, 0);
            const hours = toNum(l.hours, 0);
            const total = rate * hours;

            const sourceBadge = isAuto
              ? `<span class="pill pill-purple" style="display:inline-flex; margin-top:6px;"><div class="dot bg-purple"></div> Aufgabe</span>`
              : `<span class="pill pill-green" style="display:inline-flex; margin-top:6px;"><div class="dot bg-green"></div> Manuell</span>`;

            const rateCell = isAuto
              ? `
            <div class="price-control" style="justify-content:center; opacity:.65; cursor:not-allowed;" title="Stundensatz wird aus Aufgabe → Personal übernommen.">
              <input type="text" class="price-input" style="width:70px; text-align:center; cursor:not-allowed;" value="${rate.toFixed(2)}" readonly>
              <span style="font-size:10px;">€/h</span>
            </div>
            <div style="font-size:0.6rem; color:#94a3b8; margin-top:4px; font-weight:700;">Auto aus Aufgaben</div>
          `
              : `
            <div class="price-control" style="justify-content:center;">
              <input type="number" step="0.01" class="price-input" style="width:78px; text-align:center;" value="${rate.toFixed(2)}" onchange="ui.updateLaborRate(${idx}, this.value)">
              <span style="font-size:10px;">€/h</span>
            </div>
            <div style="font-size:0.6rem; color:#94a3b8; margin-top:4px; font-weight:700;">Manuell editierbar</div>
          `;

            const hoursCell = isAuto
              ? `
            <div class="qty-control" style="display:inline-flex; background:#f8fafc; border-color:#e2e8f0; cursor:not-allowed;" title="Stunden werden automatisch aus Aufgaben summiert.">
              <input type="text" readonly class="qty-input" style="width:58px; cursor:not-allowed; color:#94a3b8;" value="${hours.toFixed(2)}">
              <span style="font-size:10px; font-weight:900; color:#cbd5e1; padding-right:8px;">h</span>
            </div>
            <div style="font-size:0.6rem; color:#94a3b8; margin-top:4px; font-weight:700;">Auto summiert</div>
          `
              : `
            <div class="qty-control" style="display:inline-flex;">
              <input type="number" step="0.25" class="qty-input" style="width:64px;" value="${hours.toFixed(2)}" onchange="ui.updateLaborHours(${idx}, this.value)">
              <span style="font-size:10px; font-weight:900; color:#94a3b8; padding-right:8px;">h</span>
            </div>
            <div style="font-size:0.6rem; color:#94a3b8; margin-top:4px; font-weight:700;">Manuell editierbar</div>
          `;
          ...
          <tr>
            <td>
              <div class="avatar-group">
                <div class="avatar-wrap">
                  <div style="width:40px; height:40px; background:${isAuto ? 'rgba(168,85,247,.10)' : 'rgba(147,194,28,.10)'}; color:${isAuto ? '#a855f7' : 'var(--accent)'}; border-radius:12px; display:flex; align-items:center; justify-content:center; border:1px solid #e2e8f0;">
                    <i class="fas ${isAuto ? 'fa-link' : 'fa-user-tag'}"></i>
                  </div>
                </div>
                <div>
                  <div style="font-weight:900; color:var(--text-main); font-size:0.95rem;">${escapeHtml(title)}</div>
                  <p style="font-size:0.65rem; font-weight:700; color:#94a3b8; text-transform:uppercase;">${escapeHtml(subTitle)}</p>
                  ${sourceBadge}
                </div>
              </div>
            </td>
            ...
            <td style="text-align:right;">
              <button
                ...
                class="btn-danger"
                title="${isAuto ? 'Qualifikation aus allen Aufgaben entfernen' : 'Manuelle Personalposition entfernen'}"
              >
                <i class="fas fa-trash-alt"></i>
              </button>
            </td>
        ...
        // 5. Update manual labor rate/hours
        updateLaborRate(idx, val) {
          const row = state.editingSet?.labor?.[idx];
          if (!row) return;

          const source = row.source || (row.is_auto ? 'tasks' : 'manual');
          if (source === 'tasks') return;

          row.rate = toNum(val, 0);
          row.hourly_rate = row.rate;
          row.source = 'manual';
          row.is_manual = true;
          row.is_auto = false;
        ...
        updateLaborHours(idx, val) {
          ...
          row.hours = Math.max(0, toNum(val, 0));
          row.source = 'manual';
          row.is_manual = true;
          row.is_auto = false;
        ...
        removeLabor(idx) {
          ...
          // Always remove the clicked row from Personal table first.
          state.editingSet.labor.splice(idx, 1);

          if (isAuto && qid && Array.isArray(state.editingSet.tasks)) {
            // Auto rows are task-derived. Removing them means removing this qualification from tasks.
            state.editingSet.tasks.forEach(t => {
              if (Array.isArray(t.task_labor)) {
                ...
                t.hours = t.task_labor.reduce((sum, tl) => sum + toNum(tl.hours, 0), 0);
              }
            });

            this.syncGlobalLaborFromTasks();
          } else {
            // Manual rows are independent from tasks.
            this.recalculateLocalStats();
          }
          ...
          app.autoSave();
          this.showStatus(isAuto ? 'QUALIFIKATION AUS AUFGABEN ENTFERNT' : 'MANUELLES PERSONAL ENTFERNT');
        },
        ...
        openReport() {
          ...
          const stageLabel = (stageId, fallbackName) => {
            if (!stageId) return 'Ohne Lead-Phase';
            const st = findStageById(stageId);
            return getStageDisplayName(st) || fallbackName || ('Lead-Phase #' + stageId);
          };
          ...
          tasks.forEach((t) => {
            const label = stageLabel(getTaskLeadStageId(t), t.stage_name);
          ...
          `;
            ...
            <div class="pdf-block">
              ...
              <div class="pdf-section-subtitle">Nach Lead-Phase gruppiert.</div>
              ${sections}
            </div>
          `;
          }
      ...
      window.TaskWizard = (() => {
        ...
        function next() {
          ...
          if (step === 2) {
            if (getCheckedValues('.tw_stage_cb').length === 0) {
              alert('Bitte mindestens eine Lead-Phase auswählen.');
              return;
            }
            go(3); return;
          }
        ...
        function buildReview() {
          ...
          <div><strong>Lead-Phasen:</strong> ${stage_ids.length}</div>
          <div><strong>Sektionen:</strong> ${section_keys.join(', ')}</div>
          <div><strong>Produkte:</strong> ${product_ids.length}</div>
          <hr/>
          <div class="text-muted">
            Es werden Phasen je Kombination (Produkt × Sektion × Lead-Phase) erstellt und die Aktivitäten übernommen.
          </div>
        `;
