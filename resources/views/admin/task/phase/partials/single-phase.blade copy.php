    ...
    <div class="folder-toggle"
        ...
        <div class="button-container">
           ...
           @php $setCount = \App\Models\MasterSet::where('task_phase_id', $phase->id)->count(); @endphp
            ...
            {{-- FIXED BUTTON: Uses js-open-master-set class instead of onclick --}}
            <button type="button" 
                class="btn btn-icon btn-flat-info js-open-master-set" 
                title="Master Set verknüpfen"
                data-type="phase"
                data-target-id="{{ $phase->id }}"
                data-product-id="{{ $phase->product_id }}">
                <i class="feather icon-layers"></i>
                @if($setCount > 0)
                    <span class="badge badge-pill badge-primary badge-up">{{ $setCount }}</span>
                @endif
            </button>
