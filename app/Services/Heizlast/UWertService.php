...
class UWertService
{
    ...
    /**
     * U-Wert aus einer Fenster-Produktspec (U_w aus Datenblatt, Konfidenz hoch).
     *
     * @return array<string, mixed>
     */
    public function ausFensterSpec(FensterSpec $spec): array
    {
        return $this->ergebnis(
            (float) $spec->u_w, 'A', 'hoch', $spec->quelle ?: 'Produkt-Datenblatt (Fenster)',
            $spec->g_wert !== null ? ['g_wert' => (float) $spec->g_wert] : [],
        );
    }
